...
template <typename _Alloc> struct allocator_traits : __allocator_traits_base {
      ...
      private:
        ...
        template <typename _Tp, typename... _Args>
        static _Require<
                __and_<__not_<__has_construct<_Tp, _Args...>>, is_constructible<_Tp, _Args...>>>
        _S_construct(_Alloc &, _Tp *__p, _Args &&...__args) noexcept(
                noexcept(::new((void *)__p) _Tp(std::forward<_Args>(__args)...)))
        {
        ...
        template <typename _Alloc2, typename _Tp>
        static void _S_destroy(_Alloc2 &, _Tp *__p, ...) noexcept(noexcept(__p->~_Tp()))
        {
                __p->~_Tp();
        }
        ...
        template <typename _Alloc2> static size_type _S_max_size(_Alloc2 &, ...)
        {
...
template <typename _Tp> struct allocator_traits<allocator<_Tp>> {
         ...
         *  Calls @c a.allocate(n)
         */
        _GLIBCXX_NODISCARD static pointer allocate(allocator_type &__a, size_type __n)
        {
                return __a.allocate(__n);
        }
        ...
        static void construct(allocator_type &__a, _Up *__p, _Args &&...__args) noexcept(
                noexcept(__a.construct(__p, std::forward<_Args>(__args)...)))
        {
                __a.construct(__p, std::forward<_Args>(__args)...);
        }
...
template <typename _Alloc> inline void __do_alloc_on_copy(_Alloc &, const _Alloc &, false_type)
{
}
...
template <typename _Alloc> inline void __do_alloc_on_move(_Alloc &__one, _Alloc &__two, true_type)
{
        __one = std::move(__two);
}

template <typename _Alloc> inline void __do_alloc_on_move(_Alloc &, _Alloc &, false_type)
{
}
...
template <typename _Alloc> inline void __do_alloc_on_swap(_Alloc &__one, _Alloc &__two, true_type)
{
        using std::swap;
        swap(__one, __two);
}

template <typename _Alloc> inline void __do_alloc_on_swap(_Alloc &, _Alloc &, false_type)
{
}
...
template <typename _Alloc, typename _Tp,
          typename _ValueT = __remove_cvref_t<typename _Alloc::value_type>, typename = void>
...
struct __is_copy_insertable
        : __is_alloc_insertable_impl<_Alloc, typename _Alloc::value_type const &>::type {
};
