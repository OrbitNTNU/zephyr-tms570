...
template <typename> class regex_traits;
...
template <typename, bool, bool> struct _BracketMatcher;
...
template <typename _TraitsT> class _Compiler
{
      public:
        ...
        _Compiler(_IterT __b, _IterT __e, const typename _TraitsT::locale_type &__traits,
                  _FlagT __flags);
      ...
      private:
        ...
        template <bool __icase, bool __collate> void _M_insert_any_matcher_ecma();

        template <bool __icase, bool __collate> void _M_insert_any_matcher_posix();

        template <bool __icase, bool __collate> void _M_insert_char_matcher();

        template <bool __icase, bool __collate> void _M_insert_character_class_matcher();

        template <bool __icase, bool __collate> void _M_insert_bracket_matcher(bool __neg);

        // Returns true if successfully matched one term and should continue.
        // Returns false if the compiler should move on.
        template <bool __icase, bool __collate>
        bool _M_expression_term(pair<bool, _CharT> &__last_char,
...
template <typename _Tp> struct __is_contiguous_iter : is_pointer<_Tp>::type {
};

template <typename _Tp, typename _Cont>
struct __is_contiguous_iter<__gnu_cxx::__normal_iterator<_Tp *, _Cont>> : true_type {
};

template <typename _Iter, typename _TraitsT>
using __enable_if_contiguous_iter =
        __enable_if_t<__is_contiguous_iter<_Iter>::value, std::shared_ptr<const _NFA<_TraitsT>>>;
...
__compile_nfa(_FwdIter __first, _FwdIter __last, const typename _TraitsT::locale_type &__loc,
              ...
              regex_constants::syntax_option_type __flags)
{
        ...
        return __compile_nfa<_TraitsT>(__str.data(), __str.data() + __str.size(), __loc, __flags);
}

// [28.13.14]
template <typename _TraitsT, bool __icase, bool __collate> class _RegexTranslatorBase
{
      public:
        typedef typename _TraitsT::char_type _CharT;
        typedef typename _TraitsT::string_type _StringT;
        ...
        explicit _RegexTranslatorBase(const _TraitsT &__traits) : _M_traits(__traits)
        {
        }
...
class _RegexTranslator : public _RegexTranslatorBase<_TraitsT, __icase, __collate>
{
      public:
...
{
      public:
        typedef _RegexTranslatorBase<std::regex_traits<_CharType>, true, true> _Base;
        typedef typename _Base::_CharT _CharT;
...
template <typename _TraitsT> class _RegexTranslator<_TraitsT, false, false>
{
      public:
        typedef typename _TraitsT::char_type _CharT;
        typedef _CharT _StrTransT;
...
template <typename _TraitsT, bool __is_ecma, bool __icase, bool __collate> struct _AnyMatcher;
...
template <typename _TraitsT, bool __icase, bool __collate> struct _CharMatcher {
        typedef _RegexTranslator<_TraitsT, __icase, __collate> _TransT;
        typedef typename _TransT::_CharT _CharT;
...
template <typename _TraitsT, bool __icase, bool __collate> struct _BracketMatcher {
      public:
        typedef _RegexTranslator<_TraitsT, __icase, __collate> _TransT;
        typedef typename _TransT::_CharT _CharT;
      ...
      public:
        ...
        {
                auto __st = _M_traits.lookup_collatename(__s.data(), __s.data() + __s.size());
                if (__st.empty()) {
        ...
        void _M_add_equivalence_class(const _StringT &__s)
        {
                ...
                __st = _M_traits.transform_primary(__st.data(), __st.data() + __st.size());
        ...
        {
                auto __mask =
                        _M_traits.lookup_classname(__s.data(), __s.data() + __s.size(), __icase);
                if (__mask == 0) {
                        __throw_regex_error(regex_constants::error_collate,
      ...
      private:
        ...
        typedef typename std::conditional<_UseCache::value, std::bitset<_S_cache_size>,
                                          _Dummy>::type _CacheT;
