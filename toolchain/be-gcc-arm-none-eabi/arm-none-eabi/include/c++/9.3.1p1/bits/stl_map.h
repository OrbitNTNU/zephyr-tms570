...
template <typename _Key, typename _Tp, typename _Compare, typename _Alloc> class multimap;
...
class map
{
      ...
      public:
        ...
        map(map &&__m,
            const allocator_type &__a) noexcept(is_nothrow_copy_constructible<_Compare>::value &&
        ...
        template <typename, typename> friend class std::_Rb_tree_merge_helper;

        template <typename _C2> void merge(map<_Key, _Tp, _C2, _Alloc> &__source)
        {
        ...
        template <typename _C2> void merge(multimap<_Key, _Tp, _C2, _Alloc> &__source)
        {
                using _Merge_helper = _Rb_tree_merge_helper<map, _C2>;
                _M_t._M_merge_unique(_Merge_helper::_S_get_tree(__source));
        }
...
#define __cpp_lib_map_try_emplace 201411
        ...
        iterator try_emplace(const_iterator __hint, const key_type &__k, _Args &&...__args)
        {
                ...
                if (__true_hint.second) {
                        __i = emplace_hint(iterator(__true_hint.second), std::piecewise_construct,
                                           std::forward_as_tuple(__k),
                                           std::forward_as_tuple(std::forward<_Args>(__args)...));
                } else {
...
#define __cpp_lib_map_insertion 201411
        ...
        iterator insert_or_assign(const_iterator __hint, const key_type &__k, _Obj &&__obj)
        {
                ...
                auto __true_hint = _M_t._M_get_insert_hint_unique_pos(__hint, __k);
                if (__true_hint.second) {
                        return emplace_hint(iterator(__true_hint.second), std::piecewise_construct,
                                            std::forward_as_tuple(__k),
                                            std::forward_as_tuple(std::forward<_Obj>(__obj)));
                }
                __i = iterator(__true_hint.first);
        ...
        auto equal_range(const _Kt &__x) const
                -> decltype(pair<const_iterator, const_iterator>(_M_t._M_equal_range_tr(__x)))
        {
...
template <typename _Key, typename _Val, typename _Cmp1, typename _Alloc, typename _Cmp2>
struct _Rb_tree_merge_helper<_GLIBCXX_STD_C::map<_Key, _Val, _Cmp1, _Alloc>, _Cmp2> {
      private:
        friend class _GLIBCXX_STD_C::map<_Key, _Val, _Cmp1, _Alloc>;

        static auto &_S_get_tree(_GLIBCXX_STD_C::map<_Key, _Val, _Cmp2, _Alloc> &__map)
        {
                return __map._M_t;
        }

        static auto &_S_get_tree(_GLIBCXX_STD_C::multimap<_Key, _Val, _Cmp2, _Alloc> &__map)
        {
                return __map._M_t;
        }
};
