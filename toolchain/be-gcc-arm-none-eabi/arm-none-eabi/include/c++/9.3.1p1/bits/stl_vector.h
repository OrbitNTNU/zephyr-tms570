...
template <typename _Tp, typename _Alloc> struct _Vector_base {
        ...
        struct _Vector_impl : public _Tp_alloc_type, public _Vector_impl_data {
...
#if _GLIBCXX_SANITIZE_STD_ALLOCATOR && _GLIBCXX_SANITIZE_VECTOR
                template <typename = _Tp_alloc_type> struct _Asan {
                ...
                template <typename _Up> struct _Asan<allocator<_Up>> {
                        typedef typename __gnu_cxx::__alloc_traits<_Tp_alloc_type>::size_type
                                size_type;

                        // Adjust ASan annotation for [_M_start, _M_end_of_storage) to
                        // mark end of valid region as __curr instead of __prev.
                        static void _S_adjust(_Vector_impl &__impl, pointer __prev, pointer __curr)
                        {
                        ...
                        {
                                if (__impl._M_start) {
                                        _S_adjust(__impl, __impl._M_finish,
                                                  __impl._M_end_of_storage);
                                }
                        }
                        ...
                        struct _Reinit {
                                ...
                                {
                                        // Mark unused capacity as valid again before deallocating
                                        // it.
...
#define _GLIBCXX_ASAN_ANNOTATE_REINIT                                                              \
        typename _Base::_Vector_impl::template _Asan<>::_Reinit const __attribute__((__unused__))  \
        __reinit_guard(this->_M_impl)
#define _GLIBCXX_ASAN_ANNOTATE_GROW(n)                                                             \
        typename _Base::_Vector_impl::template _Asan<>::_Grow __attribute__((__unused__))          \
        __grow_guard(this->_M_impl, (n))
...
class vector : protected _Vector_base<_Tp, _Alloc>
{
      ...
      private:
        ...
        static pointer _S_relocate(pointer __first, pointer __last, pointer __result,
      ...
      protected:
        ...
        {
                this->_M_impl._M_start = _M_allocate(
      ...
      private:
        ...
        template <typename _Up> _Up *_M_data_ptr(_Up *__ptr) const _GLIBCXX_NOEXCEPT
        {
                return __ptr;
        }
...
#else
        template <typename _Up> _Up *_M_data_ptr(_Up *__ptr) _GLIBCXX_NOEXCEPT
        {
                return __ptr;
        }

        template <typename _Ptr> value_type *_M_data_ptr(_Ptr __ptr)
        {
                return empty() ? (value_type *)0 : __ptr.operator->();
        }

        template <typename _Ptr> const value_type *_M_data_ptr(_Ptr __ptr) const
        {
