...
class path
{
        ...
        using __is_encoded_char =
                __or_<is_same<_Ch, char>,
                      ...
                      is_same<_Ch, char8_t>,
#endif
                      is_same<_Ch, wchar_t>, is_same<_Ch, char16_t>, is_same<_Ch, char32_t>>;

        template <typename _Iter, typename _Iter_traits = std::iterator_traits<_Iter>>
        using __is_path_iter_src = __and_<
                __is_encoded_char<typename _Iter_traits::value_type>,
                std::is_base_of<std::input_iterator_tag, typename _Iter_traits::iterator_category>>;

        template <typename _Iter> static __is_path_iter_src<_Iter> __is_path_src(_Iter, int);
        ...
        static __is_encoded_char<_CharT> __is_path_src(const basic_string_view<_CharT, _Traits> &,
                                                       int);

        template <typename _Unknown> static std::false_type __is_path_src(const _Unknown &, ...);

        template <typename _Tp1, typename _Tp2> struct __constructible_from;
        ...
        using _Path = typename std::enable_if<__and_<__not_<is_same<remove_cv_t<_Tp1>, path>>,
                                                     __not_<is_void<remove_pointer_t<_Tp1>>>,
                                                     __constructible_from<_Tp1, _Tp2>>::value,
                                              path>::type;

        template <typename _Source> static _Source _S_range_begin(_Source __begin)
        {
                return __begin;
        }
        ...
        template <typename _Source> static __null_terminated _S_range_end(_Source)
        {
                return {};
        }

        template <typename _CharT, typename _Traits, typename _Alloc>
        static const _CharT *_S_range_begin(const basic_string<_CharT, _Traits, _Alloc> &__str)
        {
                return __str.data();
        }

        template <typename _CharT, typename _Traits, typename _Alloc>
        static const _CharT *_S_range_end(const basic_string<_CharT, _Traits, _Alloc> &__str)
        {
                return __str.data() + __str.size();
        }

        template <typename _CharT, typename _Traits>
        static const _CharT *_S_range_begin(const basic_string_view<_CharT, _Traits> &__str)
        {
                return __str.data();
        }

        template <typename _CharT, typename _Traits>
        static const _CharT *_S_range_end(const basic_string_view<_CharT, _Traits> &__str)
        {
                return __str.data() + __str.size();
        }

        template <typename _Tp, typename _Iter = decltype(_S_range_begin(std::declval<_Tp>())),
      ...
      public:
        ...
        path(path &&__p)
                ...
                noexcept
#endif
                : _M_pathname(std::move(__p._M_pathname)), _M_cmpts(std::move(__p._M_cmpts))
        {
                __p.clear();
        }

        path(string_type &&__source, format = auto_format) : _M_pathname(std::move(__source))
        {
                _M_split_cmpts();
        }
        ...
        path(_Source const &__source, format = auto_format)
                : _M_pathname(_S_convert(_S_range_begin(__source), _S_range_end(__source)))
        {
                _M_split_cmpts();
        }
        ...
        path(_Source const &__source, const locale &__loc, format = auto_format)
                : _M_pathname(
                          _S_convert_loc(_S_range_begin(__source), _S_range_end(__source), __loc))
        {
                _M_split_cmpts();
        }
        ...
        template <typename _InputIterator, typename _Traits = std::iterator_traits<_InputIterator>,
                  typename _CharT = typename std::remove_cv_t<typename _Traits::value_type>>
        static std::basic_string<_CharT> _S_string_from_iter(_InputIterator __source)
        {
      ...
      private:
        ...
        path(basic_string_view<value_type> __str, _Type __type) : _M_pathname(__str)
        {
        ...
        template <typename _CharT> struct _Cvt;

        static basic_string_view<value_type> _S_convert(value_type *__src, __null_terminated)
        {
                return __src;
        }

        static basic_string_view<value_type> _S_convert(const value_type *__src, __null_terminated)
        {
                return __src;
        }

        static basic_string_view<value_type> _S_convert(value_type *__first, value_type *__last)
        {
        ...
        template <typename _Iter> static string_type _S_convert(_Iter __first, _Iter __last)
        {
                using __value_type = typename std::iterator_traits<_Iter>::value_type;
                return _Cvt<typename remove_cv<__value_type>::type>::_S_convert(__first, __last);
        }

        template <typename _InputIterator>
        static string_type _S_convert(_InputIterator __src, __null_terminated)
        {
        ...
        template <typename _Iter>
        static string_type _S_convert_loc(_Iter __first, _Iter __last, const std::locale &__loc)
        {
        ...
        template <typename _InputIterator>
        static string_type _S_convert_loc(_InputIterator __src, __null_terminated,
                                          const std::locale &__loc)
        {
        ...
        static basic_string<_CharT, _Traits, _Allocator> _S_str_convert(const string_type &,
...
class filesystem_error : public std::system_error
{
      public:
        ...
        filesystem_error(const string &__what_arg, const path &__p1, const path &__p2,
                         error_code __ec);
...
template <typename _InputIterator>
inline auto u8path(_InputIterator __first, _InputIterator __last)
...
/// Create a path from a UTF-8-encoded sequence of char
template <typename _Source>
inline auto u8path(const _Source &__source)
...
template <> struct path::_Cvt<path::value_type> {
        template <typename _Iter> static string_type _S_convert(_Iter __first, _Iter __last)
        {
...
template <> struct path::_Cvt<char8_t> {
        template <typename _Iter> static string_type _S_convert(_Iter __first, _Iter __last)
        {
                return string_type(__first, __last);
        }
...
template <typename _CharT> struct path::_Cvt {
        ...
        {
                return _S_convert(const_cast<const _CharT *>(__f), const_cast<const _CharT *>(__l));
        }
        ...
        static string_type _S_convert(__gnu_cxx::__normal_iterator<_Iter, _Cont> __first,
...
class path::iterator
{
      ...
      private:
        ...
        friend difference_type __path_iter_distance(const iterator &__first, const iterator &__last)
        {
        ...
        iterator(const path *__path, bool __at_end) : _M_path(__path), _M_cur(), _M_at_end(__at_end)
        {
        }
...
template <typename _CharT> inline path::_Path<_CharT *, _CharT *> &path::operator+=(_CharT __x)
{
...
template <typename _CharT, typename _Traits, typename _Allocator>
std::basic_string<_CharT, _Traits, _Allocator> path::_S_str_convert(const string_type &__str,
                                                                    const _Allocator &__a)
{
...
template <typename _CharT, typename _Traits, typename _Allocator>
inline basic_string<_CharT, _Traits, _Allocator> path::string(const _Allocator &__a) const
{
...
inline ptrdiff_t distance(filesystem::path::iterator __first, filesystem::path::iterator __last)
{
