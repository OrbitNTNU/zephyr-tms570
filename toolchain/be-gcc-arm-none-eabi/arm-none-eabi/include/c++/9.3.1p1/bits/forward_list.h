...
struct _Fwd_list_node_base {
        ...
        _Fwd_list_node_base(_Fwd_list_node_base &&__x) noexcept : _M_next(__x._M_next)
        {
                __x._M_next = nullptr;
        }
 ...
 */
template <typename _Tp> struct _Fwd_list_node : public _Fwd_list_node_base {
 ...
 */
template <typename _Tp> struct _Fwd_list_iterator {
        ...
        explicit _Fwd_list_iterator(_Fwd_list_node_base *__n) noexcept : _M_node(__n)
        {
        }
...
template <typename _Tp> struct _Fwd_list_const_iterator {
        ...
        explicit _Fwd_list_const_iterator(const _Fwd_list_node_base *__n) noexcept : _M_node(__n)
        {
        }

        _Fwd_list_const_iterator(const iterator &__iter) noexcept : _M_node(__iter._M_node)
        {
        }
 ...
 */
template <typename _Tp, typename _Alloc> struct _Fwd_list_base {
      protected:
        ...
        struct _Fwd_list_impl : public _Node_alloc_type {
                ...
                _Fwd_list_impl() noexcept(is_nothrow_default_constructible<_Node_alloc_type>::value)
                ...
                _Fwd_list_impl(_Node_alloc_type &&__a) : _Node_alloc_type(std::move(__a)), _M_head()
                {
                }
        };
      ...
      protected:
        ...
        template <typename... _Args> _Node *_M_create_node(_Args &&...__args)
        {
                ...
                __try {
                        ...
                        _Node_alloc_traits::construct(_M_get_Node_allocator(), __node->_M_valptr(),
...
class forward_list : private _Fwd_list_base<_Tp, _Alloc>
{
      ...
      public:
         ...
         */
        explicit forward_list(const _Alloc &__al) noexcept : _Base(_Node_alloc_type(__al))
        {
        }
         ...
         *  @param  __al    An allocator object.
         */
        forward_list(const forward_list &__list, const _Alloc &__al) : _Base(_Node_alloc_type(__al))
        {
      ...
      private:
                ...
                : _Base(std::move(__list), std::move(__al))
        {
                ...
                insert_after(cbefore_begin(), std::__make_move_if_noexcept_iterator(__list.begin()),
                             std::__make_move_if_noexcept_iterator(__list.end()));
        }

        forward_list(forward_list &&__list, _Node_alloc_type &&__al, true_type) noexcept
      ...
      public:
        ...
        forward_list(forward_list &&__list,
                     const _Alloc &__al) noexcept(_Node_alloc_traits::_S_always_equal())
        ...
        forward_list(const forward_list &__list)
                : _Base(_Node_alloc_traits::_S_select_on_copy(__list._M_get_Node_allocator()))
        {
                _M_range_initialize(__list.begin(), __list.end());
        }
         ...
         */
        forward_list &
        operator=(forward_list &&__list) noexcept(_Node_alloc_traits::_S_nothrow_move())
        {
        ...
        {
                return iterator(this->_M_insert_after(__pos, std::forward<_Args>(__args)...));
        }
        ...
        {
                return iterator(this->_M_erase_after(const_cast<_Node_base *>(__pos._M_node)));
        }
         ...
         *  @{
         */
        void splice_after(const_iterator __pos, forward_list &&, const_iterator __before,
...
#define _GLIBCXX_FWDLIST_REMOVE_RETURN_TYPE_TAG __attribute__((__abi_tag__("__cxx20")))
#else
...
#undef _GLIBCXX_FWDLIST_REMOVE_RETURN_TYPE_TAG
      ...
      private:
        ...
        void _M_move_assign(forward_list &&__list, true_type) noexcept
        {
                ...
                std::__alloc_on_move(this->_M_get_Node_allocator(), __list._M_get_Node_allocator());
        }
 ...
 */
template <typename _Tp, typename _Alloc>
inline bool operator<(const forward_list<_Tp, _Alloc> &__lx, const forward_list<_Tp, _Alloc> &__ly)
{
        return std::lexicographical_compare(__lx.cbegin(), __lx.cend(), __ly.cbegin(), __ly.cend());
}
...
template <typename _Tp, typename _Alloc>
inline void swap(forward_list<_Tp, _Alloc> &__lx,
