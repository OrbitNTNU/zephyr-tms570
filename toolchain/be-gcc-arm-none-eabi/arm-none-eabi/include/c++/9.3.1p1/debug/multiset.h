...
class multiset : public __gnu_debug::_Safe_container<multiset<_Key, _Compare, _Allocator>,
                                                     _Allocator, __gnu_debug::_Safe_node_sequence>,
                 public _GLIBCXX_STD_C::multiset<_Key, _Compare, _Allocator>
{
        typedef _GLIBCXX_STD_C::multiset<_Key, _Compare, _Allocator> _Base;
        typedef __gnu_debug::_Safe_container<multiset, _Allocator, __gnu_debug::_Safe_node_sequence>
                _Safe;
      ...
      public:
        ...
        multiset(const multiset &__m, const allocator_type &__a) : _Base(__m, __a)
        {
        }

        multiset(multiset &&__m,
                 const allocator_type &__a) noexcept(noexcept(_Base(std::move(__m._M_base()), __a)))
                : _Safe(std::move(__m._M_safe()), __a), _Base(std::move(__m._M_base()), __a)
        {
        }

        multiset(initializer_list<value_type> __l, const allocator_type &__a) : _Base(__l, __a)
        {
        }
        ...
        template <typename _InputIterator>
        multiset(_InputIterator __first, _InputIterator __last, const _Compare &__comp = _Compare(),
        ...
        template <typename... _Args> iterator emplace_hint(const_iterator __pos, _Args &&...__args)
        {
                ...
                return {_Base::emplace_hint(__pos.base(), std::forward<_Args>(__args)...), this};
        }
#endif
        ...
        {
                std::pair<_Base_iterator, _Base_iterator> __victims = _Base::equal_range(__x);
                size_type __count = 0;
        ...
        iterator erase(const_iterator __first, const_iterator __last)
        {
                ...
                for (_Base_const_iterator __victim = __first.base(); __victim != __last.base();
                     ++__victim) {
        ...
        void swap(multiset &__x) _GLIBCXX_NOEXCEPT_IF(noexcept(declval<_Base &>().swap(__x)))
        {
                _Safe::_M_swap(__x);
                _Base::swap(__x);
        }
        ...
        std::pair<iterator, iterator> equal_range(const key_type &__x)
        {
                ...
                return std::make_pair(iterator(__res.first, this), iterator(__res.second, this));
        }
...
template <typename _InputIterator,
          typename _Compare = less<typename iterator_traits<_InputIterator>::value_type>,
