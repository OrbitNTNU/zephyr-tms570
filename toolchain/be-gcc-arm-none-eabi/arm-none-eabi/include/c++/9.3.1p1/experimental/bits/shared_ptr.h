...
constexpr bool __sp_compatible_v = std::__sp_compatible_with<_Yp *, _Tp *>::value;

template <typename _Tp, typename _Yp>
constexpr bool __sp_is_constructible_v = std::__sp_is_constructible<_Tp, _Yp>::value;
...
template <typename _Tp> class shared_ptr : public __shared_ptr<_Tp>
{
      ...
      private:
        ...
        template <typename _Yp> using _SafeConv = enable_if_t<__sp_is_constructible_v<_Tp, _Yp>>;

        template <typename _Tp1, typename _Res = void>
        using _Compatible = enable_if_t<__sp_compatible_v<_Tp1, _Tp>, _Res>;

        template <typename _Tp1, typename _Del,
                  typename _Ptr = typename unique_ptr<_Tp1, _Del>::pointer, typename _Res = void>
      ...
      public:
        ...
        template <typename _Tp1, typename _Deleter, typename _Alloc, typename = _SafeConv<_Tp1>>
        shared_ptr(_Tp1 *__p, _Deleter __d, _Alloc __a) : _Base_type(__p, __d, __a)
        {
                _M_enable_shared_from_this_with(__p);
        }

        template <typename _Deleter> shared_ptr(nullptr_t __p, _Deleter __d) : _Base_type(__p, __d)
        {
        }
        ...
        shared_ptr(const shared_ptr<_Tp1> &__r, element_type *__p) noexcept : _Base_type(__r, __p)
        {
        }
        ...
        template <typename _Tp1, typename _Del, typename = _UniqCompatible<_Tp1, _Del>>
        ...
        template <typename _Tp1>
        _Compatible<_Tp1, shared_ptr &> operator=(const shared_ptr<_Tp1> &__r) noexcept
        {
        ...
        template <typename _Tp1, typename _Del>
        _UniqCompatible<_Tp1, _Del, shared_ptr &> operator=(unique_ptr<_Tp1, _Del> &&__r)
        {
                _Base_type::operator=(std::move(__r));
                return *this;
        }
      ...
      private:
        ...
        shared_ptr(const weak_ptr<_Tp> &__r, std::nothrow_t) : _Base_type(__r, std::nothrow)
        {
        }
        ...
        using __esft_base_t = decltype(__expt_enable_shared_from_this_base(std::declval<_Yp *>()));

        // Detect an accessible and unambiguous enable_shared_from_this base.
        template <typename _Yp, typename = void> struct __has_esft_base : false_type {
        };

        template <typename _Yp>
        struct __has_esft_base<_Yp, __void_t<__esft_base_t<_Yp>>>
                : __bool_constant<!is_array_v<_Tp>> {
...
template <typename _Tp> inline bool operator==(const shared_ptr<_Tp> &__a, nullptr_t) noexcept
{
        return !__a;
}

template <typename _Tp> inline bool operator==(nullptr_t, const shared_ptr<_Tp> &__a) noexcept
{
        return !__a;
}

template <typename _Tp1, typename _Tp2>
inline bool operator!=(const shared_ptr<_Tp1> &__a, const shared_ptr<_Tp2> &__b) noexcept
{
        return __a.get() != __b.get();
}
...
inline shared_ptr<_Tp> static_pointer_cast(const shared_ptr<_Tp1> &__r) noexcept
{
        using __elem_t = typename shared_ptr<_Tp>::element_type;
...
template <typename _Tp, typename _Tp1>
inline shared_ptr<_Tp> dynamic_pointer_cast(const shared_ptr<_Tp1> &__r) noexcept
{
        using __elem_t = typename shared_ptr<_Tp>::element_type;
...
template <typename _Tp, typename _Tp1>
inline shared_ptr<_Tp> const_pointer_cast(const shared_ptr<_Tp1> &__r) noexcept
{
        using __elem_t = typename shared_ptr<_Tp>::element_type;
...
template <typename _Tp, typename _Tp1>
inline shared_ptr<_Tp> reinterpret_pointer_cast(const shared_ptr<_Tp1> &__r) noexcept
{
        using __elem_t = typename shared_ptr<_Tp>::element_type;
...
template <typename _Tp> class weak_ptr : public __weak_ptr<_Tp>
{
      ...
      public:
        ...
        template <typename _Tp1>
        _Compatible<_Tp1, weak_ptr &> operator=(const shared_ptr<_Tp1> &__r) noexcept
        {
                this->_Base_type::operator=(__r);
                return *this;
        }
...
inline std::basic_ostream<_Ch, _Tr> &operator<<(std::basic_ostream<_Ch, _Tr> &__os,
...
template <typename _Tp> shared_ptr<_Tp> atomic_load(const shared_ptr<_Tp> *__p)
{
...
template <typename _Tp> void atomic_store(shared_ptr<_Tp> *__p, shared_ptr<_Tp> __r)
{
        return std::atomic_store<_Tp>(__p, __r);
}

template <typename _Tp>
shared_ptr<_Tp> atomic_store_explicit(const shared_ptr<_Tp> *__p, shared_ptr<_Tp> __r,
                                      memory_order __mo)
{
        return std::atomic_store_explicit<_Tp>(__p, __r, __mo);
}

template <typename _Tp> void atomic_exchange(shared_ptr<_Tp> *__p, shared_ptr<_Tp> __r)
{
        return std::atomic_exchange<_Tp>(__p, __r);
}

template <typename _Tp>
shared_ptr<_Tp> atomic_exchange_explicit(const shared_ptr<_Tp> *__p, shared_ptr<_Tp> __r,
                                         memory_order __mo)
{
...
template <typename _Tp>
bool atomic_compare_exchange_weak_explicit(shared_ptr<_Tp> *__p, shared_ptr<_Tp> *__v,
                                           shared_ptr<_Tp> __w, memory_order __success,
                                           memory_order __failure)
{
        return std::atomic_compare_exchange_weak_explicit<_Tp>(__p, __v, __w, __success, __failure);
}

template <typename _Tp>
bool atomic_compare_exchange_strong_explicit(shared_ptr<_Tp> *__p, shared_ptr<_Tp> *__v,
                                             shared_ptr<_Tp> __w, memory_order __success,
                                             memory_order __failure)
{
        return std::atomic_compare_exchange_strong_explicit<_Tp>(__p, __v, __w, __success,
                                                                 __failure);
}
