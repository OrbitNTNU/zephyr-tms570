...
template <typename _CompletionToken, typename _Signature, typename> class async_result;

// A type denoted by DEDUCED in the TS.
template <typename _CompletionToken, typename _Signature>
using __deduced_t = typename async_result<decay_t<_CompletionToken>, _Signature, void>::return_type;
...
using __is_value_constructible =
        typename __and_<is_copy_constructible<_Tp>, is_move_constructible<_Tp>,
                        is_constructible<_Tp, _Tp &>, is_constructible<_Tp, const _Tp &&>>::type;
...
template <typename _Tp> struct __sockopt_base {
        ...
        template <typename _Protocol> void *data(const _Protocol &) noexcept
        {
                return std::addressof(_M_value);
        }

        template <typename _Protocol> const void *data(const _Protocol &) const noexcept
        {
                return std::addressof(_M_value);
        }

        template <typename _Protocol> size_t size(const _Protocol &) const noexcept
        {
                return sizeof(_M_value);
        }

        template <typename _Protocol> void resize(const _Protocol &, size_t __s)
        {
...
template <> struct __sockopt_base<bool> : __sockopt_base<int> {
        __sockopt_base() = default;
...
template <typename _Derived, typename _Tp = int> struct __sockopt_crtp : __sockopt_base<_Tp> {
        ...
        template <typename _Protocol> int level(const _Protocol &) const noexcept
        {
                return _Derived::_S_level;
        }

        template <typename _Protocol> int name(const _Protocol &) const noexcept
        {
