...
template <class _Index, class _RealBody> class __parallel_for_body
{
      public:
...
void __parallel_for(_ExecutionPolicy &&, _Index __first, _Index __last, _Fp __f)
{
...
template <class _ExecutionPolicy, class _Value, class _Index, typename _RealBody,
          typename _Reduction>
...
template <class _Index, class _Up, class _Tp, class _Cp, class _Rp> struct __par_trans_red_body {
        alignas(_Tp) char _M_sum_storage[sizeof(_Tp)]; // Holds generalized non-commutative sum when
                                                       // has_sum==true
        ...
        __par_trans_red_body(__par_trans_red_body &__left, tbb::split)
                : _M_brick_reduce(__left._M_brick_reduce), _M_u(__left._M_u),
                  _M_combine(__left._M_combine), _M_has_sum(false)
        {
        }
        ...
        void operator()(const tbb::blocked_range<_Index> &__range)
        {
                ...
                if (!_M_has_sum) {
                        __PSTL_ASSERT_MSG(__range.size() > 1,
                                          "there should be at least 2 elements");
                        new (&_M_sum_storage) _Tp(_M_combine(
                                _M_u(__i), _M_u(__i + 1))); // The condition i+1 < j is provided by
                                                            // the grain size of 3
                        _M_has_sum = true;
                        std::advance(__i, 2);
                        if (__i == __j) {
...
_Tp __parallel_transform_reduce(_ExecutionPolicy &&, _Index __first, _Index __last, _Up __u,
...
class __trans_scan_body
{
      ...
      public:
        __trans_scan_body(_Up __u, _Tp __init, _Cp __combine, _Rp __reduce, _Sp __scan)
                : _M_brick_reduce(__reduce), _M_u(__u), _M_combine(__combine), _M_scan(__scan),
                  _M_has_sum(true)
        {
                new (_M_sum_storage) _Tp(__init);
        }
        ...
        __trans_scan_body(__trans_scan_body &__b, tbb::split)
                : _M_brick_reduce(__b._M_brick_reduce), _M_u(__b._M_u), _M_combine(__b._M_combine),
                  _M_scan(__b._M_scan), _M_has_sum(false)
        {
        }
        ...
        void assign(__trans_scan_body &__b)
        {
                sum() = __b.sum();
        }
};

template <typename _Index> _Index __split(_Index __m)
{
        _Index __k = 1;
        while (2 * __k < __m) {
                __k *= 2;
...
void __upsweep(_Index __i, _Index __m, _Index __tilesize, _Tp *__r, _Index __lastsize, _Rp __reduce,
               _Cp __combine)
{
        if (__m == 1) {
                ...
                tbb::parallel_invoke(
                        ...
                        [=] {
                                __par_backend::__upsweep(__i + __k, __m - __k, __tilesize,
                                                         __r + __k, __lastsize, __reduce,
                                                         __combine);
                        });
                if (__m == 2 * __k) {
...
void __downsweep(_Index __i, _Index __m, _Index __tilesize, _Tp *__r, _Index __lastsize,
                 _Tp __initial, _Cp __combine, _Sp __scan)
{
        if (__m == 1) {
...
// Let i:len denote a counted interval of length n starting at i.  s denotes a generalized-sum
// value. Expected actions of the functors are:
...
// For example, it's useful for allocating a __buffer used by scan but whose size is the sum of all
// reduction values. T must have a trivial constructor and destructor.
template <class _ExecutionPolicy, typename _Index, typename _Tp, typename _Rp, typename _Cp,
          typename _Sp, typename _Ap>
void __parallel_strict_scan(_ExecutionPolicy &&, _Index __n, _Tp __initial, _Rp __reduce,
...
template <class _ExecutionPolicy, class _Index, class _Up, class _Tp, class _Cp, class _Rp,
          class _Sp>
_Tp __parallel_transform_scan(_ExecutionPolicy &&, _Index __n, _Up __u, _Tp __init, _Cp __combine,
                              _Rp __brick_reduce, _Sp __scan)
{
        __trans_scan_body<_Index, _Up, _Tp, _Cp, _Rp, _Sp> __body(__u, __init, __combine,
...
class __merge_task : public tbb::task
{
      ...
      public:
        __merge_task(_RandomAccessIterator1 __xs, _RandomAccessIterator1 __xe,
                ...
                : _M_xs(__xs), _M_xe(__xe), _M_ys(__ys), _M_ye(__ye), _M_zs(__zs), _M_comp(__comp),
                  _M_cleanup(__cleanup), _M_leaf_merge(__leaf_merge)
        {
        }
};
...
template <typename _RandomAccessIterator1, typename _RandomAccessIterator2,
          typename _RandomAccessIterator3, typename __M_Compare, typename _Cleanup,
          typename _LeafMerge>
tbb::task *__merge_task<_RandomAccessIterator1, _RandomAccessIterator2, _RandomAccessIterator3,
                        __M_Compare, _Cleanup, _LeafMerge>::execute()
{
        ...
        if (__n <= __merge_cut_off) {
                ...
                tbb::task *__right = new (tbb::task::allocate_additional_child_of(*parent()))
                        __merge_task(__xm, _M_xe, __ym, _M_ye, __zm, _M_comp, _M_cleanup,
                                     _M_leaf_merge);
...
template <typename _RandomAccessIterator1, typename _RandomAccessIterator2, typename _Compare,
          typename _LeafSort>
class __stable_sort_task : public tbb::task
{
      ...
      public:
        __stable_sort_task(_RandomAccessIterator1 __xs, _RandomAccessIterator1 __xe,
                           _RandomAccessIterator2 __zs, int32_t __inplace, _Compare __comp,
                           _LeafSort __leaf_sort, _SizeType __n)
                : _M_xs(__xs), _M_xe(__xe), _M_zs(__zs), _M_comp(__comp), _M_leaf_sort(__leaf_sort),
                  _M_inplace(__inplace), _M_nsort(__n)
        {
        }
};
...
struct __binary_no_op {
        template <typename _T> void operator()(_T, _T)
        {
        }
};
...
template <class _ExecutionPolicy, typename _RandomAccessIterator, typename _Compare,
          typename _LeafSort>
                ...
                if (__n > __sort_cut_off) {
                        ...
                        task::spawn_root_and_wait(
                                *new (task::allocate_root())
                                        __stable_sort_task<_RandomAccessIterator, _ValueType *,
                                                           _Compare, _LeafSort>(
                                                __xs, __xe, (_ValueType *)__buf.get(), 2, __comp,
                                                __leaf_sort, __nsort));
                        return;
                }
                // serial sort
...
void __parallel_merge(_ExecutionPolicy &&, _RandomAccessIterator1 __xs, _RandomAccessIterator1 __xe,
                      _RandomAccessIterator2 __ys, _RandomAccessIterator2 __ye,
        ...
        if (__n <= __merge_cut_off) {
                ...
                tbb::this_task_arena::isolate([=]() {
                        ...
                        tbb::task::spawn_root_and_wait(*new (tbb::task::allocate_root()) _TaskType(
                                __xs, __xe, __ys, __ye, __zs, __comp,
...
{
        // TODO: a version of tbb::this_task_arena::isolate with variadic arguments pack should be
        // added in the future
        tbb::this_task_arena::isolate(
