...
template <typename _Fp> typename std::result_of<_Fp()>::type __except_handler(_Fp __f)
{
        try {
                return __f();
...
template <typename _Fp> void __invoke_if(std::true_type, _Fp __f)
{
        __f();
}
...
typename std::result_of<_F1()>::type __invoke_if_else(std::true_type, _F1 __f1, _F2 __f2)
{
        return __f1();
}

template <typename _F1, typename _F2>
typename std::result_of<_F2()>::type __invoke_if_else(std::false_type, _F1 __f1, _F2 __f2)
{
        return __f2();
}
...
template <typename _Pred> class __reorder_pred
{
      ...
      public:
        ...
        template <typename _FTp, typename _STp> bool operator()(_FTp &&__a, _STp &&__b)
        {
...
class __pstl_equal
{
      public:
        ...
        template <typename _Xp, typename _Yp> bool operator()(_Xp &&__x, _Yp &&__y) const
        {
...
_ForwardIterator __cmp_iterators_by_values(_ForwardIterator __a, _ForwardIterator __b,
                                           _Compare __comp)
{
