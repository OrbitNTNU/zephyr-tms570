...
_Iterator __simd_walk_1(_Iterator __first, _DifferenceType __n, _Function __f) noexcept
{
        __PSTL_PRAGMA_SIMD
...
_Iterator2 __simd_walk_2(_Iterator1 __first1, _DifferenceType __n, _Iterator2 __first2,
...
template <class _Iterator1, class _DifferenceType, class _Iterator2, class _Iterator3,
          class _Function>
_Iterator3 __simd_walk_3(_Iterator1 __first1, _DifferenceType __n, _Iterator2 __first2,
                         _Iterator3 __first3, _Function __f) noexcept
{
        __PSTL_PRAGMA_SIMD
        for (_DifferenceType __i = 0; __i < __n; ++__i) {
...
_Index __simd_first(_Index __first, _DifferenceType __begin, _DifferenceType __end,
...
std::pair<_Index1, _Index2> __simd_first(_Index1 __first1, _DifferenceType __n, _Index2 __first2,
...
template <class _Index, class _DifferenceType, class _Pred>
_DifferenceType __simd_count(_Index __index, _DifferenceType __n, _Pred __pred) noexcept
{
...
_OutputIterator __simd_assign(_InputIterator __first, _DifferenceType __n, _OutputIterator __result,
...
_OutputIterator __simd_copy_if(_InputIterator __first, _DifferenceType __n,
                               _OutputIterator __result, _UnaryPredicate __pred) noexcept
{
...
_DifferenceType __simd_calc_mask_2(_InputIterator __first, _DifferenceType __n, bool *__mask,
...
_DifferenceType __simd_calc_mask_1(_InputIterator __first, _DifferenceType __n, bool *__mask,
...
template <class _InputIterator, class _DifferenceType, class _OutputIterator, class _Assigner>
void __simd_copy_by_mask(_InputIterator __first, _DifferenceType __n, _OutputIterator __result,
                         bool *__mask, _Assigner __assigner) noexcept
{
        _DifferenceType __cnt = 0;
        __PSTL_PRAGMA_SIMD
        for (_DifferenceType __i = 0; __i < __n; ++__i) {
...
_Index __simd_fill_n(_Index __first, _DifferenceType __n, const _Tp &__value) noexcept
{
        __PSTL_USE_NONTEMPORAL_STORES_IF_ALLOWED
        __PSTL_PRAGMA_SIMD
        for (_DifferenceType __i = 0; __i < __n; ++__i) {
...
_Index __simd_generate_n(_Index __first, _DifferenceType __size, _Generator __g) noexcept
{
        __PSTL_USE_NONTEMPORAL_STORES_IF_ALLOWED
        __PSTL_PRAGMA_SIMD
...
_Index __simd_adjacent_find(_Index __first, _Index __last, _BinaryPredicate __pred,
                            bool __or_semantic) noexcept
{
...
#if __PSTL_EARLYEXIT_PRESENT
        // Some compiler versions fail to compile the following loop when iterators are used.
        ...
        while (__last - __first >= __block_size) {
                ...
                if (__found) {
                        ...
                        return __first + __i; // As far as found is true a __result (__lane[__i] is
                                              // true) is guaranteed
                }
                __first += __block_size;
        }
...
template <class _InputIterator, class _Size, class _OutputIterator, class _UnaryOperation,
...
__simd_scan(_InputIterator __first, _Size __n, _OutputIterator __result, _UnaryOperation __unary_op,
            _Tp __init, _BinaryOperation, /*Inclusive*/ std::false_type)
{
...
// As soon as we cannot call __binary_op in "combiner" we create a wrapper over _Tp to encapsulate
// __binary_op
...
typename std::enable_if<!is_arithmetic_plus<_Tp, _BinaryOperation>::value,
                        std::pair<_OutputIterator, _Tp>>::type
__simd_scan(_InputIterator __first, _Size __n, _OutputIterator __result, _UnaryOperation __unary_op,
            _Tp __init, _BinaryOperation __binary_op, /*Inclusive*/ std::false_type)
{
...
typename std::enable_if<is_arithmetic_plus<_Tp, _BinaryOperation>::value,
                        std::pair<_OutputIterator, _Tp>>::type
__simd_scan(_InputIterator __first, _Size __n, _OutputIterator __result, _UnaryOperation __unary_op,
            _Tp __init, _BinaryOperation, /*Inclusive*/ std::true_type)
{
        __PSTL_PRAGMA_SIMD_SCAN(+ : __init)
        for (_Size __i = 0; __i < __n; ++__i) {
                __init += __unary_op(__first[__i]);
...
// DefaultConstructible. complexity [violation] - We will have at most (__n-1 + number_of_lanes)
// comparisons instead of at most __n-1.
template <typename _ForwardIterator, typename _Size, typename _Compare>
_ForwardIterator __simd_min_element(_ForwardIterator __first, _Size __n, _Compare __comp) noexcept
{
        if (__n == 0) {
                return __first;
        }
...
// [restriction] - std::iterator_traits<_ForwardIterator>::value_type should be
// DefaultConstructible. complexity [violation] - We will have at most (2*(__n-1) +
// 4*number_of_lanes) comparisons instead of at most [1.5*(__n-1)].
template <typename _ForwardIterator, typename _Size, typename _Compare>
...
__simd_minmax_element(_ForwardIterator __first, _Size __n, _Compare __comp) noexcept
{
        ...
        struct _ComplexType {
                ...
                _ComplexType()
                        : __min_val{}, __max_val{}, __min_ind{}, __max_ind{}, __minmax_comp(nullptr)
                {
                }
                ...
                void operator()(const _ComplexType &__obj)
                {
                        ...
                        if ((*__minmax_comp)(__obj.__min_val, __min_val)) {
                                ...
                                __min_val = __obj.__min_val;
                                __min_ind = (__min_ind - __obj.__min_ind < 0) ? __min_ind
                                                                              : __obj.__min_ind;
                        }
                        ...
                        if ((*__minmax_comp)(__max_val, __obj.__max_val)) {
                                ...
                                __max_val = __obj.__max_val;
                                __max_ind = (__max_ind - __obj.__max_ind < 0) ? __obj.__max_ind
                                                                              : __max_ind;
                        }
                }
        };
...
_ForwardIterator1 __simd_find_first_of(_ForwardIterator1 __first, _ForwardIterator1 __last,
                                       ...
                                       _BinaryPredicate __pred) noexcept
{
        ...
        // Common case
        // If first sequence larger than second then we'll run simd_first with parameters of first
        // sequence. Otherwise, vice versa.
        if (__n1 < __n2) {
...
_RandomAccessIterator __simd_remove_if(_RandomAccessIterator __first, _DifferenceType __n,
                                       _UnaryPredicate __pred) noexcept
{
        ...
        // if we have in sequence only one element that pred(__current[1]) != false we can exit the
        // function
        if (__n < 2) {
                return __current;
        }
