...
template <typename _RAIter> struct _DRandomShufflingGlobalData {
...
/** @brief Local data for a thread participating in
    __gnu_parallel::__parallel_random_shuffle().
  */
template <typename _RAIter, typename _RandomNumberGenerator> struct _DRSSorterPU {
...
void __parallel_random_shuffle_drs_pu(_DRSSorterPU<_RAIter, _RandomNumberGenerator> *__pus)
{
        ...
        _DifferenceType __length = (__sd->_M_starts[__iam + 1] - __sd->_M_starts[__iam]);
 ...
 */
template <typename _RAIter, typename _RandomNumberGenerator>
void __parallel_random_shuffle_drs(_RAIter __begin, _RAIter __end,
                                   ...
                                   _ThreadIndex __num_threads, _RandomNumberGenerator &__rng)
{
...
#if _GLIBCXX_RANDOM_SHUFFLE_CONSIDER_TLB
                // 2 TLB entries needed per bin.
                __num_bins = std::min(static_cast<_DifferenceType>(__s.TLB_size / 2), __num_bins);
#endif
                __num_bins = __round_up_to_pow2(__num_bins);
#if _GLIBCXX_RANDOM_SHUFFLE_CONSIDER_L1
        }
        ...
        __num_bins = __round_up_to_pow2(std::max<_BinIndex>(__num_threads, __num_bins));
...
#pragma omp single
                {
                        ...
                        for (_ThreadIndex __i = 0; __i < __num_threads; ++__i) {
                                ...
                                __bin_cursor += (__i < __bin_split ? (__bin_chunk_length + 1)
                                                                   : __bin_chunk_length);
 ...
 */
template <typename _RAIter, typename _RandomNumberGenerator>
inline void __parallel_random_shuffle(_RAIter __begin, _RAIter __end,
