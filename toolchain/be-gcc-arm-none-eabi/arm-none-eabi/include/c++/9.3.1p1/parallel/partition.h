...
__parallel_partition(_RAIter __begin, _RAIter __end, _Predicate __pred, _ThreadIndex __num_threads)
{
        ...
        _GLIBCXX_VOLATILE _DifferenceType __left = 0, __right = __n - 1, __dist = __n,
                                          __leftover_left, __leftover_right, __leftnew, __rightnew;
...
#pragma omp barrier
                        ...
                        if (__thread_left <= __thread_left_border &&
                            __thread_left_border < __leftnew) {
                                ...
                                std::swap_ranges(
                                        __begin + __thread_left_border - (__chunk_size - 1),
                                        __begin + __thread_left_border + 1, __begin + __swapstart);
                        }
        ...
        while (static_cast<_SequenceIndex>(__end - __begin) >= __minimum_length) {
                ...
                if ((__split_pos1 + 1 - __begin) < (__n >> 7) ||
                    (__end - __split_pos1) < (__n >> 7)) {
                        ...
                        __split_pos2 = __gnu_sequential::partition(__split_pos1 + 1, __end, __pred);
                } else {
                        // Only skip the pivot.
