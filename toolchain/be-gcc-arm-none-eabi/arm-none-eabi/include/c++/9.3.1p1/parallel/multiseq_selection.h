...
class _Lexicographic : public std::binary_function<std::pair<_T1, _T2>, std::pair<_T1, _T2>, bool>
{
      private:
        _Compare &_M_comp;
...
void multiseq_partition(
        _RanSeqs __begin_seqs, _RanSeqs __end_seqs, _RankType __rank, _RankIterator __begin_offsets,
        _Compare __comp = std::less<typename std::iterator_traits<typename std::iterator_traits<
                _RanSeqs>::value_type::first_type>::value_type>()) // std::less<_Tp>
{
        ...
        // including padding).
        _DifferenceType __m = std::distance(__begin_seqs, __end_seqs), __nn = 0, __nmax, __n, __r;
...
#define __S(__i) (__begin_seqs[__i].first)
        ...
        _SeqNumber __j;
        for (__j = 0; __j < __localrank && ((__n + 1) <= __ns[__sample[__j].second]); ++__j) {
                ...
                if (__skew > 0) {
                        ...
                        for (; __skew != 0 && !__pq.empty(); --__skew) {
                                ...
                                if (__b[__source] < __ns[__source]) {
                                        __pq.push(std::make_pair(__S(__source)[__b[__source]],
                                                                 __source));
                                }
                        }
                } else if (__skew < 0) {
                        ...
                        for (; __skew != 0; ++__skew) {
                                ...
                                if (__a[__source] > 0) {
                                        __pq.push(std::make_pair(__S(__source)[__a[__source] - 1],
                                                                 __source));
                                }
                        }
                }
...
_Tp multiseq_selection(_RanSeqs __begin_seqs, _RanSeqs __end_seqs, _RankType __rank,
...
#define __S(__i) (__begin_seqs[__i].first)
        ...
        __gnu_sequential::sort(__sample.begin(), __sample.end(), __lcomp, sequential_tag());
