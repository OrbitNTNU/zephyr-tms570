...
void __shrink_and_double(std::vector<_IIter> &__os_starts, size_t &__count_to_two,
                         size_t &__range_length, const bool __make_twice)
{
        ++__count_to_two;
...
{
        for (typename std::vector<_IIter>::size_type __i = 0; __i <= (__os_starts.size() / 2);
             ++__i) {
...
size_t list_partition(const _IIter __begin, const _IIter __end, _IIter *__starts, size_t *__lengths,
