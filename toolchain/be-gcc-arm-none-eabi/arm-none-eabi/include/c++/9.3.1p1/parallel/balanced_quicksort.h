...
template <typename _RAIter> struct _QSBThreadLocal {
...
__qsb_divide(_RAIter __begin, _RAIter __end, _Compare __comp, _ThreadIndex __num_threads)
{
        ...
        _RAIter __pivot_pos = __median_of_three_iterators(__begin, __begin + (__end - __begin) / 2,
                                                          __end - 1, __comp);
        ...
        __gnu_parallel::__binder2nd<_Compare, _ValueType, _ValueType, bool> __pred(__comp,
                                                                                   *__pivot_pos);
...
template <typename _RAIter, typename _Compare>
void __qsb_conquer(_QSBThreadLocal<_RAIter> **__tls, _RAIter __begin, _RAIter __end,
                   _Compare __comp, _ThreadIndex __iam, _ThreadIndex __num_threads,
                   bool __parent_wait)
{
        typedef std::iterator_traits<_RAIter> _TraitsType;
        typedef typename _TraitsType::value_type _ValueType;
        ...
        _ThreadIndex __num_threads_leftside = std::max<_ThreadIndex>(
                1, std::min<_ThreadIndex>(__num_threads - 1, __split_pos * __num_threads / __n));
 ...
 */
template <typename _RAIter, typename _Compare>
void __qsb_local_sort_with_helping(_QSBThreadLocal<_RAIter> **__tls, _Compare &__comp,
                        ...
                        if ((__split_pos1 + 1 - __begin) < (__n >> 7) ||
                            (__end - __split_pos1) < (__n >> 7)) {
                                ...
                                __gnu_parallel::__unary_negate<
                                        __gnu_parallel::__binder1st<_Compare, _ValueType,
                                                                    _ValueType, bool>,
                                        _ValueType>
                                ...
                                __split_pos2 = __gnu_sequential::partition(__split_pos1 + 1, __end,
                                                                           __pred);
                        } else {
                                // Only skip the pivot.
                         ...
                         _ThreadIndex __num_threads)
{
        ...
        _DifferenceType __queue_size = (__num_threads * (_ThreadIndex)(__rd_log2(__n) + 1));
