...
{
      public:
        __list2slist_stack_info(const __list2slist_info &__o) : __list2slist_info(__o)
        {
        }
};

class __trace_list_to_slist : public __trace_base<__list2slist_info, __list2slist_stack_info>
{
      public:
        ...
        __trace_list_to_slist() : __trace_base<__list2slist_info, __list2slist_stack_info>()
        {
                __id = "list-to-slist";
        }

        void __destruct(__list2slist_info *__obj_info)
        {
...
inline void __trace_list_to_slist_free()
{
        delete _GLIBCXX_PROFILE_DATA(_S_list_to_slist);
}

inline void __trace_list_to_slist_report(FILE *__f, __warning_vector_t &__warnings)
{
        __trace_report(_GLIBCXX_PROFILE_DATA(_S_list_to_slist), __f, __warnings);
}

inline __list2slist_info *__trace_list_to_slist_construct()
{
        if (!__profcxx_init()) {
                return 0;
        }
...
inline void __trace_list_to_slist_rewind(__list2slist_info *__obj_info)
{
        if (!__obj_info) {
                return;
        }
...
inline void __trace_list_to_slist_operation(__list2slist_info *__obj_info)
{
        if (!__obj_info) {
                return;
        }
...
inline void __trace_list_to_slist_destruct(__list2slist_info *__obj_info)
{
        if (!__obj_info) {
                return;
        }
