...
struct __warning_data {
        ...
        __warning_data() : __magnitude(0.0), __context(0), __warning_id(0)
        {
        }

        __warning_data(float __m, __stack_t __c, const char *__id, const std::string &__msg)
                : __magnitude(__m), __context(__c), __warning_id(__id), __warning_message(__msg)
        {
        }
...
inline std::size_t __stack_max_depth()
{
        return _GLIBCXX_PROFILE_DATA(_S_max_stack_depth);
}

inline std::size_t __max_mem()
{
        return _GLIBCXX_PROFILE_DATA(_S_max_mem);
}

/** @brief Base class for all trace producers.  */
template <typename __object_info, typename __stack_info> class __trace_base
{
      public:
        // Do not pick the initial size too large, as we don't know which
        // diagnostics are more active.
        __trace_base()
                : __objects_byte_size(0), __stack_table(10000), __stack_table_byte_size(0), __id(0)
        {
        }

        ~__trace_base()
        {
                for (typename __stack_table_t::iterator __it = __stack_table.begin();
                     __it != __stack_table.end(); ++__it) {
                        delete __it->first;
                }
        }
      ...
      private:
        ...
        typedef _GLIBCXX_IMPL_UNORDERED_MAP<__stack_t, __stack_info, __stack_hash, __stack_hash>
                __stack_table_t;
...
__object_info *__trace_base<__object_info, __stack_info>::__add_object(__stack_t __stack)
{
...
template <typename __object_info, typename __stack_info>
void __trace_base<__object_info, __stack_info>::__retire_object(__object_info *__obj_info)
{
        if (!__obj_info) {
                return;
        }
        ...
        typename __stack_table_t::iterator __stack_it = __stack_table.find(__stack);
...
template <typename __object_info, typename __stack_info>
void __trace_base<__object_info, __stack_info>::__collect_warnings(__warning_vector_t &__warnings)
{
        for (typename __stack_table_t::iterator __it = __stack_table.begin();
             __it != __stack_table.end(); ++__it) {
                __warnings.push_back(__warning_data(__it->second.__magnitude(), __it->first, __id,
                                                    __it->second.__advice()));
        }
}

template <typename __object_info, typename __stack_info>
inline void __trace_report(__trace_base<__object_info, __stack_info> *__cont, FILE *__f,
...
inline std::size_t __env_to_size_t(const char *__env_var, std::size_t __default_value)
{
        ...
        if (__env_value) {
                ...
                if (errno || __converted_value < 0) {
                        std::fprintf(stderr, "Bad value for environment variable '%s'.\n",
                                     __env_var);
                        std::abort();
                } else {
...
inline FILE *__open_output_file(const char *__extension)
{
        ...
        __builtin_memcpy(__file_name, _GLIBCXX_PROFILE_DATA(_S_trace_file_name), __root_len);
        *(__file_name + __root_len) = '.';
        __builtin_memcpy(__file_name + __root_len + 1, __extension, __ext_len + 1);
...
struct __warn {
        ...
        void operator()(const __warning_data &__info)
        {
                ...
                std::fprintf(__file, ": improvement = %d", __log_magnitude(__info.__magnitude));
...
inline void __report()
{
        ...
        std::size_t __cutoff =
                std::min(_GLIBCXX_PROFILE_DATA(_S_max_warn_count), __warnings.size());
        __top_n(__warnings, __top_warnings, __cutoff);

        FILE *__warn_file = __open_output_file("txt");
        __for_each(__top_warnings.begin(), __top_warnings.end(), __warn(__warn_file));
        std::fclose(__warn_file);
}

inline void __report_and_free()
{
        __report();
...
{
        char *__env_max_warn_count_str = std::getenv(_GLIBCXX_PROFILE_MAX_WARN_COUNT_ENV_VAR);
...
inline void __write_cost_factors()
{
        ...
        __for_each(_GLIBCXX_PROFILE_DATA(__cost_factors)->begin(),
                   _GLIBCXX_PROFILE_DATA(__cost_factors)->end(), __cost_factor_writer(__file));
        std::fclose(__file);
}
