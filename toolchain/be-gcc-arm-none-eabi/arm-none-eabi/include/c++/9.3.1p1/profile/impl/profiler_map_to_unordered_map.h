...
{
inline int __log2(std::size_t __size)
{
        for (int __bit_count = sizeof(std::size_t) - 1; __bit_count >= 0; --__bit_count) {
...
class __map2umap_info : public __object_info_base
{
      public:
        __map2umap_info(__stack_t __stack)
                : __object_info_base(__stack), _M_insert(0), _M_erase(0), _M_find(0), _M_iterate(0),
                  _M_umap_cost(0.0), _M_map_cost(0.0)
        {
        }
        ...
        {
                std::fprintf(__f, "%Zu %Zu %Zu %Zu %.0f %.0f\n", _M_insert, _M_erase, _M_find,
                             _M_iterate, _M_map_cost, _M_umap_cost);
        }
        ...
        void __record_erase(std::size_t __size, std::size_t __count)
        {
                ...
                if (__count) {
                        ...
                        _M_umap_cost +=
                                (__count * _GLIBCXX_PROFILE_DATA(__umap_erase_cost_factor).__value);
                }
        }
        ...
        {
                _M_umap_cost +=
                        (_M_iterate * _GLIBCXX_PROFILE_DATA(__umap_iterate_cost_factor).__value);
                _M_map_cost +=
                        (_M_iterate * _GLIBCXX_PROFILE_DATA(__map_iterate_cost_factor).__value);
        }
...
{
      public:
        __map2umap_stack_info(const __map2umap_info &__o) : __map2umap_info(__o)
        {
        }
};

/** @brief Map-to-unordered_map instrumentation producer.  */
class __trace_map2umap : public __trace_base<__map2umap_info, __map2umap_stack_info>
{
      public:
        __trace_map2umap() : __trace_base<__map2umap_info, __map2umap_stack_info>()
        {
...
inline void __trace_map_to_unordered_map_free()
{
        delete _GLIBCXX_PROFILE_DATA(_S_map2umap);
}

inline void __trace_map_to_unordered_map_report(FILE *__f, __warning_vector_t &__warnings)
{
        __trace_report(_GLIBCXX_PROFILE_DATA(_S_map2umap), __f, __warnings);
}

inline __map2umap_info *__trace_map_to_unordered_map_construct()
{
        if (!__profcxx_init()) {
                return 0;
        }
...
inline void __trace_map_to_unordered_map_insert(__map2umap_info *__info, std::size_t __size,
                                                std::size_t __count)
{
        if (!__info) {
                return;
...
inline void __trace_map_to_unordered_map_erase(__map2umap_info *__info, std::size_t __size,
                                               std::size_t __count)
{
        if (!__info) {
                return;
...
inline void __trace_map_to_unordered_map_find(__map2umap_info *__info, std::size_t __size)
{
        if (!__info) {
                return;
        }
...
inline void __trace_map_to_unordered_map_iterate(__map2umap_info *__info, int)
{
        if (!__info) {
                return;
        }
...
inline void __trace_map_to_unordered_map_invalidate(__map2umap_info *__info)
{
        if (!__info) {
                return;
        }
...
inline void __trace_map_to_unordered_map_destruct(__map2umap_info *__info)
{
        if (!__info) {
                return;
        }
