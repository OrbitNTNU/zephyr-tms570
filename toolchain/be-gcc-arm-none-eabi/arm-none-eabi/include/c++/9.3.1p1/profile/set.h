...
class set : public _GLIBCXX_STD_C::set<_Key, _Compare, _Allocator>,
            public _Ordered_profile<set<_Key, _Compare, _Allocator>>
{
      ...
      public:
        ...
        set(const set &__x, const _Allocator &__a) : _Base(__x, __a)
        {
        }
        ...
        set(initializer_list<value_type> __l, const _Allocator &__a) : _Base(__l, __a)
        {
        }
        ...
        {
                __profcxx_map2umap_insert(this->_M_map2umap_info, this->size(), 1);
                std::pair<_Base_iterator, bool> __base_ret = _Base::insert(std::move(__x));
                return std::make_pair(iterator(__base_ret.first, this), __base_ret.second);
        }
#endif
