 ...
 */
template <typename _ToType> struct _Caster {
        typedef typename _ToType::element_type *type;
};

template <typename _ToType> struct _Caster<_ToType *> {
...
{
        return _ToType(reinterpret_cast<typename _Caster<_ToType>::type>(__arg.get()));
}
