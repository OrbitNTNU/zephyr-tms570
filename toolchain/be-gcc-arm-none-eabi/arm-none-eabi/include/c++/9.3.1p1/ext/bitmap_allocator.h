 ...
 */
template <typename _Tp> class __mini_vector
{
...
template <typename _Tp> void __mini_vector<_Tp>::insert(iterator __pos, const_reference __x)
{
        if (this->_M_space_left()) {
...
template <typename _Tp> void __mini_vector<_Tp>::erase(iterator __pos) throw()
{
...
template <typename _Tp> struct __mv_iter_traits {
        typedef typename _Tp::value_type value_type;
        typedef typename _Tp::difference_type difference_type;
};

template <typename _Tp> struct __mv_iter_traits<_Tp *> {
        typedef _Tp value_type;
        typedef ptrdiff_t difference_type;
};
...
class _Functor_Ref : public std::unary_function<typename _Functor::argument_type,
 ...
 */
// _Tp should be a pointer type.
template <typename _Tp> class _Bitmap_counter
{
        typedef typename __detail::__mini_vector<typename std::pair<_Tp, _Tp>> _BPVector;
      ...
      public:
        ...
        void _M_reset(long __index = -1) throw()
        {
                ...
                _M_curr_bmap = reinterpret_cast<size_t *>(_M_vbp[_M_curr_index].first) - 1;
        ...
        _Index_type _M_offset() const throw()
        {
                return size_t(bits_per_block) *
                       ((reinterpret_cast<size_t *>(this->_M_base()) - _M_curr_bmap) - 1);
        }
 ...
 */
inline void __bit_allocate(size_t *__pbmap, size_t __pos) throw()
{
 ...
 *  corresponding bit in the bit-map.
 */
inline void __bit_free(size_t *__pbmap, size_t __pos) throw()
{
        size_t __mask = 1 << __pos;
        *__pbmap |= __mask;
}
 ...
 */
inline size_t _Bit_scan_forward(size_t __num)
{
...
class free_list
{
      ...
      private:
        struct _LT_pointer_compare {
                bool operator()(const size_t *__pui, const size_t __cui) const throw()
                {
                        return *__pui < __cui;
                }
        };
        ...
        void _M_validate(size_t *__addr) throw()
        {
                ...
                iterator __temp = __detail::__lower_bound(__free_list.begin(), __free_list.end(),
      ...
      public:
         ...
         */
        inline void _M_insert(size_t *__addr) throw()
        {
#if defined __GTHREADS
         ...
         */
        size_t *_M_get(size_t __sz) _GLIBCXX_THROW(std::bad_alloc);
 ...
 * @ingroup allocators
 */
template <typename _Tp> class bitmap_allocator : private free_list
{
      public:
        typedef size_t size_type;
        typedef ptrdiff_t difference_type;
      ...
      private:
        template <size_t _BSize, size_t _AlignSize> struct aligned_size {
                enum {
        ...
        template <typename _Predicate> static _BPiter _S_find(_Predicate __p)
        {
        ...
        void _S_refill_pool() _GLIBCXX_THROW(std::bad_alloc)
        {
                ...
                *__temp = 0;
                ...
                _Block_pair __bp =
                        std::make_pair(reinterpret_cast<_Alloc_block *>(__temp + __num_bitmaps),
      ...
      public:
        ...
        void _M_deallocate_single_object(pointer __p) throw()
        {
                ...
                size_t *__puse_count = reinterpret_cast<size_t *>(_S_mem_blocks[__diff].first) -
...
template <typename _Tp>
size_t bitmap_allocator<_Tp>::_S_block_size = 2 * size_t(__detail::bits_per_block);

template <typename _Tp>
typename bitmap_allocator<_Tp>::_BPVector::size_type bitmap_allocator<_Tp>::_S_last_dealloc_index =
        0;
...
#if defined __GTHREADS
template <typename _Tp> typename bitmap_allocator<_Tp>::__mutex_type bitmap_allocator<_Tp>::_S_mut;
#endif
