 ...
 * @ingroup allocators
 */
template <typename _Tp> class __pool_alloc : private __pool_alloc_base
{
      private:
      ...
      public:
        ...
        template <typename _Tp1> __pool_alloc(const __pool_alloc<_Tp1> &) _GLIBCXX_USE_NOEXCEPT
        {
        }
...
template <typename _Tp> inline bool operator==(const __pool_alloc<_Tp> &, const __pool_alloc<_Tp> &)
{
        return true;
}
...
template <typename _Tp> void __pool_alloc<_Tp>::deallocate(pointer __p, size_type __n)
{
