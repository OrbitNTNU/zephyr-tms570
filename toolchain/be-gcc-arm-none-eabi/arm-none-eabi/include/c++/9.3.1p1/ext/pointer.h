 ...
 */
template <typename _Tp> class _Std_pointer_impl
{
      public:
        ...
        inline void set(element_type *__arg)
        {
                _M_value = __arg;
        }
 ...
 */
template <typename _Tp> class _Relative_pointer_impl
{
      public:
        typedef _Tp element_type;
      ...
      private:
        ...
        typedef __gnu_cxx::__conditional_type<(sizeof(unsigned long) >= sizeof(void *)),
 ...
 */
template <typename _Tp> class _Relative_pointer_impl<const _Tp>
{
      public:
        ...
        const _Tp *get() const
        {
                if (_M_diff == 1) {
                        ...
                        return reinterpret_cast<const _Tp *>(reinterpret_cast<_UIntPtrType>(this) +
                                                             _M_diff);
                }
        }
...
template <typename _Tp> struct _Reference_type {
        typedef _Tp &reference;
};

template <> struct _Reference_type<void> {
        typedef _Invalid_type &reference;
};

template <> struct _Reference_type<const void> {
        typedef const _Invalid_type &reference;
};

template <> struct _Reference_type<volatile void> {
        typedef volatile _Invalid_type &reference;
};

template <> struct _Reference_type<volatile const void> {
 ...
 */
template <typename _Tp> struct _Unqualified_type {
        typedef _Tp type;
};

template <typename _Tp> struct _Unqualified_type<const _Tp> {
        typedef _Tp type;
};
 ...
 */
template <typename _Storage_policy> class _Pointer_adapter : public _Storage_policy
{
      public:
      ...
      public:
        ...
        // Pointer differences
        inline friend std::ptrdiff_t operator-(const _Pointer_adapter &__lhs, element_type *__rhs)
        {
                return (__lhs.get() - __rhs);
        }

        inline friend std::ptrdiff_t operator-(element_type *__lhs, const _Pointer_adapter &__rhs)
        {
                return (__lhs - __rhs.get());
        }

        template <typename _Up>
        inline friend std::ptrdiff_t operator-(const _Pointer_adapter &__lhs, _Up *__rhs)
        {
                return (__lhs.get() - __rhs);
        }

        template <typename _Up>
        inline friend std::ptrdiff_t operator-(_Up *__lhs, const _Pointer_adapter &__rhs)
        {
                return (__lhs - __rhs.get());
        }
...
#define _CXX_POINTER_ARITH_OPERATOR_SET(INT_TYPE)                                                  \
        inline friend _Pointer_adapter operator+(const _Pointer_adapter &__lhs, INT_TYPE __offset) \
        {                                                                                          \
                return _Pointer_adapter(__lhs.get() + __offset);                                   \
        }                                                                                          \
                                                                                                   \
        inline friend _Pointer_adapter operator+(INT_TYPE __offset, const _Pointer_adapter &__rhs) \
        {                                                                                          \
...
#define _GCC_CXX_POINTER_COMPARISON_OPERATION_SET(OPERATOR)                                        \
        ...
        inline bool operator OPERATOR(const _Pointer_adapter<_Tp1> &__lhs, _Tp2 __rhs)             \
        {                                                                                          \
                return __lhs.get() OPERATOR __rhs;                                                 \
        }                                                                                          \
                                                                                                   \
        template <typename _Tp1, typename _Tp2>                                                    \
        inline bool operator OPERATOR(_Tp1 __lhs, const _Pointer_adapter<_Tp2> &__rhs)             \
        {                                                                                          \
...
template <typename _Tp> inline bool operator==(const _Pointer_adapter<_Tp> &__lhs, int __rhs)
{
        return __lhs.get() == reinterpret_cast<void *>(__rhs);
}

template <typename _Tp> inline bool operator==(int __lhs, const _Pointer_adapter<_Tp> &__rhs)
{
...
inline bool operator==(const _Pointer_adapter<_Tp> &__lhs, const _Pointer_adapter<_Tp> &__rhs)
{
