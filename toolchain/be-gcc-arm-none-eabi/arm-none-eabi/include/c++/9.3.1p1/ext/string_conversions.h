...
_Ret __stoa(_TRet (*__convf)(const _CharT *, _CharT **, _Base...), const char *__name,
            const _CharT *__str, std::size_t *__idx, _Base... __base)
{
        _Ret __ret;
        ...
        struct _Range_chk {
                ...
                static bool _S_chk(_TRet __val, std::true_type) // only called when _Ret is int
                {
        ...
        } else if (errno == ERANGE || _Range_chk::_S_chk(__tmp, std::is_same<_Ret, int>{})) {
...
_String __to_xstring(int (*__convf)(_CharT *, std::size_t, const _CharT *, __builtin_va_list),
