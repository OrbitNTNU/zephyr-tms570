...
inline tree find_type_usage(tree t, bool (*pred)(const_tree))
{
        enum tree_code code;
        if (pred(t)) {
                return t;
        ...
        if (code == POINTER_TYPE || code == REFERENCE_TYPE || code == PARM_DECL ||
            code == OFFSET_TYPE || code == FUNCTION_TYPE || code == METHOD_TYPE ||
            code == ARRAY_TYPE) {
