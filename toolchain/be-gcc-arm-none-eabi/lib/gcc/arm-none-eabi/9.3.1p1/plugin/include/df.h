...
#define DF_REF_REAL_REG(REF)                                                                       \
        (GET_CODE((REF)->base.reg) == SUBREG ? SUBREG_REG((REF)->base.reg) : ((REF)->base.reg))
#define DF_REF_REGNO(REF) ((REF)->base.regno)
#define DF_REF_REAL_LOC(REF)                                                                       \
        (GET_CODE(*((REF)->regular_ref.loc)) == SUBREG ? &SUBREG_REG(*((REF)->regular_ref.loc))    \
                                                       : ((REF)->regular_ref.loc))
#define DF_REF_REG(REF) ((REF)->base.reg)
#define DF_REF_LOC(REF) (DF_REF_CLASS(REF) == DF_REF_REGULAR ? (REF)->regular_ref.loc : NULL)
#define DF_REF_BB(REF)                                                                             \
        (DF_REF_IS_ARTIFICIAL(REF) ? (REF)->artificial_ref.bb : BLOCK_FOR_INSN(DF_REF_INSN(REF)))
...
#define DF_REF_REG_MEM_P(REF)       (DF_REF_REG_MEM_STORE_P(REF) || DF_REF_REG_MEM_LOAD_P(REF))
...
#define DF_INSN_UID_SAFE_GET(UID) (((unsigned)(UID) < DF_INSN_SIZE()) ? DF_INSN_UID_GET(UID) : NULL)
...
#define FOR_EACH_ARTIFICIAL_USE(ITER, BB_INDEX)                                                    \
        for (ITER = df_get_artificial_uses(BB_INDEX); ITER; ITER = DF_REF_NEXT_LOC(ITER))

#define FOR_EACH_ARTIFICIAL_DEF(ITER, BB_INDEX)                                                    \
        for (ITER = df_get_artificial_defs(BB_INDEX); ITER; ITER = DF_REF_NEXT_LOC(ITER))
...
extern void df_simple_dataflow(enum df_flow_dir, df_init_function, df_confluence_function_0,
                               df_confluence_function_n, df_transfer_function, bitmap, int *, int);
...
extern bool can_move_insns_across(rtx_insn *, rtx_insn *, rtx_insn *, rtx_insn *, basic_block,
                                  regset, regset, rtx_insn **);
...
static inline struct df_scan_bb_info *df_scan_get_bb_info(unsigned int index)
{
...
static inline struct df_rd_bb_info *df_rd_get_bb_info(unsigned int index)
{
...
static inline struct df_lr_bb_info *df_lr_get_bb_info(unsigned int index)
{
...
static inline struct df_md_bb_info *df_md_get_bb_info(unsigned int index)
{
...
static inline struct df_live_bb_info *df_live_get_bb_info(unsigned int index)
{
...
static inline struct df_word_lr_bb_info *df_word_lr_get_bb_info(unsigned int index)
{
...
static inline struct df_mir_bb_info *df_mir_get_bb_info(unsigned int index)
{
...
static inline bitmap df_get_live_out(basic_block bb)
{
...
static inline bitmap df_get_live_in(basic_block bb)
{
        gcc_checking_assert(df_lr);
...
static inline df_ref df_get_artificial_defs(unsigned int bb_index)
{
...
static inline df_ref df_get_artificial_uses(unsigned int bb_index)
{
...
static inline df_ref df_single_def(const df_insn_info *info)
{
...
static inline df_ref df_single_use(const df_insn_info *info)
{
