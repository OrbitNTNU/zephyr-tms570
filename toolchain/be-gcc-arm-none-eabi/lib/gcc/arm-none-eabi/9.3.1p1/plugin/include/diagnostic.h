...
typedef void (*diagnostic_finalizer_fn)(diagnostic_context *, diagnostic_info *, diagnostic_t);
...
static inline void diagnostic_inhibit_notes(diagnostic_context *context)
{
...
#define diagnostic_report_warnings_p(DC, LOC)                                                      \
        (!(DC)->dc_inhibit_warnings && !(in_system_header_at(LOC) && !(DC)->dc_warn_system_headers))

/* Override the option index to be used for reporting a
   diagnostic.  */

static inline void diagnostic_override_option_index(diagnostic_info *info, int optidx)
{
...
extern void diagnostic_show_locus(diagnostic_context *, rich_location *richloc,
                                  diagnostic_t diagnostic_kind);

/* Force diagnostics controlled by OPTIDX to be kind KIND.  */
extern diagnostic_t diagnostic_classify_diagnostic(diagnostic_context *, int /* optidx */,
...
extern bool diagnostic_report_diagnostic(diagnostic_context *, diagnostic_info *);
#ifdef ATTRIBUTE_GCC_DIAG
extern void diagnostic_set_info(diagnostic_info *, const char *, va_list *, rich_location *,
                                diagnostic_t) ATTRIBUTE_GCC_DIAG(2, 0);
extern void diagnostic_set_info_translated(diagnostic_info *, const char *, va_list *,
                                           rich_location *, diagnostic_t) ATTRIBUTE_GCC_DIAG(2, 0);
extern void diagnostic_append_note(diagnostic_context *, location_t, const char *, ...)
        ATTRIBUTE_GCC_DIAG(3, 4);
#endif
extern char *diagnostic_build_prefix(diagnostic_context *, const diagnostic_info *);
void default_diagnostic_starter(diagnostic_context *, diagnostic_info *);
void default_diagnostic_start_span_fn(diagnostic_context *, expanded_location);
void default_diagnostic_finalizer(diagnostic_context *, diagnostic_info *, diagnostic_t);
...
static inline location_t diagnostic_location(const diagnostic_info *diagnostic, int which = 0)
{
...
static inline unsigned int diagnostic_num_locations(const diagnostic_info *diagnostic)
{
...
static inline expanded_location diagnostic_expand_location(const diagnostic_info *diagnostic,
                                                           int which = 0)
{
...
static inline bool diagnostic_same_line(const diagnostic_context *context, expanded_location s1,
...
extern void diagnostic_output_format_init(diagnostic_context *, enum diagnostics_output_format);
