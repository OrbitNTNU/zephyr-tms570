...
static inline void bitmap_check_index(const_sbitmap map, int index)
{
...
static inline void bitmap_check_sizes(const_sbitmap a, const_sbitmap b)
{
        gcc_checking_assert(a->n_bits == b->n_bits);
}

/* Test if bit number bitno in the bitmap is set.  */
static inline SBITMAP_ELT_TYPE bitmap_bit_p(const_sbitmap map, int bitno)
{
...
static inline void bitmap_set_bit(sbitmap map, int bitno)
{
        bitmap_check_index(map, bitno);

        map->elms[bitno / SBITMAP_ELT_BITS] |= (SBITMAP_ELT_TYPE)1 << (bitno) % SBITMAP_ELT_BITS;
}

/* Reset bit number BITNO in the sbitmap MAP.  */

static inline void bitmap_clear_bit(sbitmap map, int bitno)
{
        bitmap_check_index(map, bitno);
...
static inline void bmp_iter_set_init(sbitmap_iterator *i, const_sbitmap bmp, unsigned int min,
                                     unsigned *bit_no ATTRIBUTE_UNUSED)
{
        ...
        if (i->word_num >= i->size) {
                ...
                i->word = (i->ptr[i->word_num] >> (i->bit_num % (unsigned int)SBITMAP_ELT_BITS));
        }
}
...
static inline bool bmp_iter_set(sbitmap_iterator *i, unsigned int *n)
{
...
static inline void bmp_iter_next(sbitmap_iterator *i, unsigned *bit_no ATTRIBUTE_UNUSED)
{
        i->word >>= 1;
        i->bit_num++;
}
...
extern void dump_bitmap_vector(FILE *, const char *, const char *, sbitmap *, int);
...
extern bool bitmap_ior_and_compl(sbitmap, const_sbitmap, const_sbitmap, const_sbitmap);
extern void bitmap_and_compl(sbitmap, const_sbitmap, const_sbitmap);
extern void bitmap_not(sbitmap, const_sbitmap);
extern bool bitmap_or_and(sbitmap, const_sbitmap, const_sbitmap, const_sbitmap);
extern bool bitmap_and_or(sbitmap, const_sbitmap, const_sbitmap, const_sbitmap);
...
{
      public:
        explicit auto_sbitmap(unsigned int size) : m_bitmap(sbitmap_alloc(size))
        {
        }
        ~auto_sbitmap()
        {
