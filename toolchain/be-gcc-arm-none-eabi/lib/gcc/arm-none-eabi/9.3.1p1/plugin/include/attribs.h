...
/* The backbone of lookup_attribute().  ATTR_LEN is the string length
   ...
   for size.  */
extern tree private_lookup_attribute(const char *attr_name, size_t attr_len, tree list);

extern unsigned decls_mismatched_attributes(tree, tree, tree, const char *const[],
                                            pretty_printer *);
...
static inline tree canonicalize_attr_name(tree attr_name)
{
...
static inline bool cmp_attribs(const char *attr1, size_t attr1_len, const char *attr2,
                               size_t attr2_len)
{
...
static inline bool cmp_attribs(const char *attr1, const char *attr2)
{
...
static inline bool is_attribute_p(const char *attr_name, const_tree ident)
{
        return cmp_attribs(attr_name, strlen(attr_name), IDENTIFIER_POINTER(ident),
...
static inline tree lookup_attribute(const char *attr_name, tree list)
{
...
static inline tree lookup_attribute_by_prefix(const char *attr_name, tree list)
{
        gcc_checking_assert(attr_name[0] != '_');
        /* In most cases, list is NULL_TREE.  */
        if (list == NULL_TREE) {
