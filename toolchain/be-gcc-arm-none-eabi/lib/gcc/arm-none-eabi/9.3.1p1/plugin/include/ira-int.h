...
static inline rtx allocno_emit_reg(ira_allocno_t a)
{
...
static inline void minmax_set_iter_init(minmax_set_iterator *i, IRA_INT_TYPE *vec, int min, int max)
{
        i->vec = vec;
        i->word_num = 0;
...
/* Return TRUE if we have more allocnos to visit, in which case *N is
   ...
   FALSE.  */
static inline bool minmax_set_iter_cond(minmax_set_iterator *i, int *n)
{
...
static inline void minmax_set_iter_next(minmax_set_iterator *i)
{
        i->word >>= 1;
        i->bit_num++;
}
...
extern void ira_traverse_loop_tree(bool, ira_loop_tree_node_t, void (*)(ira_loop_tree_node_t),
...
extern ira_copy_t ira_create_copy(ira_allocno_t, ira_allocno_t, int, bool, rtx_insn *,
                                  ira_loop_tree_node_t);
extern ira_copy_t ira_add_allocno_copy(ira_allocno_t, ira_allocno_t, int, bool, rtx_insn *,
                                       ira_loop_tree_node_t);
...
static inline bool ira_equiv_no_lvalue_p(int regno)
{
...
static inline void ira_init_register_move_cost_if_necessary(machine_mode mode)
{
...
static inline void ira_allocno_iter_init(ira_allocno_iterator *i)
{
        i->n = 0;
}
...
static inline bool ira_allocno_iter_cond(ira_allocno_iterator *i, ira_allocno_t *a)
{
        int n;
...
/* Initialize the iterator I.  */
static inline void ira_object_iter_init(ira_object_iterator *i)
{
        i->n = 0;
}
...
static inline bool ira_object_iter_cond(ira_object_iterator *i, ira_object_t *obj)
{
        int n;
...
/* Initialize the iterator I.  */
static inline void ira_allocno_object_iter_init(ira_allocno_object_iterator *i)
{
        i->n = 0;
}
...
/* Return TRUE if we have more objects to visit in allocno A, in which
   ...
   FALSE.  */
static inline bool ira_allocno_object_iter_cond(ira_allocno_object_iterator *i, ira_allocno_t a,
                                                ira_object_t *o)
{
        int n = i->n++;
...
/* Initialize the iterator I.  */
static inline void ira_pref_iter_init(ira_pref_iterator *i)
{
        i->n = 0;
}
...
static inline bool ira_pref_iter_cond(ira_pref_iterator *i, ira_pref_t *pref)
{
        int n;
...
/* Loop over all prefs.  In each iteration, P is set to the next
   ...
   the prefs.  */
#define FOR_EACH_PREF(P, ITER) for (ira_pref_iter_init(&(ITER)); ira_pref_iter_cond(&(ITER), &(P));)
...
/* Initialize the iterator I.  */
static inline void ira_copy_iter_init(ira_copy_iterator *i)
{
        i->n = 0;
}
...
static inline bool ira_copy_iter_cond(ira_copy_iterator *i, ira_copy_t *cp)
{
        int n;
...
/* Loop over all copies.  In each iteration, C is set to the next
   ...
   the copies.  */
#define FOR_EACH_COPY(C, ITER) for (ira_copy_iter_init(&(ITER)); ira_copy_iter_cond(&(ITER), &(C));)
...
static inline void ira_object_conflict_iter_init(ira_object_conflict_iterator *i, ira_object_t obj)
{
        ...
        if (i->conflict_vec_p) {
                ...
                if (OBJECT_MIN(obj) > OBJECT_MAX(obj)) {
                        ...
                        i->size = ((OBJECT_MAX(obj) - OBJECT_MIN(obj) + IRA_INT_BITS) /
                                   IRA_INT_BITS) *
                                  sizeof(IRA_INT_TYPE);
                }
                i->bit_num = 0;
...
/* Return TRUE if we have more conflicting allocnos to visit, in which
   ...
   FALSE.  */
static inline bool ira_object_conflict_iter_cond(ira_object_conflict_iterator *i,
                                                 ira_object_t *pobj)
{
        ira_object_t obj;
...
/* The function returns TRUE if at least one hard register from ones
   ...
   HARD_REGSET.  */
static inline bool ira_hard_reg_set_intersection_p(int hard_regno, machine_mode mode,
...
static inline int hard_reg_set_size(HARD_REG_SET set)
{
        int i, size;
...
/* The function returns TRUE if hard registers starting with
   ...
   HARD_REGSET.  */
static inline bool ira_hard_reg_in_set_p(int hard_regno, machine_mode mode,
                                         HARD_REG_SET hard_regset)
{
        int i;
...
static inline void ira_allocate_and_set_costs(int **vec, reg_class_t aclass, int val)
{
        int i, *reg_costs;
        int len;
...
static inline void ira_allocate_and_copy_costs(int **vec, enum reg_class aclass, int *src)
{
        int len;
...
/* Allocate cost vector *VEC for hard registers of ACLASS and add
   values of vector SRC into the vector if it is necessary */
static inline void ira_allocate_and_accumulate_costs(int **vec, enum reg_class aclass, int *src)
{
        int i, len;
...
/* Allocate cost vector *VEC for hard registers of ACLASS and copy
   ...
   SRC is null).  */
static inline void ira_allocate_and_set_or_copy_costs(int **vec, enum reg_class aclass, int val,
                                                      int *src)
{
        int i, *reg_costs;
        int len;
