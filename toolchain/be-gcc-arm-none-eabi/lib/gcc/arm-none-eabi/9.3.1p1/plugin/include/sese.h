...
static inline basic_block get_entry_bb(sese_l &s)
{
        return s.entry->dest;
}
...
static inline basic_block get_exit_bb(sese_l &s)
{
        return s.exit->src;
}

/* Returns the index of V where ELEM can be found. -1 Otherwise.  */
template <typename T> int vec_find(const vec<T> &v, const T &elem)
{
        int i;
        T t;
...
static inline unsigned sese_nb_params(sese_info_p region)
{
...
static inline bool bb_in_region(const_basic_block bb, const_basic_block entry,
...
static inline bool bb_in_sese_p(basic_block bb, const sese_l &r)
{
...
static inline bool stmt_in_sese_p(gimple *stmt, const sese_l &r)
{
...
static inline bool defined_in_sese_p(tree name, const sese_l &r)
{
...
static inline bool loop_in_sese_p(struct loop *loop, const sese_l &region)
{
        return (bb_in_sese_p(loop->header, region) && bb_in_sese_p(loop->latch, region));
}
...
static inline unsigned int sese_loop_depth(const sese_l &region, loop_p loop)
{
...
static inline edge if_region_entry(ifsese if_region)
{
        return if_region->region->region.entry;
}

static inline edge if_region_exit(ifsese if_region)
{
        return if_region->region->region.exit;
}

static inline basic_block if_region_get_condition_block(ifsese if_region)
{
...
static inline struct loop *gbb_loop(gimple_poly_bb_p gbb)
{
...
static inline loop_p gbb_loop_at_index(gimple_poly_bb_p gbb, sese_l &region, int index)
{
...
static inline int nb_common_loops(sese_l &region, gimple_poly_bb_p gbb1, gimple_poly_bb_p gbb2)
{
