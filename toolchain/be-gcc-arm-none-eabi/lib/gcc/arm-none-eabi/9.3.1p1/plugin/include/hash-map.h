...
template <typename KeyId, typename Value, typename Traits> class GTY((user)) hash_map
{
        ...
        struct hash_entry {
              ...
              private:
                ...
                static void pch_nx_helper(int, gt_pointer_operator, void *)
                {
                }

                static void pch_nx_helper(unsigned int, gt_pointer_operator, void *)
                {
                }

                static void pch_nx_helper(bool, gt_pointer_operator, void *)
                {
                }
      ...
      public:
        ...
        explicit hash_map(const hash_map &h, bool ggc = false,
                          bool gather_mem_stats = GATHER_STATISTICS CXX_MEM_STAT_INFO)
                : m_table(h.m_table, ggc, gather_mem_stats, HASH_MAP_ORIGIN PASS_MEM_STAT)
        {
        }
        ...
        template <typename Arg, bool (*f)(const typename Traits::key_type &, const Value &, Arg)>
        ...
        template <typename Arg, bool (*f)(const typename Traits::key_type &, Value *, Arg)>
        void traverse(Arg a) const
        {
                for (typename hash_table<hash_entry>::iterator iter = m_table.begin();
                     iter != m_table.end(); ++iter) {
        ...
        class iterator
        {
              public:
                ...
                struct reference_pair {
                        ...
                        template <typename K, typename V> operator std::pair<K, V>() const
                        {
...
template <typename K, typename V, typename H> static inline void gt_ggc_mx(hash_map<K, V, H> *h)
{
        gt_ggc_mx(&h->m_table);
}

template <typename K, typename V, typename H> static inline void gt_pch_nx(hash_map<K, V, H> *h)
{
        gt_pch_nx(&h->m_table);
}

template <typename K, typename V, typename H>
static inline void gt_cleare_cache(hash_map<K, V, H> *h)
{
        if (h) {
...
template <typename K, typename V, typename H>
static inline void gt_pch_nx(hash_map<K, V, H> *h, gt_pointer_operator op, void *cookie)
{
