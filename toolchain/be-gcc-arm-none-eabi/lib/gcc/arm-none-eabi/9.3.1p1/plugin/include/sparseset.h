...
static inline void sparseset_clear(sparseset s)
{
        s->members = 0;
        s->iterating = false;
}
...
static inline SPARSESET_ELT_TYPE sparseset_cardinality(sparseset s)
{
        return s->members;
}
...
static inline SPARSESET_ELT_TYPE sparseset_size(sparseset s)
{
        return s->size;
}
...
static inline bool sparseset_bit_p(sparseset s, SPARSESET_ELT_TYPE e)
{
...
static inline void sparseset_insert_bit(sparseset s, SPARSESET_ELT_TYPE e, SPARSESET_ELT_TYPE idx)
{
        s->sparse[e] = idx;
        s->dense[idx] = e;
}
...
static inline void sparseset_set_bit(sparseset s, SPARSESET_ELT_TYPE e)
{
...
static inline SPARSESET_ELT_TYPE sparseset_pop(sparseset s)
{
...
static inline void sparseset_iter_init(sparseset s)
{
        s->iter = 0;
        s->iter_inc = 1;
        s->iterating = true;
...
static inline bool sparseset_iter_p(sparseset s)
{
...
static inline SPARSESET_ELT_TYPE sparseset_iter_elm(sparseset s)
{
        return s->dense[s->iter];
}

static inline void sparseset_iter_next(sparseset s)
{
        s->iter += s->iter_inc;
        s->iter_inc = 1;
}
...
#define EXECUTE_IF_SET_IN_SPARSESET(SPARSESET, ITER)                                               \
        for (sparseset_iter_init(SPARSESET);                                                       \
             sparseset_iter_p(SPARSESET) && (((ITER) = sparseset_iter_elm(SPARSESET)) || 1);       \
