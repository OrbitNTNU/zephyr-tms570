...
#define VECTORIZABLE_CYCLE_DEF(D)                                                                  \
        (((D) == vect_reduction_def) || ((D) == vect_double_reduction_def) ||                      \
         ((D) == vect_nested_cycle))
...
typedef hash_map<tree_operand_hash, innermost_loop_behavior *> vec_base_alignments;
...
struct vec_lower_bound {
        ...
        vec_lower_bound(tree e, bool u, poly_uint64 m) : expr(e), unsigned_p(u), min_value(m)
        {
        }
...
template <> template <> inline bool is_a_helper<_loop_vec_info *>::test(vec_info *i)
{
        return i->kind == vec_info::loop;
}

template <> template <> inline bool is_a_helper<_bb_vec_info *>::test(vec_info *i)
{
...
#define LOOP_REQUIRES_VERSIONING_FOR_ALIAS(L)                                                      \
        ((L)->comp_alias_ddrs.length() > 0 || (L)->check_unequal_addrs.length() > 0 ||             \
...
#define LOOP_REQUIRES_VERSIONING(L)                                                                \
        (LOOP_REQUIRES_VERSIONING_FOR_ALIGNMENT(L) || LOOP_REQUIRES_VERSIONING_FOR_ALIAS(L) ||     \
         LOOP_REQUIRES_VERSIONING_FOR_NITERS(L) || LOOP_REQUIRES_VERSIONING_FOR_SIMD_IF_COND(L))
...
static inline loop_vec_info loop_vec_info_for_loop(struct loop *loop)
{
...
static inline bb_vec_info vec_info_for_bb(basic_block bb)
{
...
inline loop_vec_info STMT_VINFO_LOOP_VINFO(stmt_vec_info stmt_vinfo)
{
...
inline bb_vec_info STMT_VINFO_BB_VINFO(stmt_vec_info stmt_vinfo)
{
...
static inline bool nested_in_vect_loop_p(struct loop *loop, stmt_vec_info stmt_info)
{
...
static inline bool is_pattern_stmt_p(stmt_vec_info stmt_info)
{
...
inline stmt_vec_info vect_orig_stmt(stmt_vec_info stmt_info)
{
...
inline stmt_vec_info vect_stmt_to_vectorize(stmt_vec_info stmt_info)
{
...
static inline bool is_loop_header_bb_p(basic_block bb)
{
...
static inline int vect_pow2(int x)
{
        int i, res = 1;
...
static inline int builtin_vectorization_cost(enum vect_cost_for_stmt type_of_cost, tree vectype,
                                             int misalign)
{
        return targetm.vectorize.builtin_vectorization_cost(type_of_cost, vectype, misalign);
}

/* Get cost by calling cost target builtin.  */

static inline int vect_get_stmt_cost(enum vect_cost_for_stmt type_of_cost)
{
...
static inline void *init_cost(struct loop *loop_info)
{
        return targetm.vectorize.init_cost(loop_info);
}

extern void dump_stmt_cost(FILE *, void *, int, enum vect_cost_for_stmt, stmt_vec_info, int,
                           unsigned, enum vect_cost_model_location);

/* Alias targetm.vectorize.add_stmt_cost.  */

static inline unsigned add_stmt_cost(void *data, int count, enum vect_cost_for_stmt kind,
                                     stmt_vec_info stmt_info, int misalign,
                                     enum vect_cost_model_location where)
{
        unsigned cost =
                targetm.vectorize.add_stmt_cost(data, count, kind, stmt_info, misalign, where);
        if (dump_file && (dump_flags & TDF_DETAILS)) {
                dump_stmt_cost(dump_file, data, count, kind, stmt_info, misalign, cost, where);
        }
        return cost;
}
...
static inline void finish_cost(void *data, unsigned *prologue_cost, unsigned *body_cost,
...
static inline void destroy_cost_data(void *data)
{
        targetm.vectorize.destroy_cost_data(data);
}

inline void add_stmt_costs(void *data, stmt_vector_for_cost *cost_vec)
{
        stmt_info_for_cost *cost;
        unsigned i;
        FOR_EACH_VEC_ELT(*cost_vec, i, cost)
        add_stmt_cost(data, cost->count, cost->kind, cost->stmt_info, cost->misalign, cost->where);
}
...
inline void set_dr_misalignment(dr_vec_info *dr_info, int val)
{
        dr_info->misalignment = val;
}

inline int dr_misalignment(dr_vec_info *dr_info)
{
...
static inline bool aligned_access_p(dr_vec_info *dr_info)
{
...
static inline bool known_alignment_for_access_p(dr_vec_info *dr_info)
{
...
static inline unsigned int vect_known_alignment_in_bytes(dr_vec_info *dr_info)
{
...
static inline innermost_loop_behavior *vect_dr_behavior(dr_vec_info *dr_info)
{
        stmt_vec_info stmt_info = dr_info->stmt;
        loop_vec_info loop_vinfo = STMT_VINFO_LOOP_VINFO(stmt_info);
        if (loop_vinfo == NULL || !nested_in_vect_loop_p(LOOP_VINFO_LOOP(loop_vinfo), stmt_info)) {
...
static inline bool unlimited_cost_model(loop_p loop)
{
...
static inline bool vect_use_loop_mask_for_alignment_p(loop_vec_info loop_vinfo)
{
...
static inline unsigned int vect_get_num_vectors(poly_uint64 nunits, tree vectype)
{
...
static inline unsigned int vect_get_num_copies(loop_vec_info loop_vinfo, tree vectype)
{
...
static inline void vect_update_max_nunits(poly_uint64 *max_nunits, poly_uint64 nunits)
{
...
static inline void vect_update_max_nunits(poly_uint64 *max_nunits, tree vectype)
{
...
static inline unsigned int vect_vf_for_cost(loop_vec_info loop_vinfo)
{
...
static inline unsigned int vect_nunits_for_cost(tree vec_type)
{
...
static inline unsigned HOST_WIDE_INT vect_max_vf(loop_vec_info loop_vinfo)
{
...
inline unsigned int vect_get_scalar_dr_size(dr_vec_info *dr_info)
{
...
extern void vect_set_loop_condition(struct loop *, loop_vec_info, tree, tree, tree, bool);
extern bool slpeel_can_duplicate_loop_p(const struct loop *, const_edge);
struct loop *slpeel_tree_duplicate_loop_to_edge_cfg(struct loop *, struct loop *, edge);
struct loop *vect_loop_versioning(loop_vec_info, unsigned int, bool, poly_uint64);
extern struct loop *vect_do_peeling(loop_vec_info, tree, tree, tree *, tree *, tree *, int, bool,
                                    bool);
...
extern bool vect_is_simple_use(tree, vec_info *, enum vect_def_type *, stmt_vec_info * = NULL,
                               gimple ** = NULL);
extern bool vect_is_simple_use(tree, vec_info *, enum vect_def_type *, tree *,
                               stmt_vec_info * = NULL, gimple ** = NULL);
extern bool supportable_widening_operation(enum tree_code, stmt_vec_info, tree, tree,
                                           enum tree_code *, enum tree_code *, int *, vec<tree> *);
extern bool supportable_narrowing_operation(enum tree_code, tree, tree, enum tree_code *, int *,
                                            vec<tree> *);
extern unsigned record_stmt_cost(stmt_vector_for_cost *, int, enum vect_cost_for_stmt,
                                 stmt_vec_info, int, enum vect_cost_model_location);
extern stmt_vec_info vect_finish_replace_stmt(stmt_vec_info, gimple *);
extern stmt_vec_info vect_finish_stmt_generation(stmt_vec_info, gimple *, gimple_stmt_iterator *);
...
extern void vect_get_vec_defs(tree, tree, stmt_vec_info, vec<tree> *, vec<tree> *, slp_tree);
extern void vect_get_vec_defs_for_stmt_copy(vec_info *, vec<tree> *, vec<tree> *);
extern tree vect_init_vector(stmt_vec_info, tree, tree, gimple_stmt_iterator *);
extern tree vect_get_vec_def_for_stmt_copy(vec_info *, tree);
extern bool vect_transform_stmt(stmt_vec_info, gimple_stmt_iterator *, slp_tree, slp_instance);
extern void vect_remove_stores(stmt_vec_info);
extern opt_result vect_analyze_stmt(stmt_vec_info, bool *, slp_tree, slp_instance,
                                    stmt_vector_for_cost *);
extern bool vectorizable_condition(stmt_vec_info, gimple_stmt_iterator *, stmt_vec_info *, bool,
                                   slp_tree, stmt_vector_for_cost *);
extern bool vectorizable_shift(stmt_vec_info, gimple_stmt_iterator *, stmt_vec_info *, slp_tree,
                               stmt_vector_for_cost *);
extern void vect_get_load_cost(stmt_vec_info, int, bool, unsigned int *, unsigned int *,
                               stmt_vector_for_cost *, stmt_vector_for_cost *, bool);
extern void vect_get_store_cost(stmt_vec_info, int, unsigned int *, stmt_vector_for_cost *);
...
extern opt_result vect_get_vector_types_for_stmt(stmt_vec_info, tree *, tree *);
extern opt_tree vect_get_mask_type_for_stmt(stmt_vec_info);

/* In tree-vect-data-refs.c.  */
extern bool vect_can_force_dr_alignment_p(const_tree, poly_uint64);
extern enum dr_alignment_support vect_supportable_dr_alignment(dr_vec_info *, bool);
extern tree vect_get_smallest_scalar_type(stmt_vec_info, HOST_WIDE_INT *, HOST_WIDE_INT *);
...
extern bool vect_gather_scatter_fn_p(bool, bool, tree, tree, unsigned int, signop, int,
                                     internal_fn *, tree *);
extern bool vect_check_gather_scatter(stmt_vec_info, loop_vec_info, gather_scatter_info *);
extern opt_result vect_find_stmt_data_reference(loop_p, gimple *, vec<data_reference_p> *);
extern opt_result vect_analyze_data_refs(vec_info *, poly_uint64 *);
extern void vect_record_base_alignments(vec_info *);
extern tree vect_create_data_ref_ptr(stmt_vec_info, tree, struct loop *, tree, tree *,
                                     gimple_stmt_iterator *, gimple **, bool, tree = NULL_TREE,
                                     tree = NULL_TREE);
extern tree bump_vector_ptr(tree, gimple *, gimple_stmt_iterator *, stmt_vec_info, tree);
...
extern void vect_permute_store_chain(vec<tree>, unsigned int, stmt_vec_info, gimple_stmt_iterator *,
                                     vec<tree> *);
extern tree vect_setup_realignment(stmt_vec_info, gimple_stmt_iterator *, tree *,
                                   enum dr_alignment_support, tree, struct loop **);
extern void vect_transform_grouped_load(stmt_vec_info, vec<tree>, int, gimple_stmt_iterator *);
extern void vect_record_grouped_load_vectors(stmt_vec_info, vec<tree>);
extern tree vect_get_new_vect_var(tree, enum vect_var_kind, const char *);
extern tree vect_get_new_ssa_name(tree, enum vect_var_kind, const char * = NULL);
extern tree vect_create_addr_base_for_vector_ref(stmt_vec_info, gimple_seq *, tree,
                                                 tree = NULL_TREE);

/* In tree-vect-loop.c.  */
/* FORNOW: Used in tree-parloops.c.  */
extern stmt_vec_info vect_force_simple_reduction(loop_vec_info, stmt_vec_info, bool *, bool);
/* Used in gimple-loop-interchange.c.  */
extern bool check_reduction_path(dump_user_location_t, loop_p, gphi *, tree, enum tree_code);
/* Drive for loop analysis stage.  */
extern opt_loop_vec_info vect_analyze_loop(struct loop *, loop_vec_info, vec_info_shared *);
extern tree vect_build_loop_niters(loop_vec_info, bool * = NULL);
extern void vect_gen_vector_loop_niters(loop_vec_info, tree, tree *, tree *, bool);
extern tree vect_halve_mask_nunits(tree);
extern tree vect_double_mask_nunits(tree);
extern void vect_record_loop_mask(loop_vec_info, vec_loop_masks *, unsigned int, tree);
extern tree vect_get_loop_mask(gimple_stmt_iterator *, vec_loop_masks *, unsigned int, tree,
                               unsigned int);

/* Drive for loop transformation stage.  */
extern struct loop *vect_transform_loop(loop_vec_info);
extern opt_loop_vec_info vect_analyze_loop_form(struct loop *, vec_info_shared *);
extern bool vectorizable_live_operation(stmt_vec_info, gimple_stmt_iterator *, slp_tree, int,
                                        stmt_vec_info *, stmt_vector_for_cost *);
extern bool vectorizable_reduction(stmt_vec_info, gimple_stmt_iterator *, stmt_vec_info *, slp_tree,
                                   slp_instance, stmt_vector_for_cost *);
extern bool vectorizable_induction(stmt_vec_info, gimple_stmt_iterator *, stmt_vec_info *, slp_tree,
                                   stmt_vector_for_cost *);
extern tree get_initial_def_for_reduction(stmt_vec_info, tree, tree *);
extern bool vect_worthwhile_without_simd_p(vec_info *, tree_code);
extern int vect_get_known_peeling_cost(loop_vec_info, int, int *, stmt_vector_for_cost *,
...
extern bool vect_transform_slp_perm_load(slp_tree, vec<tree>, gimple_stmt_iterator *, poly_uint64,
...
extern bool can_duplicate_and_interleave_p(unsigned int, machine_mode, unsigned int * = NULL,
                                           tree * = NULL, tree * = NULL);
extern void duplicate_and_interleave(gimple_seq *, tree, vec<tree>, unsigned int, vec<tree> &);
