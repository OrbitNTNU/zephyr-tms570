...
/* spellcheck.c  */
extern edit_distance_t get_edit_distance(const char *s, int len_s, const char *t, int len_t);

extern edit_distance_t get_edit_distance(const char *s, const char *t);

extern const char *find_closest_string(const char *target,
...
template <typename TYPE> struct edit_distance_traits {
};

/* Specialization of edit_distance_traits for C-style strings.  */

template <> struct edit_distance_traits<const char *> {
...
extern edit_distance_t get_edit_distance_cutoff(size_t goal_len, size_t candidate_len);
...
template <typename GOAL_TYPE, typename CANDIDATE_TYPE> class best_match
{
      public:
        ...
        best_match(GOAL_TYPE goal, edit_distance_t best_distance_so_far = MAX_EDIT_DISTANCE)
                : m_goal(goal_traits::get_string(goal)), m_goal_len(goal_traits::get_length(goal)),
                  m_best_candidate(NULL), m_best_distance(best_distance_so_far)
        {
        }
        ...
        void consider(candidate_t candidate)
        {
                ...
                edit_distance_t dist = get_edit_distance(
                        m_goal, m_goal_len, candidate_traits::get_string(candidate), candidate_len);
        ...
        void set_best_so_far(CANDIDATE_TYPE best_candidate, edit_distance_t best_distance,
