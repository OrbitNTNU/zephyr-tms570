...
extern rtx lra_create_new_reg_with_unique_value(machine_mode, rtx, enum reg_class, const char *);
...
extern void lra_process_new_insns(rtx_insn *, rtx_insn *, rtx_insn *, const char *);
...
extern lra_live_range_t lra_merge_live_ranges(lra_live_range_t, lra_live_range_t);
...
extern rtx lra_eliminate_regs_1(rtx_insn *, rtx, machine_mode, bool, bool, poly_int64, bool);
...
/* Return the hard register which given pseudo REGNO assigned to.
   ...
   allocation yet.  */
static inline int lra_get_regno_hard_regno(int regno)
{
        resize_reg_info();
...
static void inline lra_change_class(int regno, enum reg_class new_class, const char *title,
                                    bool nl_p)
{
        lra_assert(regno >= FIRST_PSEUDO_REGISTER);
        if (lra_dump_file != NULL) {
                fprintf(lra_dump_file, "%s class %s for r%d", title, reg_class_names[new_class],
                        regno);
        }
...
static inline void lra_update_dup(lra_insn_recog_data_t id, int nop)
{
        int i;
...
static inline void lra_update_operator_dups(lra_insn_recog_data_t id)
{
        int i;
        struct lra_static_insn_data *static_id = id->insn_static_data;
...
static inline lra_insn_recog_data_t lra_get_insn_recog_data(rtx_insn *insn)
{
        lra_insn_recog_data_t data;
        unsigned int uid = INSN_UID(insn);

        if (lra_insn_recog_data_len > (int)uid && (data = lra_insn_recog_data[uid]) != NULL) {
...
static inline void lra_update_reg_val_offset(int val, poly_int64 incr)
{
        int i;
...
static inline bool lra_reg_val_equal_p(int regno, int val, poly_int64 offset)
{
        if (lra_reg_info[regno].val == val && known_eq(lra_reg_info[regno].offset, offset)) {
                return true;
        }
...
static inline void lra_assign_reg_val(int from, int to)
{
