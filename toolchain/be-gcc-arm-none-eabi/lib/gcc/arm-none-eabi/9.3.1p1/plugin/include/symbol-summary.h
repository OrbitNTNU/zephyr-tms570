...
template <class T> class function_summary_base
{
      public:
        ...
        function_summary_base(symbol_table *symtab)
                : m_symtab(symtab), m_insertion_enabled(true), m_released(false)
        {
        }
...
template <typename T> void function_summary_base<T>::unregister_hooks()
{
...
template <class T> class GTY((user)) function_summary<T *> : public function_summary_base<T>
{
      public:
        /* Default construction takes SYMTAB as an argument.  */
        ...
        /* Traverses all summarys with a function F called with
           ARG as argument.  */
        template <typename Arg, bool (*f)(const T &, Arg)> void traverse(Arg a) const
        {
                m_map.traverse<f>(a);
        }
        ...
        : function_summary_base<T>(symtab), m_ggc(ggc), m_map(13, ggc)
{
        this->m_symtab_insertion_hook =
                this->m_symtab->add_cgraph_insertion_hook(function_summary::symtab_insertion, this);
        this->m_symtab_removal_hook =
                this->m_symtab->add_cgraph_removal_hook(function_summary::symtab_removal, this);
        this->m_symtab_duplication_hook = this->m_symtab->add_cgraph_duplication_hook(
                function_summary::symtab_duplication, this);
}

template <typename T> void function_summary<T *>::release()
{
        if (this->m_released) {
                return;
        }
...
template <typename T> void function_summary<T *>::symtab_insertion(cgraph_node *node, void *data)
{
...
template <typename T> void function_summary<T *>::symtab_removal(cgraph_node *node, void *data)
{
        gcc_checking_assert(node->get_uid());
        function_summary *summary = (function_summary<T *> *)(data);
        summary->remove(node);
...
template <typename T>
void function_summary<T *>::symtab_duplication(cgraph_node *node, cgraph_node *node2, void *data)
{
        function_summary *summary = (function_summary<T *> *)(data);
...
template <typename T> void gt_ggc_mx(function_summary<T *> *const &summary)
{
        gcc_checking_assert(summary->m_ggc);
        gt_ggc_mx(&summary->m_map);
}

template <typename T> void gt_pch_nx(function_summary<T *> *const &summary)
{
        gcc_checking_assert(summary->m_ggc);
...
template <typename T>
void gt_pch_nx(function_summary<T *> *const &summary, gt_pointer_operator op, void *cookie)
{
        gcc_checking_assert(summary->m_ggc);
...
template <class T, class V> class fast_function_summary
{
      private:
...
class GTY((user)) fast_function_summary<T *, V> : public function_summary_base<T>
{
      public:
        ...
        T *get_create(cgraph_node *node)
        {
                ...
                if ((unsigned int)id >= m_vector->length()) {
                        vec_safe_grow_cleared(m_vector, this->m_symtab->cgraph_max_summary_id);
                }
        ...
        {
                int id = node->get_summary_id();
                return (id != -1 && (unsigned int)id < m_vector->length() &&
                        (*m_vector)[id] != NULL);
        }
      ...
      private:
        ...
        template <typename U>
        friend void gt_pch_nx(fast_function_summary<U *, va_gc> *const &, gt_pointer_operator,
                              void *);
};
        ...
        : function_summary_base<T>(symtab), m_vector(NULL)
{
        ...
        this->m_symtab_insertion_hook = this->m_symtab->add_cgraph_insertion_hook(
                fast_function_summary::symtab_insertion, this);
        this->m_symtab_removal_hook = this->m_symtab->add_cgraph_removal_hook(
                fast_function_summary::symtab_removal, this);
        this->m_symtab_duplication_hook = this->m_symtab->add_cgraph_duplication_hook(
                fast_function_summary::symtab_duplication, this);
}

template <typename T, typename V> void fast_function_summary<T *, V>::release()
{
        if (this->m_released) {
                return;
        }
...
template <typename T, typename V>
void fast_function_summary<T *, V>::symtab_insertion(cgraph_node *node, void *data)
{
        gcc_checking_assert(node->get_uid());
...
template <typename T, typename V>
void fast_function_summary<T *, V>::symtab_removal(cgraph_node *node, void *data)
{
        gcc_checking_assert(node->get_uid());
        fast_function_summary *summary = (fast_function_summary<T *, V> *)(data);
...
template <typename T, typename V>
void fast_function_summary<T *, V>::symtab_duplication(cgraph_node *node, cgraph_node *node2,
                                                       void *data)
{
        fast_function_summary *summary = (fast_function_summary<T *, V> *)(data);
        T *v = summary->get(node);
...
template <typename T, typename V> inline bool fast_function_summary<T *, V>::is_ggc()
{
        return is_same<V, va_gc>::value;
}

template <typename T> void gt_ggc_mx(fast_function_summary<T *, va_heap> *const &)
{
}

template <typename T> void gt_pch_nx(fast_function_summary<T *, va_heap> *const &)
{
}
...
template <typename T> void gt_ggc_mx(fast_function_summary<T *, va_gc> *const &summary)
{
        ggc_test_and_set_mark(summary->m_vector);
        gt_ggc_mx(summary->m_vector);
}

template <typename T> void gt_pch_nx(fast_function_summary<T *, va_gc> *const &summary)
{
        gt_pch_nx(summary->m_vector);
}

template <typename T>
void gt_pch_nx(fast_function_summary<T *, va_gc> *const &summary, gt_pointer_operator op,
               void *cookie)
{
...
template <class T> class call_summary_base
{
      public:
        /* Default construction takes SYMTAB as an argument.  */
        call_summary_base(symbol_table *symtab)
                : m_symtab(symtab), m_initialize_when_cloning(true), m_released(false)
        {
        }
...
template <typename T> void call_summary_base<T>::unregister_hooks()
{
...
template <class T> class GTY((user)) call_summary<T *> : public call_summary_base<T>
{
      public:
        /* Default construction takes SYMTAB as an argument.  */
        ...
        {
                this->m_symtab_removal_hook =
                        this->m_symtab->add_edge_removal_hook(call_summary::symtab_removal, this);
                this->m_symtab_duplication_hook = this->m_symtab->add_edge_duplication_hook(
...
template <typename T> void call_summary<T *>::release()
{
        if (this->m_released) {
                return;
        }
...
template <typename T> void call_summary<T *>::symtab_removal(cgraph_edge *edge, void *data)
{
...
template <typename T>
void call_summary<T *>::symtab_duplication(cgraph_edge *edge1, cgraph_edge *edge2, void *data)
{
        call_summary *summary = (call_summary<T *> *)(data);
        ...
        if (edge1_summary) {
                summary->duplicate(edge1, edge2, edge1_summary, summary->get_create(edge2));
        }
}

template <typename T> void gt_ggc_mx(call_summary<T *> *const &summary)
{
        gcc_checking_assert(summary->m_ggc);
        gt_ggc_mx(&summary->m_map);
}

template <typename T> void gt_pch_nx(call_summary<T *> *const &summary)
{
        gcc_checking_assert(summary->m_ggc);
        gt_pch_nx(&summary->m_map);
}
...
template <typename T>
void gt_pch_nx(call_summary<T *> *const &summary, gt_pointer_operator op, void *cookie)
{
        gcc_checking_assert(summary->m_ggc);
        gt_pch_nx(&summary->m_map, op, cookie);
}
...
template <class T, class V> class fast_call_summary
{
      private:
        fast_call_summary();
};
...
class GTY((user)) fast_call_summary<T *, V> : public call_summary_base<T>
{
      public:
        /* Default construction takes SYMTAB as an argument.  */
        fast_call_summary(symbol_table *symtab) : call_summary_base<T>(symtab), m_vector(NULL)
        {
                vec_alloc(m_vector, 13);
                this->m_symtab_removal_hook = this->m_symtab->add_edge_removal_hook(
        ...
        {
                int id = edge->get_summary_id();
                return (id != -1 && (unsigned)id < m_vector->length() && (*m_vector)[id] != NULL);
        }
      ...
      private:
        ...
        template <typename U>
        friend void gt_pch_nx(fast_call_summary<U *, va_gc> *const &, gt_pointer_operator, void *);
};

template <typename T, typename V> void fast_call_summary<T *, V>::release()
{
        if (this->m_released) {
                return;
        }
...
template <typename T, typename V>
void fast_call_summary<T *, V>::symtab_removal(cgraph_edge *edge, void *data)
{
...
template <typename T, typename V>
void fast_call_summary<T *, V>::symtab_duplication(cgraph_edge *edge1, cgraph_edge *edge2,
                                                   void *data)
{
        fast_call_summary *summary = (fast_call_summary<T *, V> *)(data);
        T *edge1_summary = NULL;
...
template <typename T, typename V> inline bool fast_call_summary<T *, V>::is_ggc()
{
        return is_same<V, va_gc>::value;
}

template <typename T> void gt_ggc_mx(fast_call_summary<T *, va_heap> *const &summary)
{
}

template <typename T> void gt_pch_nx(fast_call_summary<T *, va_heap> *const &summary)
{
}

template <typename T>
void gt_pch_nx(fast_call_summary<T *, va_heap> *const &summary, gt_pointer_operator op,
               void *cookie)
{
}

template <typename T> void gt_ggc_mx(fast_call_summary<T *, va_gc> *const &summary)
{
        ggc_test_and_set_mark(summary->m_vector);
        gt_ggc_mx(&summary->m_vector);
}

template <typename T> void gt_pch_nx(fast_call_summary<T *, va_gc> *const &summary)
{
        gt_pch_nx(&summary->m_vector);
}

template <typename T>
void gt_pch_nx(fast_call_summary<T *, va_gc> *const &summary, gt_pointer_operator op, void *cookie)
{
        gt_pch_nx(&summary->m_vector, op, cookie);
}
