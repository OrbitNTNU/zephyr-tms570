...
class dump_context
{
      ...
      public:
        ...
        void dump_gimple_stmt(const dump_metadata_t &metadata, dump_flags_t extra_dump_flags,
                              gimple *gs, int spc);

        void dump_gimple_stmt_loc(const dump_metadata_t &metadata, const dump_user_location_t &loc,
                                  dump_flags_t extra_dump_flags, gimple *gs, int spc);

        void dump_gimple_expr(const dump_metadata_t &metadata, dump_flags_t extra_dump_flags,
                              gimple *gs, int spc);

        void dump_gimple_expr_loc(const dump_metadata_t &metadata, const dump_user_location_t &loc,
                                  dump_flags_t extra_dump_flags, gimple *gs, int spc);

        void dump_generic_expr(const dump_metadata_t &metadata, dump_flags_t extra_dump_flags,
                               tree t);

        void dump_generic_expr_loc(const dump_metadata_t &metadata, const dump_user_location_t &loc,
                                   dump_flags_t extra_dump_flags, tree t);

        void dump_printf_va(const dump_metadata_t &metadata, const char *format, va_list *ap)
                ATTRIBUTE_GCC_DUMP_PRINTF(3, 0);

        void dump_printf_loc_va(const dump_metadata_t &metadata, const dump_user_location_t &loc,
                                const char *format, va_list *ap) ATTRIBUTE_GCC_DUMP_PRINTF(4, 0);
        ...
        void begin_scope(const char *name, const dump_user_location_t &user_location,
...
class dump_pretty_printer : public pretty_printer
{
      ...
      private:
        ...
        static bool format_decoder_cb(pretty_printer *pp, text_info *text, const char *spec,
                                      int /*precision*/, bool /*wide*/, bool /*set_locus*/,
                                      bool /*verbose*/, bool * /*quoted*/, const char **buffer_ptr);
...
{
      public:
        temp_dump_context(bool forcibly_enable_optinfo, bool forcibly_enable_dumping,
...
extern void verify_dumped_text(const location &loc, temp_dump_context *context,
...
void verify_item(const location &loc, const optinfo_item *item,
                 enum optinfo_item_kind expected_kind, location_t expected_location,
                 const char *expected_text);
...
#define ASSERT_IS_TEXT(ITEM, EXPECTED_TEXT)                                                        \
        SELFTEST_BEGIN_STMT                                                                        \
        verify_item(SELFTEST_LOCATION, (ITEM), OPTINFO_ITEM_KIND_TEXT, UNKNOWN_LOCATION,           \
                    (EXPECTED_TEXT));                                                              \
        SELFTEST_END_STMT
...
#define ASSERT_IS_TREE(ITEM, EXPECTED_LOCATION, EXPECTED_TEXT)                                     \
        SELFTEST_BEGIN_STMT                                                                        \
        verify_item(SELFTEST_LOCATION, (ITEM), OPTINFO_ITEM_KIND_TREE, (EXPECTED_LOCATION),        \
                    (EXPECTED_TEXT));                                                              \
        SELFTEST_END_STMT
...
#define ASSERT_IS_GIMPLE(ITEM, EXPECTED_LOCATION, EXPECTED_TEXT)                                   \
        SELFTEST_BEGIN_STMT                                                                        \
        verify_item(SELFTEST_LOCATION, (ITEM), OPTINFO_ITEM_KIND_GIMPLE, (EXPECTED_LOCATION),      \
                    (EXPECTED_TEXT));                                                              \
        SELFTEST_END_STMT
...
#define ASSERT_IS_SYMTAB_NODE(ITEM, EXPECTED_LOCATION, EXPECTED_TEXT)                              \
        SELFTEST_BEGIN_STMT                                                                        \
        verify_item(SELFTEST_LOCATION, (ITEM), OPTINFO_ITEM_KIND_SYMTAB_NODE, (EXPECTED_LOCATION), \
                    (EXPECTED_TEXT));                                                              \
        SELFTEST_END_STMT
