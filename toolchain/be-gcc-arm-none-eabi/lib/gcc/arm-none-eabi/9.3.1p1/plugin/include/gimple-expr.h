...
static inline bool types_compatible_p(tree type1, tree type2)
{
        return (type1 == type2 || (useless_type_conversion_p(type1, type2) &&
                                   useless_type_conversion_p(type2, type1)));
}

/* Return true if TYPE is a suitable type for a scalar register variable.  */

static inline bool is_gimple_reg_type(tree type)
{
...
static inline bool is_gimple_variable(tree t)
{
        return (TREE_CODE(t) == VAR_DECL || TREE_CODE(t) == PARM_DECL ||
                TREE_CODE(t) == RESULT_DECL || TREE_CODE(t) == SSA_NAME);
}

/*  Return true if T is a GIMPLE identifier (something with an address).  */

static inline bool is_gimple_id(tree t)
{
        return (is_gimple_variable(t) || TREE_CODE(t) == FUNCTION_DECL ||
...
static inline bool virtual_operand_p(tree op)
{
...
static inline bool is_gimple_addressable(tree t)
{
        return (is_gimple_id(t) || handled_component_p(t) || TREE_CODE(t) == TARGET_MEM_REF ||
                TREE_CODE(t) == MEM_REF);
}

/* Return true if T is a valid gimple constant.  */

static inline bool is_gimple_constant(const_tree t)
{
        switch (TREE_CODE(t)) {
        case INTEGER_CST:
        case POLY_INT_CST:
...
static inline void extract_ops_from_tree(tree expr, enum tree_code *code, tree *op0, tree *op1)
{
        tree op2;
...
static inline tree gimple_call_addr_fndecl(const_tree fn)
{
