...
static inline void loop_constraint_set(struct loop *loop, unsigned c)
{
        loop->constraints |= c;
}

/* Clear C from the LOOP constraint.  */
static inline void loop_constraint_clear(struct loop *loop, unsigned c)
{
        loop->constraints &= ~c;
}

/* Check if C is set in the LOOP constraint.  */
static inline bool loop_constraint_set_p(struct loop *loop, unsigned c)
{
...
#define LOOPS_NORMAL                                                                               \
        (LOOPS_HAVE_PREHEADERS | LOOPS_HAVE_SIMPLE_LATCHES | LOOPS_HAVE_MARKED_IRREDUCIBLE_REGIONS)
...
extern void flow_loops_dump(FILE *, void (*)(const struct loop *, FILE *, int), int);
extern void flow_loop_dump(const struct loop *, FILE *, void (*)(const struct loop *, FILE *, int),
                           int);
...
extern void flow_loop_tree_node_add(struct loop *, struct loop *, struct loop * = NULL);
...
extern unsigned get_loop_body_with_size(const struct loop *, basic_block *, unsigned);
...
gcov_type expected_loop_iterations_unbounded(const struct loop *, bool *read_profile_p = NULL,
                                             bool by_profile_only = false);
...
extern bool iv_analyze_expr(rtx_insn *, scalar_int_mode, rtx, struct rtx_iv *);
...
static inline struct niter_desc *simple_loop_desc(struct loop *loop)
{
...
static inline struct loop *get_loop(struct function *fn, unsigned num)
{
...
static inline unsigned loop_depth(const struct loop *loop)
{
...
static inline struct loop *loop_outer(const struct loop *loop)
{
...
static inline bool loop_has_exit_edges(const struct loop *loop)
{
...
inline vec<loop_p, va_gc> *get_loops(struct function *fn)
{
...
static inline unsigned number_of_loops(struct function *fn)
{
        struct loops *loops = loops_for_fn(fn);
        if (!loops) {
                return 0;
...
static inline bool loops_state_satisfies_p(function *fn, unsigned flags)
{
        return (loops_for_fn(fn)->state & flags) == flags;
}

static inline bool loops_state_satisfies_p(unsigned flags)
{
...
static inline void loops_state_set(function *fn, unsigned flags)
{
        loops_for_fn(fn)->state |= flags;
}

static inline void loops_state_set(unsigned flags)
{
...
static inline void loops_state_clear(function *fn, unsigned flags)
{
        loops_for_fn(fn)->state &= ~flags;
}

static inline void loops_state_clear(unsigned flags)
{
        if (!current_loops) {
                return;
        }
...
static inline void checking_verify_loop_structure(void)
{
...
inline loop_p loop_iterator::next()
{
        int anum;
...
inline loop_iterator::loop_iterator(function *fn, loop_p *loop, unsigned flags)
{
        struct loop *aloop;
        unsigned i;
        int mn;
        ...
        if (flags & LI_ONLY_INNERMOST) {
                for (i = 0; vec_safe_iterate(loops_for_fn(fn)->larray, i, &aloop); i++) {
                        if (aloop != NULL && aloop->inner == NULL && aloop->num >= mn) {
                                this->to_visit.quick_push(aloop->num);
                        }
                }
        } else if (flags & LI_FROM_INNERMOST) {
                /* Push the loops to LI->TO_VISIT in postorder.  */
                for (aloop = loops_for_fn(fn)->tree_root; aloop->inner != NULL;
                     aloop = aloop->inner) {
                        continue;
                }
                ...
                while (1) {
                        ...
                        if (aloop->next) {
                                for (aloop = aloop->next; aloop->inner != NULL;
                                     aloop = aloop->inner) {
                                        continue;
                                }
...
inline loop_iterator::~loop_iterator()
{
        this->to_visit.release();
}

#define FOR_EACH_LOOP(LOOP, FLAGS)                                                                 \
        for (loop_iterator li(cfun, &(LOOP), FLAGS); (LOOP); (LOOP) = li.next())

#define FOR_EACH_LOOP_FN(FN, LOOP, FLAGS)                                                          \
        for (loop_iterator li(FN, &(LOOP), FLAGS); (LOOP); (LOOP) = li.next())
...
static inline struct loop *loop_outermost(struct loop *loop)
{
        unsigned n = vec_safe_length(loop->superloops);
...
static inline widest_int gcov_type_to_wide_int(gcov_type val)
{
        HOST_WIDE_INT a[2];
