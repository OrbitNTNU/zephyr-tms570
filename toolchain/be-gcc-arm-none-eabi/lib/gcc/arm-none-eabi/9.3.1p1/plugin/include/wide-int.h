...
#define WI_BINARY_RESULT_VAR(RESULT, VAL, T1, X, T2, Y)                                            \
        ...
        RESULT = wi::int_traits<WI_BINARY_RESULT(T1, T2)>::get_binary_result(X, Y);                \
        HOST_WIDE_INT *VAL = RESULT.write_val()

/* Similar for the result of a unary operation on X, which has type T.  */
#define WI_UNARY_RESULT_VAR(RESULT, VAL, T, X)                                                     \
        WI_UNARY_RESULT(T) RESULT = wi::int_traits<WI_UNARY_RESULT(T)>::get_binary_result(X, X);   \
        HOST_WIDE_INT *VAL = RESULT.write_val()
...
template <bool SE, bool HDP = true> struct wide_int_ref_storage;
...
template <typename T1, typename T2, enum precision_type P1 = int_traits<T1>::precision_type,
...
template <typename T1, typename T2> struct binary_traits<T1, T2, CONST_PRECISION, CONST_PRECISION> {
...
template <typename T1, typename T2> struct binary_traits<T1, T2, VAR_PRECISION, VAR_PRECISION> {
        typedef wide_int result_type;
        typedef result_type operator_result;
        typedef bool predicate_result;
};
...
template <typename T1, typename T2> unsigned int get_binary_precision(const T1 &, const T2 &);
...
template <typename T> HOST_WIDE_INT sign_mask(const T &);
...
template <typename T1, typename T2> int cmp(const T1 &, const T2 &, signop);

template <typename T1, typename T2> int cmps(const T1 &, const T2 &);

template <typename T1, typename T2> int cmpu(const T1 &, const T2 &);
...
template <typename T1, typename T2> bool multiple_of_p(const T1 &, const T2 &, signop);
...
template <typename T> unsigned HOST_WIDE_INT extract_uhwi(const T &, unsigned int, unsigned int);

template <typename T> unsigned int min_precision(const T &, signop);
...
inline ::wi::storage_ref::storage_ref(const HOST_WIDE_INT *val_in, unsigned int len_in,
                                      unsigned int precision_in)
        : val(val_in), len(len_in), precision(precision_in)
{
}

inline unsigned int wi::storage_ref::get_len() const
{
        return len;
}

inline unsigned int wi::storage_ref::get_precision() const
{
        return precision;
}

inline const HOST_WIDE_INT *wi::storage_ref::get_val() const
{
        return val;
}
...
template <typename storage> class GTY(()) generic_wide_int : public storage
{
      public:
        generic_wide_int();
        ...
        template <typename T> generic_wide_int(const T &, unsigned int);
        ...
        template <typename T> generic_wide_int &operator=(const T &);

#define ASSIGNMENT_OPERATOR(OP, F)                                                                 \
        template <typename T> generic_wide_int &OP(const T &c)                                     \
        {                                                                                          \
                return (*this = wi::F(*this, c));                                                  \
        }
...
template <typename storage> inline generic_wide_int<storage>::generic_wide_int()
{
}
...
template <typename storage>
template <typename T>
inline generic_wide_int<storage>::generic_wide_int(const T &x, unsigned int precision)
        : storage(x, precision)
{
}
...
template <typename storage>
inline HOST_WIDE_INT generic_wide_int<storage>::to_shwi(unsigned int precision) const
{
...
template <typename storage> inline HOST_WIDE_INT generic_wide_int<storage>::to_shwi() const
{
        if (is_sign_extended) {
                return this->get_val()[0];
        } else {
...
/* Return THIS as an unsigned HOST_WIDE_INT, zero-extending from
   ...
   is lost.  */
template <typename storage>
inline unsigned HOST_WIDE_INT generic_wide_int<storage>::to_uhwi(unsigned int precision) const
{
        if (precision < HOST_BITS_PER_WIDE_INT) {
...
template <typename storage> inline unsigned HOST_WIDE_INT generic_wide_int<storage>::to_uhwi() const
{
...
/* TODO: The compiler is half converted from using HOST_WIDE_INT to
   ...
   unconverted code.  */
template <typename storage> inline HOST_WIDE_INT generic_wide_int<storage>::to_short_addr() const
{
        return this->get_val()[0];
}

/* Return the implicit value of blocks above get_len ().  */
template <typename storage> inline HOST_WIDE_INT generic_wide_int<storage>::sign_mask() const
{
...
/* Return the signed value of the least-significant explicitly-encoded
   block.  */
template <typename storage> inline HOST_WIDE_INT generic_wide_int<storage>::slow() const
{
        return this->get_val()[0];
}
...
/* Return the signed value of the most-significant explicitly-encoded
   block.  */
template <typename storage> inline HOST_WIDE_INT generic_wide_int<storage>::shigh() const
{
...
template <typename storage> inline unsigned HOST_WIDE_INT generic_wide_int<storage>::ulow() const
{
        return this->get_val()[0];
}
...
/* Return the unsigned value of the most-significant
   explicitly-encoded block.  */
template <typename storage> inline unsigned HOST_WIDE_INT generic_wide_int<storage>::uhigh() const
{
        return this->get_val()[this->get_len() - 1];
}
...
template <typename storage>
inline HOST_WIDE_INT generic_wide_int<storage>::elt(unsigned int i) const
{
        if (i >= this->get_len()) {
                return sign_mask();
        } else {
...
template <typename storage>
template <typename T>
inline generic_wide_int<storage> &generic_wide_int<storage>::operator=(const T &x)
{
        storage::operator=(x);
        return *this;
}
...
template <typename storage> void generic_wide_int<storage>::dump() const
{
        unsigned int len = this->get_len();
...
template <typename storage>
inline unsigned int
wi::int_traits<generic_wide_int<storage>>::get_precision(const generic_wide_int<storage> &x)
{
...
wi::int_traits<generic_wide_int<storage>>::decompose(HOST_WIDE_INT *, unsigned int precision,
...
template <bool SE, bool HDP> struct wide_int_ref_storage : public wi::storage_ref {
      private:
      ...
      public:
        ...
        template <typename T> wide_int_ref_storage(const T &, unsigned int);
};

/* Create a reference from an existing reference.  */
template <bool SE, bool HDP>
inline wide_int_ref_storage<SE, HDP>::wide_int_ref_storage(const wi::storage_ref &x)
        : storage_ref(x)
{
}
...
inline wide_int_ref_storage<SE, HDP>::wide_int_ref_storage(const T &x)
        : storage_ref(wi::int_traits<T>::decompose(scratch, wi::get_precision(x), x))
{
}
...
template <bool SE, bool HDP>
template <typename T>
inline wide_int_ref_storage<SE, HDP>::wide_int_ref_storage(const T &x, unsigned int precision)
...
namespace wi
{
unsigned int force_to_size(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int, unsigned int,
                           unsigned int, signop sgn);
unsigned int from_array(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int, unsigned int,
                        bool = true);
} // namespace wi
...
class GTY(()) wide_int_storage
{
      ...
      public:
        ...
        template <typename T> wide_int_storage &operator=(const T &);
...
template <typename T> inline wide_int_storage &wide_int_storage::operator=(const T &x)
{
        {
                STATIC_ASSERT(!wi::int_traits<T>::host_dependent_precision);
        }
...
inline unsigned int wide_int_storage::get_precision() const
{
        return precision;
}

inline const HOST_WIDE_INT *wide_int_storage::get_val() const
{
        return val;
}

inline unsigned int wide_int_storage::get_len() const
{
        return len;
}

inline HOST_WIDE_INT *wide_int_storage::write_val()
{
        return val;
}
...
/* Treat X as having signedness SGN and convert it to a PRECISION-bit
   number.  */
inline wide_int wide_int_storage::from(const wide_int_ref &x, unsigned int precision, signop sgn)
{
        wide_int result = wide_int::create(precision);
        result.set_len(
                wi::force_to_size(result.write_val(), x.val, x.len, x.precision, precision, sgn));
        return result;
}
...
inline wide_int wide_int_storage::from_array(const HOST_WIDE_INT *val, unsigned int len,
                                             unsigned int precision, bool need_canon_p)
{
        wide_int result = wide_int::create(precision);
        result.set_len(wi::from_array(result.write_val(), val, len, precision, need_canon_p));
        return result;
}

/* Return an uninitialized wide_int with precision PRECISION.  */
inline wide_int wide_int_storage::create(unsigned int precision)
{
        wide_int x;
...
template <typename T1, typename T2>
inline wide_int wi::int_traits<wide_int_storage>::get_binary_result(const T1 &x, const T2 &y)
{
...
template <int N> class GTY(()) fixed_wide_int_storage
{
      ...
      public:
        ...
        static FIXED_WIDE_INT(N) from_array(const HOST_WIDE_INT *, unsigned int, bool = true);
};
...
template <int N> inline fixed_wide_int_storage<N>::fixed_wide_int_storage()
{
}
...
template <int N> inline unsigned int fixed_wide_int_storage<N>::get_precision() const
{
        return N;
}

template <int N> inline const HOST_WIDE_INT *fixed_wide_int_storage<N>::get_val() const
{
        return val;
}

template <int N> inline unsigned int fixed_wide_int_storage<N>::get_len() const
{
        return len;
}

template <int N> inline HOST_WIDE_INT *fixed_wide_int_storage<N>::write_val()
{
        return val;
}

template <int N> inline void fixed_wide_int_storage<N>::set_len(unsigned int l, bool)
{
        len = l;
...
template <int N>
inline FIXED_WIDE_INT(N) fixed_wide_int_storage<N>::from(const wide_int_ref &x, signop sgn)
{
        FIXED_WIDE_INT(N) result;
        result.set_len(wi::force_to_size(result.write_val(), x.val, x.len, x.precision, N, sgn));
        return result;
}
...
/* Create a FIXED_WIDE_INT (N) from the explicit block encoding given by
   ...
   trailing blocks.  */
template <int N>
inline FIXED_WIDE_INT(N) fixed_wide_int_storage<N>::from_array(const HOST_WIDE_INT *val,
                                                               unsigned int len, bool need_canon_p)
{
        FIXED_WIDE_INT(N) result;
        result.set_len(wi::from_array(result.write_val(), val, len, N, need_canon_p));
        return result;
}

template <int N>
template <typename T1, typename T2>
inline FIXED_WIDE_INT(N) wi::int_traits<fixed_wide_int_storage<N>>::get_binary_result(const T1 &,
                                                                                      const T2 &)
{
...
class trailing_wide_int_storage
{
      ...
      public:
        ...
        template <typename T> trailing_wide_int_storage &operator=(const T &);
};
...
namespace wi
{
template <> struct int_traits<trailing_wide_int_storage> : public int_traits<wide_int_storage> {
};
} // namespace wi
...
template <int N> class GTY((user)) trailing_wide_ints
{
      private:
...
inline trailing_wide_int_storage::trailing_wide_int_storage(unsigned int precision,
                                                            unsigned char *len, HOST_WIDE_INT *val)
        : m_precision(precision), m_len(len), m_val(val)
{
}

inline unsigned int trailing_wide_int_storage::get_len() const
{
        return *m_len;
}

inline unsigned int trailing_wide_int_storage::get_precision() const
{
        return m_precision;
}

inline const HOST_WIDE_INT *trailing_wide_int_storage::get_val() const
{
        return m_val;
}

inline HOST_WIDE_INT *trailing_wide_int_storage::write_val()
{
        return m_val;
}

inline void trailing_wide_int_storage::set_len(unsigned int len, bool is_sign_extended)
{
        *m_len = len;
        if (!is_sign_extended && len * HOST_BITS_PER_WIDE_INT > m_precision) {
                m_val[len - 1] = sext_hwi(m_val[len - 1], m_precision % HOST_BITS_PER_WIDE_INT);
        }
}

template <typename T>
inline trailing_wide_int_storage &trailing_wide_int_storage::operator=(const T &x)
{
...
/* Initialize the structure and record that all elements have precision
   PRECISION.  */
template <int N> inline void trailing_wide_ints<N>::set_precision(unsigned int precision)
{
        m_precision = precision;
        m_max_len = ((precision + HOST_BITS_PER_WIDE_INT - 1) / HOST_BITS_PER_WIDE_INT);
}

/* Return a reference to element INDEX.  */
template <int N> inline trailing_wide_int trailing_wide_ints<N>::operator[](unsigned int index)
{
        return trailing_wide_int_storage(m_precision, &m_len[index], &m_val[index * m_max_len]);
}
...
{
        return wi::storage_ref(&m_val[index * m_max_len], m_len[index], m_precision);
}
...
template <int N> inline size_t trailing_wide_ints<N>::extra_size(unsigned int precision)
{
        unsigned int max_len = ((precision + HOST_BITS_PER_WIDE_INT - 1) / HOST_BITS_PER_WIDE_INT);
...
#define TRAILING_WIDE_INT_ACCESSOR(NAME, FIELD, I)                                                 \
        trailing_wide_int get_##NAME()                                                             \
        {                                                                                          \
                return FIELD[I];                                                                   \
        }                                                                                          \
        template <typename T> void set_##NAME(const T &x)                                          \
        {                                                                                          \
                FIELD[I] = x;                                                                      \
        }
...
template <typename T, bool signed_p> struct primitive_int_traits {
...
inline unsigned int wi::primitive_int_traits<T, signed_p>::get_precision(T)
{
        return sizeof(T) * CHAR_BIT;
}

template <typename T, bool signed_p>
inline wi::storage_ref wi::primitive_int_traits<T, signed_p>::decompose(HOST_WIDE_INT *scratch,
...
namespace wi
{
template <> struct int_traits<unsigned char> : public primitive_int_traits<unsigned char, false> {
};

template <> struct int_traits<unsigned short> : public primitive_int_traits<unsigned short, false> {
};

template <> struct int_traits<int> : public primitive_int_traits<int, true> {
};

template <> struct int_traits<unsigned int> : public primitive_int_traits<unsigned int, false> {
};

template <> struct int_traits<long> : public primitive_int_traits<long, true> {
};

template <> struct int_traits<unsigned long> : public primitive_int_traits<unsigned long, false> {
};

#if defined HAVE_LONG_LONG
template <> struct int_traits<long long> : public primitive_int_traits<long long, true> {
};

template <>
struct int_traits<unsigned long long> : public primitive_int_traits<unsigned long long, false> {
};
#endif
} // namespace wi
...
inline wi::hwi_with_prec::hwi_with_prec(HOST_WIDE_INT v, unsigned int p, signop s)
        : precision(p), sgn(s)
{
        if (precision < HOST_BITS_PER_WIDE_INT) {
...
inline wi::hwi_with_prec wi::shwi(HOST_WIDE_INT val, unsigned int precision)
{
        return hwi_with_prec(val, precision, SIGNED);
}

/* Return an unsigned integer that has value VAL and precision PRECISION.  */
inline wi::hwi_with_prec wi::uhwi(unsigned HOST_WIDE_INT val, unsigned int precision)
{
        return hwi_with_prec(val, precision, UNSIGNED);
}

/* Return a wide int of -1 with precision PRECISION.  */
inline wi::hwi_with_prec wi::minus_one(unsigned int precision)
{
        return wi::shwi(-1, precision);
}

/* Return a wide int of 0 with precision PRECISION.  */
inline wi::hwi_with_prec wi::zero(unsigned int precision)
{
        return wi::shwi(0, precision);
}

/* Return a wide int of 1 with precision PRECISION.  */
inline wi::hwi_with_prec wi::one(unsigned int precision)
{
        return wi::shwi(1, precision);
}

/* Return a wide int of 2 with precision PRECISION.  */
inline wi::hwi_with_prec wi::two(unsigned int precision)
{
...
template <typename T, precision_type = int_traits<T>::precision_type> struct ints_for {
...
template <typename T> inline wi::hwi_with_prec wi::ints_for<T, wi::VAR_PRECISION>::zero(const T &x)
{
...
inline unsigned int wi::int_traits<wi::hwi_with_prec>::get_precision(const wi::hwi_with_prec &x)
{
        return x.precision;
}

inline wi::storage_ref wi::int_traits<wi::hwi_with_prec>::decompose(HOST_WIDE_INT *scratch,
                                                                    unsigned int precision,
...
namespace wi
{
bool eq_p_large(const HOST_WIDE_INT *, unsigned int, const HOST_WIDE_INT *, unsigned int,
                unsigned int);
bool lts_p_large(const HOST_WIDE_INT *, unsigned int, unsigned int, const HOST_WIDE_INT *,
                 unsigned int);
bool ltu_p_large(const HOST_WIDE_INT *, unsigned int, unsigned int, const HOST_WIDE_INT *,
                 unsigned int);
int cmps_large(const HOST_WIDE_INT *, unsigned int, unsigned int, const HOST_WIDE_INT *,
               unsigned int);
int cmpu_large(const HOST_WIDE_INT *, unsigned int, unsigned int, const HOST_WIDE_INT *,
               unsigned int);
unsigned int sext_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int, unsigned int,
                        unsigned int);
unsigned int zext_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int, unsigned int,
                        unsigned int);
unsigned int set_bit_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int, unsigned int,
                           unsigned int);
unsigned int lshift_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int, unsigned int,
                          unsigned int);
unsigned int lrshift_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int, unsigned int,
                           unsigned int, unsigned int);
unsigned int arshift_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int, unsigned int,
                           unsigned int, unsigned int);
unsigned int and_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int, const HOST_WIDE_INT *,
                       unsigned int, unsigned int);
unsigned int and_not_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int,
                           const HOST_WIDE_INT *, unsigned int, unsigned int);
unsigned int or_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int, const HOST_WIDE_INT *,
                      unsigned int, unsigned int);
unsigned int or_not_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int,
                          const HOST_WIDE_INT *, unsigned int, unsigned int);
unsigned int xor_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int, const HOST_WIDE_INT *,
                       unsigned int, unsigned int);
unsigned int add_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int, const HOST_WIDE_INT *,
                       unsigned int, unsigned int, signop, overflow_type *);
unsigned int sub_large(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int, const HOST_WIDE_INT *,
                       unsigned int, unsigned int, signop, overflow_type *);
unsigned int mul_internal(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int,
                          const HOST_WIDE_INT *, unsigned int, unsigned int, signop,
                          overflow_type *, bool);
unsigned int divmod_internal(HOST_WIDE_INT *, unsigned int *, HOST_WIDE_INT *,
                             const HOST_WIDE_INT *, unsigned int, unsigned int,
                             const HOST_WIDE_INT *, unsigned int, unsigned int, signop,
                             overflow_type *);
} // namespace wi
...
template <typename T1, typename T2>
inline unsigned int wi::get_binary_precision(const T1 &x, const T2 &y)
{
        return get_precision(wi::int_traits<WI_BINARY_RESULT(T1, T2)>::get_binary_result(x, y));
}
...
template <typename T> inline bool wi::fits_shwi_p(const T &x)
{
        WIDE_INT_REF_FOR(T) xi(x);
        return xi.len == 1;
}
...
/* Return true if X fits in an unsigned HOST_WIDE_INT with no loss of
   precision.  */
template <typename T> inline bool wi::fits_uhwi_p(const T &x)
{
        WIDE_INT_REF_FOR(T) xi(x);
...
template <typename T> inline bool wi::neg_p(const T &x, signop sgn)
{
        WIDE_INT_REF_FOR(T) xi(x);
        if (sgn == UNSIGNED) {
                return false;
...
template <typename T> inline HOST_WIDE_INT wi::sign_mask(const T &x)
{
        WIDE_INT_REF_FOR(T) xi(x);
        return xi.sign_mask();
}
...
template <typename T1, typename T2> inline bool wi::eq_p(const T1 &x, const T2 &y)
{
...
/* Return true if X != Y.  X and Y must be binary-compatible.  */
template <typename T1, typename T2> inline bool wi::ne_p(const T1 &x, const T2 &y)
{
        return !eq_p(x, y);
}

/* Return true if X < Y when both are treated as signed values.  */
template <typename T1, typename T2> inline bool wi::lts_p(const T1 &x, const T2 &y)
{
        unsigned int precision = get_binary_precision(x, y);
        WIDE_INT_REF_FOR(T1) xi(x, precision);
        WIDE_INT_REF_FOR(T2) yi(y, precision);
...
template <typename T1, typename T2> inline bool wi::ltu_p(const T1 &x, const T2 &y)
{
        unsigned int precision = get_binary_precision(x, y);
        WIDE_INT_REF_FOR(T1) xi(x, precision);
        WIDE_INT_REF_FOR(T2) yi(y, precision);
...
template <typename T1, typename T2> inline bool wi::lt_p(const T1 &x, const T2 &y, signop sgn)
{
        if (sgn == SIGNED) {
                return lts_p(x, y);
        } else {
...
/* Return true if X <= Y when both are treated as signed values.  */
template <typename T1, typename T2> inline bool wi::les_p(const T1 &x, const T2 &y)
{
        return !lts_p(y, x);
}

/* Return true if X <= Y when both are treated as unsigned values.  */
template <typename T1, typename T2> inline bool wi::leu_p(const T1 &x, const T2 &y)
{
        return !ltu_p(y, x);
}

/* Return true if X <= Y.  Signedness of X and Y is indicated by SGN.  */
template <typename T1, typename T2> inline bool wi::le_p(const T1 &x, const T2 &y, signop sgn)
{
        if (sgn == SIGNED) {
                return les_p(x, y);
        } else {
...
/* Return true if X > Y when both are treated as signed values.  */
template <typename T1, typename T2> inline bool wi::gts_p(const T1 &x, const T2 &y)
{
        return lts_p(y, x);
}

/* Return true if X > Y when both are treated as unsigned values.  */
template <typename T1, typename T2> inline bool wi::gtu_p(const T1 &x, const T2 &y)
{
        return ltu_p(y, x);
}

/* Return true if X > Y.  Signedness of X and Y is indicated by SGN.  */
template <typename T1, typename T2> inline bool wi::gt_p(const T1 &x, const T2 &y, signop sgn)
{
        if (sgn == SIGNED) {
                return gts_p(x, y);
        } else {
...
/* Return true if X >= Y when both are treated as signed values.  */
template <typename T1, typename T2> inline bool wi::ges_p(const T1 &x, const T2 &y)
{
        return !lts_p(x, y);
}

/* Return true if X >= Y when both are treated as unsigned values.  */
template <typename T1, typename T2> inline bool wi::geu_p(const T1 &x, const T2 &y)
{
        return !ltu_p(x, y);
}

/* Return true if X >= Y.  Signedness of X and Y is indicated by SGN.  */
template <typename T1, typename T2> inline bool wi::ge_p(const T1 &x, const T2 &y, signop sgn)
{
        if (sgn == SIGNED) {
                return ges_p(x, y);
        } else {
...
/* Return -1 if X < Y, 0 if X == Y and 1 if X > Y.  Treat both X and Y
   as signed values.  */
template <typename T1, typename T2> inline int wi::cmps(const T1 &x, const T2 &y)
{
        unsigned int precision = get_binary_precision(x, y);
        WIDE_INT_REF_FOR(T1) xi(x, precision);
        WIDE_INT_REF_FOR(T2) yi(y, precision);
...
/* Return -1 if X < Y, 0 if X == Y and 1 if X > Y.  Treat both X and Y
   as unsigned values.  */
template <typename T1, typename T2> inline int wi::cmpu(const T1 &x, const T2 &y)
{
        unsigned int precision = get_binary_precision(x, y);
        WIDE_INT_REF_FOR(T1) xi(x, precision);
        WIDE_INT_REF_FOR(T2) yi(y, precision);
...
template <typename T1, typename T2> inline int wi::cmp(const T1 &x, const T2 &y, signop sgn)
{
        if (sgn == SIGNED) {
                return cmps(x, y);
        } else {
...
/* Return ~x.  */
template <typename T> inline WI_UNARY_RESULT(T) wi::bit_not(const T &x)
{
...
/* Return -x.  */
template <typename T> inline WI_UNARY_RESULT(T) wi::neg(const T &x)
{
        return sub(0, x);
}
...
/* Return -x.  Indicate in *OVERFLOW if performing the negation would
   cause an overflow.  */
template <typename T> inline WI_UNARY_RESULT(T) wi::neg(const T &x, overflow_type *overflow)
{
...
template <typename T> inline WI_UNARY_RESULT(T) wi::abs(const T &x)
{
        return neg_p(x) ? neg(x) : WI_UNARY_RESULT(T)(x);
}

/* Return the result of sign-extending the low OFFSET bits of X.  */
template <typename T> inline WI_UNARY_RESULT(T) wi::sext(const T &x, unsigned int offset)
{
        WI_UNARY_RESULT_VAR(result, val, T, x);
...
template <typename T> inline WI_UNARY_RESULT(T) wi::zext(const T &x, unsigned int offset)
{
        WI_UNARY_RESULT_VAR(result, val, T, x);
        unsigned int precision = get_precision(result);
        WIDE_INT_REF_FOR(T) xi(x, precision);
...
/* Return the result of extending the low OFFSET bits of X according to
   signedness SGN.  */
template <typename T> inline WI_UNARY_RESULT(T) wi::ext(const T &x, unsigned int offset, signop sgn)
{
        return sgn == SIGNED ? sext(x, offset) : zext(x, offset);
}

/* Return an integer that represents X | (1 << bit).  */
template <typename T> inline WI_UNARY_RESULT(T) wi::set_bit(const T &x, unsigned int bit)
{
        WI_UNARY_RESULT_VAR(result, val, T, x);
        unsigned int precision = get_precision(result);
        WIDE_INT_REF_FOR(T) xi(x, precision);
...
/* Return the mininum of X and Y, treating them both as having
   signedness SGN.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::min(const T1 &x, const T2 &y, signop sgn)
{
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::smin(const T1 &x, const T2 &y)
{
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::umin(const T1 &x, const T2 &y)
{
...
/* Return the maxinum of X and Y, treating them both as having
   signedness SGN.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::max(const T1 &x, const T2 &y, signop sgn)
{
        WI_BINARY_RESULT_VAR(result, val ATTRIBUTE_UNUSED, T1, x, T2, y);
        unsigned int precision = get_precision(result);
        if (wi::ge_p(x, y, sgn)) {
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::smax(const T1 &x, const T2 &y)
{
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::umax(const T1 &x, const T2 &y)
{
...
/* Return X & Y.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::bit_and(const T1 &x, const T2 &y)
{
        ...
        if (__builtin_expect(xi.len + yi.len == 2, true)) {
                ...
                result.set_len(and_large(val, xi.val, xi.len, yi.val, yi.len, precision),
                               is_sign_extended);
        }
        return result;
}
...
/* Return X & ~Y.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::bit_and_not(const T1 &x, const T2 &y)
{
        WI_BINARY_RESULT_VAR(result, val, T1, x, T2, y);
        unsigned int precision = get_precision(result);
        WIDE_INT_REF_FOR(T1) xi(x, precision);
        ...
        if (__builtin_expect(xi.len + yi.len == 2, true)) {
                ...
                result.set_len(and_not_large(val, xi.val, xi.len, yi.val, yi.len, precision),
                               is_sign_extended);
        }
        return result;
}
...
/* Return X | Y.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::bit_or(const T1 &x, const T2 &y)
{
        WI_BINARY_RESULT_VAR(result, val, T1, x, T2, y);
        unsigned int precision = get_precision(result);
        WIDE_INT_REF_FOR(T1) xi(x, precision);
...
/* Return X | ~Y.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::bit_or_not(const T1 &x, const T2 &y)
{
        WI_BINARY_RESULT_VAR(result, val, T1, x, T2, y);
        unsigned int precision = get_precision(result);
        WIDE_INT_REF_FOR(T1) xi(x, precision);
        ...
        if (__builtin_expect(xi.len + yi.len == 2, true)) {
                ...
                result.set_len(or_not_large(val, xi.val, xi.len, yi.val, yi.len, precision),
                               is_sign_extended);
        }
        return result;
}
...
/* Return X ^ Y.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::bit_xor(const T1 &x, const T2 &y)
{
        WI_BINARY_RESULT_VAR(result, val, T1, x, T2, y);
        unsigned int precision = get_precision(result);
        WIDE_INT_REF_FOR(T1) xi(x, precision);
...
/* Return X + Y.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::add(const T1 &x, const T2 &y)
{
        WI_BINARY_RESULT_VAR(result, val, T1, x, T2, y);
        unsigned int precision = get_precision(result);
        WIDE_INT_REF_FOR(T1) xi(x, precision);
        ...
        /* If the precision is known at compile time to be greater than
           ...
           point handling them inline.  */
                ...
                result.set_len(1 +
                               (((resultl ^ xl) & (resultl ^ yl)) >> (HOST_BITS_PER_WIDE_INT - 1)));
        } else {
                result.set_len(
                        add_large(val, xi.val, xi.len, yi.val, yi.len, precision, UNSIGNED, 0));
        }
        return result;
}
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::add(const T1 &x, const T2 &y, signop sgn,
                                        overflow_type *overflow)
{
        ...
        if (precision <= HOST_BITS_PER_WIDE_INT) {
                ...
                unsigned HOST_WIDE_INT resultl = xl + yl;
                if (sgn == SIGNED) {
                        if ((((resultl ^ xl) & (resultl ^ yl)) >> (precision - 1)) & 1) {
                                if (xl > resultl) {
                ...
                result.set_len(
                        add_large(val, xi.val, xi.len, yi.val, yi.len, precision, sgn, overflow));
        }
        return result;
}
...
/* Return X - Y.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::sub(const T1 &x, const T2 &y)
{
        WI_BINARY_RESULT_VAR(result, val, T1, x, T2, y);
        unsigned int precision = get_precision(result);
        WIDE_INT_REF_FOR(T1) xi(x, precision);
        ...
        /* If the precision is known at compile time to be greater than
           ...
           point handling them inline.  */
                ...
                val[0] = resultl;
                val[1] = (HOST_WIDE_INT)resultl < 0 ? 0 : -1;
                result.set_len(1 + (((resultl ^ xl) & (xl ^ yl)) >> (HOST_BITS_PER_WIDE_INT - 1)));
        } else {
                result.set_len(
                        sub_large(val, xi.val, xi.len, yi.val, yi.len, precision, UNSIGNED, 0));
        }
        return result;
}
...
/* Return X - Y.  Treat X and Y as having the signednes given by SGN
   and indicate in *OVERFLOW whether the operation overflowed.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::sub(const T1 &x, const T2 &y, signop sgn,
                                        overflow_type *overflow)
{
        WI_BINARY_RESULT_VAR(result, val, T1, x, T2, y);
        unsigned int precision = get_precision(result);
        ...
        if (precision <= HOST_BITS_PER_WIDE_INT) {
                ...
                result.set_len(
                        sub_large(val, xi.val, xi.len, yi.val, yi.len, precision, sgn, overflow));
        }
        return result;
}
...
/* Return X * Y.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::mul(const T1 &x, const T2 &y)
{
        WI_BINARY_RESULT_VAR(result, val, T1, x, T2, y);
        unsigned int precision = get_precision(result);
        WIDE_INT_REF_FOR(T1) xi(x, precision);
...
/* Return X * Y.  Treat X and Y as having the signednes given by SGN
   and indicate in *OVERFLOW whether the operation overflowed.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::mul(const T1 &x, const T2 &y, signop sgn,
                                        overflow_type *overflow)
{
        WI_BINARY_RESULT_VAR(result, val, T1, x, T2, y);
        unsigned int precision = get_precision(result);
        ...
        WIDE_INT_REF_FOR(T2) yi(y, precision);
        result.set_len(
                mul_internal(val, xi.val, xi.len, yi.val, yi.len, precision, sgn, overflow, false));
        return result;
}
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::smul(const T1 &x, const T2 &y, overflow_type *overflow)
{
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::umul(const T1 &x, const T2 &y, overflow_type *overflow)
{
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::mul_high(const T1 &x, const T2 &y, signop sgn)
{
        WI_BINARY_RESULT_VAR(result, val, T1, x, T2, y);
        unsigned int precision = get_precision(result);
        WIDE_INT_REF_FOR(T1) xi(x, precision);
        WIDE_INT_REF_FOR(T2) yi(y, precision);
        result.set_len(mul_internal(val, xi.val, xi.len, yi.val, yi.len, precision, sgn, 0, true));
        return result;
}
...
/* Return X / Y, rouding towards 0.  Treat X and Y as having the
   ...
   overflows.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::div_trunc(const T1 &x, const T2 &y, signop sgn,
                                              overflow_type *overflow)
{
        ...
        quotient.set_len(divmod_internal(quotient_val, 0, 0, xi.val, xi.len, precision, yi.val,
                                         yi.len, yi.precision, sgn, overflow));
        return quotient;
}

/* Return X / Y, rouding towards 0.  Treat X and Y as signed values.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::sdiv_trunc(const T1 &x, const T2 &y)
{
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::udiv_trunc(const T1 &x, const T2 &y)
{
...
/* Return X / Y, rouding towards -inf.  Treat X and Y as having the
   ...
   overflows.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::div_floor(const T1 &x, const T2 &y, signop sgn,
                                              overflow_type *overflow)
{
        WI_BINARY_RESULT_VAR(quotient, quotient_val, T1, x, T2, y);
        ...
        quotient.set_len(divmod_internal(quotient_val, &remainder_len, remainder_val, xi.val,
                                         xi.len, precision, yi.val, yi.len, yi.precision, sgn,
                                         overflow));
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::sdiv_floor(const T1 &x, const T2 &y)
{
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::udiv_floor(const T1 &x, const T2 &y)
{
...
/* Return X / Y, rouding towards +inf.  Treat X and Y as having the
   ...
   overflows.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::div_ceil(const T1 &x, const T2 &y, signop sgn,
                                             overflow_type *overflow)
{
        WI_BINARY_RESULT_VAR(quotient, quotient_val, T1, x, T2, y);
        WI_BINARY_RESULT_VAR(remainder, remainder_val, T1, x, T2, y);
...
/* Return X / Y, rouding towards +inf.  Treat X and Y as unsigned values.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::udiv_ceil(const T1 &x, const T2 &y)
{
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::div_round(const T1 &x, const T2 &y, signop sgn,
                                              overflow_type *overflow)
{
        WI_BINARY_RESULT_VAR(quotient, quotient_val, T1, x, T2, y);
        WI_BINARY_RESULT_VAR(remainder, remainder_val, T1, x, T2, y);
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::divmod_trunc(const T1 &x, const T2 &y, signop sgn,
                                                 WI_BINARY_RESULT(T1, T2) * remainder_ptr)
{
        ...
        unsigned int remainder_len;
        quotient.set_len(divmod_internal(quotient_val, &remainder_len, remainder_val, xi.val,
                                         xi.len, precision, yi.val, yi.len, yi.precision, sgn, 0));
        remainder.set_len(remainder_len);
...
/* Compute the greatest common divisor of two numbers A and B using
   Euclid's algorithm.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::gcd(const T1 &a, const T2 &b, signop sgn)
{
        T1 x, y, z;
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::mod_trunc(const T1 &x, const T2 &y, signop sgn,
                                              overflow_type *overflow)
{
        WI_BINARY_RESULT_VAR(remainder, remainder_val, T1, x, T2, y);
        ...
        unsigned int remainder_len;
        divmod_internal(0, &remainder_len, remainder_val, xi.val, xi.len, precision, yi.val, yi.len,
                        yi.precision, sgn, overflow);
        remainder.set_len(remainder_len);
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::smod_trunc(const T1 &x, const T2 &y)
{
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::umod_trunc(const T1 &x, const T2 &y)
{
...
/* Compute X / Y, rouding towards -inf, and return the remainder.
   ...
   in *OVERFLOW if the division overflows.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::mod_floor(const T1 &x, const T2 &y, signop sgn,
                                              overflow_type *overflow)
{
        WI_BINARY_RESULT_VAR(quotient, quotient_val, T1, x, T2, y);
        WI_BINARY_RESULT_VAR(remainder, remainder_val, T1, x, T2, y);
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::umod_floor(const T1 &x, const T2 &y)
{
...
/* Compute X / Y, rouding towards +inf, and return the remainder.
   ...
   in *OVERFLOW if the division overflows.  */
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::mod_ceil(const T1 &x, const T2 &y, signop sgn,
                                             overflow_type *overflow)
{
        WI_BINARY_RESULT_VAR(quotient, quotient_val, T1, x, T2, y);
        WI_BINARY_RESULT_VAR(remainder, remainder_val, T1, x, T2, y);
...
template <typename T1, typename T2>
inline WI_BINARY_RESULT(T1, T2) wi::mod_round(const T1 &x, const T2 &y, signop sgn,
                                              overflow_type *overflow)
{
        WI_BINARY_RESULT_VAR(quotient, quotient_val, T1, x, T2, y);
        WI_BINARY_RESULT_VAR(remainder, remainder_val, T1, x, T2, y);
...
template <typename T1, typename T2>
inline bool wi::multiple_of_p(const T1 &x, const T2 &y, signop sgn)
{
...
template <typename T1, typename T2>
inline bool wi::multiple_of_p(const T1 &x, const T2 &y, signop sgn, WI_BINARY_RESULT(T1, T2) * res)
{
...
/* Return X << Y.  Return 0 if Y is greater than or equal to
   the precision of X.  */
template <typename T1, typename T2> inline WI_UNARY_RESULT(T1) wi::lshift(const T1 &x, const T2 &y)
{
        ...
        if (geu_p(yi, precision)) {
                ...
                if (STATIC_CONSTANT_P(xi.precision > HOST_BITS_PER_WIDE_INT)
                            ? (STATIC_CONSTANT_P(shift < HOST_BITS_PER_WIDE_INT - 1) &&
                               xi.len == 1 &&
                               xi.val[0] <=
                                       (HOST_WIDE_INT)((unsigned HOST_WIDE_INT)HOST_WIDE_INT_MAX >>
                                                       shift))
...
template <typename T1, typename T2> inline WI_UNARY_RESULT(T1) wi::lrshift(const T1 &x, const T2 &y)
{
        WI_UNARY_RESULT_VAR(result, val, T1, x);
        ...
        if (geu_p(yi, xi.precision)) {
                ...
                /* For fixed-precision integers like offset_int and widest_int,
                   ...
                   and sub-HWI integers inline.  */
                if (STATIC_CONSTANT_P(xi.precision > HOST_BITS_PER_WIDE_INT)
                            ? (shift < HOST_BITS_PER_WIDE_INT && xi.len == 1 && xi.val[0] >= 0)
...
template <typename T1, typename T2> inline WI_UNARY_RESULT(T1) wi::arshift(const T1 &x, const T2 &y)
{
        WI_UNARY_RESULT_VAR(result, val, T1, x);
        /* Do things in the precision of the input rather than the output,
           since the result can be no larger than that.  */
...
template <typename T1, typename T2>
inline WI_UNARY_RESULT(T1) wi::rshift(const T1 &x, const T2 &y, signop sgn)
{
        if (sgn == UNSIGNED) {
                return lrshift(x, y);
        } else {
...
/* Return 0 if the number of 1s in X is even and 1 if the number of 1s
   is odd.  */
inline int wi::parity(const wide_int_ref &x)
{
        return popcount(x) & 1;
}
...
template <typename T>
inline unsigned HOST_WIDE_INT wi::extract_uhwi(const T &x, unsigned int bitpos, unsigned int width)
{
...
template <typename T> inline unsigned int wi::min_precision(const T &x, signop sgn)
{
        if (sgn == SIGNED) {
...
#define SIGNED_BINARY_PREDICATE(OP, F)                                                             \
        template <typename T1, typename T2>                                                        \
        inline WI_SIGNED_BINARY_PREDICATE_RESULT(T1, T2) OP(const T1 &x, const T2 &y)              \
        {                                                                                          \
                return wi::F(x, y);                                                                \
        }
...
template <typename T1, typename T2>
inline WI_SIGNED_SHIFT_RESULT(T1, T2) operator>>(const T1 & x, const T2 & y)
{
...
template <typename T> void gt_ggc_mx(generic_wide_int<T> *)
{
}

template <typename T> void gt_pch_nx(generic_wide_int<T> *)
{
}

template <typename T> void gt_pch_nx(generic_wide_int<T> *, void (*)(void *, void *), void *)
{
}

template <int N> void gt_ggc_mx(trailing_wide_ints<N> *)
{
}

template <int N> void gt_pch_nx(trailing_wide_ints<N> *)
{
}

template <int N> void gt_pch_nx(trailing_wide_ints<N> *, void (*)(void *, void *), void *)
{
}
...
template <typename T> T mask(unsigned int, bool);

template <typename T> T shifted_mask(unsigned int, unsigned int, bool);

template <typename T> T set_bit_in_zero(unsigned int);
...
unsigned int from_array(HOST_WIDE_INT *, const HOST_WIDE_INT *, unsigned int, unsigned int, bool);
} // namespace wi
...
inline wide_int wi::mask(unsigned int width, bool negate_p, unsigned int precision)
{
        wide_int result = wide_int::create(precision);
...
inline wide_int wi::shifted_mask(unsigned int start, unsigned int width, bool negate_p,
                                 unsigned int precision)
{
        wide_int result = wide_int::create(precision);
        result.set_len(shifted_mask(result.write_val(), start, width, negate_p, precision));
        return result;
}

/* Return a PRECISION-bit integer in which bit BIT is set and all the
   others are clear.  */
inline wide_int wi::set_bit_in_zero(unsigned int bit, unsigned int precision)
{
...
/* Return an integer of type T in which the low WIDTH bits are set
   and the other bits are clear, or the inverse if NEGATE_P.  */
template <typename T> inline T wi::mask(unsigned int width, bool negate_p)
{
        STATIC_ASSERT(wi::int_traits<T>::precision);
        T result;
        result.set_len(mask(result.write_val(), width, negate_p, wi::int_traits<T>::precision));
        return result;
}
...
/* Return an integer of type T in which the low START bits are clear,
   ...
   inverse if NEGATE_P.  */
template <typename T>
inline T wi::shifted_mask(unsigned int start, unsigned int width, bool negate_p)
{
        STATIC_ASSERT(wi::int_traits<T>::precision);
        T result;
...
/* Return an integer of type T in which bit BIT is set and all the
   others are clear.  */
template <typename T> inline T wi::set_bit_in_zero(unsigned int bit)
{
...
static inline void wi::accumulate_overflow(wi::overflow_type &overflow,
