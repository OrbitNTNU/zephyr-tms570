...
inline bool safe_scale_64bit(uint64_t a, uint64_t b, uint64_t c, uint64_t *res)
{
#if (GCC_VERSION >= 5000)
        uint64_t tmp;
        if (!__builtin_mul_overflow(a, b, &tmp) && !__builtin_add_overflow(tmp, c / 2, &tmp)) {
                *res = tmp / c;
                return true;
        }
        if (c == 1) {
...
#else
        if (a < ((uint64_t)1 << 31) && b < ((uint64_t)1 << 31) && c < ((uint64_t)1 << 31)) {
                *res = (a * b + (c / 2)) / c;
                return true;
        }
#endif
...
class GTY((user)) profile_probability
{
      ...
      public:
        ...
        static profile_probability very_unlikely()
        {
                ...
                profile_probability r = profile_probability::guessed_always().apply_scale(1, 2000);
                r.m_val--;
                return r;
        }
        static profile_probability unlikely()
        {
                ...
                profile_probability r = profile_probability::guessed_always().apply_scale(1, 5);
                r.m_val--;
                return r;
        }
        ...
        int to_reg_br_prob_note() const
        {
                ...
                gcc_checking_assert(profile_probability::from_reg_br_prob_note(ret) == *this);
                return ret;
        }
        ...
        profile_probability operator/(const profile_probability &other) const
        {
                ...
                if (m_val >= other.m_val) {
                        ret.m_val = max_probability;
                        ret.m_quality = MIN(MIN(m_quality, other.m_quality), profile_guessed);
                        return ret;
                } else if (!m_val) {
                        ret.m_val = 0;
                } else {
                        gcc_checking_assert(other.m_val);
                        ret.m_val = MIN(RDIV((uint64_t)m_val * max_probability, other.m_val),
                                        max_probability);
                }
                ret.m_quality = MIN(MIN(m_quality, other.m_quality), profile_adjusted);
                return ret;
        ...
        profile_probability &operator/=(const profile_probability &other)
        {
                ...
                if (!initialized_p() || !other.initialized_p()) {
                        ...
                        if (m_val > other.m_val) {
                                ...
                                m_quality = MIN(MIN(m_quality, other.m_quality), profile_guessed);
                                return *this;
                        } else if (!m_val)
                                ;
                        else {
                                gcc_checking_assert(other.m_val);
                                m_val = MIN(RDIV((uint64_t)m_val * max_probability, other.m_val),
                                            max_probability);
                        }
                        m_quality = MIN(MIN(m_quality, other.m_quality), profile_adjusted);
                }
        ...
        /* COUNT1 times event happens with *THIS probability, COUNT2 times OTHER
           ...
           OTHER happens.  */
        profile_probability combine_with_count(profile_count count1, profile_probability other,
...
#undef UINT64_BIT_FIELD_ALIGN
      ...
      public:
        ...
        {
                if (m_quality < profile_adjusted || other.m_quality < profile_adjusted) {
                        return true;
                }
                return !(other < *this);
        }
        ...
        {
                if (*this == other || !other.initialized_p() || m_quality > other.m_quality) {
                        return *this;
                }
                if (other.m_quality > m_quality || other > *this) {
                        return other;
                }
                return *this;
        }
        ...
        profile_count max(profile_count other) const
        {
                ...
                gcc_checking_assert(compatible_p(other));
                if (m_val < other.m_val || (m_val == other.m_val && m_quality < other.m_quality)) {
                        return other;
                }
                return *this;
        }
        ...
        profile_count apply_probability(profile_probability prob) const
        {
                ...
                profile_count ret;
                uint64_t tmp;
                safe_scale_64bit(m_val, prob.m_val, profile_probability::max_probability, &tmp);
                ret.m_val = tmp;
        ...
        profile_count apply_scale(profile_count num, profile_count den) const
        {
                ...
                ret.m_quality =
                        MIN(MIN(MIN(m_quality, profile_adjusted), num.m_quality), den.m_quality);
        ...
        {
                if (*this == profile_count::zero() && !(overall == profile_count::zero())) {
                        return profile_probability::never();
                }
                if (!initialized_p() || !overall.initialized_p() || !overall.m_val) {
                        return profile_probability::uninitialized();
                }
                ...
                if (overall.m_val < m_val) {
                        ...
                        ret.m_val =
                                RDIV(m_val * profile_probability::max_probability, overall.m_val);
                }
                ret.m_quality = MIN(MAX(MIN(m_quality, overall.m_quality), profile_guessed),
                                    profile_adjusted);
                return ret;
        }
