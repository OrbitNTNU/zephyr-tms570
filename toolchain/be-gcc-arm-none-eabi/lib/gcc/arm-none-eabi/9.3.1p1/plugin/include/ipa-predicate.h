      ...
      public:
        ...
        inline bool operator==(const predicate &p2) const
        {
                ...
                for (i = 0; m_clause[i]; i++) {
                        ...
                        gcc_checking_assert(!p2.m_clause[i] || p2.m_clause[i] > p2.m_clause[i + 1]);
        ...
        predicate remap_after_inlining(struct ipa_fn_summary *, struct ipa_fn_summary *, vec<int>,
                                       vec<int>, clause_t, const predicate &);
...
predicate add_condition(struct ipa_fn_summary *summary, int operand_num, HOST_WIDE_INT size,
                        struct agg_position_info *aggpos, enum tree_code code, tree val);
