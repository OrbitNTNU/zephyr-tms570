...
static inline bool leaf_code_p(enum rtx_code code)
{
...
/* Used to iterate over subrtxes of an rtx.  T abstracts the type of
   access.  */
template <typename T> class generic_subrtx_iterator
{
...
template <typename T> inline generic_subrtx_iterator<T>::array_type::array_type() : heap(0)
{
}

template <typename T> inline generic_subrtx_iterator<T>::array_type::~array_type()
{
...
template <typename T>
inline generic_subrtx_iterator<T>::generic_subrtx_iterator(array_type &array, value_type x,
                                                           const rtx_subrtx_bound_info *bounds)
        : m_bounds(bounds), m_array(array), m_current(x), m_base(m_array.stack), m_end(0),
          m_done(false), m_skip(false), m_substitute(false)
{
}

/* Return the current subrtx.  */

template <typename T> inline typename T::value_type generic_subrtx_iterator<T>::operator*() const
{
        return m_current;
}
...
template <typename T> inline bool generic_subrtx_iterator<T>::at_end() const
{
        return m_done;
}
...
template <typename T> inline void generic_subrtx_iterator<T>::next()
{
        if (m_substitute) {
                m_substitute = false;
                m_skip = false;
...
template <typename T> inline void generic_subrtx_iterator<T>::skip_subrtxes()
{
        m_skip = true;
}
...
template <typename T> inline void generic_subrtx_iterator<T>::substitute(value_type x)
{
        m_substitute = true;
        m_current = x;
}
...
#define FOR_EACH_SUBRTX(ITER, ARRAY, X, TYPE)                                                      \
        for (subrtx_iterator ITER(ARRAY, X, TYPE##_BOUNDS); !ITER.at_end(); ITER.next())

/* Like FOR_EACH_SUBRTX, but iterate over subrtxes of an rtx X.  */
#define FOR_EACH_SUBRTX_VAR(ITER, ARRAY, X, TYPE)                                                  \
        for (subrtx_var_iterator ITER(ARRAY, X, TYPE##_BOUNDS); !ITER.at_end(); ITER.next())
...
#define FOR_EACH_SUBRTX_PTR(ITER, ARRAY, X, TYPE)                                                  \
        for (subrtx_ptr_iterator ITER(ARRAY, X, TYPE##_BOUNDS); !ITER.at_end(); ITER.next())
