...
class timer
{
      ...
      private:
        ...
        void pop_internal();
        static void print_row(FILE *fp, const timevar_time_def *total, const char *name,
...
static inline void timevar_push(timevar_id_t tv)
{
        if (g_timer) {
                g_timer->push(tv);
        }
...
static inline void timevar_pop(timevar_id_t tv)
{
        if (g_timer) {
                g_timer->pop(tv);
        }
...
{
      public:
        auto_timevar(timer *t, timevar_id_t tv) : m_timer(t), m_tv(tv)
        {
                if (m_timer) {
                        m_timer->push(m_tv);
                }
        ...
        explicit auto_timevar(timevar_id_t tv) : m_timer(g_timer), m_tv(tv)
        {
                if (m_timer) {
                        m_timer->push(m_tv);
                }
