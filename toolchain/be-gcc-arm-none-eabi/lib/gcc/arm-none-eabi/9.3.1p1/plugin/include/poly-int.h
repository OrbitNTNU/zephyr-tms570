...
template <typename T> struct poly_coeff_traits<T, wi::FLEXIBLE_PRECISION> {
        typedef T result;
        typedef T int_type;
        static const int signedness = (T(0) >= T(-1));
        static const int precision = sizeof(T) * CHAR_BIT;
        static const T max_value =
                (signedness ? ((T(1) << (precision - 2)) + ((T(1) << (precision - 2)) - 1))
                            : T(-1));
        static const int rank = sizeof(T) * 2 + !signedness;
};

template <typename T> struct poly_coeff_traits<T, wi::VAR_PRECISION> {
        typedef T result;
...
template <typename T> struct poly_coeff_traits<T, wi::CONST_PRECISION> {
...
template <typename T1, typename T2> struct poly_coeff_pair_traits {
        ...
        static const bool lossless_p =
                (poly_coeff_traits<T1>::signedness == poly_coeff_traits<T2>::signedness
                         ? (poly_coeff_traits<T1>::precision >= poly_coeff_traits<T2>::precision)
...
#define RANK(X) poly_coeff_traits<X>::rank
        static const int result_kind =
                ((RANK(T1) <= RANK(HOST_WIDE_INT) && RANK(T2) <= RANK(HOST_WIDE_INT)) ? 0
...
struct if_lossless;
template <typename T1, typename T2, typename T3> struct if_lossless<T1, T2, T3, true> {
        typedef T3 type;
};
...
template <typename T> struct poly_int_traits {
...
template <unsigned int N, typename C> struct poly_int_traits<poly_int_pod<N, C>> {
...
template <typename T1, typename T2> struct if_nonpoly<T1, T2, false> {
        typedef T2 type;
};

/* SFINAE class that makes T3 available as "type" if both T1 and T2 are
   non-polynomial types.  */
template <typename T1, typename T2, typename T3, bool is_poly1 = poly_int_traits<T1>::is_poly,
          bool is_poly2 = poly_int_traits<T2>::is_poly>
struct if_nonpoly2 {
};
template <typename T1, typename T2, typename T3> struct if_nonpoly2<T1, T2, T3, false, false> {
        typedef T3 type;
};
...
template <typename T1, typename T2> struct if_poly<T1, T2, true> {
        typedef T2 type;
};
...
template <typename T1, typename T2> struct poly_result<T1, T2, 0> {
...
template <typename T1, typename T2> struct poly_result<T1, T2, 1> {
...
template <typename T1, typename T2> struct poly_result<T1, T2, 2> {
...
#define CONST_CONST_RESULT(N, T1, T2)                                                              \
        POLY_POLY_COEFF(typename if_nonpoly<T1>::type, typename if_nonpoly<T2>::type)
...
template <unsigned int N, typename C> class poly_int_pod
{
      public:
        template <typename Ca> poly_int_pod &operator=(const poly_int_pod<N, Ca> &);
        template <typename Ca> typename if_nonpoly<Ca, poly_int_pod>::type &operator=(const Ca &);
        ...
        template <typename T> typename if_lossless<T, C, bool>::type is_constant(T *) const;

        C to_constant() const;

        template <typename Ca>
        static poly_int<N, C> from(const poly_int_pod<N, Ca> &, unsigned int, signop);
        template <typename Ca> static poly_int<N, C> from(const poly_int_pod<N, Ca> &, signop);
...
template <unsigned int N, typename C>
template <typename Ca>
inline poly_int_pod<N, C> &poly_int_pod<N, C>::operator=(const poly_int_pod<N, Ca> &a)
{
...
template <unsigned int N, typename C>
template <typename Ca>
inline typename if_nonpoly<Ca, poly_int_pod<N, C>>::type &poly_int_pod<N, C>::operator=(const Ca &a)
{
...
template <unsigned int N, typename C>
inline poly_int_pod<N, C> &poly_int_pod<N, C>::operator<<=(unsigned int a)
{
        for (unsigned int i = 0; i < N; i++) {
                this->coeffs[i] <<= a;
        }
...
template <unsigned int N, typename C> inline bool poly_int_pod<N, C>::is_constant() const
{
        if (N >= 2) {
                for (unsigned int i = 1; i < N; i++) {
                        if (this->coeffs[i] != 0) {
...
template <unsigned int N, typename C>
template <typename T>
inline typename if_lossless<T, C, bool>::type poly_int_pod<N, C>::is_constant(T *const_value) const
{
        if (is_constant()) {
...
template <unsigned int N, typename C> inline C poly_int_pod<N, C>::to_constant() const
{
...
template <unsigned int N, typename C>
template <typename Ca>
inline poly_int<N, C> poly_int_pod<N, C>::from(const poly_int_pod<N, Ca> &a, unsigned int bitsize,
                                               signop sgn)
{
        poly_int<N, C> r;
        for (unsigned int i = 0; i < N; i++) {
...
template <unsigned int N, typename C>
template <typename Ca>
inline poly_int<N, C> poly_int_pod<N, C>::from(const poly_int_pod<N, Ca> &a, signop sgn)
{
        poly_int<N, C> r;
        for (unsigned int i = 0; i < N; i++) {
...
template <unsigned int N, typename C>
inline bool poly_int_pod<N, C>::to_shwi(poly_int_pod<N, HOST_WIDE_INT> *r) const
{
        for (unsigned int i = 0; i < N; i++) {
...
template <unsigned int N, typename C>
inline bool poly_int_pod<N, C>::to_uhwi(poly_int_pod<N, unsigned HOST_WIDE_INT> *r) const
{
        for (unsigned int i = 0; i < N; i++) {
...
template <unsigned int N, typename C>
inline poly_int<N, HOST_WIDE_INT> poly_int_pod<N, C>::force_shwi() const
{
...
template <unsigned int N, typename C>
inline poly_int<N, unsigned HOST_WIDE_INT> poly_int_pod<N, C>::force_uhwi() const
{
...
template <unsigned int N, typename C> inline poly_int_pod<N, C>::operator C() const
{
...
template <unsigned int N, typename C> class poly_int : public poly_int_pod<N, C>
{
      public:
        poly_int()
        {
        ...
        template <typename Ca> poly_int(const poly_int<N, Ca> &);
        template <typename Ca> poly_int(const poly_int_pod<N, Ca> &);
        template <typename C0> poly_int(const C0 &);
        template <typename C0, typename C1> poly_int(const C0 &, const C1 &);

        template <typename Ca> poly_int &operator=(const poly_int_pod<N, Ca> &);
        template <typename Ca> typename if_nonpoly<Ca, poly_int>::type &operator=(const Ca &);
...
template <unsigned int N, typename C>
template <typename Ca>
inline poly_int<N, C>::poly_int(const poly_int<N, Ca> &a)
{
        for (unsigned int i = 0; i < N; i++) {
                POLY_SET_COEFF(C, *this, i, a.coeffs[i]);
        }
...
template <unsigned int N, typename C>
template <typename Ca>
inline poly_int<N, C>::poly_int(const poly_int_pod<N, Ca> &a)
{
        for (unsigned int i = 0; i < N; i++) {
                POLY_SET_COEFF(C, *this, i, a.coeffs[i]);
        }
...
inline poly_int<N, C>::poly_int(const C0 &c0)
{
...
inline poly_int<N, C>::poly_int(const C0 &c0, const C1 &c1)
{
        STATIC_ASSERT(N >= 2);
        POLY_SET_COEFF(C, *this, 0, c0);
...
template <unsigned int N, typename C>
template <typename Ca>
inline poly_int<N, C> &poly_int<N, C>::operator=(const poly_int_pod<N, Ca> &a)
{
        for (unsigned int i = 0; i < N; i++) {
                this->coeffs[i] = a.coeffs[i];
        }
...
template <unsigned int N, typename C>
template <typename Ca>
inline typename if_nonpoly<Ca, poly_int<N, C>>::type &poly_int<N, C>::operator=(const Ca &a)
{
        this->coeffs[0] = a;
        if (N >= 2) {
                for (unsigned int i = 1; i < N; i++) {
...
template <unsigned int N, typename C>
inline poly_int<N, C> &poly_int<N, C>::operator<<=(unsigned int a)
{
        for (unsigned int i = 0; i < N; i++) {
                this->coeffs[i] <<= a;
        }
...
inline typename if_nonpoly<Ca, bool>::type coeffs_in_range_p(const Ca &a, const Cb &b, const Cc &c)
{
        return a >= b && a <= c;
}

template <unsigned int N, typename Ca, typename Cb, typename Cc>
inline typename if_nonpoly<Ca, bool>::type coeffs_in_range_p(const poly_int_pod<N, Ca> &a,
                                                             const Cb &b, const Cc &c)
{
        for (unsigned int i = 0; i < N; i++) {
...
inline poly_int<N, hwi_with_prec> shwi(const poly_int_pod<N, HOST_WIDE_INT> &a,
...
template <unsigned int N>
inline poly_int<N, hwi_with_prec> uhwi(const poly_int_pod<N, unsigned HOST_WIDE_INT> &a,
                                       unsigned int precision)
{
        poly_int<N, hwi_with_prec> r;
        for (unsigned int i = 0; i < N; i++) {
...
inline POLY_POLY_RESULT(N, Ca, Ca) sext(const poly_int_pod<N, Ca> &a, unsigned int precision)
{
...
template <unsigned int N, typename Ca>
inline POLY_POLY_RESULT(N, Ca, Ca) zext(const poly_int_pod<N, Ca> &a, unsigned int precision)
{
        typedef POLY_POLY_COEFF(Ca, Ca) C;
        poly_int<N, C> r;
        for (unsigned int i = 0; i < N; i++) {
...
inline POLY_POLY_RESULT(N, Ca, Cb) operator+(const poly_int_pod<N, Ca> &a,
...
template <unsigned int N, typename Ca, typename Cb>
inline POLY_CONST_RESULT(N, Ca, Cb) operator+(const poly_int_pod<N, Ca> &a, const Cb & b)
{
        typedef POLY_CAST(Ca, Cb) NCa;
...
template <unsigned int N, typename Ca, typename Cb>
inline CONST_POLY_RESULT(N, Ca, Cb) operator+(const Ca & a, const poly_int_pod<N, Cb> &b)
{
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> add(const poly_int_pod<N, Ca> &a,
                                                 const poly_int_pod<N, Cb> &b)
{
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> add(const poly_int_pod<N, Ca> &a, const Cb &b)
{
        typedef WI_BINARY_RESULT(Ca, Cb) C;
        poly_int<N, C> r;
        POLY_SET_COEFF(C, r, 0, wi::add(a.coeffs[0], b));
        for (unsigned int i = 1; i < N; i++) {
                POLY_SET_COEFF(C, r, i, wi::add(a.coeffs[i], wi::ints_for<Cb>::zero(b)));
        }
        return r;
}

template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> add(const Ca &a, const poly_int_pod<N, Cb> &b)
{
        typedef WI_BINARY_RESULT(Ca, Cb) C;
        poly_int<N, C> r;
        POLY_SET_COEFF(C, r, 0, wi::add(a, b.coeffs[0]));
        for (unsigned int i = 1; i < N; i++) {
                POLY_SET_COEFF(C, r, i, wi::add(wi::ints_for<Ca>::zero(a), b.coeffs[i]));
        }
        return r;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> add(const poly_int_pod<N, Ca> &a,
                                                 const poly_int_pod<N, Cb> &b, signop sgn,
                                                 wi::overflow_type *overflow)
{
        ...
        for (unsigned int i = 1; i < N; i++) {
                ...
                POLY_SET_COEFF(C, r, i, wi::add(a.coeffs[i], b.coeffs[i], sgn, &suboverflow));
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> sub(const poly_int_pod<N, Ca> &a,
                                                 const poly_int_pod<N, Cb> &b)
{
        typedef WI_BINARY_RESULT(Ca, Cb) C;
        poly_int<N, C> r;
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> sub(const poly_int_pod<N, Ca> &a, const Cb &b)
{
        typedef WI_BINARY_RESULT(Ca, Cb) C;
        poly_int<N, C> r;
        POLY_SET_COEFF(C, r, 0, wi::sub(a.coeffs[0], b));
        for (unsigned int i = 1; i < N; i++) {
                POLY_SET_COEFF(C, r, i, wi::sub(a.coeffs[i], wi::ints_for<Cb>::zero(b)));
        }
        return r;
}

template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> sub(const Ca &a, const poly_int_pod<N, Cb> &b)
{
        typedef WI_BINARY_RESULT(Ca, Cb) C;
        poly_int<N, C> r;
        POLY_SET_COEFF(C, r, 0, wi::sub(a, b.coeffs[0]));
        for (unsigned int i = 1; i < N; i++) {
                POLY_SET_COEFF(C, r, i, wi::sub(wi::ints_for<Ca>::zero(a), b.coeffs[i]));
        }
        return r;
}
                                                 ...
                                                 wi::overflow_type *overflow)
{
        ...
        for (unsigned int i = 1; i < N; i++) {
                wi::overflow_type suboverflow;
                POLY_SET_COEFF(C, r, i, wi::sub(a.coeffs[i], b.coeffs[i], sgn, &suboverflow));
                wi::accumulate_overflow(*overflow, suboverflow);
        }
        return r;
}
...
template <unsigned int N, typename Ca>
inline POLY_POLY_RESULT(N, Ca, Ca) operator-(const poly_int_pod<N, Ca> &a)
{
...
template <unsigned int N, typename Ca>
inline poly_int<N, WI_UNARY_RESULT(Ca)> neg(const poly_int_pod<N, Ca> &a)
{
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> mul(const poly_int_pod<N, Ca> &a, const Cb &b)
{
        typedef WI_BINARY_RESULT(Ca, Cb) C;
        poly_int<N, C> r;
        for (unsigned int i = 0; i < N; i++) {
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Cb)> mul(const Ca &a, const poly_int_pod<N, Cb> &b)
{
        typedef WI_BINARY_RESULT(Ca, Cb) C;
        poly_int<N, C> r;
        for (unsigned int i = 0; i < N; i++) {
...
template <unsigned int N, typename Ca, typename Cb>
inline POLY_POLY_RESULT(N, Ca, Ca) operator<<(const poly_int_pod<N, Ca> &a, const Cb & b)
{
        typedef POLY_CAST(Ca, Ca) NCa;
        typedef POLY_POLY_COEFF(Ca, Ca) C;
        poly_int<N, C> r;
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, WI_BINARY_RESULT(Ca, Ca)> lshift(const poly_int_pod<N, Ca> &a, const Cb &b)
{
...
inline bool maybe_eq_2(const Ca &a0, const Ca &a1, const Cb &b0, const Cb &b1)
{
        if (a1 != b1) {
                /*      a0 + a1 * x == b0 + b1 * x
                  ==> (a1 - b1) * x == b0 - a0
                ...
                return (a1 < b1 ? b0 <= a0 && (a0 - b0) % (b1 - a1) == 0
                                : b0 >= a0 && (b0 - a0) % (a1 - b1) == 0);
        }
        return a0 == b0;
}
...
template <typename Ca, typename Cb> inline bool maybe_eq_2(const Ca &a0, const Ca &a1, const Cb &b)
{
        if (a1 != 0) {
                /*      a0 + a1 * x == b
                  ==>             x == (b - a0) / a1
                ...
                return (a1 < 0 ? b <= a0 && (a0 - b) % a1 == 0 : b >= a0 && (b - a0) % a1 == 0);
        }
        return a0 == b;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline bool maybe_eq(const poly_int_pod<N, Ca> &a, const poly_int_pod<N, Cb> &b)
{
        STATIC_ASSERT(N <= 2);
        if (N == 2) {
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type maybe_eq(const poly_int_pod<N, Ca> &a, const Cb &b)
{
        STATIC_ASSERT(N <= 2);
        if (N == 2) {
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Ca, bool>::type maybe_eq(const Ca &a, const poly_int_pod<N, Cb> &b)
{
        STATIC_ASSERT(N <= 2);
        if (N == 2) {
...
template <typename Ca, typename Cb>
inline typename if_nonpoly2<Ca, Cb, bool>::type maybe_eq(const Ca &a, const Cb &b)
{
        return a == b;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline bool maybe_ne(const poly_int_pod<N, Ca> &a, const poly_int_pod<N, Cb> &b)
{
        if (N >= 2) {
                for (unsigned int i = 1; i < N; i++) {
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type maybe_ne(const poly_int_pod<N, Ca> &a, const Cb &b)
{
        if (N >= 2) {
                for (unsigned int i = 1; i < N; i++) {
                        if (a.coeffs[i] != 0) {
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Ca, bool>::type maybe_ne(const Ca &a, const poly_int_pod<N, Cb> &b)
{
        if (N >= 2) {
                for (unsigned int i = 1; i < N; i++) {
                        if (b.coeffs[i] != 0) {
...
template <typename Ca, typename Cb>
inline typename if_nonpoly2<Ca, Cb, bool>::type maybe_ne(const Ca &a, const Cb &b)
{
        return a != b;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline bool maybe_le(const poly_int_pod<N, Ca> &a, const poly_int_pod<N, Cb> &b)
{
        if (N >= 2) {
                for (unsigned int i = 1; i < N; i++) {
                        if (a.coeffs[i] < b.coeffs[i]) {
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type maybe_le(const poly_int_pod<N, Ca> &a, const Cb &b)
{
        if (N >= 2) {
                for (unsigned int i = 1; i < N; i++) {
                        if (a.coeffs[i] < 0) {
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Ca, bool>::type maybe_le(const Ca &a, const poly_int_pod<N, Cb> &b)
{
        if (N >= 2) {
                for (unsigned int i = 1; i < N; i++) {
                        if (b.coeffs[i] > 0) {
...
template <typename Ca, typename Cb>
inline typename if_nonpoly2<Ca, Cb, bool>::type maybe_le(const Ca &a, const Cb &b)
{
        return a <= b;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline bool maybe_lt(const poly_int_pod<N, Ca> &a, const poly_int_pod<N, Cb> &b)
{
        if (N >= 2) {
                for (unsigned int i = 1; i < N; i++) {
                        if (a.coeffs[i] < b.coeffs[i]) {
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type maybe_lt(const poly_int_pod<N, Ca> &a, const Cb &b)
{
        if (N >= 2) {
                for (unsigned int i = 1; i < N; i++) {
                        if (a.coeffs[i] < 0) {
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Ca, bool>::type maybe_lt(const Ca &a, const poly_int_pod<N, Cb> &b)
{
        if (N >= 2) {
                for (unsigned int i = 1; i < N; i++) {
                        if (b.coeffs[i] > 0) {
...
template <typename Ca, typename Cb>
inline typename if_nonpoly2<Ca, Cb, bool>::type maybe_lt(const Ca &a, const Cb &b)
{
        return a < b;
}
...
template <typename T1, typename T2> inline bool ordered_p(const T1 &a, const T2 &b)
{
        return ((poly_int_traits<T1>::num_coeffs == 1 && poly_int_traits<T2>::num_coeffs == 1) ||
                known_le(a, b) || known_le(b, a));
}
...
template <unsigned int N, typename Ca, typename Cb>
inline CONST_POLY_RESULT(N, Ca, Cb) ordered_min(const Ca &a, const poly_int_pod<N, Cb> &b)
{
        if (known_le(a, b)) {
                return a;
        } else {
...
template <unsigned int N, typename Ca, typename Cb>
inline POLY_CONST_RESULT(N, Ca, Cb) ordered_min(const poly_int_pod<N, Ca> &a, const Cb &b)
{
        if (known_le(a, b)) {
                return a;
        } else {
...
template <unsigned int N, typename Ca, typename Cb>
inline CONST_POLY_RESULT(N, Ca, Cb) ordered_max(const Ca &a, const poly_int_pod<N, Cb> &b)
{
        if (known_le(a, b)) {
                return b;
        } else {
...
template <unsigned int N, typename Ca, typename Cb>
inline POLY_CONST_RESULT(N, Ca, Cb) ordered_max(const poly_int_pod<N, Ca> &a, const Cb &b)
{
        if (known_le(a, b)) {
                return b;
        } else {
...
template <unsigned int N, typename Ca> inline Ca constant_lower_bound(const poly_int_pod<N, Ca> &a)
{
...
template <unsigned int N, typename Ca, typename Cb>
inline POLY_CONST_RESULT(N, Ca, Cb) lower_bound(const poly_int_pod<N, Ca> &a, const Cb &b)
{
        typedef POLY_CAST(Ca, Cb) NCa;
        typedef POLY_CAST(Cb, Ca) NCb;
...
template <unsigned int N, typename Ca, typename Cb>
inline CONST_POLY_RESULT(N, Ca, Cb) lower_bound(const Ca &a, const poly_int_pod<N, Cb> &b)
{
...
template <typename Ca, typename Cb>
inline CONST_CONST_RESULT(N, Ca, Cb) lower_bound(const Ca &a, const Cb &b)
{
        return a < b ? a : b;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline POLY_CONST_RESULT(N, Ca, Cb) upper_bound(const poly_int_pod<N, Ca> &a, const Cb &b)
{
        typedef POLY_CAST(Ca, Cb) NCa;
        typedef POLY_CAST(Cb, Ca) NCb;
        typedef POLY_INT_TYPE(Cb) ICb;
...
template <unsigned int N, typename Ca, typename Cb>
inline CONST_POLY_RESULT(N, Ca, Cb) upper_bound(const Ca &a, const poly_int_pod<N, Cb> &b)
{
...
template <unsigned int N, typename Ca>
inline POLY_BINARY_COEFF(Ca, Ca) coeff_gcd(const poly_int_pod<N, Ca> &a)
{
...
template <unsigned int N, typename Ca, typename Cb>
inline CONST_POLY_RESULT(N, Ca, Cb) common_multiple(const Ca &a, const poly_int_pod<N, Cb> &b)
{
...
force_common_multiple(const poly_int_pod<N, Ca> &a, const poly_int_pod<N, Cb> &b)
{
        if (b.is_constant()) {
...
template <unsigned int N, typename Ca, typename Cb>
inline int compare_sizes_for_sort(const poly_int_pod<N, Ca> &a, const poly_int_pod<N, Cb> &b)
{
...
template <unsigned int N, typename Ca, typename Cb>
inline bool can_align_p(const poly_int_pod<N, Ca> &value, Cb align)
{
        for (unsigned int i = 1; i < N; i++) {
...
template <unsigned int N, typename Ca, typename Cb>
inline bool can_align_up(const poly_int_pod<N, Ca> &value, Cb align, poly_int_pod<N, Ca> *aligned)
{
...
template <unsigned int N, typename Ca, typename Cb>
inline bool can_align_down(const poly_int_pod<N, Ca> &value, Cb align, poly_int_pod<N, Ca> *aligned)
{
        if (!can_align_p(value, align)) {
                return false;
        }
...
template <unsigned int N, typename Ca, typename Cb, typename Cc>
inline bool known_equal_after_align_up(const poly_int_pod<N, Ca> &a, const poly_int_pod<N, Cb> &b,
                                       Cc align)
{
        poly_int<N, Ca> aligned_a;
        poly_int<N, Cb> aligned_b;
        return (can_align_up(a, align, &aligned_a) && can_align_up(b, align, &aligned_b) &&
...
template <unsigned int N, typename Ca, typename Cb, typename Cc>
inline bool known_equal_after_align_down(const poly_int_pod<N, Ca> &a, const poly_int_pod<N, Cb> &b,
                                         Cc align)
{
        poly_int<N, Ca> aligned_a;
        poly_int<N, Cb> aligned_b;
        return (can_align_down(a, align, &aligned_a) && can_align_down(b, align, &aligned_b) &&
                known_eq(aligned_a, aligned_b));
}
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> force_align_up(const poly_int_pod<N, Ca> &value, Cb align)
{
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> force_align_down(const poly_int_pod<N, Ca> &value, Cb align)
{
        gcc_checking_assert(can_align_p(value, align));
        return value - (value.coeffs[0] & (align - 1));
}
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> aligned_lower_bound(const poly_int_pod<N, Ca> &value, Cb align)
{
        poly_int<N, Ca> r;
        for (unsigned int i = 0; i < N; i++) {
                /* This form copes correctly with more type combinations than
                   value.coeffs[i] & -align would.  */
                POLY_SET_COEFF(Ca, r, i, (value.coeffs[i] - (value.coeffs[i] & (align - 1))));
        }
        return r;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> aligned_upper_bound(const poly_int_pod<N, Ca> &value, Cb align)
{
        poly_int<N, Ca> r;
        for (unsigned int i = 0; i < N; i++) {
                POLY_SET_COEFF(Ca, r, i, (value.coeffs[i] + (-value.coeffs[i] & (align - 1))));
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> force_align_down_and_div(const poly_int_pod<N, Ca> &value, Cb align)
{
        gcc_checking_assert(can_align_p(value, align));

        poly_int<N, Ca> r;
        POLY_SET_COEFF(Ca, r, 0, ((value.coeffs[0] - (value.coeffs[0] & (align - 1))) / align));
        if (N >= 2) {
                for (unsigned int i = 1; i < N; i++) {
...
template <unsigned int N, typename Ca, typename Cb>
inline poly_int<N, Ca> force_align_up_and_div(const poly_int_pod<N, Ca> &value, Cb align)
{
        gcc_checking_assert(can_align_p(value, align));
...
inline bool known_misalignment(const poly_int_pod<N, Ca> &value, Cb align, Cm *misalign)
{
...
template <unsigned int N, typename Ca, typename Cb>
inline POLY_BINARY_COEFF(Ca, Ca) force_get_misalignment(const poly_int_pod<N, Ca> &a, Cb align)
{
...
template <unsigned int N, typename Ca>
inline POLY_BINARY_COEFF(Ca, Ca) known_alignment(const poly_int_pod<N, Ca> &a)
{
        typedef POLY_BINARY_COEFF(Ca, Ca) C;
        C r = a.coeffs[0];
        for (unsigned int i = 1; i < N; ++i) {
...
inline typename if_nonpoly<Cb, bool>::type can_ior_p(const poly_int_pod<N, Ca> &a, Cb b, Cr *result)
{
...
template <unsigned int N, typename Ca, typename Cb, typename Cm>
inline typename if_nonpoly<Cb, bool>::type constant_multiple_p(const poly_int_pod<N, Ca> &a, Cb b,
                                                               Cm *multiple)
{
        typedef POLY_CAST(Ca, Cb) NCa;
        typedef POLY_CAST(Cb, Ca) NCb;
...
template <unsigned int N, typename Ca, typename Cb, typename Cm>
inline typename if_nonpoly<Ca, bool>::type constant_multiple_p(Ca a, const poly_int_pod<N, Cb> &b,
                                                               Cm *multiple)
{
        typedef POLY_CAST(Ca, Cb) NCa;
        typedef POLY_CAST(Cb, Ca) NCb;
        ...
        /* Do the modulus before the constant check, to catch divide by
           zero errors.  */
        if (NCa(a) % NCb(b.coeffs[0]) != 0 || (a != int_type(0) && !b.is_constant())) {
                return false;
        }
...
template <unsigned int N, typename Ca, typename Cb, typename Cm>
inline bool constant_multiple_p(const poly_int_pod<N, Ca> &a, const poly_int_pod<N, Cb> &b,
                                Cm *multiple)
{
        typedef POLY_CAST(Ca, Cb) NCa;
        typedef POLY_CAST(Cb, Ca) NCb;
        ...
        for (unsigned int i = 1; i < N; ++i) {
                if (b.coeffs[i] == ICb(0) ? a.coeffs[i] != ICa(0)
...
template <typename Ca, typename Cb>
inline typename if_nonpoly2<Ca, Cb, bool>::type multiple_p(Ca a, Cb b)
{
        return a % b == 0;
}
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Cb, bool>::type multiple_p(const poly_int_pod<N, Ca> &a, Cb b)
{
        for (unsigned int i = 0; i < N; ++i) {
                if (a.coeffs[i] % b != 0) {
                        return false;
...
template <unsigned int N, typename Ca, typename Cb>
inline typename if_nonpoly<Ca, bool>::type multiple_p(Ca a, const poly_int_pod<N, Cb> &b)
{
        typedef POLY_INT_TYPE(Ca) int_type;
...
template <unsigned int N, typename Ca, typename Cb>
inline bool multiple_p(const poly_int_pod<N, Ca> &a, const poly_int_pod<N, Cb> &b)
{
        if (b.is_constant()) {
...
inline typename if_nonpoly2<Ca, Cb, bool>::type multiple_p(Ca a, Cb b, Cm *multiple)
{
        if (a % b != 0) {
                return false;
        }
...
template <unsigned int N, typename Ca, typename Cb>
inline POLY_CONST_RESULT(N, Ca, Cb) exact_div(const poly_int_pod<N, Ca> &a, Cb b)
{
        typedef POLY_CONST_COEFF(Ca, Cb) C;
        poly_int<N, C> r;
        for (unsigned int i = 0; i < N; i++) {
...
inline typename if_nonpoly2<Cb, Cq, bool>::type can_div_trunc_p(const poly_int_pod<N, Ca> &a, Cb b,
                                                                Cq *quotient)
{
        typedef POLY_CAST(Ca, Cb) NCa;
        typedef POLY_CAST(Cb, Ca) NCb;
...
can_div_trunc_p(const poly_int_pod<N, Ca> &a, const poly_int_pod<N, Cb> &b, Cq *quotient)
{
                        ...
                        if (q == 0) {
                                ...
                                if (a.coeffs[i] != ICa(0)) {
                                        /* Use negative absolute to avoid overflow, i.e.
                                           -|ai| >= -|bi|.  */
                                        C neg_abs_a =
                                                (a.coeffs[i] < 0 ? a.coeffs[i] : -a.coeffs[i]);
                                        C neg_abs_b =
                                                (b.coeffs[i] < 0 ? b.coeffs[i] : -b.coeffs[i]);
...
inline typename if_nonpoly<Cq, bool>::type can_div_trunc_p(const poly_int_pod<N, Ca> &a,
                                                           const poly_int_pod<N, Cb> &b,
...
template <unsigned int N, typename Ca, typename Cb, typename Cq>
inline typename if_nonpoly<Cb, bool>::type can_div_trunc_p(const poly_int_pod<N, Ca> &a, Cb b,
...
can_div_trunc_p(const poly_int_pod<N, Ca> &a, Cb b, poly_int_pod<N, Cq> *quotient, Cr *remainder)
{
        if (!can_div_trunc_p(a, b, quotient)) {
                return false;
        }
...
template <unsigned int N, typename Ca, typename Cb, typename Cq>
inline bool can_div_trunc_p(const poly_int_pod<N, Ca> &a, const poly_int_pod<N, Cb> &b,
                            poly_int_pod<N, Cq> *quotient)
{
        if (b.is_constant()) {
...
template <unsigned int N, typename Ca, typename Cb, typename Cq>
inline typename if_nonpoly<Cq, bool>::type
can_div_away_from_zero_p(const poly_int_pod<N, Ca> &a, const poly_int_pod<N, Cb> &b, Cq *quotient)
{
        if (!can_div_trunc_p(a, b, quotient)) {
                return false;
        }
...
template <unsigned int N, typename C>
void print_dec(const poly_int_pod<N, C> &value, FILE *file, signop sgn)
{
...
template <unsigned int N, typename C> void print_dec(const poly_int_pod<N, C> &value, FILE *file)
{
        STATIC_ASSERT(poly_coeff_traits<C>::signedness >= 0);
        print_dec(value, file, poly_coeff_traits<C>::signedness ? SIGNED : UNSIGNED);
}

/* Use print_hex to print VALUE to FILE.  */

template <unsigned int N, typename C> void print_hex(const poly_int_pod<N, C> &value, FILE *file)
{
        if (value.is_constant()) {
...
template <typename T1, typename T2,
          typename Res = POLY_BINARY_COEFF(POLY_BINARY_COEFF(T1, T2), unsigned HOST_WIDE_INT)>
struct poly_span_traits {
        template <typename T> static const T &cast(const T &x)
        {
                return x;
        }
};

template <typename T1, typename T2> struct poly_span_traits<T1, T2, unsigned HOST_WIDE_INT> {
        template <typename T>
        static typename if_nonpoly<T, unsigned HOST_WIDE_INT>::type cast(const T &x)
        {
                return x;
        }

        template <unsigned int N, typename T>
        static poly_int<N, unsigned HOST_WIDE_INT> cast(const poly_int_pod<N, T> &x)
        {
                return x;
        }
};
...
template <typename T> inline bool known_size_p(const T &a)
{
...
template <typename T1, typename T2, typename T3>
inline bool maybe_in_range_p(const T1 &val, const T2 &pos, const T3 &size)
{
        ...
        if ((poly_int_traits<T1>::num_coeffs > 1 || poly_int_traits<T2>::num_coeffs > 1) &&
            maybe_lt(val, pos)) {
                /* In this case we don't know whether VAL >= POS is true at compile
                   time, so we can't prove that VAL >= POS + SIZE.  */
                return true;
        }
        return maybe_lt(start_span::cast(val) - start_span::cast(pos), size_span::cast(size));
}
...
template <typename T1, typename T2, typename T3>
inline bool known_in_range_p(const T1 &val, const T2 &pos, const T3 &size)
{
        typedef poly_span_traits<T1, T2> start_span;
        typedef poly_span_traits<T3, T3> size_span;
        return (known_size_p(size) && known_ge(val, pos) &&
                known_lt(start_span::cast(val) - start_span::cast(pos), size_span::cast(size)));
}
...
inline bool ranges_maybe_overlap_p(const T1 &pos1, const T2 &size1, const T3 &pos2, const T4 &size2)
{
...
template <typename T1, typename T2, typename T3, typename T4>
inline bool ranges_known_overlap_p(const T1 &pos1, const T2 &size1, const T3 &pos2, const T4 &size2)
{
        ...
        return (known_size_p(size1) && known_size_p(size2) &&
                known_lt(start_span::cast(pos2) - start_span::cast(lower_bound(pos1, pos2)),
                         size1_span::cast(size1)) &&
                known_lt(start_span::cast(pos1) - start_span::cast(lower_bound(pos1, pos2)),
...
template <typename T1, typename T2, typename T3, typename T4>
inline bool known_subrange_p(const T1 &pos1, const T2 &size1, const T3 &pos2, const T4 &size2)
{
        ...
        return (known_gt(size1, POLY_INT_TYPE(T2)(0)) &&
                (poly_coeff_traits<C2>::signedness > 0 || known_size_p(size1)) &&
                known_size_p(size2) && known_ge(pos1, pos2) && known_le(size1, size2) &&
...
template <typename T>
inline typename if_nonpoly<T, bool>::type endpoint_representable_p(const T &pos, const T &size)
{
        return (!known_size_p(size) || pos <= poly_coeff_traits<T>::max_value - size);
}

template <unsigned int N, typename C>
inline bool endpoint_representable_p(const poly_int_pod<N, C> &pos, const poly_int_pod<N, C> &size)
{
        if (known_size_p(size)) {
                for (unsigned int i = 0; i < N; ++i) {
...
template <unsigned int N, typename C> void gt_ggc_mx(poly_int_pod<N, C> *)
{
}

template <unsigned int N, typename C> void gt_pch_nx(poly_int_pod<N, C> *)
{
}

template <unsigned int N, typename C>
void gt_pch_nx(poly_int_pod<N, C> *, void (*)(void *, void *), void *)
{
}
