...
inline bool region_contains_p(var_map map, basic_block bb)
{
...
static inline unsigned num_var_partitions(var_map map)
{
...
static inline tree partition_to_var(var_map map, int i)
{
        tree name;
...
static inline int var_to_partition(var_map map, tree var)
{
        int part;
...
static inline tree var_to_partition_to_var(var_map map, tree var)
{
        int part;
...
static inline int basevar_index(var_map map, int partition)
{
        gcc_checking_assert(partition >= 0 && partition <= (int)num_var_partitions(map));
        return map->partition_to_base_index[partition];
}

/* Return the number of different base variables in MAP.  */

static inline int num_basevars(var_map map)
{
...
static inline int partition_is_global(tree_live_info_p live, int p)
{
...
static inline bitmap live_on_entry(tree_live_info_p live, basic_block bb)
{
        gcc_checking_assert(live->livein && bb != ENTRY_BLOCK_PTR_FOR_FN(cfun) &&
...
static inline bitmap live_on_exit(tree_live_info_p live, basic_block bb)
{
        gcc_checking_assert(live->liveout && bb != ENTRY_BLOCK_PTR_FOR_FN(cfun) &&
                            bb != EXIT_BLOCK_PTR_FOR_FN(cfun));
...
static inline var_map live_var_map(tree_live_info_p live)
{
        return live->map;
}
...
static inline void make_live_on_entry(tree_live_info_p live, basic_block bb, int p)
{
