...
template <typename Type> inline Type *xcallocator<Type>::data_alloc(size_t count)
{
...
template <typename Type> inline void xcallocator<Type>::data_free(Type *memory)
{
        return ::free(memory);
}
...
extern unsigned int hash_table_higher_prime_index(unsigned long n) ATTRIBUTE_PURE;
...
inline hashval_t mul_mod(hashval_t x, hashval_t y, hashval_t inv, int shift)
{
...
inline hashval_t hash_table_mod1(hashval_t hash, unsigned int index)
{
...
inline hashval_t hash_table_mod2(hashval_t hash, unsigned int index)
{
        const struct prime_ent *p = &prime_tab[index];
        gcc_checking_assert(sizeof(hashval_t) * CHAR_BIT <= 32);
...
class hash_table
{
      ...
      public:
        explicit hash_table(size_t, bool ggc = false, bool gather_mem_stats = GATHER_STATISTICS,
        ...
        static hash_table *create_ggc(size_t n CXX_MEM_STAT_INFO)
        {
                hash_table *table = ggc_alloc<hash_table>();
                new (table) hash_table(n, true, GATHER_STATISTICS, HASH_TABLE_ORIGIN PASS_MEM_STAT);
                return table;
        }
        ...
        value_type *find_slot_with_hash(const compare_type &comparable, hashval_t hash,
        ...
        template <typename Argument, int (*Callback)(value_type *slot, Argument argument)>
        ...
        class iterator
        {
              public:
                ...
                iterator(value_type *slot, value_type *limit) : m_slot(slot), m_limit(limit)
                {
                }
      ...
      private:
        ...
        template <typename T> friend void gt_pch_nx(hash_table<T> *, gt_pointer_operator, void *);
...
hash_table<Descriptor, Lazy, Allocator>::hash_table(size_t size, bool ggc,
                                                    bool gather_mem_stats ATTRIBUTE_UNUSED,
                                                    mem_alloc_origin origin MEM_STAT_DECL)
        : m_n_elements(0), m_n_deleted(0), m_searches(0), m_collisions(0), m_ggc(ggc)
#if GATHER_STATISTICS
          ,
...
template <typename Descriptor, bool Lazy, template <typename Type> class Allocator>
hash_table<Descriptor, Lazy, Allocator>::hash_table(const hash_table &h, bool ggc,
                                                    bool gather_mem_stats ATTRIBUTE_UNUSED,
                                                    mem_alloc_origin origin MEM_STAT_DECL)
        : m_n_elements(h.m_n_elements), m_n_deleted(h.m_n_deleted), m_searches(0), m_collisions(0),
          m_ggc(ggc)
#if GATHER_STATISTICS
          ,
          m_gather_mem_stats(gather_mem_stats)
...
hash_table<Descriptor, Lazy, Allocator>::~hash_table()
{
        if (!Lazy || m_entries) {
                ...
                if (m_gather_mem_stats) {
                        hash_table_usage().release_instance_overhead(
                                this, sizeof(value_type) * m_size, true);
                }
...
hash_table<Descriptor, Lazy, Allocator>::alloc_entries(size_t n MEM_STAT_DECL) const
{
        value_type *nentries;
...
hash_table<Descriptor, Lazy, Allocator>::find_empty_slot_for_expand(hashval_t hash)
{
...
template <typename Descriptor, bool Lazy, template <typename Type> class Allocator>
inline bool hash_table<Descriptor, Lazy, Allocator>::too_empty_p(unsigned int elts)
{
...
void hash_table<Descriptor, Lazy, Allocator>::expand()
{
        ...
        if (m_gather_mem_stats) {
                hash_table_usage().release_instance_overhead(this, sizeof(value_type) * osize);
        }
...
{
        gcc_checking_assert(!(slot < m_entries || slot >= m_entries + size() || is_empty(*slot) ||
                              is_deleted(*slot)));
...
template <typename Descriptor, bool Lazy, template <typename Type> class Allocator>
typename hash_table<Descriptor, Lazy, Allocator>::value_type &
hash_table<Descriptor, Lazy, Allocator>::find_with_hash(const compare_type &comparable,
                                                        hashval_t hash)
{
        m_searches++;
        size_t size = m_size;
        ...
        if (is_empty(*entry) || (!is_deleted(*entry) && Descriptor::equal(*entry, comparable))) {
                return *entry;
        }
...
template <typename Descriptor, bool Lazy, template <typename Type> class Allocator>
typename hash_table<Descriptor, Lazy, Allocator>::value_type *
hash_table<Descriptor, Lazy, Allocator>::find_slot_with_hash(const compare_type &comparable,
                                                             hashval_t hash,
                                                             enum insert_option insert)
{
        if (Lazy && m_entries == NULL) {
...
#define FOR_EACH_HASH_TABLE_ELEMENT(HTAB, RESULT, TYPE, ITER)                                      \
        for ((ITER) = (HTAB).begin(); (ITER) != (HTAB).end() ? (RESULT = *(ITER), true) : false;   \
             ++(ITER))

/* ggc walking routines.  */

template <typename E> static inline void gt_ggc_mx(hash_table<E> *h)
{
        ...
        for (size_t i = 0; i < h->m_size; i++) {
                if (table::is_empty(h->m_entries[i]) || table::is_deleted(h->m_entries[i])) {
                        continue;
                }
...
template <typename D>
static inline void hashtab_entry_note_pointers(void *obj, void *h, gt_pointer_operator op,
                                               void *cookie)
{
        ...
        for (size_t i = 0; i < map->m_size; i++) {
                ...
                if (table::is_empty(map->m_entries[i]) || table::is_deleted(map->m_entries[i])) {
                        continue;
                }
...
template <typename D> static void gt_pch_nx(hash_table<D> *h)
{
        bool success = gt_pch_note_object(h->m_entries, h, hashtab_entry_note_pointers<D>);
...
template <typename D>
static inline void gt_pch_nx(hash_table<D> *h, gt_pointer_operator op, void *cookie)
{
        op(&h->m_entries, cookie);
}

template <typename H> inline void gt_cleare_cache(hash_table<H> *h)
{
