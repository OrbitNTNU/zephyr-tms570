...
class hash_set
{
      public:
        ...
        {
                if (Lazy) {
                        return (m_table.find_slot_with_hash(k, Traits::hash(k), NO_INSERT) != NULL);
                }
        ...
        {
              public:
                explicit iterator(const typename hash_table<Traits, Lazy>::iterator &iter)
                        : m_iter(iter)
                {
                }
      ...
      private:
        template <typename T, typename U> friend void gt_ggc_mx(hash_set<T, false, U> *);
        template <typename T, typename U> friend void gt_pch_nx(hash_set<T, false, U> *);
...
template <typename T> void debug_helper(hash_set<T> &ref)
{
        for (typename hash_set<T>::iterator it = ref.begin(); it != ref.end(); ++it) {
                debug_slim(*it);
                fputc('\n', stderr);
        }
}
...
#define DEFINE_DEBUG_HASH_SET(T)                                                                   \
        ...
        DEBUG_FUNCTION void debug(hash_set<T> &ref)                                                \
        {                                                                                          \
                debug_helper<T>(ref);                                                              \
        }                                                                                          \
        DEBUG_FUNCTION void debug(hash_set<T> *ptr)                                                \
        {                                                                                          \
                if (ptr)                                                                           \
                        debug(*ptr);                                                               \
                else                                                                               \
...
template <typename K, typename H> static inline void gt_ggc_mx(hash_set<K, false, H> *h)
{
        gt_ggc_mx(&h->m_table);
}

template <typename K, typename H> static inline void gt_pch_nx(hash_set<K, false, H> *h)
{
        gt_pch_nx(&h->m_table);
}

template <typename K, typename H>
static inline void gt_pch_nx(hash_set<K, false, H> *h, gt_pointer_operator op, void *cookie)
{
