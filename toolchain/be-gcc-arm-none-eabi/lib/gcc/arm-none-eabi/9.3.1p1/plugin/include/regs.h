...
static inline int REG_N_REFS(int regno)
{
...
#define REG_FREQ_FROM_BB(bb)                                                                       \
        (optimize_function_for_size_p(cfun) ? REG_FREQ_MAX                                         \
         : ((bb)->count.to_frequency(cfun) * REG_FREQ_MAX / BB_FREQ_MAX)                           \
                 ? ((bb)->count.to_frequency(cfun) * REG_FREQ_MAX / BB_FREQ_MAX)                   \
                 : 1)
...
static inline unsigned int end_hard_regno(machine_mode mode, unsigned int regno)
{
...
static inline void add_to_hard_reg_set(HARD_REG_SET *regs, machine_mode mode, unsigned int regno)
{
...
static inline void remove_from_hard_reg_set(HARD_REG_SET *regs, machine_mode mode,
                                            unsigned int regno)
{
        unsigned int end_regno;
...
static inline bool in_hard_reg_set_p(const HARD_REG_SET regs, machine_mode mode, unsigned int regno)
{
        unsigned int end_regno;
...
static inline bool overlaps_hard_reg_set_p(const HARD_REG_SET regs, machine_mode mode,
                                           unsigned int regno)
{
        unsigned int end_regno;
...
static inline void add_range_to_hard_reg_set(HARD_REG_SET *regs, unsigned int regno, int nregs)
{
        while (nregs-- > 0) {
...
static inline void remove_range_from_hard_reg_set(HARD_REG_SET *regs, unsigned int regno, int nregs)
{
        while (nregs-- > 0) {
...
/* Like overlaps_hard_reg_set_p, but use a REGNO/NREGS range instead of
   REGNO and MODE.  */
static inline bool range_overlaps_hard_reg_set_p(const HARD_REG_SET set, unsigned regno, int nregs)
{
        while (nregs-- > 0) {
...
/* Like in_hard_reg_set_p, but use a REGNO/NREGS range instead of
   REGNO and MODE.  */
static inline bool range_in_hard_reg_set_p(const HARD_REG_SET set, unsigned regno, int nregs)
{
        while (nregs-- > 0) {
                if (!TEST_HARD_REG_BIT(set, regno + nregs)) {
                        return false;
...
extern bool get_call_reg_set_usage(rtx_insn *insn, HARD_REG_SET *reg_set, HARD_REG_SET default_set);
