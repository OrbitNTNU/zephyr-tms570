...
opt_result dr_analyze_innermost(innermost_loop_behavior *, tree, struct loop *, const gimple *);
extern bool compute_data_dependences_for_loop(struct loop *, bool, vec<loop_p> *,
                                              vec<data_reference_p> *, vec<ddr_p> *);
...
extern opt_result find_data_references_in_stmt(struct loop *, gimple *, vec<data_reference_p> *);
extern bool graphite_find_data_references_in_stmt(edge, loop_p, gimple *, vec<data_reference_p> *);
tree find_data_references_in_loop(struct loop *, vec<data_reference_p> *);
bool loop_nest_has_data_refs(loop_p loop);
struct data_reference *create_data_ref(edge, loop_p, tree, gimple *, bool, bool);
extern bool find_loop_nest(struct loop *, vec<loop_p> *);
extern struct data_dependence_relation *
initialize_data_dependence_relation(struct data_reference *, struct data_reference *, vec<loop_p>);
extern void compute_affine_dependence(struct data_dependence_relation *, loop_p);
extern void compute_self_dependence(struct data_dependence_relation *);
extern bool compute_all_dependences(vec<data_reference_p>, vec<ddr_p> *, vec<loop_p>, bool);
extern tree find_data_references_in_bb(struct loop *, basic_block, vec<data_reference_p> *);
...
inline unsigned int dr_alignment(data_reference *dr)
{
        return dr_alignment(&DR_INNERMOST(dr));
}

extern bool dr_may_alias_p(const struct data_reference *, const struct data_reference *,
                           struct loop *);
extern bool dr_equal_offsets_p(struct data_reference *, struct data_reference *);

extern opt_result runtime_alias_check_p(ddr_p, struct loop *, bool);
extern int data_ref_compare_tree(tree, tree);
extern void prune_runtime_alias_test_list(vec<dr_with_seg_len_pair_t> *, poly_uint64);
extern void create_runtime_alias_checks(struct loop *, vec<dr_with_seg_len_pair_t> *, tree *);
...
static inline bool same_data_refs_base_objects(data_reference_p a, data_reference_p b)
{
...
static inline bool same_data_refs(data_reference_p a, data_reference_p b)
{
        unsigned int i;
...
inline bool known_dependences_p(vec<ddr_p> dependence_relations)
{
        ddr_p ddr;
        unsigned int i;
...
static inline unsigned dependence_level(lambda_vector dist_vect, int length)
{
        int i;
...
static inline unsigned ddr_dependence_level(ddr_p ddr)
{
        unsigned vector;
        unsigned level = 0;
        ...
        for (vector = 1; vector < DDR_NUM_DIST_VECTS(ddr); vector++) {
                level = MIN(level, dependence_level(DDR_DIST_VECT(ddr, vector), DDR_NB_LOOPS(ddr)));
        }
        return level;
}
...
static inline int index_in_loop_nest(int var, vec<loop_p> loop_nest)
{
        struct loop *loopi;
        int var_index;
...
static inline bool adjacent_dr_p(struct data_reference *dr)
{
        /* If this is a bitfield store bail out.  */
        if (TREE_CODE(DR_REF(dr)) == COMPONENT_REF && DECL_BIT_FIELD(TREE_OPERAND(DR_REF(dr), 1))) {
                return false;
        }

        if (!DR_STEP(dr) || TREE_CODE(DR_STEP(dr)) != INTEGER_CST) {
                return false;
        }

        return tree_int_cst_equal(fold_unary(ABS_EXPR, TREE_TYPE(DR_STEP(dr)), DR_STEP(dr)),
...
static inline lambda_int lambda_vector_gcd(lambda_vector vector, int size)
{
        int i;
        lambda_int gcd1 = 0;
...
static inline lambda_vector lambda_vector_new(int size)
{
...
static inline void lambda_vector_clear(lambda_vector vec1, int size)
{
...
static inline bool lambda_vector_lexico_pos(lambda_vector v, unsigned n)
{
        unsigned i;
        for (i = 0; i < n; i++) {
                if (v[i] == 0) {
...
static inline bool lambda_vector_zerop(lambda_vector vec1, int size)
{
        int i;
        for (i = 0; i < size; i++) {
                if (vec1[i] != 0) {
...
static inline lambda_matrix lambda_matrix_new(int m, int n, struct obstack *lambda_obstack)
{
        lambda_matrix mat;
        int i;
