...
/* Finishes constructing a NULL-terminated character string representing
   the buffered text.  */
static inline const char *output_buffer_formatted_text(output_buffer *buff)
{
...
static inline void output_buffer_append_r(output_buffer *buff, const char *start, int length)
{
...
static inline const char *output_buffer_last_position_in_text(const output_buffer *buff)
{
        const char *p = NULL;
...
static inline const char *pp_get_prefix(const pretty_printer *pp)
{
        return pp->prefix;
}
...
#define pp_identifier(PP, ID)                                                                      \
        pp_string(PP, (pp_translate_identifiers(PP) ? identifier_to_locale(ID) : (ID)))
...
#define ATTRIBUTE_GCC_PPDIAG(m, n)                                                                 \
        __attribute__((__format__(GCC_PPDIAG_STYLE, m, n))) ATTRIBUTE_NONNULL(m)
#else
#define ATTRIBUTE_GCC_PPDIAG(m, n) ATTRIBUTE_NONNULL(m)
#endif
extern void pp_printf(pretty_printer *, const char *, ...) ATTRIBUTE_GCC_PPDIAG(2, 3);

extern void pp_verbatim(pretty_printer *, const char *, ...) ATTRIBUTE_GCC_PPDIAG(2, 3);
...
static inline pp_wrapping_mode_t pp_set_verbatim_wrapping_(pretty_printer *pp)
{
...
inline void pp_wide_integer(pretty_printer *pp, HOST_WIDE_INT i)
{
