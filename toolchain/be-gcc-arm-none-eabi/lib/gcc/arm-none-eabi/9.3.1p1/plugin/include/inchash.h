...
class hash
{
      public:
        ...
        template <unsigned int N, typename T> void add_poly_int(const poly_int_pod<N, T> &v)
        {
        ...
        template <unsigned int N, typename T> void add_poly_hwi(const poly_int_pod<N, T> &v)
        {
                for (unsigned int i = 0; i < N; ++i) {
                        add_hwi(v.coeffs[i]);
                }
        ...
        template <typename T> void add_wide_int(const generic_wide_int<T> &x)
        {
                add_int(x.get_len());
...
inline hashval_t iterative_hash_hashval_t(hashval_t val, hashval_t val2)
{
...
inline hashval_t iterative_hash_host_wide_int(HOST_WIDE_INT val, hashval_t val2)
{
