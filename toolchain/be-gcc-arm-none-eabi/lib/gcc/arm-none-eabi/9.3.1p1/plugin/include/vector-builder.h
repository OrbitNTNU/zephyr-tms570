...
template <typename T, typename Derived> class vector_builder : public auto_vec<T, 32>
{
      public:
        vector_builder();
...
inline const Derived *vector_builder<T, Derived>::derived() const
{
        return static_cast<const Derived *>(this);
}

template <typename T, typename Derived>
inline vector_builder<T, Derived>::vector_builder()
        : m_full_nelts(0), m_npatterns(0), m_nelts_per_pattern(0)
{
}
...
template <typename T, typename Derived>
inline unsigned int vector_builder<T, Derived>::encoded_nelts() const
{
...
template <typename T, typename Derived>
inline bool vector_builder<T, Derived>::encoded_full_vector_p() const
{
...
template <typename T, typename Derived>
void vector_builder<T, Derived>::new_vector(poly_uint64 full_nelts, unsigned int npatterns,
...
template <typename T, typename Derived>
bool vector_builder<T, Derived>::operator==(const Derived &other) const
{
        if (maybe_ne(m_full_nelts, other.m_full_nelts) || m_npatterns != other.m_npatterns ||
...
template <typename T, typename Derived> T vector_builder<T, Derived>::elt(unsigned int i) const
{
        ...
        return derived()->apply_step(final, count - 2, derived()->step(prev, final));
}
...
template <typename T, typename Derived>
void vector_builder<T, Derived>::reshape(unsigned int npatterns, unsigned int nelts_per_pattern)
{
...
template <typename T, typename Derived>
bool vector_builder<T, Derived>::repeating_sequence_p(unsigned int start, unsigned int end,
                                                      unsigned int step)
{
...
template <typename T, typename Derived>
bool vector_builder<T, Derived>::stepped_sequence_p(unsigned int start, unsigned int end,
                                                    unsigned int step)
{
        ...
        for (unsigned int i = start + step * 2; i < end; ++i) {
                ...
                if (!derived()->integral_p(elt1) || !derived()->integral_p(elt2) ||
                    !derived()->integral_p(elt3)) {
                        return false;
                }

                if (maybe_ne(derived()->step(elt1, elt2), derived()->step(elt2, elt3))) {
                        return false;
                }
...
template <typename T, typename Derived>
bool vector_builder<T, Derived>::try_npatterns(unsigned int npatterns)
{
...
template <typename T, typename Derived> void vector_builder<T, Derived>::finalize()
{
        ...
        if (m_full_nelts.is_constant(&const_full_nelts) && const_full_nelts <= encoded_nelts()) {
        ...
        /* Try to whittle down the number of elements per pattern.  That is:
           ...
           2. If we have background fill values that are the same as the
              ...
              from 2 to 1.  */
        while (m_nelts_per_pattern > 1 && repeating_sequence_p(encoded_nelts() - m_npatterns * 2,
        ...
        if (pow2p_hwi(m_npatterns)) {
                ...
                if (m_nelts_per_pattern == 1 && m_full_nelts.is_constant(&const_full_nelts) &&
                    this->length() >= const_full_nelts && (m_npatterns & 3) == 0 &&
                    stepped_sequence_p(m_npatterns / 4, const_full_nelts, m_npatterns / 4)) {
