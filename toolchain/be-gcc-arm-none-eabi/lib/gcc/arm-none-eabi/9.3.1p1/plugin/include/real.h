...
#define REAL_EXP(REAL)                                                                             \
        ((int)((REAL)->uexp ^ (unsigned int)(1 << (EXP_BITS - 1))) - (1 << (EXP_BITS - 1)))
...
/* Verify the guess.  */
extern char test_real_width[sizeof(REAL_VALUE_TYPE) <= REAL_WIDTH * sizeof(HOST_WIDE_INT) ? 1 : -1];
...
extern const struct real_format
        *real_format_for_mode[MAX_MODE_FLOAT - MIN_MODE_FLOAT + 1 + MAX_MODE_DECIMAL_FLOAT -
                              MIN_MODE_DECIMAL_FLOAT + 1];

#define REAL_MODE_FORMAT(MODE)                                                                     \
        (real_format_for_mode[DECIMAL_FLOAT_MODE_P(MODE) ? (((MODE)-MIN_MODE_DECIMAL_FLOAT) +      \
                                                            (MAX_MODE_FLOAT - MIN_MODE_FLOAT + 1)) \
                              : GET_MODE_CLASS(MODE) == MODE_FLOAT ? ((MODE)-MIN_MODE_FLOAT)       \
                                                                   : (gcc_unreachable(), 0)])
...
#define MODE_COMPOSITE_P(MODE)                                                                     \
        (FLOAT_MODE_P(MODE) && FLOAT_MODE_FORMAT(MODE)->pnan < FLOAT_MODE_FORMAT(MODE)->p)
...
#define MODE_HAS_SIGN_DEPENDENT_ROUNDING(MODE)                                                     \
        (FLOAT_MODE_P(MODE) && FLOAT_MODE_FORMAT(MODE)->has_sign_dependent_rounding)
...
template <typename T>
inline format_helper::format_helper(const T &m) : m_format(m == VOIDmode ? 0 : REAL_MODE_FORMAT(m))
{
}
...
extern void real_convert(REAL_VALUE_TYPE *, format_helper, const REAL_VALUE_TYPE *);

/* Return true if truncating to NEW is exact.  */
extern bool exact_real_truncate(format_helper, const REAL_VALUE_TYPE *);

/* Render R as a decimal floating point constant.  */
extern void real_to_decimal(char *, const REAL_VALUE_TYPE *, size_t, size_t, int);
...
extern void real_to_decimal_for_mode(char *, const REAL_VALUE_TYPE *, size_t, size_t, int,
                                     machine_mode);

/* Render R as a hexadecimal floating point constant.  */
extern void real_to_hexadecimal(char *, const REAL_VALUE_TYPE *, size_t, size_t, int);
...
extern void real_from_target(REAL_VALUE_TYPE *, const long *, format_helper);
...
#define REAL_VALUE_TO_TARGET_LONG_DOUBLE(IN, OUT)                                                  \
        real_to_target(OUT, &(IN), float_mode_for_size(LONG_DOUBLE_TYPE_SIZE).require())
...
#define REAL_VALUE_TO_TARGET_DECIMAL32(IN, OUT)                                                    \
        ((OUT) = real_to_target(NULL, &(IN), decimal_float_mode_for_size(32).require()))
...
extern bool real_powi(REAL_VALUE_TYPE *, format_helper, const REAL_VALUE_TYPE *, HOST_WIDE_INT);

/* Standard round to integer value functions.  */
extern void real_trunc(REAL_VALUE_TYPE *, format_helper, const REAL_VALUE_TYPE *);
extern void real_floor(REAL_VALUE_TYPE *, format_helper, const REAL_VALUE_TYPE *);
extern void real_ceil(REAL_VALUE_TYPE *, format_helper, const REAL_VALUE_TYPE *);
extern void real_round(REAL_VALUE_TYPE *, format_helper, const REAL_VALUE_TYPE *);
...
extern bool real_nextafter(REAL_VALUE_TYPE *, format_helper, const REAL_VALUE_TYPE *,
                           const REAL_VALUE_TYPE *);
...
extern void real_from_integer(REAL_VALUE_TYPE *, format_helper, const wide_int_ref &, signop);
#endif
