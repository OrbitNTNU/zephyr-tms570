...
static inline bool sel_sched_p(void)
{
...
#define SET_INSN_LUID(INSN, LUID) (sched_luids[INSN_UID(INSN)] = (LUID))
...
extern void sched_create_recovery_edges(basic_block, basic_block, basic_block);
...
#define INSN_AUTOPREF_MULTIPASS_DATA(INSN)         (HID(INSN)->autopref_multipass_data)
...
#define IS_SPECULATION_SIMPLE_CHECK_P(INSN) (RECOVERY_BLOCK(INSN) == EXIT_BLOCK_PTR_FOR_FN(cfun))
...
#define IS_SPECULATION_BRANCHY_CHECK_P(INSN)                                                       \
        (RECOVERY_BLOCK(INSN) != NULL && RECOVERY_BLOCK(INSN) != EXIT_BLOCK_PTR_FOR_FN(cfun))
...
#define NOTE_NOT_BB_P(NOTE) (NOTE_P(NOTE) && (NOTE_KIND(NOTE) != NOTE_INSN_BASIC_BLOCK))
...
extern bool sched_insns_conditions_mutex_p(const rtx_insn *, const rtx_insn *);
...
extern int haifa_classify_insn(const_rtx);
extern void get_ebb_head_tail(basic_block, basic_block, rtx_insn **, rtx_insn **);
...
#define EBB_LAST_BB(ebb)  BASIC_BLOCK_FOR_FN(cfun, rgn_bb_table[ebb_head[ebb + 1] - 1])
...
static inline sd_iterator_def sd_iterator_start(rtx insn, sd_list_types_def types)
{
...
static inline bool sd_iterator_cond(sd_iterator_def *it_ptr, dep_t *dep_ptr)
{
        while (true) {
                ...
                if (link != NULL) {
                        *dep_ptr = DEP_LINK_DEP(link);
                        ...
                        /* Switch to next list.  */
                        {
                                ...
                                sd_next_list(it_ptr->insn, &it_ptr->types, &list,
                                             &it_ptr->resolved_p);
...
/* Advance iterator.  */
static inline void sd_iterator_next(sd_iterator_def *it_ptr)
{
...
#define FOR_EACH_DEP(INSN, LIST_TYPES, ITER, DEP)                                                  \
        for ((ITER) = sd_iterator_start((INSN), (LIST_TYPES)); sd_iterator_cond(&(ITER), &(DEP));  \
