...
template <class K, class V> class fibonacci_node
{
      ...
      public:
        /* Default constructor.  */
        fibonacci_node()
                : m_parent(NULL), m_child(NULL), m_left(this), m_right(this), m_degree(0), m_mark(0)
        {
        }

        /* Constructor for a node with given KEY.  */
        fibonacci_node(K key, V *data = NULL)
                : m_parent(NULL), m_child(NULL), m_left(this), m_right(this), m_key(key),
                  m_data(data), m_degree(0), m_mark(0)
        {
        }
...
template <class K, class V> class fibonacci_heap
{
      ...
      public:
        /* Default constructor.  */
        fibonacci_heap(K global_min_key)
                : m_nodes(0), m_min(NULL), m_root(NULL), m_global_min_key(global_min_key)
        {
        }
...
template <class K, class V> fibonacci_node<K, V> *fibonacci_node<K, V>::remove()
{
...
template <class K, class V> void fibonacci_node<K, V>::link(fibonacci_node<K, V> *parent)
{
...
template <class K, class V> void fibonacci_node<K, V>::insert_after(fibonacci_node<K, V> *b)
{
...
template <class K, class V> fibonacci_node<K, V> *fibonacci_heap<K, V>::insert(K key, V *data)
{
        /* Create the new node.  */
...
fibonacci_node<K, V> *fibonacci_heap<K, V>::insert(fibonacci_node_t *node, K key, V *data)
{
        /* Set the node's data.  */
        node->m_data = data;
        node->m_key = key;
...
template <class K, class V>
V *fibonacci_heap<K, V>::replace_key_data(fibonacci_node<K, V> *node, K key, V *data)
{
        K okey;
...
template <class K, class V> V *fibonacci_heap<K, V>::extract_min(bool release)
{
...
template <class K, class V>
V *fibonacci_heap<K, V>::delete_node(fibonacci_node<K, V> *node, bool release)
{
        V *ret = node->m_data;
...
template <class K, class V> void fibonacci_heap<K, V>::insert_root(fibonacci_node_t *node)
{
...
template <class K, class V>
void fibonacci_heap<K, V>::cut(fibonacci_node<K, V> *node, fibonacci_node<K, V> *parent)
{
        node->remove();
        parent->m_degree--;
        insert_root(node);
...
template <class K, class V> void fibonacci_heap<K, V>::cascading_cut(fibonacci_node<K, V> *y)
{
        fibonacci_node<K, V> *z;
...
template <class K, class V> fibonacci_node<K, V> *fibonacci_heap<K, V>::extract_minimum_node()
{
...
template <class K, class V> void fibonacci_heap<K, V>::remove_root(fibonacci_node<K, V> *node)
{
        if (node->m_left == node) {
                m_root = NULL;
        } else {
...
template <class K, class V> void fibonacci_heap<K, V>::consolidate()
{
