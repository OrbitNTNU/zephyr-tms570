...
class GTY((desc("%h.type"), tag("SYMTAB_SYMBOL"), chain_next("%h.next"),
           chain_prev("%h.previous"))) symtab_node
{
      public:
        ...
        {
                gcc_checking_assert(!group || TREE_CODE(group) == IDENTIFIER_NODE || DECL_P(group));
        ...
        /* Call callback on symtab node and aliases associated to this node.
           ...
           skipped.  */
        bool call_for_symbol_and_aliases(bool (*callback)(symtab_node *, void *), void *data,
        ...
        inline bool can_be_discarded_p(void)
        {
                return (DECL_EXTERNAL(decl) || ((get_comdat_group() || DECL_COMMON(decl) ||
      ...
      protected:
        ...
        bool call_for_symbol_and_aliases_1(bool (*callback)(symtab_node *, void *), void *data,
                                           bool include_overwrite);
...
inline void symtab_node::checking_verify_symtab_nodes(void)
{
        if (flag_checking) {
...
struct GTY((tag("SYMTAB_FUNCTION"))) cgraph_node : public symtab_node {
      public:
        ...
        /* Walk the alias chain to return the function cgraph_node is alias of.
           ...
           When REF is non-NULL, assume that reference happens in symbol REF
           when determining the availability.  */
        cgraph_node *function_or_virtual_thunk_symbol(enum availability *avail = NULL,
                                                      struct symtab_node *ref = NULL);
        ...
        cgraph_node *create_clone(tree decl, profile_count count, bool update_original,
                                  vec<cgraph_edge *> redirect_callers, bool call_duplication_hook,
                                  cgraph_node *new_inlined_to, bitmap args_to_skip,
        ...
        cgraph_node *create_version_clone(tree new_decl, vec<cgraph_edge *> redirect_callers,
                                          bitmap bbs_to_copy, const char *suffix = NULL);
        ...
        cgraph_node *create_version_clone_with_body(vec<cgraph_edge *> redirect_callers,
                                                    vec<ipa_replace_map *, va_gc> *tree_map,
                                                    bitmap args_to_skip, bool skip_return,
        ...
        cgraph_node *create_thunk(tree alias, tree, bool this_adjusting, HOST_WIDE_INT fixed_offset,
                                  HOST_WIDE_INT virtual_value, HOST_WIDE_INT indirect_offset,
                                  tree virtual_offset, tree real_alias);
        ...
        cgraph_edge *create_edge(cgraph_node *callee, gcall *call_stmt, profile_count count);

        /* Create an indirect edge with a yet-undetermined callee where the call
           statement destination is a formal parameter of the caller with index
           PARAM_INDEX. */
        cgraph_edge *create_indirect_edge(gcall *call_stmt, int ecf_flags, profile_count count,
                                          bool compute_indirect_info = true);

        /* Like cgraph_create_edge walk the clone tree and update all clones sharing
         same function body.  If clones already have edge for OLD_STMT; only
         update the edge same way as cgraph_set_call_stmt_including_clones does.  */
        void create_edge_including_clones(cgraph_node *callee, gimple *old_stmt, gcall *stmt,
                                          profile_count count, cgraph_inline_failed_t reason);
        ...
        bool call_for_symbol_and_aliases(bool (*callback)(cgraph_node *, void *), void *data,
      ...
      private:
        ...
        bool call_for_symbol_and_aliases_1(bool (*callback)(cgraph_node *, void *), void *data,
                                           bool include_overwritable);
};
...
class GTY(()) ipa_polymorphic_call_context
{
      public:
        ...
        /* Walk container types and modify context to point to actual class
           ...
           of TYPE.  */
        bool restrict_to_inner_class(tree otr_type, bool consider_placement_new = true,
...
struct GTY((chain_next("%h.next_caller"), chain_prev("%h.prev_caller"), for_user)) cgraph_edge {
        ...
        cgraph_edge *clone(cgraph_node *n, gcall *call_stmt, unsigned stmt_uid, profile_count num,
                           profile_count den, bool update_original);
...
class GTY((tag("SYMTAB_VARIABLE"))) varpool_node : public symtab_node
{
      public:
        ...
        /* Call calback on varpool symbol and aliases associated to varpool symbol.
           When INCLUDE_OVERWRITABLE is false, overwritable aliases and thunks are
           skipped. */
        bool call_for_symbol_and_aliases(bool (*callback)(varpool_node *, void *), void *data,
                                         bool include_overwritable);
      ...
      private:
        ...
        bool call_for_symbol_and_aliases_1(bool (*callback)(varpool_node *, void *), void *data,
                                           bool include_overwritable);
};
...
template <> template <> inline bool is_a_helper<cgraph_node *>::test(symtab_node *p)
{
...
template <> template <> inline bool is_a_helper<varpool_node *>::test(symtab_node *p)
{
...
class GTY((tag("SYMTAB"))) symbol_table
{
      public:
        ...
        symbol_table()
                : cgraph_max_uid(1), cgraph_max_summary_id(0), edges_max_uid(1),
                  edges_max_summary_id(0)
        {
        }
        ...
        inline void clear_asm_symbols(void)
        {
                asmnodes = NULL;
                asm_last_node = NULL;
        }
        ...
        inline asm_node *first_asm_symbol(void)
        {
                return asmnodes;
        }
      ...
      private:
        ...
        /* Allocate a cgraph_edge structure and fill it with data according to the
           ...
           edge).  */
        cgraph_edge *create_edge(cgraph_node *caller, cgraph_node *callee, gcall *call_stmt,
                                 profile_count count, bool indir_unknown_callee);
...
inline hashval_t asmname_hasher::hash(symtab_node *n)
{
        return symbol_table::decl_assembler_name_hash(DECL_ASSEMBLER_NAME(n->decl));
}

inline bool asmname_hasher::equal(symtab_node *n, const_tree t)
{
...
void tree_function_versioning(tree, tree, vec<ipa_replace_map *, va_gc> *, bool, bitmap, bool,
                              bitmap, basic_block);

void dump_callgraph_transformation(const cgraph_node *original, const cgraph_node *clone,
                                   const char *suffix);
tree cgraph_build_function_type_skip_args(tree orig_type, bitmap args_to_skip, bool skip_return);
...
/* Return true when the symbol is real symbol, i.e. it is not inline clone
   or abstract function kept for debug info purposes only.  */
inline bool symtab_node::real_symbol_p(void)
{
        cgraph_node *cnode;
...
static inline bool decl_in_symtab_p(const_tree decl)
{
        return (TREE_CODE(decl) == FUNCTION_DECL ||
                (TREE_CODE(decl) == VAR_DECL && (TREE_STATIC(decl) || DECL_EXTERNAL(decl))));
}

inline bool symtab_node::in_same_comdat_group_p(symtab_node *target)
{
...
inline symtab_node *symtab_node::get_alias_target(void)
{
        ipa_ref *ref = NULL;
...
inline symtab_node *symtab_node::next_defined_symbol(void)
{
...
inline ipa_ref *symtab_node::iterate_reference(unsigned i, ipa_ref *&ref)
{
...
inline ipa_ref *symtab_node::iterate_referring(unsigned i, ipa_ref *&ref)
{
...
inline ipa_ref *symtab_node::iterate_direct_aliases(unsigned i, ipa_ref *&ref)
{
        ref_list.referring.iterate(i, &ref);
...
inline bool symtab_node::has_aliases_p(void)
{
        ipa_ref *ref = NULL;
...
inline bool resolution_used_from_other_file_p(enum ld_plugin_symbol_resolution resolution)
{
        return (resolution == LDPR_PREVAILING_DEF || resolution == LDPR_PREEMPTED_REG ||
                resolution == LDPR_RESOLVED_EXEC || resolution == LDPR_RESOLVED_DYN);
}

/* Return true when symtab_node is known to be used from other (non-LTO)
   object file. Known only when doing LTO via linker plugin.  */

inline bool symtab_node::used_from_object_file_p(void)
{
...
inline varpool_node *varpool_node::get(const_tree decl)
{
...
inline void symbol_table::register_symbol(symtab_node *node)
{
        node->next = nodes;
        node->previous = NULL;
...
/* Unregister a symbol NODE.  */
inline void symbol_table::unregister(symtab_node *node)
{
        if (node->previous) {
...
inline void symbol_table::release_symbol(cgraph_node *node)
{
        cgraph_count--;
...
inline cgraph_node *symbol_table::allocate_cgraph_symbol(void)
{
        cgraph_node *node;
...
/* Return first static symbol with definition.  */
inline symtab_node *symbol_table::first_symbol(void)
{
        return nodes;
}

/* Walk all symbols.  */
#define FOR_EACH_SYMBOL(node) for ((node) = symtab->first_symbol(); (node); (node) = (node)->next)

/* Return first static symbol with definition.  */
inline symtab_node *symbol_table::first_defined_symbol(void)
{
        symtab_node *node;
...
#define FOR_EACH_DEFINED_SYMBOL(node)                                                              \
        for ((node) = symtab->first_defined_symbol(); (node); (node) = node->next_defined_symbol())

/* Return first variable.  */
inline varpool_node *symbol_table::first_variable(void)
{
        symtab_node *node;
        for (node = nodes; node; node = node->next) {
...
/* Return next variable after NODE.  */
inline varpool_node *symbol_table::next_variable(varpool_node *node)
{
...
/* Return first static variable with initializer.  */
inline varpool_node *symbol_table::first_static_initializer(void)
{
        symtab_node *node;
        for (node = nodes; node; node = node->next) {
...
/* Return next static variable with initializer after NODE.  */
inline varpool_node *symbol_table::next_static_initializer(varpool_node *node)
{
        symtab_node *node1 = node->next;
        for (; node1; node1 = node1->next) {
...
/* Return first static variable with definition.  */
inline varpool_node *symbol_table::first_defined_variable(void)
{
        symtab_node *node;
        for (node = nodes; node; node = node->next) {
                varpool_node *vnode = dyn_cast<varpool_node *>(node);
...
/* Return next static variable with definition after NODE.  */
inline varpool_node *symbol_table::next_defined_variable(varpool_node *node)
{
        symtab_node *node1 = node->next;
        for (; node1; node1 = node1->next) {
                varpool_node *vnode1 = dyn_cast<varpool_node *>(node1);
...
/* Return first function with body defined.  */
inline cgraph_node *symbol_table::first_defined_function(void)
{
        symtab_node *node;
        for (node = nodes; node; node = node->next) {
...
/* Return next function with body defined after NODE.  */
inline cgraph_node *symbol_table::next_defined_function(cgraph_node *node)
{
        symtab_node *node1 = node->next;
        for (; node1; node1 = node1->next) {
...
/* Return first function.  */
inline cgraph_node *symbol_table::first_function(void)
{
        symtab_node *node;
        for (node = nodes; node; node = node->next) {
...
/* Return next function.  */
inline cgraph_node *symbol_table::next_function(cgraph_node *node)
{
        symtab_node *node1 = node->next;
        for (; node1; node1 = node1->next) {
...
/* Return first function with body defined.  */
inline cgraph_node *symbol_table::first_function_with_gimple_body(void)
{
        symtab_node *node;
        for (node = nodes; node; node = node->next) {
                cgraph_node *cn = dyn_cast<cgraph_node *>(node);
...
/* Return next reachable static variable with initializer after NODE.  */
inline cgraph_node *symbol_table::next_function_with_gimple_body(cgraph_node *node)
{
        symtab_node *node1 = node->next;
        for (; node1; node1 = node1->next) {
                cgraph_node *cn1 = dyn_cast<cgraph_node *>(node1);
...
#define FOR_EACH_FUNCTION(node)                                                                    \
        for ((node) = symtab->first_function(); (node); (node) = symtab->next_function((node)))
...
inline bool cgraph_node::has_gimple_body_p(void)
{
...
inline bool cgraph_node::former_thunk_p(void)
{
        return (!thunk.thunk_p &&
                (thunk.fixed_offset || thunk.virtual_offset_p || thunk.indirect_offset));
}
...
inline bool cgraph_node::only_called_directly_or_aliased_p(void)
{
        gcc_assert(!global.inlined_to);
        return (!force_output && !address_taken && !ifunc_resolver && !used_from_other_partition &&
                !DECL_VIRTUAL_P(decl) && !DECL_STATIC_CONSTRUCTOR(decl) &&
                !DECL_STATIC_DESTRUCTOR(decl) && !used_from_object_file_p() && !externally_visible);
}

/* Return true when function can be removed from callgraph
   if all direct calls are eliminated.  */

inline bool cgraph_node::can_remove_if_no_direct_calls_and_refs_p(void)
{
        ...
        if (externally_visible &&
            (!DECL_COMDAT(decl) || forced_by_abi || used_from_object_file_p())) {
                return false;
        }
        return true;
}
...
inline void cgraph_node::checking_verify_cgraph_nodes(void)
{
        if (flag_checking) {
...
inline bool varpool_node::can_remove_if_no_refs_p(void)
{
        if (DECL_EXTERNAL(decl)) {
                return true;
        }
        return (!force_output && !used_from_other_partition &&
                ((DECL_COMDAT(decl) && !forced_by_abi && !used_from_object_file_p()) ||
                 !externally_visible || DECL_HAS_VALUE_EXPR_P(decl)));
}
...
inline bool varpool_node::all_refs_explicit_p()
{
        return (definition && !externally_visible && !used_from_other_partition && !force_output);
}
...
inline cgraph_node *cgraph_node::get_alias_target(void)
{
...
inline varpool_node *varpool_node::get_alias_target(void)
{
...
inline symtab_node *symtab_node::ultimate_alias_target(enum availability *availability,
                                                       symtab_node *ref)
{
        if (!alias) {
                if (availability) {
...
inline cgraph_node *cgraph_node::ultimate_alias_target(enum availability *availability,
                                                       symtab_node *ref)
{
        cgraph_node *n =
                dyn_cast<cgraph_node *>(symtab_node::ultimate_alias_target(availability, ref));
        if (!n && availability) {
...
inline varpool_node *varpool_node::ultimate_alias_target(availability *availability,
                                                         symtab_node *ref)
{
        varpool_node *n =
                dyn_cast<varpool_node *>(symtab_node::ultimate_alias_target(availability, ref));
...
inline void cgraph_edge::set_callee(cgraph_node *n)
{
        prev_caller = NULL;
        if (n->callers) {
...
inline void cgraph_edge::redirect_callee(cgraph_node *n)
{
...
inline bool cgraph_edge::recursive_p(void)
{
...
inline void cgraph_edge::remove_callee(void)
{
...
inline bool cgraph_edge::binds_to_current_def_p()
{
        if (callee) {
...
inline int cgraph_edge::frequency()
{
        return count.to_cgraph_frequency(
                caller->global.inlined_to ? caller->global.inlined_to->count : caller->count);
}

/* Return true if the TM_CLONE bit is set for a given FNDECL.  */
static inline bool decl_is_tm_clone(const_tree fndecl)
{
...
inline void cgraph_node::mark_force_output(void)
{
        force_output = 1;
        gcc_checking_assert(!global.inlined_to);
}
...
inline bool cgraph_node::optimize_for_size_p(void)
{
...
inline enum availability symtab_node::get_availability(symtab_node *ref)
{
        if (is_a<cgraph_node *>(this)) {
...
inline bool symtab_node::call_for_symbol_and_aliases(bool (*callback)(symtab_node *, void *),
                                                     void *data, bool include_overwritable)
{
        if (include_overwritable || get_availability() > AVAIL_INTERPOSABLE) {
...
inline bool cgraph_node::call_for_symbol_and_aliases(bool (*callback)(cgraph_node *, void *),
                                                     void *data, bool include_overwritable)
{
        if (include_overwritable || get_availability() > AVAIL_INTERPOSABLE) {
                if (callback(this, data)) {
...
inline bool varpool_node::call_for_symbol_and_aliases(bool (*callback)(varpool_node *, void *),
                                                      void *data, bool include_overwritable)
{
        if (include_overwritable || get_availability() > AVAIL_INTERPOSABLE) {
                if (callback(this, data)) {
...
inline bool ipa_ref::address_matters_p()
{
        if (use != IPA_REF_ADDR) {
                return false;
        }
        /* Addresses taken from virtual tables are never compared.  */
        if (is_a<varpool_node *>(referring) && DECL_VIRTUAL_P(referring->decl)) {
                return false;
        }
...
inline ipa_polymorphic_call_context::ipa_polymorphic_call_context(cgraph_edge *e)
{
...
inline ipa_polymorphic_call_context::ipa_polymorphic_call_context()
{
        clear_speculation();
        clear_outer_type();
        invalid = false;
...
inline void ipa_polymorphic_call_context::clear_speculation()
{
...
inline void ipa_polymorphic_call_context::clear_outer_type(tree otr_type)
{
...
inline void ipa_polymorphic_call_context::offset_by(HOST_WIDE_INT off)
{
        if (outer_type) {
                offset += off;
        }
...
inline bool ipa_polymorphic_call_context::useless_p() const
{
...
static inline const char *xstrdup_for_dump(const char *transient_str)
{
...
/* During LTO stream-in this predicate can be used to check whether node
   in question prevails in the linking to save some memory usage.  */
inline bool symtab_node::prevailing_p(void)
{
        return definition &&
               ((!TREE_PUBLIC(decl) && !DECL_EXTERNAL(decl)) || previous_sharing_asm_name == NULL);
}
