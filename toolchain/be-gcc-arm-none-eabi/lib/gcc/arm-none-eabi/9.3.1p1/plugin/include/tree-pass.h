...
{
      protected:
        gimple_opt_pass(const pass_data &data, gcc::context *ctxt) : opt_pass(data, ctxt)
        {
        }
};
...
{
      protected:
        rtl_opt_pass(const pass_data &data, gcc::context *ctxt) : opt_pass(data, ctxt)
        {
        }
};
...
class ipa_opt_pass_d : public opt_pass
{
      ...
      protected:
        ipa_opt_pass_d(const pass_data &data, gcc::context *ctxt, void (*generate_summary)(void),
                       void (*write_summary)(void), void (*read_summary)(void),
                       void (*write_optimization_summary)(void),
                       void (*read_optimization_summary)(void),
                       void (*stmt_fixup)(struct cgraph_node *, gimple **),
                       unsigned int function_transform_todo_flags_start,
                       unsigned int (*function_transform)(struct cgraph_node *),
                       void (*variable_transform)(varpool_node *))
                : opt_pass(data, ctxt), generate_summary(generate_summary),
                  ...
                  function_transform(function_transform), variable_transform(variable_transform)
        {
        }
};
...
{
      protected:
        simple_ipa_opt_pass(const pass_data &data, gcc::context *ctxt) : opt_pass(data, ctxt)
        {
        }
};
...
extern void register_pass(opt_pass *pass, pass_positioning_ops pos, const char *ref_pass_name,
...
extern gimple_opt_pass *make_pass_remove_cgraph_callee_edges(gcc::context *ctxt);
...
extern ipa_opt_pass_d *make_pass_ipa_whole_program_visibility(gcc::context *ctxt);
extern simple_ipa_opt_pass *make_pass_ipa_increase_alignment(gcc::context *ctxt);
...
extern simple_ipa_opt_pass *make_pass_materialize_all_clones(gcc::context *ctxt);

extern gimple_opt_pass *make_pass_cleanup_cfg_post_optimizing(gcc::context *ctxt);
...
extern rtl_opt_pass *make_pass_value_profile_transformations(gcc::context *ctxt);
...
extern rtl_opt_pass *make_pass_branch_target_load_optimize1(gcc::context *ctxt);
extern rtl_opt_pass *make_pass_thread_prologue_and_epilogue(gcc::context *ctxt);
...
extern rtl_opt_pass *make_pass_branch_target_load_optimize2(gcc::context *ctxt);
