...
#define OBJC_IS_CXX_KEYWORD(rid)                                                                   \
        (rid == RID_CLASS || rid == RID_SYNCHRONIZED || rid == RID_PUBLIC ||                       \
         rid == RID_PROTECTED || rid == RID_PRIVATE || rid == RID_TRY || rid == RID_THROW ||       \
         rid == RID_CATCH)
...
#define C_CPP_HASHNODE(id) (&(((struct c_common_identifier *)(id))->node))
#define C_RID_CODE(id)     ((enum rid)(((struct c_common_identifier *)(id))->node.rid_code))
...
#define abi_compat_version_crosses(N)                                                              \
        (abi_version_at_least(N) !=                                                                \
         (flag_abi_compat_version == 0 || flag_abi_compat_version >= (N)))
...
#define abi_version_crosses(N)                                                                     \
        (abi_version_at_least(N) != (warn_abi_version == 0 || warn_abi_version >= (N)))
...
extern bool check_function_arguments(location_t loc, const_tree, const_tree, int, tree *,
                                     vec<location_t> *);
extern void check_function_arguments_recurse(void (*)(void *, tree, unsigned HOST_WIDE_INT), void *,
                                             tree, unsigned HOST_WIDE_INT);
extern bool check_builtin_function_arguments(location_t, vec<location_t>, tree, int, tree *);
extern void check_function_format(const_tree, tree, int, tree *, vec<location_t> *);
extern bool attribute_fallthrough_p(tree);
extern tree handle_format_attribute(tree *, tree, tree, int, bool *);
extern tree handle_format_arg_attribute(tree *, tree, tree, int, bool *);
extern bool c_common_handle_option(size_t, const char *, HOST_WIDE_INT, int, location_t,
                                   const struct cl_option_handlers *);
extern bool default_handle_c_option(size_t, const char *, int);
extern tree c_common_type_for_mode(machine_mode, int);
extern tree c_common_type_for_size(unsigned int, int);
extern tree c_common_fixed_point_type_for_size(unsigned int, unsigned int, int, int);
extern tree c_common_unsigned_type(tree);
extern tree c_common_signed_type(tree);
extern tree c_common_signed_or_unsigned_type(int, tree);
extern void c_common_init_ts(void);
extern tree c_build_bitfield_integer_type(unsigned HOST_WIDE_INT, int);
extern enum conversion_safety unsafe_conversion_p(location_t, tree, tree, tree, bool);
...
extern tree shorten_compare(location_t, tree *, tree *, tree *, enum tree_code *);

extern tree pointer_int_sum(location_t, enum tree_code, tree, tree, bool = true);
...
#define C_MAYBE_CONST_EXPR_PRE(NODE)          TREE_OPERAND(C_MAYBE_CONST_EXPR_CHECK(NODE), 0)
#define C_MAYBE_CONST_EXPR_EXPR(NODE)         TREE_OPERAND(C_MAYBE_CONST_EXPR_CHECK(NODE), 1)
#define C_MAYBE_CONST_EXPR_INT_OPERANDS(NODE) TREE_LANG_FLAG_0(C_MAYBE_CONST_EXPR_CHECK(NODE))
#define C_MAYBE_CONST_EXPR_NON_CONST(NODE)    TREE_LANG_FLAG_1(C_MAYBE_CONST_EXPR_CHECK(NODE))
#define EXPR_INT_CONST_OPERANDS(EXPR)                                                              \
        (INTEGRAL_TYPE_P(TREE_TYPE(EXPR)) &&                                                       \
         (TREE_CODE(EXPR) == INTEGER_CST ||                                                        \
          (TREE_CODE(EXPR) == C_MAYBE_CONST_EXPR && C_MAYBE_CONST_EXPR_INT_OPERANDS(EXPR))))
...
#define DECL_UNNAMED_BIT_FIELD(NODE) (DECL_C_BIT_FIELD(NODE) && !DECL_NAME(NODE))
...
extern tree build_function_call_vec(location_t, vec<location_t>, tree, vec<tree, va_gc> *,
                                    vec<tree, va_gc> *);
...
extern bool c_cpp_diagnostic(cpp_reader *, enum cpp_diagnostic_level, enum cpp_warning_reason,
                             rich_location *, const char *, va_list *) ATTRIBUTE_GCC_DIAG(5, 0);
...
extern tree fold_offsetof(tree, tree = size_type_node, tree_code ctx = ERROR_MARK);
...
/* Callback for libcpp for offering spelling suggestions for misspelled
   directives.  */
extern const char *cb_get_suggestion(cpp_reader *, const char *, const char *const *);

extern GTY(()) string_concat_db *g_string_concat_db;

class substring_loc;
extern const char *c_get_substring_location(const substring_loc &substr_loc, location_t *out_loc);
...
/* In c-lex.c.  */
extern enum cpp_ttype conflict_marker_get_final_tok_kind(enum cpp_ttype tok1_kind);
...
extern tree c_finish_omp_atomic(location_t, enum tree_code, enum tree_code, tree, tree, tree, tree,
                                tree, bool, enum omp_memory_order, bool = false);
extern bool c_omp_depend_t_p(tree);
extern void c_finish_omp_depobj(location_t, tree, enum omp_clause_depend_kind, tree);
extern void c_finish_omp_flush(location_t, int);
extern void c_finish_omp_taskwait(location_t);
extern void c_finish_omp_taskyield(location_t);
extern tree c_finish_omp_for(location_t, enum tree_code, tree, tree, tree, tree, tree, tree, tree,
                             bool);
extern bool c_omp_check_loop_iv(tree, tree, walk_tree_lh);
extern bool c_omp_check_loop_iv_exprs(location_t, tree, tree, tree, tree, walk_tree_lh);
extern tree c_finish_oacc_wait(location_t, tree, tree);
extern tree c_oacc_split_loop_clauses(tree, tree *, bool);
extern void c_omp_split_clauses(location_t, enum tree_code, omp_clause_mask, tree, tree *);
...
static inline tree c_tree_chain_next(tree t)
{
...
#define USERDEF_LITERAL_TYPE(NODE) (TREE_TYPE(USERDEF_LITERAL_VALUE(NODE)))

extern tree build_userdef_literal(tree suffix_id, tree value, enum overflow_type overflow,
                                  tree num_string);
...
extern enum stv_conv scalar_to_vector(location_t loc, enum tree_code code, tree op0, tree op1,
                                      bool);
...
extern void warn_logical_operator(location_t, enum tree_code, tree, enum tree_code, tree,
                                  enum tree_code, tree);
...
extern void sizeof_pointer_memaccess_warning(location_t *, tree, vec<tree, va_gc> *, tree *,
                                             bool (*)(tree, tree));
...
extern void warn_about_parentheses(location_t, enum tree_code, enum tree_code, tree, enum tree_code,
                                   tree);
extern void warn_for_unused_label(tree label);
extern void warn_for_div_by_zero(location_t, tree divisor);
extern void warn_for_memset(location_t, tree, tree, int);
extern void warn_for_sign_compare(location_t, tree orig_op0, tree orig_op1, tree op0, tree op1,
...
extern tree positional_argument(const_tree, const_tree, tree, tree_code, int = 0,
                                int = posargflags());
...
extern void maybe_suggest_missing_token_insertion(rich_location *richloc, enum cpp_ttype token_type,
