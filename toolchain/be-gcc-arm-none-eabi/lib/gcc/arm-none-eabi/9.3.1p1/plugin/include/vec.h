...
inline unsigned vec_prefix::calculate_allocation(vec_prefix *pfx, unsigned reserve, bool exact)
{
        if (exact) {
...
template <typename T>
inline void va_heap::reserve(vec<T, va_heap, vl_embed> *&v, unsigned reserve,
                             bool exact MEM_STAT_DECL)
{
        size_t elt_size = sizeof(T);
        unsigned alloc = vec_prefix::calculate_allocation(v ? &v->m_vecpfx : 0, reserve, exact);
        gcc_checking_assert(alloc);

        if (GATHER_STATISTICS && v) {
                v->m_vecpfx.release_overhead(v, elt_size * v->allocated(), v->allocated(), false);
        }
...
template <typename T> void va_heap::release(vec<T, va_heap, vl_embed> *&v)
{
        size_t elt_size = sizeof(T);
        if (v == NULL) {
                return;
        ...
        if (GATHER_STATISTICS) {
                v->m_vecpfx.release_overhead(v, elt_size * v->allocated(), v->allocated(), true);
        }
        ::free(v);
        v = NULL;
}
...
struct va_gc {
        ...
        template <typename T, typename A> static void release(vec<T, A, vl_embed> *&v);
};

/* Free GC memory used by V and reset V to NULL.  */

template <typename T, typename A> inline void va_gc::release(vec<T, A, vl_embed> *&v)
{
        if (v) {
                ::ggc_free(v);
        }
...
template <typename T, typename A>
void va_gc::reserve(vec<T, A, vl_embed> *&v, unsigned reserve, bool exact MEM_STAT_DECL)
{
        unsigned alloc = vec_prefix::calculate_allocation(v ? &v->m_vecpfx : 0, reserve, exact);
        if (!alloc) {
                ::ggc_free(v);
        ...
        unsigned nelem = v ? v->length() : 0;
        v = static_cast<vec<T, A, vl_embed> *>(::ggc_realloc(v, size PASS_MEM_STAT));
        v->embedded_init(alloc, nelem);
}
...
template <typename T> void debug_helper(vec<T> &ref)
{
        unsigned i;
...
template <typename T> void debug_helper(vec<T, va_gc> &ref)
{
        unsigned i;
        for (i = 0; i < ref.length(); ++i) {
                fprintf(stderr, "[%d] = ", i);
...
#define DEFINE_DEBUG_VEC(T)                                                                        \
        ...
        DEBUG_FUNCTION void debug(vec<T> &ref)                                                     \
        {                                                                                          \
                debug_helper<T>(ref);                                                              \
        }                                                                                          \
        DEBUG_FUNCTION void debug(vec<T> *ptr)                                                     \
        {                                                                                          \
                if (ptr)                                                                           \
                        debug(*ptr);                                                               \
                else                                                                               \
        ...
        DEBUG_FUNCTION void debug(vec<T, va_gc> &ref)                                              \
        {                                                                                          \
                debug_helper<T>(ref);                                                              \
        }                                                                                          \
        DEBUG_FUNCTION void debug(vec<T, va_gc> *ptr)                                              \
        {                                                                                          \
                if (ptr)                                                                           \
                        debug(*ptr);                                                               \
                else                                                                               \
...
template <typename T> inline void vec_default_construct(T *dst, unsigned n)
{
...
template <typename T> inline void vec_copy_construct(T *dst, const T *src, unsigned n)
{
        for (; n; ++dst, ++src, --n) {
...
struct vnull {
        template <typename T, typename A, typename L> CONSTEXPR operator vec<T, A, L>()
        {
                return vec<T, A, L>();
        }
};
...
template <typename T, typename A> struct GTY((user)) vec<T, A, vl_embed> {
      public:
...
/* If V contains no room for NELEMS elements, return false. Otherwise,
   return true.  */
template <typename T, typename A>
inline bool vec_safe_space(const vec<T, A, vl_embed> *v, unsigned nelems)
{
...
template <typename T, typename A> inline T *vec_safe_address(vec<T, A, vl_embed> *v)
{
        return v ? v->address() : NULL;
}

/* If V is NULL, return true.  Otherwise, return V->is_empty().  */
template <typename T, typename A> inline bool vec_safe_is_empty(vec<T, A, vl_embed> *v)
{
...
template <typename T, typename A>
inline bool vec_safe_reserve(vec<T, A, vl_embed> *&v, unsigned nelems,
...
template <typename T, typename A>
inline bool vec_safe_reserve_exact(vec<T, A, vl_embed> *&v, unsigned nelems CXX_MEM_STAT_INFO)
{
...
template <typename T, typename A>
inline void vec_alloc(vec<T, A, vl_embed> *&v, unsigned nelems CXX_MEM_STAT_INFO)
{
        v = NULL;
...
template <typename T, typename A> inline void vec_free(vec<T, A, vl_embed> *&v)
{
        A::release(v);
}
...
template <typename T, typename A>
inline void vec_safe_grow(vec<T, A, vl_embed> *&v, unsigned len CXX_MEM_STAT_INFO)
{
...
template <typename T, typename A>
inline void vec_safe_grow_cleared(vec<T, A, vl_embed> *&v, unsigned len CXX_MEM_STAT_INFO)
{
        unsigned oldlen = vec_safe_length(v);
...
template <typename T>
inline void vec_safe_grow_cleared(vec<T, va_heap, vl_ptr> *&v, unsigned len CXX_MEM_STAT_INFO)
{
...
template <typename T, typename A>
inline bool vec_safe_iterate(const vec<T, A, vl_embed> *v, unsigned ix, T **ptr)
{
        if (v) {
...
/* If V has no room for one more element, reallocate it.  Then call
   V->quick_push(OBJ).  */
template <typename T, typename A>
inline T *vec_safe_push(vec<T, A, vl_embed> *&v, const T &obj CXX_MEM_STAT_INFO)
{
...
template <typename T, typename A>
inline void vec_safe_insert(vec<T, A, vl_embed> *&v, unsigned ix, const T &obj CXX_MEM_STAT_INFO)
{
        vec_safe_reserve(v, 1, false PASS_MEM_STAT);
        v->quick_insert(ix, obj);
}
...
template <typename T, typename A>
inline void vec_safe_truncate(vec<T, A, vl_embed> *v, unsigned size)
{
        if (v) {
                v->truncate(size);
        }
...
template <typename T, typename A>
inline vec<T, A, vl_embed> *vec_safe_copy(vec<T, A, vl_embed> *src CXX_MEM_STAT_INFO)
{
...
template <typename T, typename A>
inline void vec_safe_splice(vec<T, A, vl_embed> *&dst,
                            const vec<T, A, vl_embed> *src CXX_MEM_STAT_INFO)
{
        unsigned src_len = vec_safe_length(src);
        if (src_len) {
                vec_safe_reserve_exact(dst, vec_safe_length(dst) + src_len PASS_MEM_STAT);
                dst->splice(*src);
        }
}
...
template <typename T, typename A>
inline bool vec_safe_contains(vec<T, A, vl_embed> *v, const T &search)
{
...
template <typename T, typename A> inline const T &vec<T, A, vl_embed>::operator[](unsigned ix) const
{
        gcc_checking_assert(ix < m_vecpfx.m_num);
        return m_vecdata[ix];
}

template <typename T, typename A> inline T &vec<T, A, vl_embed>::operator[](unsigned ix)
{
        gcc_checking_assert(ix < m_vecpfx.m_num);
        return m_vecdata[ix];
}
...
template <typename T, typename A> inline T &vec<T, A, vl_embed>::last(void)
{
...
template <typename T, typename A> inline bool vec<T, A, vl_embed>::space(unsigned nelems) const
{
...
template <typename T, typename A>
inline bool vec<T, A, vl_embed>::iterate(unsigned ix, T *ptr) const
{
...
template <typename T, typename A>
inline vec<T, A, vl_embed> *vec<T, A, vl_embed>::copy(ALONE_MEM_STAT_DECL) const
{
...
template <typename T, typename A>
inline void vec<T, A, vl_embed>::splice(const vec<T, A, vl_embed> &src)
{
...
template <typename T, typename A> inline T *vec<T, A, vl_embed>::quick_push(const T &obj)
{
...
template <typename T, typename A> inline T &vec<T, A, vl_embed>::pop(void)
{
...
template <typename T, typename A> inline void vec<T, A, vl_embed>::truncate(unsigned size)
{
...
template <typename T, typename A>
inline void vec<T, A, vl_embed>::quick_insert(unsigned ix, const T &obj)
{
...
template <typename T, typename A> inline void vec<T, A, vl_embed>::ordered_remove(unsigned ix)
{
        gcc_checking_assert(ix < length());
        T *slot = &m_vecdata[ix];
...
#define VEC_ORDERED_REMOVE_IF_FROM_TO(vec, read_index, write_index, elem_ptr, start, end, cond)    \
        {                                                                                          \
                ...
                for (read_index = write_index = (start); read_index < (end); ++read_index) {       \
...
#define VEC_ORDERED_REMOVE_IF(vec, read_index, write_index, elem_ptr, cond)                        \
        VEC_ORDERED_REMOVE_IF_FROM_TO((vec), read_index, write_index, elem_ptr, 0, (vec).length(), \
                                      (cond))

/* Remove an element from the IXth position of this vector.  Ordering of
   remaining elements is destroyed.  This is an O(1) operation.  */

template <typename T, typename A> inline void vec<T, A, vl_embed>::unordered_remove(unsigned ix)
{
        gcc_checking_assert(ix < length());
...
template <typename T, typename A>
inline void vec<T, A, vl_embed>::block_remove(unsigned ix, unsigned len)
{
...
template <typename T, typename A>
inline void vec<T, A, vl_embed>::qsort(int (*cmp)(const void *, const void *))
{
        if (length() > 1) {
...
template <typename T, typename A>
inline T *vec<T, A, vl_embed>::bsearch(const void *key, int (*compar)(const void *, const void *))
{
...
template <typename T, typename A> inline bool vec<T, A, vl_embed>::contains(const T &search) const
{
...
template <typename T, typename A>
unsigned vec<T, A, vl_embed>::lower_bound(T obj, bool (*lessthan)(const T &, const T &)) const
{
        unsigned int len = length();
...
template <typename T, typename A> inline size_t vec<T, A, vl_embed>::embedded_size(unsigned alloc)
{
...
template <typename T, typename A>
inline void vec<T, A, vl_embed>::embedded_init(unsigned alloc, unsigned num, unsigned aut)
{
...
template <typename T, typename A> inline void vec<T, A, vl_embed>::quick_grow(unsigned len)
{
...
template <typename T, typename A> inline void vec<T, A, vl_embed>::quick_grow_cleared(unsigned len)
{
...
template <typename T> void gt_ggc_mx(vec<T, va_gc> *v)
{
...
template <typename T> void gt_ggc_mx(vec<T, va_gc_atomic, vl_embed> *v ATTRIBUTE_UNUSED)
{
...
template <typename T, typename A> void gt_pch_nx(vec<T, A, vl_embed> *v)
{
...
/* auto_vec is a subclass of vec that automatically manages creating and
   ...
   */
template <typename T, size_t N = 0> class auto_vec : public vec<T, va_heap>
{
      public:
        auto_vec()
        {
...
/* auto_vec is a sub class of vec whose storage is released when it is
  destroyed. */
template <typename T> class auto_vec<T, 0> : public vec<T, va_heap>
{
      public:
        auto_vec()
        {
...
template <typename T> inline void vec_alloc(vec<T> *&v, unsigned nelems CXX_MEM_STAT_INFO)
{
        v = new vec<T>;
...
template <typename T>
inline void vec_check_alloc(vec<T, va_heap> *&vec, unsigned nelems CXX_MEM_STAT_INFO)
{
        if (!vec) {
...
template <typename T> inline void vec_free(vec<T> *&v)
{
        if (v == NULL) {
                return;
        }
...
template <typename T> inline bool vec<T, va_heap, vl_ptr>::iterate(unsigned ix, T *ptr) const
{
        if (m_vec) {
...
#define FOR_EACH_VEC_ELT_REVERSE(V, I, P) for (I = (V).length() - 1; (V).iterate((I), &(P)); (I)--)

#define FOR_EACH_VEC_SAFE_ELT_REVERSE(V, I, P)                                                     \
        for (I = vec_safe_length(V) - 1; vec_safe_iterate((V), (I), &(P)); (I)--)
...
inline auto_string_vec::~auto_string_vec()
{
        int i;
        char *str;
...
template <typename T>
inline vec<T, va_heap, vl_ptr> vec<T, va_heap, vl_ptr>::copy(ALONE_MEM_STAT_DECL) const
{
...
template <typename T>
inline bool vec<T, va_heap, vl_ptr>::reserve(unsigned nelems, bool exact MEM_STAT_DECL)
{
        if (space(nelems)) {
                return false;
        }
...
template <typename T>
inline bool vec<T, va_heap, vl_ptr>::reserve_exact(unsigned nelems MEM_STAT_DECL)
{
...
template <typename T> inline void vec<T, va_heap, vl_ptr>::create(unsigned nelems MEM_STAT_DECL)
{
        m_vec = NULL;
        if (nelems > 0) {
...
template <typename T> inline void vec<T, va_heap, vl_ptr>::release(void)
{
        if (!m_vec) {
                return;
        }
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::splice(const vec<T, va_heap, vl_ptr> &src)
{
        if (src.length()) {
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::safe_splice(const vec<T, va_heap, vl_ptr> &src MEM_STAT_DECL)
{
        if (src.length()) {
...
template <typename T> inline T *vec<T, va_heap, vl_ptr>::quick_push(const T &obj)
{
...
template <typename T> inline T *vec<T, va_heap, vl_ptr>::safe_push(const T &obj MEM_STAT_DECL)
{
...
template <typename T> inline T &vec<T, va_heap, vl_ptr>::pop(void)
{
        return m_vec->pop();
}
...
template <typename T> inline void vec<T, va_heap, vl_ptr>::truncate(unsigned size)
{
        if (m_vec) {
                m_vec->truncate(size);
        } else {
...
template <typename T> inline void vec<T, va_heap, vl_ptr>::safe_grow(unsigned len MEM_STAT_DECL)
{
        unsigned oldlen = length();
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::safe_grow_cleared(unsigned len MEM_STAT_DECL)
{
        unsigned oldlen = length();
        size_t growby = len - oldlen;
...
template <typename T> inline void vec<T, va_heap, vl_ptr>::quick_grow(unsigned len)
{
...
template <typename T> inline void vec<T, va_heap, vl_ptr>::quick_grow_cleared(unsigned len)
{
        gcc_checking_assert(m_vec);
...
template <typename T> inline void vec<T, va_heap, vl_ptr>::quick_insert(unsigned ix, const T &obj)
{
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::safe_insert(unsigned ix, const T &obj MEM_STAT_DECL)
{
        reserve(1, false PASS_MEM_STAT);
        quick_insert(ix, obj);
}
...
template <typename T> inline void vec<T, va_heap, vl_ptr>::ordered_remove(unsigned ix)
{
...
template <typename T> inline void vec<T, va_heap, vl_ptr>::unordered_remove(unsigned ix)
{
...
template <typename T>
inline void vec<T, va_heap, vl_ptr>::qsort(int (*cmp)(const void *, const void *))
{
        if (m_vec) {
                m_vec->qsort(cmp);
        }
...
template <typename T>
inline T *vec<T, va_heap, vl_ptr>::bsearch(const void *key, int (*cmp)(const void *, const void *))
{
        if (m_vec) {
...
template <typename T>
inline unsigned vec<T, va_heap, vl_ptr>::lower_bound(T obj,
...
template <typename T> inline bool vec<T, va_heap, vl_ptr>::contains(const T &search) const
{
...
template <typename T> inline void vec<T, va_heap, vl_ptr>::reverse(void)
{
        unsigned l = length();
        T *ptr = address();
...
template <typename T> inline bool vec<T, va_heap, vl_ptr>::using_auto_storage() const
{
...
template <typename T> inline void release_vec_vec(vec<vec<T>> &vec)
{
