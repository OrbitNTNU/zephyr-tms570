...
static inline _list_t _list_alloc(void)
{
        return sched_lists_pool.allocate();
}

static inline void _list_add(_list_t *lp)
{
...
static inline void _list_remove_nofree(_list_t *lp)
{
        _list_t n = *lp;
...
static inline void _list_remove(_list_t *lp)
{
        _list_t n = *lp;
...
static inline void _list_clear(_list_t *l)
{
        while (*l) {
                _list_remove(l);
        }
...
static inline void _list_iter_start(_list_iterator *ip, _list_t *lp, bool can_remove_p)
{
        ip->lp = lp;
...
static inline void _list_iter_next(_list_iterator *ip)
{
        if (!ip->removed_p) {
...
static inline void _list_iter_remove(_list_iterator *ip)
{
...
static inline void _list_iter_remove_nofree(_list_iterator *ip)
{
        gcc_assert(!ip->removed_p && ip->can_remove_p);
...
#define _FOR_EACH(TYPE, ELEM, I, L)                                                                \
        for (_list_iter_start(&(I), &(L), false); _list_iter_cond_##TYPE(*(I).lp, &(ELEM));        \
             _list_iter_next(&(I)))

#define _FOR_EACH_1(TYPE, ELEM, I, LP)                                                             \
        for (_list_iter_start(&(I), (LP), true); _list_iter_cond_##TYPE(*(I).lp, &(ELEM));         \
             _list_iter_next(&(I)))

/* _xlist_t functions.  */

static inline void _xlist_add(_xlist_t *lp, rtx x)
{
        _list_add(lp);
        _XLIST_X(*lp) = x;
}
...
static inline bool _xlist_is_in_p(_xlist_t l, rtx x)
{
        while (l) {
                if (_XLIST_X(l) == x) {
                        return true;
...
static inline bool _list_iter_cond_x(_xlist_t l, rtx *xp)
{
        if (l) {
                *xp = _XLIST_X(l);
                return true;
...
static inline void ilist_add(ilist_t *lp, insn_t insn)
{
        _list_add(lp);
        ILIST_INSN(*lp) = insn;
}
...
static inline bool ilist_is_in_p(ilist_t l, insn_t insn)
{
        while (l) {
...
/* Used through _FOR_EACH.  */
static inline bool _list_iter_cond_insn(ilist_t l, insn_t *ip)
{
        if (l) {
                *ip = ILIST_INSN(l);
                return true;
...
inline bool _list_iter_cond_expr(av_set_t av, expr_t *exprp)
{
        if (av) {
...
static inline bool _list_iter_cond_def(def_list_t def_list, def_t *def)
{
        if (def_list) {
...
#define LOOP_PREHEADER_BLOCKS(LOOP)                                                                \
        ((size_t)((LOOP)->aux) == 1 ? NULL : ((vec<basic_block> *)(LOOP)->aux))
#define SET_LOOP_PREHEADER_BLOCKS(LOOP, BLOCKS)                                                    \
        ((LOOP)->aux = (BLOCKS != NULL ? BLOCKS : (LOOP)->aux))
...
static inline bool inner_loop_header_p(basic_block bb)
{
        ...
        if (bb == inner_loop->header && flow_bb_inside_loop_p(current_loop_nest, bb)) {
...
static inline vec<edge> get_loop_exit_edges_unique_dests(const struct loop *loop)
{
...
static bool sel_bb_empty_or_nop_p(basic_block bb)
{
...
/* Collect all loop exits recursively, skipping empty BBs between them.
   ...
   as well.  */
static inline vec<edge> get_all_loop_exits(basic_block bb)
{
        vec<edge> exits = vNULL;

        /* If bb is empty, and we're skipping to loop exits, then
           consider bb as a possible gate to the inner loop now.  */
        while (sel_bb_empty_or_nop_p(bb) && in_current_region_p(bb) && EDGE_COUNT(bb->succs) > 0) {
                bb = single_succ(bb);
        ...
        if (inner_loop_header_p(bb)) {
                ...
                for (this_loop = bb->loop_father; this_loop && this_loop != current_loop_nest;
                ...
                for (i = 0; exits.iterate(i, &e); i++) {
                        if ((in_current_region_p(e->dest) || (inner_loop_header_p(e->dest))) &&
...
/* We need to return a succ_iterator to avoid 'unitialized' warning
   during bootstrap.  */
static inline succ_iterator _succ_iter_start(insn_t *succp, insn_t insn, int flags)
{
        succ_iterator i;
...
static inline bool _succ_iter_cond(succ_iterator *ip, insn_t *succp, insn_t insn,
                        ...
                        while (ei_cond(ip->ei, &(ip->e1))) {
                                ...
                                if ((ip->flags & SUCCS_SKIP_TO_LOOP_EXITS) &&
                                    ...
                                    (!in_current_region_p(bb) ||
                                     BLOCK_TO_BB(ip->bb->index) < BLOCK_TO_BB(bb->index))) {
...
static inline void _succ_iter_next(succ_iterator *ip)
{
...
static inline bool _eligible_successor_edge_p(edge e1, succ_iterator *ip)
{
        edge e2 = e1;
        basic_block bb;
        int flags = ip->flags;
        ...
        while (1) {
                if (!sel_bb_empty_p(bb)) {
                        ...
                        if (!in_current_region_p(nbb) && !(flags & SUCCS_OUT)) {
                                break;
                        }
                ...
                if (!in_current_region_p(bb) && !(flags & SUCCS_OUT)) {
                        return false;
                }
        ...
        if (in_current_region_p(bb)) {
                ...
                bool succeeds_in_top_order = (BLOCK_TO_BB(ip->bb->index) < BLOCK_TO_BB(bb->index));
                ...
                if (succeeds_in_top_order) {
                        ...
                        gcc_assert(!src_outside_rgn || flag_sel_sched_pipelining_outer_loops);
                ...
                /* This is a back edge.  During pipelining we ignore back edges,
                   ...
                   the current loop.  */
                if (pipelining_p && e1->src->loop_father == bb->loop_father) {
                        return !!(flags & SUCCS_NORMAL);
                }
...
static inline basic_block bb_next_bb(basic_block bb)
{
...
extern int find_in_history_vect(vec<expr_history_def>, rtx, vinsn_t, bool);
extern void insert_in_history_vect(vec<expr_history_def> *, unsigned, enum local_trans_type,
