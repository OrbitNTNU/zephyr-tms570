...
class substring_loc
{
      public:
        /* Constructor.  FMT_STRING_LOC is the location of the string as
           ...
           of the string data.  */
        substring_loc(location_t fmt_string_loc, tree string_type, int caret_idx, int start_idx,
                      int end_idx)
...
{
      public:
        format_string_diagnostic_t(const substring_loc &fmt_loc, const range_label *fmt_label,
                                   location_t param_loc, const range_label *param_label,
        ...
        bool emit_warning_n_va(int opt, unsigned HOST_WIDE_INT n, const char *singular_gmsgid,
                               const char *plural_gmsgid, va_list *ap) const
                ATTRIBUTE_GCC_DIAG(4, 0) ATTRIBUTE_GCC_DIAG(5, 0);

        bool emit_warning(int opt, const char *gmsgid, ...) const ATTRIBUTE_GCC_DIAG(3, 4);

        bool emit_warning_n(int opt, unsigned HOST_WIDE_INT n, const char *singular_gmsgid,
                            const char *plural_gmsgid, ...) const ATTRIBUTE_GCC_DIAG(4, 6)
...
extern const char *get_location_within_string(cpp_reader *pfile, string_concat_db *concats,
                                              location_t strloc, enum cpp_ttype type, int caret_idx,
                                              int start_idx, int end_idx, location_t *out_loc);
