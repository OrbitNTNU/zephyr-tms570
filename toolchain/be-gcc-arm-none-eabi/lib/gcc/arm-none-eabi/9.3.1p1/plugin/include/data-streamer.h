...
void streamer_write_string_with_length(struct output_block *, struct lto_output_stream *,
                                       const char *, unsigned int, bool);
void bp_pack_string_with_length(struct output_block *, struct bitpack_d *, const char *,
                                unsigned int, bool);
...
const char *streamer_read_indexed_string(struct data_in *, struct lto_input_block *,
                                         unsigned int *);
...
static inline struct bitpack_d bitpack_create(struct lto_output_stream *s)
{
        struct bitpack_d bp;
        bp.pos = 0;
        bp.word = 0;
...
static inline void bp_pack_value(struct bitpack_d *bp, bitpack_word_t val, unsigned nbits)
{
        ...
        if (pos + nbits > BITS_PER_BITPACK_WORD) {
                streamer_write_uhwi_stream((struct lto_output_stream *)bp->stream, word);
                word = val;
                pos = nbits;
        } else {
                word |= val << pos;
...
/* Pack VAL into the bit-packing context BP, using NBITS for each
   coefficient.  */
static inline void bp_pack_poly_value(struct bitpack_d *bp,
...
static inline void streamer_write_bitpack(struct bitpack_d *bp)
{
        streamer_write_uhwi_stream((struct lto_output_stream *)bp->stream, bp->word);
        bp->word = 0;
        bp->pos = 0;
}

/* Returns a new bit-packing context for bit-unpacking from IB.  */
static inline struct bitpack_d streamer_read_bitpack(struct lto_input_block *ib)
{
        struct bitpack_d bp;
...
static inline bitpack_word_t bp_unpack_value(struct bitpack_d *bp, unsigned nbits)
{
        ...
        if (pos + nbits > BITS_PER_BITPACK_WORD) {
                bp->word = val = streamer_read_uhwi((struct lto_input_block *)bp->stream);
                bp->pos = nbits;
                return val & mask;
        }
        val = bp->word;
...
static inline void streamer_write_char_stream(struct lto_output_stream *obs, char c)
{
        /* No space left.  */
...
static inline unsigned char streamer_read_uchar(struct lto_input_block *ib)
{
        if (ib->p >= ib->len) {
...
static inline void streamer_write_hwi_in_range(struct lto_output_stream *obs, HOST_WIDE_INT min,
                                               HOST_WIDE_INT max, HOST_WIDE_INT val)
{
        HOST_WIDE_INT range = max - min;

        gcc_checking_assert(val >= min && val <= max && range > 0 && range < 0x7fffffff);
...
static inline HOST_WIDE_INT streamer_read_hwi_in_range(struct lto_input_block *ib,
                                                       const char *purpose, HOST_WIDE_INT min,
                                                       HOST_WIDE_INT max)
{
        HOST_WIDE_INT range = max - min;
...
static inline void bp_pack_int_in_range(struct bitpack_d *bp, HOST_WIDE_INT min, HOST_WIDE_INT max,
                                        HOST_WIDE_INT val)
{
        HOST_WIDE_INT range = max - min;
...
static inline HOST_WIDE_INT bp_unpack_int_in_range(struct bitpack_d *bp, const char *purpose,
...
#define streamer_read_enum(ib, enum_name, enum_last)                                               \
        (enum enum_name) streamer_read_hwi_in_range((ib), #enum_name, 0, (int)(enum_last)-1)
...
#define bp_unpack_enum(bp, enum_name, enum_last)                                                   \
        (enum enum_name) bp_unpack_int_in_range((bp), #enum_name, 0, (int)(enum_last)-1)

/* Output the start of a record with TAG to output block OB.  */

static inline void streamer_write_record_start(struct output_block *ob, enum LTO_tags tag)
{
...
static inline enum LTO_tags streamer_read_record_start(struct lto_input_block *ib)
{
