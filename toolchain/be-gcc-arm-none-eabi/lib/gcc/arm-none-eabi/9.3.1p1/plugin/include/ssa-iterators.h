...
#define FOR_EACH_IMM_USE_FAST(DEST, ITER, SSAVAR)                                                  \
        for ((DEST) = first_readonly_imm_use(&(ITER), (SSAVAR)); !end_readonly_imm_use_p(&(ITER)); \
             (void)((DEST) = next_readonly_imm_use(&(ITER))))

/* Use this iterator to visit each stmt which has a use of SSAVAR.  */

#define FOR_EACH_IMM_USE_STMT(STMT, ITER, SSAVAR)                                                  \
        for ((STMT) = first_imm_use_stmt(&(ITER), (SSAVAR)); !end_imm_use_stmt_p(&(ITER));         \
...
#define FOR_EACH_IMM_USE_ON_STMT(DEST, ITER)                                                       \
        for ((DEST) = first_imm_use_on_stmt(&(ITER)); !end_imm_use_on_stmt_p(&(ITER));             \
             (void)((DEST) = next_imm_use_on_stmt(&(ITER))))

extern bool single_imm_use_1(const ssa_use_operand_t *head, use_operand_p *use_p, gimple **stmt);
...
#define FOR_EACH_SSA_TREE_OPERAND(TREEVAR, STMT, ITER, FLAGS)                                      \
        for (TREEVAR = op_iter_init_tree(&(ITER), STMT, FLAGS); !op_iter_done(&(ITER));            \
...
#define FOR_EACH_SSA_USE_OPERAND(USEVAR, STMT, ITER, FLAGS)                                        \
        for (USEVAR = op_iter_init_use(&(ITER), STMT, FLAGS); !op_iter_done(&(ITER));              \
...
#define FOR_EACH_SSA_DEF_OPERAND(DEFVAR, STMT, ITER, FLAGS)                                        \
        for (DEFVAR = op_iter_init_def(&(ITER), STMT, FLAGS); !op_iter_done(&(ITER));              \
...
#define FOR_EACH_PHI_ARG(USEVAR, STMT, ITER, FLAGS)                                                \
        for ((USEVAR) = op_iter_init_phiuse(&(ITER), STMT, FLAGS); !op_iter_done(&(ITER));         \
             (USEVAR) = op_iter_next_use(&(ITER)))
...
#define FOR_EACH_PHI_OR_STMT_USE(USEVAR, STMT, ITER, FLAGS)                                        \
        for ((USEVAR) = (gimple_code(STMT) == GIMPLE_PHI                                           \
                                 ? op_iter_init_phiuse(&(ITER), as_a<gphi *>(STMT), FLAGS)         \
                                 : op_iter_init_use(&(ITER), STMT, FLAGS));                        \
             !op_iter_done(&(ITER)); (USEVAR) = op_iter_next_use(&(ITER)))

/* This macro will execute a loop over a stmt, regardless of whether it is
   a real stmt or a PHI node, looking at the DEF nodes matching FLAGS.  */
#define FOR_EACH_PHI_OR_STMT_DEF(DEFVAR, STMT, ITER, FLAGS)                                        \
        for ((DEFVAR) = (gimple_code(STMT) == GIMPLE_PHI                                           \
                                 ? op_iter_init_phidef(&(ITER), as_a<gphi *>(STMT), FLAGS)         \
                                 : op_iter_init_def(&(ITER), STMT, FLAGS));                        \
             !op_iter_done(&(ITER)); (DEFVAR) = op_iter_next_def(&(ITER)))
...
static inline void link_imm_use_to_list(ssa_use_operand_t *linknode, ssa_use_operand_t *list)
{
...
/* Link ssa_imm_use node LINKNODE into the chain for DEF, with use occurring
   in STMT.  */
static inline void link_imm_use_stmt(ssa_use_operand_t *linknode, tree def, gimple *stmt)
{
        if (stmt) {
...
static inline void relink_imm_use(ssa_use_operand_t *node, ssa_use_operand_t *old)
{
...
/* Relink ssa_imm_use node LINKNODE into the chain for OLD, with use occurring
   in STMT.  */
static inline void relink_imm_use_stmt(ssa_use_operand_t *linknode, ssa_use_operand_t *old,
                                       gimple *stmt)
{
        if (stmt) {
...
static inline use_operand_p first_readonly_imm_use(imm_use_iterator *imm, tree var)
{
...
static inline bool single_imm_use(const_tree var, use_operand_p *use_p, gimple **stmt)
{
        ...
        if (ptr->flags & SSA_OP_DEF) {
                while (ptr->i < ptr->numops) {
                        ...
                        if (*val) {
                                ...
                                if (TREE_CODE(*val) == SSA_NAME || is_gimple_reg(*val)) {
                                        return val;
                                }
                        }
                }
...
static inline void op_iter_init(ssa_op_iter *ptr, gimple *stmt, int flags)
{
        ...
        if (!(flags & SSA_OP_VUSE) && ptr->uses && gimple_vuse(stmt) != NULL_TREE) {
                ptr->uses = ptr->uses->next;
        }
        ptr->done = false;
        ptr->i = 0;
...
/* Initialize iterator PTR to the use operands in STMT based on FLAGS. Return
   the first use.  */
static inline use_operand_p op_iter_init_use(ssa_op_iter *ptr, gimple *stmt, int flags)
{
        gcc_checking_assert((flags & SSA_OP_ALL_DEFS) == 0 && (flags & SSA_OP_USE));
...
/* Initialize iterator PTR to the def operands in STMT based on FLAGS. Return
   the first def.  */
static inline def_operand_p op_iter_init_def(ssa_op_iter *ptr, gimple *stmt, int flags)
{
        gcc_checking_assert((flags & SSA_OP_ALL_USES) == 0 && (flags & SSA_OP_DEF));
        op_iter_init(ptr, stmt, flags);
...
/* Return the single virtual use operand in STMT if present.  Otherwise
   return NULL.  */
static inline use_operand_p ssa_vuse_operand(gimple *stmt)
{
        if (!gimple_vuse(stmt)) {
                return NULL_USE_OPERAND_P;
        }
...
static inline tree single_phi_def(gphi *stmt, int flags)
{
...
static inline use_operand_p op_iter_init_phiuse(ssa_op_iter *ptr, gphi *phi, int flags)
{
...
static inline def_operand_p op_iter_init_phidef(ssa_op_iter *ptr, gphi *phi, int flags)
{
...
static inline use_operand_p move_use_after_head(use_operand_p use_p, use_operand_p head,
...
static inline void link_use_stmts_after(use_operand_p head, imm_use_iterator *imm)
{
        use_operand_p use_p;
