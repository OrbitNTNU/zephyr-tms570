...
cluster::cluster(tree case_label_expr, basic_block case_bb, profile_probability prob,
                 profile_probability subtree_prob)
...
struct simple_cluster : public cluster {
        ...
        void dump(FILE *f, bool details ATTRIBUTE_UNUSED = false)
        {
...
simple_cluster::simple_cluster(tree low, tree high, tree case_label_expr, basic_block case_bb,
                               profile_probability prob)
        : cluster(case_label_expr, case_bb, prob, prob), m_low(low), m_high(high)
{
...
struct jump_table_cluster : public group_cluster {
        ...
        void emit(tree index_expr, tree index_type, tree default_label_expr,
...
struct bit_test_cluster : public group_cluster {
        ...
        static basic_block hoist_edge_and_branch_if_true(gimple_stmt_iterator *gsip, tree cond,
                                                         basic_block case_bb,
                                                         profile_probability prob);
...
inline case_tree_node::case_tree_node() : m_left(NULL), m_right(NULL), m_parent(NULL), m_c(NULL)
{
}

unsigned int jump_table_cluster::case_values_threshold(void)
{
...
struct switch_decision_tree {
        ...
        switch_decision_tree(gswitch *swtch)
                : m_switch(swtch), m_phi_mapping(), m_case_bbs(),
                  m_case_node_pool("struct case_node pool"), m_case_list(NULL)
        {
        }
        ...
        /* Generate a decision tree, switching on INDEX_EXPR and jumping to
           ...
           code.  */
        void emit(basic_block bb, tree index_expr, profile_probability default_prob,
                  tree index_type);
        ...
        basic_block emit_case_nodes(basic_block bb, tree index, case_tree_node *node,
                                    profile_probability default_prob, tree index_type, location_t);
        ...
        static basic_block emit_cmp_and_jump_insns(basic_block bb, tree op0, tree op1,
                                                   tree_code comparison, basic_block label_bb,
                                                   profile_probability prob, location_t);
...
struct switch_conversion {
        ...
        bool contains_linear_function_p(vec<constructor_elt, va_gc> *vec, wide_int *coeff_a,
                                        wide_int *coeff_b);
...
void switch_decision_tree::reset_out_edges_aux(gswitch *swtch)
{
