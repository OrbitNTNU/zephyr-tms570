...
inline bool convert_optab_p(optab op)
{
...
inline enum insn_code convert_optab_handler(convert_optab op, machine_mode to_mode,
...
inline enum insn_code direct_optab_handler(direct_optab op, machine_mode mode)
{
...
inline bool trapv_unoptab_p(optab unoptab)
{
        return (unoptab == negv_optab || unoptab == absv_optab);
}

/* Return true if BINOPTAB is for a trapping-on-overflow operation.  */

inline bool trapv_binoptab_p(optab binoptab)
{
        return (binoptab == addv_optab || binoptab == subv_optab || binoptab == smulv_optab);
}

/* Return insn code for a comparison operator with VMODE
   resultin MASK_MODE, unsigned if UNS is true.  */

static inline enum insn_code get_vec_cmp_icode(machine_mode vmode, machine_mode mask_mode, bool uns)
{
...
static inline enum insn_code get_vec_cmp_eq_icode(machine_mode vmode, machine_mode mask_mode)
{
...
inline enum insn_code get_vcond_icode(machine_mode vmode, machine_mode cmode, bool uns)
{
...
static inline enum insn_code get_vcond_mask_icode(machine_mode vmode, machine_mode mmode)
{
...
static inline enum insn_code get_vcond_eq_icode(machine_mode vmode, machine_mode cmode)
{
...
bool get_best_reg_extraction_insn(extraction_insn *, enum extraction_pattern,
                                  unsigned HOST_WIDE_INT, machine_mode);
bool get_best_mem_extraction_insn(extraction_insn *, enum extraction_pattern, HOST_WIDE_INT,
...
bool can_vec_perm_const_p(machine_mode, const vec_perm_indices &, bool = true);
/* Find a widening optab even if it doesn't widen as much as we want.  */
#define find_widening_optab_handler(A, B, C) find_widening_optab_handler_and_mode(A, B, C, NULL)
enum insn_code find_widening_optab_handler_and_mode(optab, machine_mode, machine_mode,
                                                    machine_mode *);
...
template <typename T>
inline enum insn_code find_widening_optab_handler_and_mode(optab op, const T &to_mode,
                                                           const T &from_mode, T *found_mode)
{
        machine_mode tmp;
        enum insn_code icode = find_widening_optab_handler_and_mode(op, machine_mode(to_mode),
                                                                    machine_mode(from_mode), &tmp);
