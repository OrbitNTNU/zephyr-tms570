...
static inline lang_bitmap get_lang_bitmap(const input_file *inpf)
{
        if (inpf == NULL) {
                return 0;
        }
...
static inline void set_lang_bitmap(input_file *inpf, lang_bitmap n)
{
        gcc_assert(inpf);
        inpf->inpbitmap = n;
}
...
#define UNION_P(x)                                                                                 \
        ((x)->kind == TYPE_UNION ||                                                                \
         ((x)->kind == TYPE_LANG_STRUCT && (x)->u.s.lang_struct->kind == TYPE_UNION))

/* Test if a type is a union or a structure, perhaps a language
   specific one.  */
static inline bool union_or_struct_p(enum typekind kind)
{
        return (kind == TYPE_UNION || kind == TYPE_STRUCT || kind == TYPE_LANG_STRUCT ||
                kind == TYPE_USER_STRUCT);
}

static inline bool union_or_struct_p(const_type_p x)
{
        return union_or_struct_p(x->kind);
}

/* Give the file location of a type, if any. */
static inline struct fileloc *type_fileloc(type_p t)
{
        if (!t) {
                return NULL;
        }
...
extern type_p new_structure(const char *name, enum typekind kind, struct fileloc *pos,
                            pair_p fields, options_p o, type_p base);
...
extern pair_p create_field_at(pair_p next, type_p type, const char *name, options_p opt,
                              struct fileloc *pos);
extern pair_p nreverse_pairs(pair_p list);
extern type_p adjust_field_type(type_p, options_p);
extern void note_variable(const char *s, type_p t, options_p o, struct fileloc *pos);
