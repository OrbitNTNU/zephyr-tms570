...
struct GTY(()) ipa_fn_summary {
        ...
        ipa_fn_summary()
                : estimated_self_stack_size(0), self_size(0), min_size(0), inlinable(false),
                  single_caller(false), fp_expressions(false), estimated_stack_size(false),
                  stack_frame_offset(false), time(0), size(0), conds(NULL), size_time_table(NULL),
                  loop_iterations(NULL), loop_stride(NULL), array_index(NULL), growth(0), scc_no(0)
        {
        }

        /* Copy constructor.  */
        ipa_fn_summary(const ipa_fn_summary &s)
                : estimated_self_stack_size(s.estimated_self_stack_size), self_size(s.self_size),
                  min_size(s.min_size), inlinable(s.inlinable), single_caller(s.single_caller),
                  fp_expressions(s.fp_expressions), estimated_stack_size(s.estimated_stack_size),
...
class GTY((user)) ipa_fn_summary_t : public fast_function_summary<ipa_fn_summary *, va_gc>
{
      public:
        ...
        {
                struct ipa_fn_summary_t *summary =
                        new (ggc_alloc<ipa_fn_summary_t>()) ipa_fn_summary_t(symtab);
        ...
        virtual void duplicate(cgraph_node *src, cgraph_node *dst, ipa_fn_summary *src_data,
                               ipa_fn_summary *dst_data);
};
...
class ipa_call_summary_t : public fast_call_summary<ipa_call_summary *, va_heap>
{
      public:
        ...
        virtual void duplicate(cgraph_edge *src, cgraph_edge *dst, ipa_call_summary *src_data,
...
void estimate_ipcp_clone_size_and_time(struct cgraph_node *, vec<tree>,
                                       vec<ipa_polymorphic_call_context>,
                                       vec<ipa_agg_jump_function_p>, int *, sreal *, sreal *,
                                       ipa_hints *);
void ipa_merge_fn_summary_after_inlining(struct cgraph_edge *edge);
void ipa_update_overall_fn_summary(struct cgraph_node *node);
void compute_fn_summary(struct cgraph_node *, bool);

void evaluate_properties_for_edge(struct cgraph_edge *e, bool inline_p, clause_t *clause_ptr,
                                  clause_t *nonspec_clause_ptr, vec<tree> *known_vals_ptr,
                                  vec<ipa_polymorphic_call_context> *known_contexts_ptr,
                                  vec<ipa_agg_jump_function_p> *);
void estimate_node_size_and_time(struct cgraph_node *node, clause_t possible_truths,
                                 clause_t nonspec_possible_truths, vec<tree> known_vals,
                                 vec<ipa_polymorphic_call_context>,
                                 vec<ipa_agg_jump_function_p> known_aggs, int *ret_size,
                                 int *ret_min_size, sreal *ret_time, sreal *ret_nonspecialized_time,
                                 ipa_hints *ret_hints,
