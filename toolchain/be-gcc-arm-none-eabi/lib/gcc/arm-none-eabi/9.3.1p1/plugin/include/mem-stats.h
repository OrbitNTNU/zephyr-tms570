...
struct mem_location {
        ...
        /* Constructor.  */
        inline mem_location(mem_alloc_origin origin, bool ggc, const char *filename = NULL,
                            int line = 0, const char *function = NULL)
                : m_filename(filename), m_function(function), m_line(line), m_origin(origin),
                  m_ggc(ggc)
        {
        }

        /* Copy constructor.  */
        inline mem_location(mem_location &other)
                : m_filename(other.m_filename), m_function(other.m_function), m_line(other.m_line),
        ...
        inline char *to_string()
        {
                ...
                sprintf(s, "%s:%i (%s)", get_trimmed_filename(), m_line, m_function);
...
struct mem_usage {
        ...
        mem_usage(size_t allocated, size_t times, size_t peak, size_t instances = 0)
                : m_allocated(allocated), m_times(times), m_peak(peak), m_instances(instances)
        {
        }
        ...
        {
                return mem_usage(m_allocated + second.m_allocated, m_times + second.m_times,
                                 m_peak + second.m_peak, m_instances + second.m_instances);
        }

        /* Equality operator.  */
        inline bool operator==(const mem_usage &second) const
        {
                return (m_allocated == second.m_allocated && m_peak == second.m_peak &&
        ...
        inline void dump(mem_location *loc, mem_usage &total) const
        {
                ...
                fprintf(stderr, "%-48s " PRsa(9) ":%5.1f%%" PRsa(9) PRsa(9) ":%5.1f%%%10s\n",
                        location_string, SIZE_AMOUNT(m_allocated),
                        get_percent(m_allocated, total.m_allocated), SIZE_AMOUNT(m_peak),
                        SIZE_AMOUNT(m_times), get_percent(m_times, total.m_times),
                        loc->m_ggc ? "ggc" : "heap");
...
/* Memory usage pair that connectes memory usage and number
   of allocated bytes.  */
template <class T> struct mem_usage_pair {
        mem_usage_pair(T *usage_, size_t allocated_) : usage(usage_), allocated(allocated_)
        {
        }
...
template <class T> class mem_alloc_description
{
      public:
        struct mem_location_hash : nofree_ptr_hash<mem_location> {
                ...
                static bool equal(value_type l1, value_type l2)
                {
                        return (l1->m_filename == l2->m_filename &&
                                l1->m_function == l2->m_function && l1->m_line == l2->m_line);
                }
        };
        ...
        /* Register memory allocation descriptor for container PTR.  ORIGIN identifies
           ...
           FUNCTION name.  */
        T *register_descriptor(const void *ptr, mem_alloc_origin origin, bool ggc, const char *name,
...
template <class T>
inline bool mem_alloc_description<T>::contains_descriptor_for_instance(const void *ptr)
{
...
template <class T> inline T *mem_alloc_description<T>::get_descriptor_for_instance(const void *ptr)
{
...
template <class T>
inline T *mem_alloc_description<T>::register_descriptor(const void *ptr, mem_location *location)
{
        T *usage = NULL;
...
template <class T>
inline T *mem_alloc_description<T>::register_descriptor(const void *ptr, mem_alloc_origin origin,
                                                        bool ggc, const char *filename, int line,
                                                        const char *function)
{
...
template <class T>
inline T *mem_alloc_description<T>::register_instance_overhead(size_t size, const void *ptr)
{
...
template <class T>
void mem_alloc_description<T>::register_object_overhead(T *usage, size_t size, const void *ptr)
{
...
template <class T>
inline T *mem_alloc_description<T>::register_overhead(size_t size, mem_alloc_origin origin,
...
template <class T>
inline T *mem_alloc_description<T>::release_instance_overhead(void *ptr, size_t size,
...
template <class T> inline void mem_alloc_description<T>::release_object_overhead(void *ptr)
{
...
/* Unregister a memory allocation descriptor registered with
   ...
   unregistered through release_instance_overhead with
   REMOVE_FROM_MAP = true.  */
template <class T> inline void mem_alloc_description<T>::unregister_descriptor(void *ptr)
{
        m_reverse_map->remove(ptr);
}
...
template <class T> inline mem_alloc_description<T>::mem_alloc_description()
{
...
template <class T>
inline typename mem_alloc_description<T>::mem_list_t *
...
template <class T> inline T mem_alloc_description<T>::get_sum(mem_alloc_origin origin)
{
        unsigned length;
...
template <class T>
inline void mem_alloc_description<T>::dump(mem_alloc_origin origin,
                                           int (*cmp)(const void *first, const void *second))
{
        unsigned length;
