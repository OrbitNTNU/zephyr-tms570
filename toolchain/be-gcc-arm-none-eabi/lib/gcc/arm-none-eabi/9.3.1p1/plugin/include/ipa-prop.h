...
static inline tree ipa_get_jf_constant(struct ipa_jump_func *jfunc)
{
        gcc_checking_assert(jfunc->type == IPA_JF_CONST);
        return jfunc->value.constant.value;
}

static inline struct ipa_cst_ref_desc *ipa_get_jf_constant_rdesc(struct ipa_jump_func *jfunc)
{
        gcc_checking_assert(jfunc->type == IPA_JF_CONST);
...
static inline tree ipa_get_jf_pass_through_operand(struct ipa_jump_func *jfunc)
{
...
static inline int ipa_get_jf_pass_through_formal_id(struct ipa_jump_func *jfunc)
{
        gcc_checking_assert(jfunc->type == IPA_JF_PASS_THROUGH);
...
static inline enum tree_code ipa_get_jf_pass_through_operation(struct ipa_jump_func *jfunc)
{
        gcc_checking_assert(jfunc->type == IPA_JF_PASS_THROUGH);
...
static inline bool ipa_get_jf_pass_through_agg_preserved(struct ipa_jump_func *jfunc)
{
        gcc_checking_assert(jfunc->type == IPA_JF_PASS_THROUGH);
...
static inline bool ipa_get_jf_pass_through_type_preserved(struct ipa_jump_func *jfunc)
{
        gcc_checking_assert(jfunc->type == IPA_JF_PASS_THROUGH);
        return jfunc->value.pass_through.agg_preserved;
}
...
static inline HOST_WIDE_INT ipa_get_jf_ancestor_offset(struct ipa_jump_func *jfunc)
{
...
static inline int ipa_get_jf_ancestor_formal_id(struct ipa_jump_func *jfunc)
{
        gcc_checking_assert(jfunc->type == IPA_JF_ANCESTOR);
...
static inline bool ipa_get_jf_ancestor_agg_preserved(struct ipa_jump_func *jfunc)
{
        gcc_checking_assert(jfunc->type == IPA_JF_ANCESTOR);
...
static inline bool ipa_get_jf_ancestor_type_preserved(struct ipa_jump_func *jfunc)
{
        gcc_checking_assert(jfunc->type == IPA_JF_ANCESTOR);
        return jfunc->value.ancestor.agg_preserved;
}
...
inline ipa_node_params::ipa_node_params()
        : descriptors(NULL), lattices(NULL), ipcp_orig_node(NULL), known_csts(vNULL),
          known_contexts(vNULL), analysis_done(0), node_enqueued(0), do_clone_for_all_contexts(0),
          is_all_contexts_clone(0), node_dead(0), node_within_scc(0), node_calling_single_call(0),
          versionable(0)
{
}
...
static inline tree ipa_get_type(struct ipa_node_params *info, int i)
{
...
static inline int ipa_get_param_move_cost(struct ipa_node_params *info, int i)
{
        gcc_checking_assert(info->descriptors);
...
/* Return how many uses described by ipa-prop a parameter has or
   ...
   structures.  */
static inline int ipa_get_controlled_uses(struct ipa_node_params *info, int i)
{
...
static inline void ipa_set_controlled_uses(struct ipa_node_params *info, int i, int val)
{
        gcc_checking_assert(info->descriptors);
...
static inline bool ipa_is_param_used(struct ipa_node_params *info, int i)
{
        gcc_checking_assert(info->descriptors);
...
static inline int ipa_get_cs_argument_count(struct ipa_edge_args *args)
{
...
static inline struct ipa_jump_func *ipa_get_ith_jump_func(struct ipa_edge_args *args, int i)
{
...
class GTY((user)) ipa_node_params_t : public function_summary<ipa_node_params *>
{
      public:
        ...
        virtual void duplicate(cgraph_node *node, cgraph_node *node2, ipa_node_params *data,
...
class GTY((user)) ipa_edge_args_sum_t : public call_summary<ipa_edge_args *>
{
      public:
        ...
        /* Hook that is called by summary when an edge is duplicated.  */
        virtual void duplicate(cgraph_edge *src, cgraph_edge *dst, ipa_edge_args *old_args,
...
class ipcp_transformation_t : public function_summary<ipcp_transformation *>
{
      public:
        ...
        {
                ipcp_transformation_t *summary = new (ggc_cleared_alloc<ipcp_transformation_t>())
...
static inline void ipa_check_create_node_params(void)
{
        if (!ipa_node_params_sum) {
                ipa_node_params_sum = (new (ggc_cleared_alloc<ipa_node_params_t>())
...
static inline bool ipa_edge_args_info_available_for_edge_p(struct cgraph_edge *edge)
{
        return ipa_edge_args_sum->exists(edge);
}

static inline ipcp_transformation *ipcp_get_transformation_summary(cgraph_node *node)
{
...
static inline struct ipa_agg_replacement_value *ipa_get_agg_replacements_for_node(cgraph_node *node)
{
...
tree ipa_get_indirect_edge_target(struct cgraph_edge *ie, vec<tree>,
                                  vec<ipa_polymorphic_call_context>, vec<ipa_agg_jump_function_p>,
                                  bool *);
struct cgraph_edge *ipa_make_edge_direct_to_target(struct cgraph_edge *, tree,
                                                   bool speculative = false);
tree ipa_impossible_devirt_target(struct cgraph_edge *, tree);
ipa_bits *ipa_get_ipa_bits_for_value(const widest_int &value, const widest_int &mask);
...
bool ipa_load_from_parm_agg(struct ipa_func_body_info *fbi,
                            vec<ipa_param_descriptor, va_gc> *descriptors, gimple *stmt, tree op,
                            int *index_p, HOST_WIDE_INT *offset_p, HOST_WIDE_INT *size_p,
...
template <typename value> class ipcp_value;

extern object_allocator<ipcp_value<tree>> ipcp_cst_values_pool;
extern object_allocator<ipcp_value<ipa_polymorphic_call_context>> ipcp_poly_ctx_values_pool;

template <typename valtype> class ipcp_value_source;
...
tree ipa_value_from_jfunc(struct ipa_node_params *info, struct ipa_jump_func *jfunc, tree type);
unsigned int ipcp_transform_function(struct cgraph_node *node);
ipa_polymorphic_call_context ipa_context_from_jfunc(ipa_node_params *, cgraph_edge *, int,
                                                    ipa_jump_func *);
