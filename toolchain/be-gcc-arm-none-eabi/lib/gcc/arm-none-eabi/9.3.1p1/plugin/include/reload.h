...
#define SECONDARY_INPUT_RELOAD_CLASS(CLASS, MODE, X)  SECONDARY_RELOAD_CLASS(CLASS, MODE, X)
#define SECONDARY_OUTPUT_RELOAD_CLASS(CLASS, MODE, X) SECONDARY_RELOAD_CLASS(CLASS, MODE, X)
#endif
...
struct target_reload {
        ...
        machine_mode(
                x_regno_save_mode[FIRST_PSEUDO_REGISTER][MAX_MOVE_MAX / MIN_UNITS_PER_WORD + 1]);
...
extern reg_class_t secondary_reload_class(bool, reg_class_t, machine_mode, rtx);
...
extern rtx find_equiv_reg(rtx, rtx_insn *, enum reg_class, int, short *, int, machine_mode);
...
extern int push_reload(rtx, rtx, rtx *, rtx *, enum reg_class, machine_mode, machine_mode, int, int,
