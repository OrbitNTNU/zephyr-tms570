...
#error In order for the format checking to accept the C++ front end diagnostic \
...
{
      public:
        cp_expr() : m_value(NULL), m_loc(UNKNOWN_LOCATION)
        {
        }

        cp_expr(tree value) : m_value(value), m_loc(cp_expr_location(m_value))
        {
        }

        cp_expr(tree value, location_t loc) : m_value(value), m_loc(loc)
        {
        }
...
inline bool operator==(const cp_expr &lhs, tree rhs)
{
...
#define FUNCTION_NEEDS_BODY_BLOCK(NODE)                                                            \
        (DECL_CONSTRUCTOR_P(NODE) || DECL_DESTRUCTOR_P(NODE) || LAMBDA_FUNCTION_P(NODE))
...
#define OVL_CHAIN(NODE)    (((struct tree_overload *)OVERLOAD_CHECK(NODE))->common.chain)
...
/* Whether this is a set of overloaded functions.  TEMPLATE_DECLS are
   ...
   here.  */
#define OVL_P(NODE)        (TREE_CODE(NODE) == FUNCTION_DECL || TREE_CODE(NODE) == OVERLOAD)
...
class ovl_iterator
{
      ...
      public:
        explicit ovl_iterator(tree o, bool allow = false) : ovl(o), allow_inner(allow)
        {
        }
...
#define MAYBE_BASELINK_FUNCTIONS(T) (BASELINK_P(T) ? BASELINK_FUNCTIONS(T) : T)
...
/* Kind bits.  */
#define IDENTIFIER_KIND_BIT_0(NODE) TREE_LANG_FLAG_0(IDENTIFIER_NODE_CHECK(NODE))
#define IDENTIFIER_KIND_BIT_1(NODE) TREE_LANG_FLAG_1(IDENTIFIER_NODE_CHECK(NODE))
#define IDENTIFIER_KIND_BIT_2(NODE) TREE_LANG_FLAG_2(IDENTIFIER_NODE_CHECK(NODE))
...
/* Nonzero if this identifier is used as a virtual function name somewhere
   (optimizes searches).  */
#define IDENTIFIER_VIRTUAL_P(NODE) TREE_LANG_FLAG_5(IDENTIFIER_NODE_CHECK(NODE))

/* True iff NAME is the DECL_ASSEMBLER_NAME for an entity with vague
   linkage which the prelinker has assigned to this translation
   unit.  */
#define IDENTIFIER_REPO_CHOSEN(NAME) (TREE_LANG_FLAG_6(IDENTIFIER_NODE_CHECK(NAME)))
...
#define IDENTIFIER_CDTOR_P(NODE) ((!IDENTIFIER_KIND_BIT_2(NODE)) & IDENTIFIER_KIND_BIT_1(NODE))

/* True if this identifier is the name of a constructor.  Value 2.  */
#define IDENTIFIER_CTOR_P(NODE) (IDENTIFIER_CDTOR_P(NODE) & (!IDENTIFIER_KIND_BIT_0(NODE)))

/* True if this identifier is the name of a destructor.  Value 3.  */
#define IDENTIFIER_DTOR_P(NODE) (IDENTIFIER_CDTOR_P(NODE) & IDENTIFIER_KIND_BIT_0(NODE))
...
#define IDENTIFIER_OVL_OP_P(NODE) (IDENTIFIER_ANY_OP_P(NODE) & (!IDENTIFIER_KIND_BIT_1(NODE)))

/* True if this identifier is for any assignment. Values 5.  */
#define IDENTIFIER_ASSIGN_OP_P(NODE) (IDENTIFIER_OVL_OP_P(NODE) & IDENTIFIER_KIND_BIT_0(NODE))

/* True if this identifier is the name of a type-conversion
   operator.  Value 7.  */
#define IDENTIFIER_CONV_OP_P(NODE)                                                                 \
        (IDENTIFIER_ANY_OP_P(NODE) & IDENTIFIER_KIND_BIT_1(NODE) & (!IDENTIFIER_KIND_BIT_0(NODE)))

/* True if this identifier is a new or delete operator.  */
#define IDENTIFIER_NEWDEL_OP_P(NODE)                                                               \
        (IDENTIFIER_OVL_OP_P(NODE) && IDENTIFIER_OVL_OP_FLAGS(NODE) & OVL_OP_FLAG_ALLOC)

/* True if this identifier is a new operator.  */
#define IDENTIFIER_NEW_OP_P(NODE)                                                                  \
        (IDENTIFIER_OVL_OP_P(NODE) &&                                                              \
         (IDENTIFIER_OVL_OP_FLAGS(NODE) & (OVL_OP_FLAG_ALLOC | OVL_OP_FLAG_DELETE)) ==             \
                 OVL_OP_FLAG_ALLOC)

/* Access a C++-specific index for identifier NODE.
   Used to optimize operator mappings etc.  */
#define IDENTIFIER_CP_INDEX(NODE) (IDENTIFIER_NODE_CHECK(NODE)->base.u.bits.address_space)
...
#define DEFERRED_NOEXCEPT_SPEC_P(NODE)                                                             \
        ((NODE) && (TREE_PURPOSE(NODE)) && (TREE_CODE(TREE_PURPOSE(NODE)) == DEFERRED_NOEXCEPT))
...
#define TRAIT_EXPR_TYPE1(NODE) (((struct tree_trait_expr *)TRAIT_EXPR_CHECK(NODE))->type1)

#define TRAIT_EXPR_TYPE2(NODE) (((struct tree_trait_expr *)TRAIT_EXPR_CHECK(NODE))->type2)

/* The specific trait that we are processing.  */
#define TRAIT_EXPR_KIND(NODE) (((struct tree_trait_expr *)TRAIT_EXPR_CHECK(NODE))->kind)
...
#define LAMBDA_TYPE_P(NODE) (CLASS_TYPE_P(NODE) && CLASSTYPE_LAMBDA_EXPR(NODE))

/* Test if FUNCTION_DECL is a lambda function.  */
#define LAMBDA_FUNCTION_P(FNDECL)                                                                  \
        (DECL_DECLARES_FUNCTION_P(FNDECL) && DECL_OVERLOADED_OPERATOR_P(FNDECL) &&                 \
         DECL_OVERLOADED_OPERATOR_IS(FNDECL, CALL_EXPR) && LAMBDA_TYPE_P(CP_DECL_CONTEXT(FNDECL)))
...
#define LAMBDA_EXPR_CAPTURE_OPTIMIZED(NODE) TREE_LANG_FLAG_2(LAMBDA_EXPR_CHECK(NODE))

/* True iff this LAMBDA_EXPR was generated in tsubst_lambda_expr.  */
#define LAMBDA_EXPR_INSTANTIATED(NODE) TREE_LANG_FLAG_3(LAMBDA_EXPR_CHECK(NODE))

/* True if this TREE_LIST in LAMBDA_EXPR_CAPTURE_LIST is for an explicit
   capture.  */
#define LAMBDA_CAPTURE_EXPLICIT_P(NODE) TREE_LANG_FLAG_0(TREE_LIST_CHECK(NODE))

/* The source location of the lambda.  */
#define LAMBDA_EXPR_LOCATION(NODE) (((struct tree_lambda_expr *)LAMBDA_EXPR_CHECK(NODE))->locus)
...
/* Non-zero if this template specialization has access violations that
   ...
   deduction.  */
#define TINFO_HAS_ACCESS_ERRORS(NODE)  (TREE_LANG_FLAG_0(TEMPLATE_INFO_CHECK(NODE)))
#define FNDECL_HAS_ACCESS_ERRORS(NODE) (TINFO_HAS_ACCESS_ERRORS(DECL_TEMPLATE_INFO(NODE)))
...
#define TINFO_USED_TEMPLATE_ID(NODE) (TREE_LANG_FLAG_1(TEMPLATE_INFO_CHECK(NODE)))
...
template <typename T> inline T *check_nonnull(T *p)
{
        gcc_assert(p);
        return p;
}
...
inline tree_constraint_info *check_constraint_info(tree t)
{
...
#define CI_TEMPLATE_REQS(NODE) check_constraint_info(check_nonnull(NODE))->template_reqs
...
// was specified.
#define CI_DECLARATOR_REQS(NODE) check_constraint_info(check_nonnull(NODE))->declarator_reqs
...
#define COMPOUND_REQ_NOEXCEPT_P(NODE) TREE_LANG_FLAG_0(TREE_CHECK(NODE, COMPOUND_REQ))

/* The constraints on an 'auto' placeholder type, used in an argument deduction
   constraint.  */
#define PLACEHOLDER_TYPE_CONSTRAINTS(NODE) DECL_SIZE_UNIT(TYPE_NAME(NODE))

/* The expression evaluated by the predicate constraint. */
#define PRED_CONSTR_EXPR(NODE) TREE_OPERAND(TREE_CHECK(NODE, PRED_CONSTR), 0)

/* The concept of a concept check. */
#define CHECK_CONSTR_CONCEPT(NODE) TREE_OPERAND(TREE_CHECK(NODE, CHECK_CONSTR), 0)

/* The template arguments of a concept check. */
#define CHECK_CONSTR_ARGS(NODE) TREE_OPERAND(TREE_CHECK(NODE, CHECK_CONSTR), 1)

/* The expression validated by the predicate constraint. */
#define EXPR_CONSTR_EXPR(NODE) TREE_OPERAND(TREE_CHECK(NODE, EXPR_CONSTR), 0)

/* The type validated by the predicate constraint. */
#define TYPE_CONSTR_TYPE(NODE) TREE_OPERAND(TREE_CHECK(NODE, TYPE_CONSTR), 0)

/* In an implicit conversion constraint, the source expression. */
#define ICONV_CONSTR_EXPR(NODE) TREE_OPERAND(TREE_CHECK(NODE, ICONV_CONSTR), 0)

/* In an implicit conversion constraint, the target type. */
#define ICONV_CONSTR_TYPE(NODE) TREE_OPERAND(TREE_CHECK(NODE, ICONV_CONSTR), 1)

/* In an argument deduction constraint, the source expression. */
#define DEDUCT_CONSTR_EXPR(NODE) TREE_OPERAND(TREE_CHECK(NODE, DEDUCT_CONSTR), 0)

/* In an argument deduction constraint, the target type pattern. */
#define DEDUCT_CONSTR_PATTERN(NODE) TREE_OPERAND(TREE_CHECK(NODE, DEDUCT_CONSTR), 1)

/* In an argument deduction constraint, the list of placeholder nodes. */
#define DEDUCT_CONSTR_PLACEHOLDER(NODE) TREE_OPERAND(TREE_CHECK(NODE, DEDUCT_CONSTR), 2)

/* The expression of an exception constraint. */
#define EXCEPT_CONSTR_EXPR(NODE) TREE_OPERAND(TREE_CHECK(NODE, EXCEPT_CONSTR), 0)

/* In a parameterized constraint, the local parameters. */
#define PARM_CONSTR_PARMS(NODE) TREE_OPERAND(TREE_CHECK(NODE, PARM_CONSTR), 0)

/* In a parameterized constraint, the operand. */
#define PARM_CONSTR_OPERAND(NODE) TREE_OPERAND(TREE_CHECK(NODE, PARM_CONSTR), 1)
...
union GTY((desc("cp_tree_node_structure (&%h)"),
        ...
        union tree_node GTY((tag("TS_CP_GENERIC"), desc("tree_node_structure (&%h)"))) generic;
        ...
        struct tree_static_assert GTY((tag("TS_CP_STATIC_ASSERT"))) static_assertion;
        struct tree_argument_pack_select
                GTY((tag("TS_CP_ARGUMENT_PACK_SELECT"))) argument_pack_select;
        struct tree_trait_expr GTY((tag("TS_CP_TRAIT_EXPR"))) trait_expression;
        struct tree_lambda_expr GTY((tag("TS_CP_LAMBDA_EXPR"))) lambda_expression;
        struct tree_template_info GTY((tag("TS_CP_TEMPLATE_INFO"))) template_info;
        struct tree_constraint_info GTY((tag("TS_CP_CONSTRAINT_INFO"))) constraint_info;
        struct tree_userdef_literal GTY((tag("TS_CP_USERDEF_LITERAL"))) userdef_literal;
};
...
struct processing_template_decl_sentinel {
        int saved;
        processing_template_decl_sentinel(bool reset = true) : saved(processing_template_decl)
        {
                if (reset) {
...
template <typename T> class temp_override
{
      ...
      public:
        ...
        temp_override(T &var, T overrider) : overridden_variable(var), saved_value(var)
        {
...
#define current_function_auto_return_pattern (cp_function_chain->x_auto_return_pattern)
...
#define WILDCARD_TYPE_P(T)                                                                         \
        (TREE_CODE(T) == TEMPLATE_TYPE_PARM || TREE_CODE(T) == TYPENAME_TYPE ||                    \
         TREE_CODE(T) == TYPEOF_TYPE || TREE_CODE(T) == BOUND_TEMPLATE_TEMPLATE_PARM ||            \
...
/* Set CLASS_TYPE_P for T to VAL.  T must be a class, struct, or
   union type.  */
#define SET_CLASS_TYPE_P(T, VAL) (TYPE_LANG_FLAG_5(RECORD_OR_UNION_CHECK(T)) = (VAL))

/* Nonzero if T is a class type.  Zero for template type parameters,
   typename types, and so forth.  */
#define CLASS_TYPE_P(T) (RECORD_OR_UNION_CODE_P(TREE_CODE(T)) && TYPE_LANG_FLAG_5(T))

/* Nonzero if T is a class type but not an union.  */
#define NON_UNION_CLASS_TYPE_P(T) (TREE_CODE(T) == RECORD_TYPE && TYPE_LANG_FLAG_5(T))
...
#define CLASSTYPE_VISIBILITY_SPECIFIED(TYPE) DECL_VISIBILITY_SPECIFIED(TYPE_MAIN_DECL(TYPE))
...
#define TYPE_HAS_CONVERSION(NODE) (LANG_TYPE_CLASS_CHECK(NODE)->has_type_conversion)
...
/* Nonzero means that NODE (a class type) has a move constructor --
   but that it has not yet been declared.  */
#define CLASSTYPE_LAZY_MOVE_CTOR(NODE) (LANG_TYPE_CLASS_CHECK(NODE)->lazy_move_ctor)

/* Nonzero means that NODE (a class type) has an assignment operator
   -- but that it has not yet been declared.  */
#define CLASSTYPE_LAZY_COPY_ASSIGN(NODE) (LANG_TYPE_CLASS_CHECK(NODE)->lazy_copy_assign)

/* Nonzero means that NODE (a class type) has an assignment operator
   -- but that it has not yet been declared.  */
#define CLASSTYPE_LAZY_MOVE_ASSIGN(NODE) (LANG_TYPE_CLASS_CHECK(NODE)->lazy_move_assign)
...
#define TYPE_HAS_CONST_COPY_ASSIGN(NODE) (LANG_TYPE_CLASS_CHECK(NODE)->has_const_copy_assign)

/* Nonzero means that this _CLASSTYPE node has an X(X&) constructor.  */
#define TYPE_HAS_COPY_CTOR(NODE)       (LANG_TYPE_CLASS_CHECK(NODE)->has_copy_ctor)
#define TYPE_HAS_CONST_COPY_CTOR(NODE) (LANG_TYPE_CLASS_CHECK(NODE)->has_const_copy_ctor)
...
#define TYPE_HAS_CONSTEXPR_CTOR(NODE) (LANG_TYPE_CLASS_CHECK(NODE)->has_constexpr_ctor)
...
#define CLASSTYPE_CONSTRUCTORS(NODE) (get_class_binding_direct(NODE, ctor_identifier))

/* A FUNCTION_DECL for the destructor for NODE.  This is the
   destructors that take an in-charge parameter.  If
   CLASSTYPE_LAZY_DESTRUCTOR is true, then this entry will be NULL
   until the destructor is created with lazily_declare_fn.  */
#define CLASSTYPE_DESTRUCTOR(NODE) (get_class_binding_direct(NODE, dtor_identifier))
...
#define IS_FAKE_BASE_TYPE(NODE)                                                                    \
        (TREE_CODE(NODE) == RECORD_TYPE && TYPE_CONTEXT(NODE) &&                                   \
         CLASS_TYPE_P(TYPE_CONTEXT(NODE)) && CLASSTYPE_AS_BASE(TYPE_CONTEXT(NODE)) == (NODE))
...
#define ABSTRACT_CLASS_TYPE_P(NODE) (CLASS_TYPE_P(NODE) && CLASSTYPE_PURE_VIRTUALS(NODE))

/* Nonzero means that this type has an X() constructor.  */
#define TYPE_HAS_DEFAULT_CONSTRUCTOR(NODE) (LANG_TYPE_CLASS_CHECK(NODE)->has_default_ctor)
...
#define CLASSTYPE_NON_LAYOUT_POD_P(NODE) (LANG_TYPE_CLASS_CHECK(NODE)->non_pod_class)

/* Nonzero means that this class type is a non-standard-layout class.  */
#define CLASSTYPE_NON_STD_LAYOUT(NODE) (LANG_TYPE_CLASS_CHECK(NODE)->non_std_layout)
...
#define LAMBDA_TYPE_EXTRA_SCOPE(NODE) (LAMBDA_EXPR_EXTRA_SCOPE(CLASSTYPE_LAMBDA_EXPR(NODE)))
...
#define CLASSTYPE_READONLY_FIELDS_NEED_INIT(NODE)                                                  \
        (TYPE_LANG_SPECIFIC(NODE) ? LANG_TYPE_CLASS_CHECK(NODE)->const_needs_init : 0)
...
#define SET_BINFO_NEW_VTABLE_MARKED(B)                                                             \
        (BINFO_NEW_VTABLE_MARKED(B) = 1, gcc_assert(!BINFO_PRIMARY_P(B) || BINFO_VIRTUAL_P(B)),    \
...
/* For FUNCTION_TYPE or METHOD_TYPE, a list of the exceptions that
   ...
   declared functions).  */
#define TYPE_RAISES_EXCEPTIONS(NODE) TYPE_LANG_SLOT_1(FUNC_OR_METHOD_CHECK(NODE))
...
struct GTY(()) lang_decl_fn {
        ...
        union lang_decl_u3 {
                ...
                struct language_function *GTY((tag("0"))) saved_language_function;
...
#define STRIP_TEMPLATE(NODE) (TREE_CODE(NODE) == TEMPLATE_DECL ? DECL_TEMPLATE_RESULT(NODE) : NODE)
...
#define LANG_DECL_FN_CHECK(NODE) (&DECL_LANG_SPECIFIC(STRIP_TEMPLATE(NODE))->u.fn)
...
#define DECL_LANGUAGE(NODE)                                                                        \
        (DECL_LANG_SPECIFIC(NODE) ? DECL_LANG_SPECIFIC(NODE)->u.base.language                      \
                                  : (TREE_CODE(NODE) == FUNCTION_DECL ? lang_c : lang_cplusplus))

/* Set the language linkage for NODE to LANGUAGE.  */
#define SET_DECL_LANGUAGE(NODE, LANGUAGE) (DECL_LANG_SPECIFIC(NODE)->u.base.language = (LANGUAGE))
...
/* Nonzero if NODE (a FUNCTION_DECL) is a constructor for a complete
   object.  */
#define DECL_COMPLETE_CONSTRUCTOR_P(NODE) (DECL_NAME(NODE) == complete_ctor_identifier)
...
/* Nonzero if NODE (a FUNCTION_DECL) is a constructor, but not either the
   ...
   constructor.  */
#define DECL_MAYBE_IN_CHARGE_CONSTRUCTOR_P(NODE) (DECL_NAME(NODE) == ctor_identifier)

/* Nonzero if NODE (a FUNCTION_DECL) is a copy constructor.  */
#define DECL_COPY_CONSTRUCTOR_P(NODE) (DECL_CONSTRUCTOR_P(NODE) && copy_fn_p(NODE) > 0)

/* Nonzero if NODE (a FUNCTION_DECL) is a move constructor.  */
#define DECL_MOVE_CONSTRUCTOR_P(NODE) (DECL_CONSTRUCTOR_P(NODE) && move_fn_p(NODE))
...
#define DECL_MAYBE_IN_CHARGE_DESTRUCTOR_P(NODE) (DECL_NAME(NODE) == dtor_identifier)

/* Nonzero if NODE (a FUNCTION_DECL) is a destructor for a complete
   object.  */
#define DECL_COMPLETE_DESTRUCTOR_P(NODE) (DECL_NAME(NODE) == complete_dtor_identifier)
...
#define DECL_DELETING_DESTRUCTOR_P(NODE) (DECL_NAME(NODE) == deleting_dtor_identifier)
...
#define DECL_MAYBE_IN_CHARGE_CDTOR_P(NODE)                                                         \
        (DECL_MAYBE_IN_CHARGE_CONSTRUCTOR_P(NODE) || DECL_MAYBE_IN_CHARGE_DESTRUCTOR_P(NODE))
...
#define DECL_DISCRIMINATOR_P(NODE)                                                                 \
        (((TREE_CODE(NODE) == VAR_DECL && TREE_STATIC(NODE)) || DECL_IMPLICIT_TYPEDEF_P(NODE)) &&  \
...
#define DECL_NEEDS_VTT_PARM_P(NODE)                                                                \
        (CLASSTYPE_VBASECLASSES(DECL_CONTEXT(NODE)) &&                                             \
         (DECL_BASE_CONSTRUCTOR_P(NODE) || DECL_BASE_DESTRUCTOR_P(NODE)))
...
#define DECL_CONV_FN_TYPE(FN) TREE_TYPE((gcc_checking_assert(DECL_CONV_FN_P(FN)), DECL_NAME(FN)))
...
#define DECL_ASSIGNMENT_OPERATOR_P(NODE) IDENTIFIER_ASSIGN_OP_P(DECL_NAME(NODE))
...
#define DECL_OVERLOADED_OPERATOR_CODE_RAW(NODE) (LANG_DECL_FN_CHECK(NODE)->ovl_op_code)
...
#define DECL_HAS_IN_CHARGE_PARM_P(NODE) (LANG_DECL_FN_CHECK(NODE)->has_in_charge_parm_p)

/* Nonzero if DECL is a declaration of __builtin_constant_p.  */
#define DECL_IS_BUILTIN_CONSTANT_P(NODE)                                                           \
        (TREE_CODE(NODE) == FUNCTION_DECL && DECL_BUILT_IN_CLASS(NODE) == BUILT_IN_NORMAL &&       \
...
#define DECL_INITIALIZED_IN_CLASS_P(DECL)                                                          \
        (DECL_LANG_SPECIFIC(VAR_OR_FUNCTION_DECL_CHECK(DECL))->u.base.initialized_in_class)

/* Nonzero if the DECL is used in the sense of 3.2 [basic.def.odr].
   Only available for decls with DECL_LANG_SPECIFIC.  */
#define DECL_ODR_USED(DECL) (DECL_LANG_SPECIFIC(VAR_OR_FUNCTION_DECL_CHECK(DECL))->u.base.odr_used)

/* Nonzero for DECL means that this decl is just a friend declaration,
   and should not be added to the list of members for this class.  */
#define DECL_FRIEND_P(NODE)                                                                        \
        (DECL_LANG_SPECIFIC(TYPE_FUNCTION_OR_TEMPLATE_DECL_CHECK(NODE))->u.base.friend_or_tls)

/* Nonzero if the thread-local variable was declared with __thread as
   opposed to thread_local.  */
#define DECL_GNU_TLS_P(NODE)                                                                       \
        (DECL_LANG_SPECIFIC(VAR_DECL_CHECK(NODE)) && DECL_LANG_SPECIFIC(NODE)->u.base.friend_or_tls)
...
#define DECL_BEFRIENDING_CLASSES(NODE) (LANG_DECL_FN_CHECK(NODE)->befriending_classes)

/* Nonzero for FUNCTION_DECL means that this decl is a static
   member function.  */
#define DECL_STATIC_FUNCTION_P(NODE) (LANG_DECL_FN_CHECK(NODE)->static_function)
...
#define DECL_CONST_MEMFUNC_P(NODE)                                                                 \
        (DECL_NONSTATIC_MEMBER_FUNCTION_P(NODE) &&                                                 \
         CP_TYPE_CONST_P(TREE_TYPE(TREE_VALUE(TYPE_ARG_TYPES(TREE_TYPE(NODE))))))

/* Nonzero for FUNCTION_DECL means that this member function
   has `this' as volatile X *const.  */
#define DECL_VOLATILE_MEMFUNC_P(NODE)                                                              \
        (DECL_NONSTATIC_MEMBER_FUNCTION_P(NODE) &&                                                 \
         CP_TYPE_VOLATILE_P(TREE_TYPE(TREE_VALUE(TYPE_ARG_TYPES(TREE_TYPE(NODE))))))
...
#define DECL_THUNKS(NODE) (DECL_VIRTUAL_P(NODE) ? LANG_DECL_FN_CHECK(NODE)->context : NULL_TREE)

/* Set DECL_THUNKS.  */
#define SET_DECL_THUNKS(NODE, THUNKS) (LANG_DECL_FN_CHECK(NODE)->context = (THUNKS))
...
#define SET_DECL_INHERITED_CTOR(NODE, INH) (LANG_DECL_FN_CHECK(NODE)->context = (INH))
...
#define DECL_THIS_THUNK_P(NODE) (DECL_THUNK_P(NODE) && LANG_DECL_FN_CHECK(NODE)->this_thunk_p)

/* Nonzero if NODE is a result pointer adjusting thunk.  */
#define DECL_RESULT_THUNK_P(NODE) (DECL_THUNK_P(NODE) && !LANG_DECL_FN_CHECK(NODE)->this_thunk_p)
...
#define DECL_REPO_AVAILABLE_P(NODE) (DECL_LANG_SPECIFIC(NODE)->u.base.repo_available_p)
...
#define DECL_DECLARED_CONCEPT_P(NODE) (DECL_LANG_SPECIFIC(NODE)->u.base.concept_p)
...
#define DECL_FNAME_P(NODE)                                                                         \
        (VAR_P(NODE) && DECL_NAME(NODE) && DECL_ARTIFICIAL(NODE) && DECL_HAS_VALUE_EXPR_P(NODE) && \
         (id_equal(DECL_NAME(NODE), "__PRETTY_FUNCTION__") ||                                      \
          id_equal(DECL_NAME(NODE), "__FUNCTION__") || id_equal(DECL_NAME(NODE), "__func__")))
...
#define DECL_FRIEND_CONTEXT(NODE)                                                                  \
        ((DECL_DECLARES_FUNCTION_P(NODE) && DECL_FRIEND_P(NODE) && !DECL_FUNCTION_MEMBER_P(NODE))  \
                 ? LANG_DECL_FN_CHECK(NODE)->context                                               \
                 : NULL_TREE)

/* Set the DECL_FRIEND_CONTEXT for NODE to CONTEXT.  */
#define SET_DECL_FRIEND_CONTEXT(NODE, CONTEXT) (LANG_DECL_FN_CHECK(NODE)->context = (CONTEXT))

#define CP_DECL_CONTEXT(NODE) (!DECL_FILE_SCOPE_P(NODE) ? DECL_CONTEXT(NODE) : global_namespace)
#define CP_TYPE_CONTEXT(NODE) (!TYPE_FILE_SCOPE_P(NODE) ? TYPE_CONTEXT(NODE) : global_namespace)
...
#define SCOPE_DEPTH(NODE) (NAMESPACE_DECL_CHECK(NODE)->base.u.bits.address_space)

/* Whether the namepace is an inline namespace.  */
#define DECL_NAMESPACE_INLINE_P(NODE) TREE_LANG_FLAG_0(NAMESPACE_DECL_CHECK(NODE))

/* In a NAMESPACE_DECL, a vector of using directives.  */
#define DECL_NAMESPACE_USING(NODE) (LANG_DECL_NS_CHECK(NODE)->usings)
...
#define DECL_NAMESPACE_STD_P(NODE)                                                                 \
        (TREE_CODE(NODE) == NAMESPACE_DECL && CP_DECL_CONTEXT(NODE) == global_namespace &&         \
...
#define DECL_PENDING_INLINE_P(NODE) (LANG_DECL_FN_CHECK(NODE)->pending_inline_p)

/* If DECL_PENDING_INLINE_P holds, this is the saved text of the
   function.  */
#define DECL_PENDING_INLINE_INFO(NODE) (LANG_DECL_FN_CHECK(NODE)->u.pending_inline_info)

/* Nonzero for TYPE_DECL means that it was written 'using name = type'.  */
#define TYPE_DECL_ALIAS_P(NODE) DECL_LANG_FLAG_6(TYPE_DECL_CHECK(NODE))

/* Nonzero for TEMPLATE_DECL means that it is a 'complex' alias template.  */
#define TEMPLATE_DECL_COMPLEX_ALIAS_P(NODE) DECL_LANG_FLAG_2(TEMPLATE_DECL_CHECK(NODE))

/* Nonzero for a type which is an alias for another type; i.e, a type
   which declaration was written 'using name-of-type =
   another-type'.  */
#define TYPE_ALIAS_P(NODE)                                                                         \
        (TYPE_P(NODE) && TYPE_NAME(NODE) && TREE_CODE(TYPE_NAME(NODE)) == TYPE_DECL &&             \
...
#define DECL_TEMPLATE_INFO(NODE)                                                                   \
        (DECL_LANG_SPECIFIC(VAR_TEMPL_TYPE_FIELD_OR_FUNCTION_DECL_CHECK(NODE))->u.min.template_info)
...
#define CLASSTYPE_TEMPLATE_INFO(NODE) (TYPE_LANG_SLOT_1(RECORD_OR_UNION_CHECK(NODE)))
...
#define TYPE_TEMPLATE_INFO(NODE)                                                                   \
        (TREE_CODE(NODE) == ENUMERAL_TYPE || TREE_CODE(NODE) == BOUND_TEMPLATE_TEMPLATE_PARM ||    \
...
#define TYPE_ALIAS_TEMPLATE_INFO(NODE)                                                             \
        (DECL_LANG_SPECIFIC(TYPE_NAME(NODE)) ? DECL_TEMPLATE_INFO(TYPE_NAME(NODE)) : NULL_TREE)
...
#define TYPE_TEMPLATE_INFO_MAYBE_ALIAS(NODE)                                                       \
        (TYPE_ALIAS_P(NODE) ? TYPE_ALIAS_TEMPLATE_INFO(NODE) : TYPE_TEMPLATE_INFO(NODE))

/* Set the template information for an ENUMERAL_, RECORD_, or
   UNION_TYPE to VAL.  */
#define SET_TYPE_TEMPLATE_INFO(NODE, VAL)                                                          \
        (TREE_CODE(NODE) == ENUMERAL_TYPE || (CLASS_TYPE_P(NODE) && !TYPE_ALIAS_P(NODE))           \
                 ? (TYPE_LANG_SLOT_1(NODE) = (VAL))                                                \
                 : (DECL_TEMPLATE_INFO(TYPE_NAME(NODE)) = (VAL)))

#define TI_TEMPLATE(NODE)                     TREE_TYPE(TEMPLATE_INFO_CHECK(NODE))
#define TI_ARGS(NODE)                         TREE_CHAIN(TEMPLATE_INFO_CHECK(NODE))
#define TI_PENDING_TEMPLATE_FLAG(NODE)        TREE_LANG_FLAG_1(TEMPLATE_INFO_CHECK(NODE))
...
#define TI_TYPEDEFS_NEEDING_ACCESS_CHECKING(NODE)                                                  \
        ((struct tree_template_info *)TEMPLATE_INFO_CHECK(NODE))->typedefs_needing_access_checking
...
#define PACK_EXPANSION_P(NODE)                                                                     \
        (TREE_CODE(NODE) == TYPE_PACK_EXPANSION || TREE_CODE(NODE) == EXPR_PACK_EXPANSION)

/* Extracts the type or expression pattern from a TYPE_PACK_EXPANSION or
   EXPR_PACK_EXPANSION.  */
#define PACK_EXPANSION_PATTERN(NODE)                                                               \
        (TREE_CODE(NODE) == TYPE_PACK_EXPANSION ? TREE_TYPE(NODE) : TREE_OPERAND(NODE, 0))
...
#define ARGUMENT_PACK_P(NODE)                                                                      \
        (TREE_CODE(NODE) == TYPE_ARGUMENT_PACK || TREE_CODE(NODE) == NONTYPE_ARGUMENT_PACK)

/* The arguments stored in an argument pack. Arguments are stored in a
   TREE_VEC, which may have length zero.  */
#define ARGUMENT_PACK_ARGS(NODE)                                                                   \
        (TREE_CODE(NODE) == TYPE_ARGUMENT_PACK ? TREE_TYPE(NODE) : TREE_OPERAND(NODE, 0))
...
#define ARGUMENT_PACK_INCOMPLETE_P(NODE) TREE_ADDRESSABLE(ARGUMENT_PACK_ARGS(NODE))
...
#define FOLD_EXPR_CHECK(NODE)                                                                      \
        TREE_CHECK4(NODE, UNARY_LEFT_FOLD_EXPR, UNARY_RIGHT_FOLD_EXPR, BINARY_LEFT_FOLD_EXPR,      \
...
#define FOLD_EXPR_P(NODE)                                                                          \
        (TREE_CODE(NODE) == UNARY_LEFT_FOLD_EXPR || TREE_CODE(NODE) == UNARY_RIGHT_FOLD_EXPR ||    \
         TREE_CODE(NODE) == BINARY_LEFT_FOLD_EXPR || TREE_CODE(NODE) == BINARY_RIGHT_FOLD_EXPR)
...
#define FOLD_EXPR_OP(NODE) TREE_OPERAND(FOLD_EXPR_CHECK(NODE), 0)
...
#define REFERENCE_VLA_OK(NODE) (TYPE_LANG_FLAG_5(REFERENCE_TYPE_CHECK(NODE)))
...
#define CALL_OR_AGGR_INIT_CHECK(NODE) TREE_CHECK2((NODE), CALL_EXPR, AGGR_INIT_EXPR)
...
#define CALL_EXPR_ORDERED_ARGS(NODE) TREE_LANG_FLAG_3(CALL_OR_AGGR_INIT_CHECK(NODE))

/* True if the arguments to NODE should be evaluated in right-to-left
   order regardless of PUSH_ARGS_REVERSED.  */
#define CALL_EXPR_REVERSE_ARGS(NODE) TREE_LANG_FLAG_5(CALL_OR_AGGR_INIT_CHECK(NODE))

/* True if CALL_EXPR was written as an operator expression, not a function
   call.  */
#define CALL_EXPR_OPERATOR_SYNTAX(NODE) TREE_LANG_FLAG_6(CALL_OR_AGGR_INIT_CHECK(NODE))
...
#define REF_PARENTHESIZED_P(NODE)                                                                  \
        TREE_LANG_FLAG_2(                                                                          \
                TREE_CHECK4((NODE), COMPONENT_REF, INDIRECT_REF, SCOPE_REF, VIEW_CONVERT_EXPR))
...
/* Nonzero if expanding this AGGR_INIT_EXPR should first zero-initialize
   the object.  */
#define AGGR_INIT_ZERO_FIRST(NODE) TREE_LANG_FLAG_2(AGGR_INIT_EXPR_CHECK(NODE))

/* Nonzero means that the call is the jump from a thunk to the
   thunked-to function.  */
#define AGGR_INIT_FROM_THUNK_P(NODE) (AGGR_INIT_EXPR_CHECK(NODE)->base.protected_flag)
...
#define AGGR_INIT_EXPR_ARG(NODE, I) TREE_OPERAND(AGGR_INIT_EXPR_CHECK(NODE), (I) + 3)
...
#define AGGR_INIT_EXPR_ARGP(NODE) (&(TREE_OPERAND(AGGR_INIT_EXPR_CHECK(NODE), 0)) + 3)
...
inline void init_aggr_init_expr_arg_iterator(tree exp, aggr_init_expr_arg_iterator *iter)
{
        iter->t = exp;
...
inline tree next_aggr_init_expr_arg(aggr_init_expr_arg_iterator *iter)
{
        tree result;
        if (iter->i >= iter->n) {
                return NULL_TREE;
...
inline tree first_aggr_init_expr_arg(tree exp, aggr_init_expr_arg_iterator *iter)
{
...
inline bool more_aggr_init_expr_args_p(const aggr_init_expr_arg_iterator *iter)
{
...
#define VEC_INIT_EXPR_IS_CONSTEXPR(NODE) TREE_LANG_FLAG_0(VEC_INIT_EXPR_CHECK(NODE))

/* Indicates that a VEC_INIT_EXPR is expressing value-initialization.  */
#define VEC_INIT_EXPR_VALUE_INIT(NODE) TREE_LANG_FLAG_1(VEC_INIT_EXPR_CHECK(NODE))

/* The condition under which this MUST_NOT_THROW_EXPR actually blocks
   exceptions.  NULL_TREE means 'true'.  */
#define MUST_NOT_THROW_COND(NODE) TREE_OPERAND(MUST_NOT_THROW_EXPR_CHECK(NODE), 1)
...
#define TYPENAME_IS_RESOLVING_P(NODE) (TREE_LANG_FLAG_2(TYPENAME_TYPE_CHECK(NODE)))
...
#define DECL_ANTICIPATED(NODE)                                                                     \
        (DECL_LANG_SPECIFIC(TYPE_FUNCTION_OR_TEMPLATE_DECL_CHECK(NODE))->u.base.anticipated_p)
...
#define TYPE_HIDDEN_P(NODE)                                                                        \
        (DECL_LANG_SPECIFIC(TYPE_NAME(NODE)) && DECL_ANTICIPATED(TYPE_NAME(NODE)))
...
#define DECL_HIDDEN_FRIEND_P(NODE) (LANG_DECL_FN_CHECK(DECL_COMMON_CHECK(NODE))->hidden_friend_p)
...
#define SET_DECL_VAR_DECLARED_INLINE_P(NODE)                                                       \
        (DECL_LANG_SPECIFIC(VAR_DECL_CHECK(NODE))->u.base.var_declared_inline_p = true)
...
#define DECL_INLINE_VAR_P(NODE)                                                                    \
        (DECL_VAR_DECLARED_INLINE_P(NODE) ||                                                       \
         (cxx_dialect >= cxx17 && DECL_DECLARED_CONSTEXPR_P(NODE) && DECL_CLASS_SCOPE_P(NODE)))

/* Nonzero if DECL was declared with '= delete'.  */
#define DECL_DELETED_FN(DECL) (LANG_DECL_FN_CHECK(DECL)->min.base.threadprivate_or_deleted_p)
...
#define DECL_DEFAULTED_OUTSIDE_CLASS_P(DECL)                                                       \
        (DECL_DEFAULTED_FN(DECL) && !(DECL_ARTIFICIAL(DECL) || DECL_INITIALIZED_IN_CLASS_P(DECL)))
...
#define SCALAR_TYPE_P(TYPE)                                                                        \
        (TYPE_PTRDATAMEM_P(TYPE) || TREE_CODE(TYPE) == ENUMERAL_TYPE || ARITHMETIC_TYPE_P(TYPE) || \
         TYPE_PTR_P(TYPE) || TYPE_PTRMEMFUNC_P(TYPE) || NULLPTR_TYPE_P(TYPE))
...
#define SCOPED_ENUM_P(TYPE) (TREE_CODE(TYPE) == ENUMERAL_TYPE && ENUM_IS_SCOPED(TYPE))

/* Determine whether this is an unscoped enumeration type.  */
#define UNSCOPED_ENUM_P(TYPE) (TREE_CODE(TYPE) == ENUMERAL_TYPE && !ENUM_IS_SCOPED(TYPE))
...
#define OPAQUE_ENUM_P(TYPE) (TREE_CODE(TYPE) == ENUMERAL_TYPE && ENUM_IS_OPAQUE(TYPE))
...
#define CP_AGGREGATE_TYPE_P(TYPE)                                                                  \
        (TREE_CODE(TYPE) == VECTOR_TYPE || TREE_CODE(TYPE) == ARRAY_TYPE ||                        \
...
#define TYPE_HAS_LATE_RETURN_TYPE(NODE) (TYPE_LANG_FLAG_2(FUNC_OR_METHOD_CHECK(NODE)))
...
#define EMPTY_CONSTRUCTOR_P(NODE)                                                                  \
        (TREE_CODE(NODE) == CONSTRUCTOR && vec_safe_is_empty(CONSTRUCTOR_ELTS(NODE)) &&            \
...
/* True if this CONSTRUCTOR is instantiation-dependent and needs to be
   substituted.  */
#define CONSTRUCTOR_IS_DEPENDENT(NODE) (TREE_LANG_FLAG_1(CONSTRUCTOR_CHECK(NODE)))

/* True if this CONSTRUCTOR should not be used as a variable initializer
   because it was loaded from a constexpr variable with mutable fields.  */
#define CONSTRUCTOR_MUTABLE_POISON(NODE) (TREE_LANG_FLAG_2(CONSTRUCTOR_CHECK(NODE)))

/* True if this typed CONSTRUCTOR represents C99 compound-literal syntax rather
   than C++11 functional cast syntax.  */
#define CONSTRUCTOR_C99_COMPOUND_LITERAL(NODE) (TREE_LANG_FLAG_3(CONSTRUCTOR_CHECK(NODE)))
...
#define CONSTRUCTOR_PLACEHOLDER_BOUNDARY(NODE) (TREE_LANG_FLAG_5(CONSTRUCTOR_CHECK(NODE)))
...
#define CONSTRUCTOR_IS_DESIGNATED_INIT(NODE) (TREE_LANG_FLAG_6(CONSTRUCTOR_CHECK(NODE)))

/* True if NODE represents a conversion for direct-initialization in a
   template.  Set by perform_implicit_conversion_flags.  */
#define IMPLICIT_CONV_EXPR_DIRECT_INIT(NODE) (TREE_LANG_FLAG_0(IMPLICIT_CONV_EXPR_CHECK(NODE)))

/* True if NODE represents a dependent conversion of a non-type template
   argument.  Set by maybe_convert_nontype_argument.  */
#define IMPLICIT_CONV_EXPR_NONTYPE_ARG(NODE) (TREE_LANG_FLAG_1(IMPLICIT_CONV_EXPR_CHECK(NODE)))

/* True if NODE represents a conversion for braced-init-list in a
   template.  Set by perform_implicit_conversion_flags.  */
#define IMPLICIT_CONV_EXPR_BRACED_INIT(NODE) (TREE_LANG_FLAG_2(IMPLICIT_CONV_EXPR_CHECK(NODE)))
...
#define TYPE_NON_AGGREGATE_CLASS(NODE) (CLASS_TYPE_P(NODE) && CLASSTYPE_NON_AGGREGATE(NODE))
...
#define TYPE_OBJ_P(NODE)                                                                           \
        (!TYPE_REF_P(NODE) && !VOID_TYPE_P(NODE) && TREE_CODE(NODE) != FUNCTION_TYPE &&            \
...
/* Returns true if NODE is a reference to an object.  Keep these checks
   in ascending tree code order.  */
#define TYPE_REF_OBJ_P(NODE) (TYPE_REF_P(NODE) && TYPE_OBJ_P(TREE_TYPE(NODE)))
...
#define TYPE_PTRFN_P(NODE) (TYPE_PTR_P(NODE) && TREE_CODE(TREE_TYPE(NODE)) == FUNCTION_TYPE)

/* Returns true if NODE is a reference to function type.  */
#define TYPE_REFFN_P(NODE) (TYPE_REF_P(NODE) && TREE_CODE(TREE_TYPE(NODE)) == FUNCTION_TYPE)
...
#define TYPE_PTRMEM_P(NODE) (TYPE_PTRDATAMEM_P(NODE) || TYPE_PTRMEMFUNC_P(NODE))
...
#define PTRMEM_OK_P(NODE) TREE_LANG_FLAG_0(TREE_CHECK3((NODE), ADDR_EXPR, OFFSET_REF, SCOPE_REF))

/* Get the POINTER_TYPE to the METHOD_TYPE associated with this
   pointer to member function.  TYPE_PTRMEMFUNC_P _must_ be true,
   before using this macro.  */
#define TYPE_PTRMEMFUNC_FN_TYPE(NODE)                                                              \
        (cp_build_qualified_type(TREE_TYPE(TYPE_FIELDS(NODE)), cp_type_quals(NODE)))
...
#define TYPE_PTRMEM_CLASS_TYPE(NODE)                                                               \
        (TYPE_PTRDATAMEM_P(NODE) ? TYPE_OFFSET_BASETYPE(NODE) : TYPE_PTRMEMFUNC_OBJECT_TYPE(NODE))

/* For a pointer-to-member type of the form `T X::*', this is `T'.  */
#define TYPE_PTRMEM_POINTED_TO_TYPE(NODE)                                                          \
        (TYPE_PTRDATAMEM_P(NODE) ? TREE_TYPE(NODE) : TREE_TYPE(TYPE_PTRMEMFUNC_FN_TYPE(NODE)))
...
#define UNDERLYING_TYPE_TYPE(NODE) (TYPE_VALUES_RAW(UNDERLYING_TYPE_CHECK(NODE)))
...
#define DECLTYPE_FOR_LAMBDA_CAPTURE(NODE) TREE_LANG_FLAG_0(DECLTYPE_TYPE_CHECK(NODE))
#define DECLTYPE_FOR_LAMBDA_PROXY(NODE)   TREE_LANG_FLAG_2(DECLTYPE_TYPE_CHECK(NODE))
#define DECLTYPE_FOR_REF_CAPTURE(NODE)    TREE_LANG_FLAG_3(DECLTYPE_TYPE_CHECK(NODE))
...
/* Nonzero for a FIELD_DECL who's NSMDI is currently being
   instantiated.  */
#define DECL_INSTANTIATING_NSDMI_P(NODE) DECL_LANG_FLAG_2(FIELD_DECL_CHECK(NODE))
...
#define DECL_TEMPLATE_INSTANTIATIONS(NODE) DECL_SIZE_UNIT(TEMPLATE_DECL_CHECK(NODE))
...
/* For a class template, this list contains the partial
   ...
   This list is not used for other templates.  */
#define DECL_TEMPLATE_SPECIALIZATIONS(NODE) DECL_SIZE(TEMPLATE_DECL_CHECK(NODE))
...
#define TEMPLATE_PARM_P(NODE)                                                                      \
        (TREE_CODE(NODE) == TEMPLATE_TYPE_PARM || TREE_CODE(NODE) == TEMPLATE_TEMPLATE_PARM ||     \
...
#define DECL_FUNCTION_TEMPLATE_P(NODE)                                                             \
        (TREE_CODE(NODE) == TEMPLATE_DECL && DECL_TEMPLATE_RESULT(NODE) != NULL_TREE &&            \
...
#define DECL_CLASS_TEMPLATE_P(NODE)                                                                \
        (DECL_TYPE_TEMPLATE_P(NODE) && DECL_IMPLICIT_TYPEDEF_P(DECL_TEMPLATE_RESULT(NODE)))

/* Nonzero for a TEMPLATE_DECL that represents an alias template.  */
#define DECL_ALIAS_TEMPLATE_P(NODE)                                                                \
        (DECL_TYPE_TEMPLATE_P(NODE) && !DECL_ARTIFICIAL(DECL_TEMPLATE_RESULT(NODE)))

/* Nonzero for a NODE which declares a type.  */
#define DECL_DECLARES_TYPE_P(NODE) (TREE_CODE(NODE) == TYPE_DECL || DECL_TYPE_TEMPLATE_P(NODE))
...
#define DECL_FRIEND_PSEUDO_TEMPLATE_INSTANTIATION(DECL)                                            \
        (DECL_LANG_SPECIFIC(DECL) && DECL_TEMPLATE_INFO(DECL) && !DECL_USE_TEMPLATE(DECL))

/* Nonzero if DECL is a function generated from a function 'temploid',
   i.e. template, member of class template, or dependent friend.  */
#define DECL_TEMPLOID_INSTANTIATION(DECL)                                                          \
        (DECL_TEMPLATE_INSTANTIATION(DECL) || DECL_FRIEND_PSEUDO_TEMPLATE_INSTANTIATION(DECL))

/* Nonzero if DECL is either defined implicitly by the compiler or
   generated from a temploid.  */
#define DECL_GENERATED_P(DECL) (DECL_TEMPLOID_INSTANTIATION(DECL) || DECL_DEFAULTED_FN(DECL))
...
#define DECL_NOT_REALLY_EXTERN(NODE) (DECL_LANG_SPECIFIC(NODE)->u.base.not_really_extern)

#define DECL_REALLY_EXTERN(NODE)                                                                   \
        (DECL_EXTERNAL(NODE) && (!DECL_LANG_SPECIFIC(NODE) || !DECL_NOT_REALLY_EXTERN(NODE)))
...
#define THUNK_VIRTUAL_OFFSET(DECL) (LANG_DECL_MIN_CHECK(FUNCTION_DECL_CHECK(DECL))->access)

/* A thunk which is equivalent to another thunk.  */
#define THUNK_ALIAS(DECL) (DECL_LANG_SPECIFIC(FUNCTION_DECL_CHECK(DECL))->u.min.template_info)
...
#define QUALIFIED_NAME_IS_TEMPLATE(NODE) (TREE_LANG_FLAG_1(SCOPE_REF_CHECK(NODE)))
...
#define OMP_FOR_GIMPLIFYING_P(NODE) (TREE_LANG_FLAG_0(OMP_LOOP_CHECK(NODE)))

/* A language-specific token attached to the OpenMP data clauses to
   hold code (or code fragments) related to ctors, dtors, and op=.
   See semantics.c for details.  */
#define CP_OMP_CLAUSE_INFO(NODE)                                                                   \
        TREE_TYPE(OMP_CLAUSE_RANGE_CHECK(NODE, OMP_CLAUSE_PRIVATE, OMP_CLAUSE_LINEAR))

/* Nonzero if this transaction expression's body contains statements.  */
#define TRANSACTION_EXPR_IS_STMT(NODE) TREE_LANG_FLAG_0(TRANSACTION_EXPR_CHECK(NODE))
...
/* True if this TARGET_EXPR is the result of list-initialization of a
   temporary.  */
#define TARGET_EXPR_LIST_INIT_P(NODE) TREE_LANG_FLAG_1(TARGET_EXPR_CHECK(NODE))

/* True if this TARGET_EXPR expresses direct-initialization of an object
   to be named later.  */
#define TARGET_EXPR_DIRECT_INIT_P(NODE) TREE_LANG_FLAG_2(TARGET_EXPR_CHECK(NODE))

/* True if NODE is a TARGET_EXPR that just expresses a copy of its INITIAL; if
   the initializer has void type, it's doing something more complicated.  */
#define SIMPLE_TARGET_EXPR_P(NODE)                                                                 \
        (TREE_CODE(NODE) == TARGET_EXPR && !VOID_TYPE_P(TREE_TYPE(TARGET_EXPR_INITIAL(NODE))))
...
#define CONVERT_EXPR_VBASE_PATH(NODE) TREE_LANG_FLAG_0(CONVERT_EXPR_CHECK(NODE))
...
struct cp_evaluated {
        int uneval;
        int inhibit;
        cp_evaluated() : uneval(cp_unevaluated_operand), inhibit(c_inhibit_evaluation_warnings)
        {
...
#define VTABLE_NAME_P(ID_NODE)                                                                     \
        (!strncmp(IDENTIFIER_POINTER(ID_NODE), VTABLE_NAME, sizeof(VTABLE_NAME) - 1))
...
#define VTABLE_NAME_P(ID_NODE)                                                                     \
        (IDENTIFIER_POINTER(ID_NODE)[1] == 'v' && IDENTIFIER_POINTER(ID_NODE)[2] == 't' &&         \
...
#define CONV_OLD_CONVERT (CONV_IMPLICIT | CONV_STATIC | CONV_CONST | CONV_REINTERPRET)
#define CONV_C_CAST                                                                                \
...
#define TEMPLATE_PARM_PARAMETER_PACK(NODE) (TREE_LANG_FLAG_0(TEMPLATE_PARM_INDEX_CHECK(NODE)))

/* These macros are for accessing the fields of TEMPLATE_TYPE_PARM,
   TEMPLATE_TEMPLATE_PARM and BOUND_TEMPLATE_TEMPLATE_PARM nodes.  */
#define TEMPLATE_TYPE_PARM_INDEX(NODE)                                                             \
        (TYPE_VALUES_RAW(TREE_CHECK3((NODE), TEMPLATE_TYPE_PARM, TEMPLATE_TEMPLATE_PARM,           \
...
#define CLASS_PLACEHOLDER_TEMPLATE(NODE) (DECL_INITIAL(TYPE_NAME(TEMPLATE_TYPE_PARM_CHECK(NODE))))
...
#define TEMPLATE_TYPE_PARM_FOR_CLASS(NODE) (TREE_LANG_FLAG_0(TEMPLATE_TYPE_PARM_CHECK(NODE)))

/* True iff this TEMPLATE_TYPE_PARM represents decltype(auto).  */
#define AUTO_IS_DECLTYPE(NODE) (TYPE_LANG_FLAG_5(TEMPLATE_TYPE_PARM_CHECK(NODE)))
...
#define TEMPLATE_TEMPLATE_PARM_TEMPLATE_DECL(NODE)                                                 \
        ((TREE_CODE(NODE) == BOUND_TEMPLATE_TEMPLATE_PARM) ? TYPE_TI_TEMPLATE(NODE)                \
                                                           : TYPE_NAME(NODE))
...
inline tree type_of_this_parm(const_tree fntype)
{
...
inline tree class_of_this_parm(const_tree fntype)
{
        return TREE_TYPE(type_of_this_parm(fntype));
}

/* True iff T is a variable template declaration. */
inline bool variable_template_p(tree t)
{
...
inline bool variable_concept_p(tree t)
{
        if (TREE_CODE(t) != TEMPLATE_DECL) {
                return false;
        }
...
inline bool concept_template_p(tree t)
{
        if (TREE_CODE(t) != TEMPLATE_DECL) {
                return false;
        }
...
extern tree build_conditional_expr(const op_location_t &, tree, tree, tree, tsubst_flags_t);
...
extern tree build_user_type_conversion(tree, tree, int, tsubst_flags_t);
extern tree build_new_function_call(tree, vec<tree, va_gc> **, tsubst_flags_t);
extern tree build_operator_new_call(tree, vec<tree, va_gc> **, tree *, tree *, tree, tree, tree *,
                                    tsubst_flags_t);
extern tree build_new_method_call(tree, tree, vec<tree, va_gc> **, tree, int, tree *,
                                  tsubst_flags_t);
extern tree build_special_member_call(tree, tree, vec<tree, va_gc> **, tree, int, tsubst_flags_t);
extern tree build_new_op(const op_location_t &, enum tree_code, int, tree, tree, tree, tree *,
                         tsubst_flags_t);
extern tree build_op_call(tree, vec<tree, va_gc> **, tsubst_flags_t);
extern bool aligned_allocation_fn_p(tree);
extern tree destroying_delete_p(tree);
extern bool usual_deallocation_fn_p(tree);
extern tree build_op_delete_call(enum tree_code, tree, tree, bool, tree, tree, tsubst_flags_t);
extern bool can_convert(tree, tree, tsubst_flags_t);
extern bool can_convert_standard(tree, tree, tsubst_flags_t);
extern bool can_convert_arg(tree, tree, tree, int, tsubst_flags_t);
extern bool can_convert_arg_bad(tree, tree, tree, int, tsubst_flags_t);
...
extern void complain_about_bad_argument(location_t arg_loc, tree from_type, tree to_type,
...
class access_failure_info
{
      public:
        access_failure_info()
                : m_was_inaccessible(false), m_basetype_path(NULL_TREE), m_decl(NULL_TREE),
...
extern bool enforce_access(tree, tree, tree, tsubst_flags_t, access_failure_info *afi = NULL);
extern void push_defarg_context(tree);
extern void pop_defarg_context(void);
extern tree convert_default_arg(tree, tree, tree, int, tsubst_flags_t);
...
extern tree initialize_reference(tree, tree, int, tsubst_flags_t);
extern tree extend_ref_init_temps(tree, tree, vec<tree, va_gc> **, tree * = NULL);
...
extern tree perform_direct_initialization_if_possible(tree, tree, bool, tsubst_flags_t);
extern tree in_charge_arg_for_name(tree);
extern tree build_cxx_call(tree, int, tree *, tsubst_flags_t);
...
extern tree build_base_path(enum tree_code, tree, tree, int, tsubst_flags_t);
extern tree convert_to_base(tree, tree, bool, bool, tsubst_flags_t);
...
/* in cvt.c */
extern tree convert_to_reference(tree, tree, int, int, tree, tsubst_flags_t);
extern tree convert_from_reference(tree);
extern tree force_rvalue(tree, tsubst_flags_t);
extern tree ocp_convert(tree, tree, int, int, tsubst_flags_t);
...
extern tree convert_to_void(tree, impl_conv_void, tsubst_flags_t);
extern tree convert_force(tree, tree, int, tsubst_flags_t);
...
extern void warn_misplaced_attr_for_class_type(location_t location, tree class_type);
...
extern bool start_function(cp_decl_specifier_seq *, const cp_declarator *, tree);
...
extern bool check_array_designated_initializer(constructor_elt *, unsigned HOST_WIDE_INT);
...
extern void grokclassfn(tree, tree, enum overload_flags);
extern tree grok_array_decl(location_t, tree, tree, bool);
extern tree delete_sanity(tree, tree, bool, int, tsubst_flags_t);
extern tree check_classfn(tree, tree, tree);
extern void check_member_template(tree);
extern tree grokfield(const cp_declarator *, cp_decl_specifier_seq *, tree, bool, tree, tree);
extern tree grokbitfield(const cp_declarator *, cp_decl_specifier_seq *, tree, tree, tree);
...
extern tree build_offset_ref_call_from_tree(tree, vec<tree, va_gc> **, tsubst_flags_t);
...
extern tree mark_use(tree expr, bool rvalue_p, bool read_p, location_t = UNKNOWN_LOCATION,
                     bool reject_builtin = true);
extern tree mark_rvalue_use(tree, location_t = UNKNOWN_LOCATION, bool reject_builtin = true);
...
extern tree build_aggr_init(tree, tree, int, tsubst_flags_t);
...
extern tree build_offset_ref(tree, tree, bool, tsubst_flags_t);
extern tree throw_bad_array_new_length(void);
extern bool type_has_new_extended_alignment(tree);
extern unsigned malloc_alignment(void);
extern tree build_new(vec<tree, va_gc> **, tree, tree, vec<tree, va_gc> **, int, tsubst_flags_t);
extern tree get_temp_regvar(tree, tree);
extern tree build_vec_init(tree, tree, tree, bool, int, tsubst_flags_t);
extern tree build_delete(tree, tree, special_function_kind, int, int, tsubst_flags_t);
extern void push_base_cleanups(void);
extern tree build_vec_delete(tree, tree, special_function_kind, int, tsubst_flags_t);
...
extern tree unqualified_name_lookup_error(tree, location_t = UNKNOWN_LOCATION);
...
extern tree implicitly_declare_fn(special_function_kind, tree, bool, tree, tree);

/* In optimize.c */
extern bool maybe_clone_body(tree);

/* In parser.c */
extern tree cp_convert_range_for(tree, tree, tree, tree, unsigned int, bool, unsigned short);
extern void cp_convert_omp_range_for(tree &, vec<tree, va_gc> *, tree &, tree &, tree &, tree &,
                                     tree &, tree &);
...
extern tree check_explicit_specialization(tree, tree, int, int, tree = NULL_TREE);
...
extern tree do_auto_deduction(tree, tree, tree, tsubst_flags_t = tf_warning_or_error,
                              auto_deduction_context = adc_unspecified, tree = NULL_TREE,
                              int = LOOKUP_NORMAL);
extern tree type_uses_auto(tree);
extern tree type_uses_auto_or_concept(tree);
extern void append_type_to_template_for_access_check(tree, tree, tree, location_t);
extern tree convert_generic_types_to_packs(tree, int, int);
extern tree splice_late_return_type(tree, tree);
extern bool is_auto(const_tree);
extern tree process_template_parm(tree, location_t, tree, bool, bool);
...
extern tree fn_type_unification(tree, tree, tree, const tree *, unsigned int, tree,
                                unification_kind_t, int, struct conversion **, bool, bool);
...
extern int comp_template_args(tree, tree, tree * = NULL, tree * = NULL, bool = false);
extern int template_args_equal(tree, tree, bool = false);
extern tree maybe_process_partial_specialization(tree);
extern tree most_specialized_instantiation(tree);
extern void print_candidates(tree);
extern void instantiate_pending_templates(int);
extern tree tsubst_default_argument(tree, int, tree, tree, tsubst_flags_t);
extern tree tsubst(tree, tree, tsubst_flags_t, tree);
extern tree tsubst_copy_and_build(tree, tree, tsubst_flags_t, tree, bool, bool);
...
extern tree lookup_base(tree, tree, base_access, base_kind *, tsubst_flags_t);
extern tree dcast_base_hint(tree, tree);
extern int accessible_p(tree, tree, bool);
extern int accessible_in_template_p(tree, tree);
extern tree lookup_field(tree, tree, int, bool);
extern tree lookup_fnfields(tree, tree, int);
extern tree lookup_member(tree, tree, int, bool, tsubst_flags_t, access_failure_info *afi = NULL);
...
extern bool perform_access_checks(vec<deferred_access_check, va_gc> *, tsubst_flags_t);
extern bool perform_deferred_access_checks(tsubst_flags_t);
extern bool perform_or_defer_access_check(tree, tree, tree, tsubst_flags_t,
...
extern tree finish_asm_stmt(int, tree, tree, tree, tree, tree, bool);
...
extern cp_expr perform_koenig_lookup(cp_expr, vec<tree, va_gc> *, tsubst_flags_t);
extern tree finish_call_expr(tree, vec<tree, va_gc> **, bool, bool, tsubst_flags_t);
...
extern cp_expr finish_unary_op_expr(location_t, enum tree_code, cp_expr, tsubst_flags_t);
...
extern cp_expr finish_id_expression(tree, tree, tree, cp_id_kind *, bool, bool, bool *, bool, bool,
                                    bool, bool, const char **, location_t);
...
extern void add_typedef_to_current_template_for_access_check(tree, tree, location_t);
extern void check_accessibility_of_qualified_id(tree, tree, tree);
extern tree finish_qualified_id_expr(tree, tree, bool, bool, bool, bool, tsubst_flags_t);
...
extern tree finish_omp_for(location_t, enum tree_code, tree, tree, tree, tree, tree, tree, tree,
                           vec<tree> *, tree);
extern tree finish_omp_for_block(tree, tree);
extern void finish_omp_atomic(location_t, enum tree_code, enum tree_code, tree, tree, tree, tree,
                              tree, tree, enum omp_memory_order);
extern void finish_omp_barrier(void);
extern void finish_omp_depobj(location_t, tree, enum omp_clause_depend_kind, tree);
...
extern bool cxx_omp_create_clause_info(tree, tree, bool, bool, bool, bool);
extern tree baselink_for_fns(tree);
extern void finish_static_assert(tree, tree, location_t, bool);
...
extern tree finish_builtin_launder(location_t, tree, tsubst_flags_t);
extern tree cp_build_vec_convert(tree, location_t, tree, tsubst_flags_t);
...
extern void lang_check_failed(const char *, int, const char *) ATTRIBUTE_NORETURN ATTRIBUTE_COLD;
...
extern tree ovl_insert(tree fn, tree maybe_ovl, bool using_p = false);
extern tree ovl_skip_hidden(tree) ATTRIBUTE_PURE;
extern void lookup_mark(tree lookup, bool val);
extern tree lookup_add(tree fns, tree lookup);
extern tree lookup_maybe_add(tree fns, tree lookup, bool deduping);
...
extern tree cp_walk_subtrees(tree *, int *, walk_tree_fn, void *, hash_set<tree> *);
#define cp_walk_tree(tp, func, data, pset) walk_tree_1(tp, func, data, pset, cp_walk_subtrees)
...
extern void cxx_print_error_function(diagnostic_context *, const char *, struct diagnostic_info *);
...
extern tree decay_conversion(tree, tsubst_flags_t, bool = true);
extern tree build_class_member_access_expr(cp_expr, tree, tree, bool, tsubst_flags_t);
extern tree finish_class_member_access_expr(cp_expr, tree, bool, tsubst_flags_t);
extern tree build_x_indirect_ref(location_t, tree, ref_operator, tsubst_flags_t);
extern tree cp_build_indirect_ref(tree, ref_operator, tsubst_flags_t);
extern tree cp_build_fold_indirect_ref(tree);
extern tree build_array_ref(location_t, tree, tree);
extern tree cp_build_array_ref(location_t, tree, tree, tsubst_flags_t);
extern tree get_member_function_from_ptrfunc(tree *, tree, tsubst_flags_t);
extern tree cp_build_function_call_nary(tree, tsubst_flags_t, ...) ATTRIBUTE_SENTINEL;
extern tree cp_build_function_call_vec(tree, vec<tree, va_gc> **, tsubst_flags_t);
extern tree build_x_binary_op(const op_location_t &, enum tree_code, tree, enum tree_code, tree,
                              enum tree_code, tree *, tsubst_flags_t);
extern tree build_x_array_ref(location_t, tree, tree, tsubst_flags_t);
extern tree build_x_unary_op(location_t, enum tree_code, cp_expr, tsubst_flags_t);
extern tree cp_build_addressof(location_t, tree, tsubst_flags_t);
extern tree cp_build_addr_expr(tree, tsubst_flags_t);
extern tree cp_build_unary_op(enum tree_code, tree, bool, tsubst_flags_t);
extern tree genericize_compound_lvalue(tree);
extern tree unary_complex_lvalue(enum tree_code, tree);
extern tree build_x_conditional_expr(location_t, tree, tree, tree, tsubst_flags_t);
extern tree build_x_compound_expr_from_list(tree, expr_list_kind, tsubst_flags_t);
extern tree build_x_compound_expr_from_vec(vec<tree, va_gc> *, const char *, tsubst_flags_t);
extern tree build_x_compound_expr(location_t, tree, tree, tsubst_flags_t);
...
extern cp_expr build_c_cast(location_t loc, tree type, cp_expr expr);
extern tree cp_build_c_cast(tree, tree, tsubst_flags_t);
extern cp_expr build_x_modify_expr(location_t, tree, enum tree_code, tree, tsubst_flags_t);
extern tree cp_build_modify_expr(location_t, tree, enum tree_code, tree, tsubst_flags_t);
extern tree convert_for_initialization(tree, tree, tree, int, impl_conv_rhs, tree, int,
                                       tsubst_flags_t);
extern int comp_ptr_ttypes(tree, tree);
extern bool comp_ptr_ttypes_const(tree, tree);
extern bool error_type_p(const_tree);
extern bool ptr_reasonably_similar(const_tree, const_tree);
extern tree build_ptrmemfunc(tree, tree, int, bool, tsubst_flags_t);
extern int cp_type_quals(const_tree);
extern int type_memfn_quals(const_tree);
extern cp_ref_qualifier type_memfn_rqual(const_tree);
extern tree apply_memfn_quals(tree, cp_cv_quals, cp_ref_qualifier = REF_QUAL_NONE);
...
extern tree composite_pointer_type(tree, tree, tree, tree, composite_pointer_operation,
                                   tsubst_flags_t);
extern tree merge_types(tree, tree);
extern tree strip_array_domain(tree);
extern tree check_return_expr(tree, bool *);
extern tree cp_build_binary_op(const op_location_t &, enum tree_code, tree, tree, tsubst_flags_t);
extern tree build_x_vec_perm_expr(location_t, tree, tree, tree, tsubst_flags_t);
...
extern tree convert_ptrmem(tree, tree, bool, bool, tsubst_flags_t);
extern int lvalue_or_else(tree, enum lvalue_use, tsubst_flags_t);
...
extern void cxx_incomplete_type_diagnostic(location_t, const_tree, const_tree, diagnostic_t);

inline location_t cp_expr_loc_or_loc(const_tree t, location_t or_loc)
{
...
inline void cxx_incomplete_type_diagnostic(const_tree value, const_tree type,
                                           diagnostic_t diag_kind)
{
        cxx_incomplete_type_diagnostic(cp_expr_loc_or_loc(value, input_location), value, type,
                                       diag_kind);
}

extern void cxx_incomplete_type_error(location_t, const_tree, const_tree);
inline void cxx_incomplete_type_error(const_tree value, const_tree type)
{
...
extern void cxx_readonly_error(location_t, tree, enum lvalue_use);
...
extern bool check_narrowing(tree, tree, tsubst_flags_t, bool = false);
extern bool ordinary_char_type_p(tree);
extern tree digest_init(tree, tree, tsubst_flags_t);
extern tree digest_init_flags(tree, tree, int, tsubst_flags_t);
extern tree digest_nsdmi_init(tree, tree, tsubst_flags_t);
extern tree build_scoped_ref(tree, tree, tree *);
extern tree build_x_arrow(location_t, tree, tsubst_flags_t);
...
extern tree fold_non_dependent_expr(tree, tsubst_flags_t = tf_warning_or_error, bool = false);
extern tree fold_non_dependent_init(tree, tsubst_flags_t = tf_warning_or_error, bool = false);
...
inline tree ovl_first(tree node)
{
...
inline hashval_t named_decl_hash::hash(const value_type decl)
{
        tree name = OVL_NAME(decl);
        return name ? IDENTIFIER_HASH_VALUE(name) : 0;
}

inline bool named_decl_hash::equal(const value_type existing, compare_type candidate)
{
        tree name = OVL_NAME(existing);
        return candidate == name;
}

inline bool null_node_p(const_tree expr)
{
