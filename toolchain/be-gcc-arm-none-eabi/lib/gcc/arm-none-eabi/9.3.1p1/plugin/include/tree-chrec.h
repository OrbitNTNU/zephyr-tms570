...
static inline bool automatically_generated_chrec_p(const_tree chrec)
{
        return (chrec == chrec_dont_know || chrec == chrec_known);
}

/* The tree nodes aka. CHRECs.  */

static inline bool tree_is_chrec(const_tree expr)
{
        if (TREE_CODE(expr) == POLYNOMIAL_CHREC || automatically_generated_chrec_p(expr)) {
                return true;
        } else {
                return false;
        }
...
extern bool convert_affine_scev(struct loop *, tree, tree *, tree *, gimple *, bool, tree = NULL);
...
static inline bool chrec_zerop(const_tree chrec)
{
        if (chrec == NULL_TREE) {
                return false;
        }
...
static inline bool no_evolution_in_loop_p(tree chrec, unsigned loop_num, bool *res)
{
        tree scev;

        if (chrec == chrec_not_analyzed_yet || chrec == chrec_dont_know ||
...
static inline tree build_polynomial_chrec(unsigned loop_num, tree left, tree right)
{
        bool val;

        if (left == chrec_dont_know || right == chrec_dont_know) {
        ...
        if (POINTER_TYPE_P(TREE_TYPE(left))) {
                ...
                gcc_checking_assert(!POINTER_TYPE_P(TREE_TYPE(right)) &&
                                    types_compatible_p(TREE_TYPE(left), TREE_TYPE(right)));
        }
...
static inline bool evolution_function_is_constant_p(const_tree chrec)
{
        if (chrec == NULL_TREE) {
                return false;
        }
...
static inline bool evolution_function_is_affine_in_loop(const_tree chrec, int loopnum)
{
        if (chrec == NULL_TREE) {
                return false;
        }
...
static inline bool evolution_function_is_affine_p(const_tree chrec)
{
        return chrec && TREE_CODE(chrec) == POLYNOMIAL_CHREC &&
...
static inline bool tree_does_not_contain_chrecs(const_tree expr)
{
...
static inline tree chrec_type(const_tree chrec)
{
...
static inline tree chrec_fold_op(enum tree_code code, tree type, tree op0, tree op1)
{
        switch (code) {
        case PLUS_EXPR:
