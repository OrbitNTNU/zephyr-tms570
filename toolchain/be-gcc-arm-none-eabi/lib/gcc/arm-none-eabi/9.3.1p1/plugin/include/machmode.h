...
template <typename T> struct mode_traits {
        ...
        enum from_int {
                dummy = MAX_MACHINE_MODE
        };
#endif
};

template <> struct mode_traits<machine_mode> {
...
#define INTEGRAL_MODE_P(MODE)                                                                      \
        (GET_MODE_CLASS(MODE) == MODE_INT || GET_MODE_CLASS(MODE) == MODE_PARTIAL_INT ||           \
         GET_MODE_CLASS(MODE) == MODE_COMPLEX_INT || GET_MODE_CLASS(MODE) == MODE_VECTOR_BOOL ||   \
         GET_MODE_CLASS(MODE) == MODE_VECTOR_INT)

/* Nonzero if MODE is a floating-point mode.  */
#define FLOAT_MODE_P(MODE)                                                                         \
        (GET_MODE_CLASS(MODE) == MODE_FLOAT || GET_MODE_CLASS(MODE) == MODE_DECIMAL_FLOAT ||       \
         GET_MODE_CLASS(MODE) == MODE_COMPLEX_FLOAT || GET_MODE_CLASS(MODE) == MODE_VECTOR_FLOAT)
...
#define VECTOR_MODE_P(MODE)                                                                        \
        (GET_MODE_CLASS(MODE) == MODE_VECTOR_BOOL || GET_MODE_CLASS(MODE) == MODE_VECTOR_INT ||    \
         GET_MODE_CLASS(MODE) == MODE_VECTOR_FLOAT || GET_MODE_CLASS(MODE) == MODE_VECTOR_FRACT || \
         GET_MODE_CLASS(MODE) == MODE_VECTOR_UFRACT ||                                             \
         GET_MODE_CLASS(MODE) == MODE_VECTOR_ACCUM || GET_MODE_CLASS(MODE) == MODE_VECTOR_UACCUM)
...
#define ALL_SCALAR_FIXED_POINT_MODE_P(MODE)                                                        \
        (SIGNED_SCALAR_FIXED_POINT_MODE_P(MODE) || UNSIGNED_SCALAR_FIXED_POINT_MODE_P(MODE))

/* Nonzero if MODE is a scalar/vector fract mode.  */
#define FRACT_MODE_P(MODE)                                                                         \
        (GET_MODE_CLASS(MODE) == MODE_FRACT || GET_MODE_CLASS(MODE) == MODE_VECTOR_FRACT)

/* Nonzero if MODE is a scalar/vector ufract mode.  */
#define UFRACT_MODE_P(MODE)                                                                        \
        (GET_MODE_CLASS(MODE) == MODE_UFRACT || GET_MODE_CLASS(MODE) == MODE_VECTOR_UFRACT)
...
#define ACCUM_MODE_P(MODE)                                                                         \
        (GET_MODE_CLASS(MODE) == MODE_ACCUM || GET_MODE_CLASS(MODE) == MODE_VECTOR_ACCUM)

/* Nonzero if MODE is a scalar/vector uaccum mode.  */
#define UACCUM_MODE_P(MODE)                                                                        \
        (GET_MODE_CLASS(MODE) == MODE_UACCUM || GET_MODE_CLASS(MODE) == MODE_VECTOR_UACCUM)
...
#define ALL_FIXED_POINT_MODE_P(MODE)                                                               \
        (SIGNED_FIXED_POINT_MODE_P(MODE) || UNSIGNED_FIXED_POINT_MODE_P(MODE))

/* Nonzero if CLASS modes can be widened.  */
#define CLASS_HAS_WIDER_MODES_P(CLASS)                                                             \
        (CLASS == MODE_INT || CLASS == MODE_PARTIAL_INT || CLASS == MODE_FLOAT ||                  \
         CLASS == MODE_DECIMAL_FLOAT || CLASS == MODE_COMPLEX_FLOAT || CLASS == MODE_FRACT ||      \
         CLASS == MODE_UFRACT || CLASS == MODE_ACCUM || CLASS == MODE_UACCUM)

/* An optional T (i.e. a T or nothing), where T is some form of mode class.  */
template <typename T> class opt_mode
{
      public:
        enum from_int {
                dummy = MAX_MACHINE_MODE
        ...
        template <typename U> ALWAYS_INLINE opt_mode(const U &m) : m_mode(T(m))
        {
        }
...
template <typename T> ALWAYS_INLINE machine_mode opt_mode<T>::else_void() const
{
        return m_mode;
}
...
template <typename T> inline machine_mode opt_mode<T>::else_blk() const
{
...
template <typename T> inline T opt_mode<T>::require() const
{
...
template <typename T> ALWAYS_INLINE bool opt_mode<T>::exists() const
{
...
template <typename T> template <typename U> inline bool opt_mode<T>::exists(U *mode) const
{
...
template <typename T> inline bool is_a(machine_mode m)
{
        return T::includes_p(m);
}

template <typename T, typename U> inline bool is_a(const opt_mode<U> &m)
{
...
template <typename T> inline T as_a(machine_mode m)
{
        gcc_checking_assert(T::includes_p(m));
        return typename mode_traits<T>::from_int(m);
}

template <typename T, typename U> inline T as_a(const opt_mode<U> &m)
{
...
template <typename T> inline opt_mode<T> dyn_cast(machine_mode m)
{
        if (T::includes_p(m)) {
...
template <typename T, typename U> inline opt_mode<T> dyn_cast(const opt_mode<U> &m)
{
...
template <typename T, typename U> inline bool is_a(machine_mode m, U *result)
{
        if (T::includes_p(m)) {
...
inline bool scalar_int_mode::includes_p(machine_mode m)
{
...
inline bool scalar_float_mode::includes_p(machine_mode m)
{
...
inline bool scalar_mode::includes_p(machine_mode m)
{
...
inline bool complex_mode::includes_p(machine_mode m)
{
...
{
#if GCC_VERSION >= 4001
        return (__builtin_constant_p(mode) ? mode_size_inline(mode) : mode_size[mode]);
#else
...
{
#if GCC_VERSION >= 4001
        return scalar_mode::from_int(__builtin_constant_p(mode) ? mode_inner_inline(mode)
                                                                : mode_inner[mode]);
#else
...
{
#if GCC_VERSION >= 4001
        return (__builtin_constant_p(mode) ? mode_unit_size_inline(mode) : mode_unit_size[mode]);
#else
...
{
#if GCC_VERSION >= 4001
        return (__builtin_constant_p(mode) ? mode_unit_precision_inline(mode)
...
{
#if GCC_VERSION >= 4001
        return (__builtin_constant_p(mode) ? mode_nunits_inline(mode) : mode_nunits[mode]);
#else
...
template <typename T>
ALWAYS_INLINE typename if_poly<typename T::measurement_type>::type GET_MODE_SIZE(const T &mode)
{
        return mode_to_bytes(mode);
}

template <typename T>
ALWAYS_INLINE typename if_nonpoly<typename T::measurement_type>::type GET_MODE_SIZE(const T &mode)
{
...
template <typename T>
ALWAYS_INLINE typename if_poly<typename T::measurement_type>::type GET_MODE_BITSIZE(const T &mode)
{
        return mode_to_bits(mode);
}
...
#if ONLY_FIXED_SIZE_MODES
#define GET_MODE_PRECISION(MODE) ((unsigned short)mode_to_precision(MODE).coeffs[0])
#else
...
template <typename T>
ALWAYS_INLINE typename if_poly<typename T::measurement_type>::type GET_MODE_PRECISION(const T &mode)
{
        return mode_to_precision(mode);
}
...
template <typename T>
ALWAYS_INLINE typename if_poly<typename T::measurement_type>::type GET_MODE_NUNITS(const T &mode)
{
        return mode_to_nunits(mode);
}

template <typename T>
ALWAYS_INLINE typename if_nonpoly<typename T::measurement_type>::type GET_MODE_NUNITS(const T &mode)
{
...
template <typename T> ALWAYS_INLINE opt_mode<T> GET_MODE_WIDER_MODE(const T &m)
{
...
template <typename T> ALWAYS_INLINE opt_mode<T> GET_MODE_2XWIDER_MODE(const T &m)
{
...
inline bool fixed_size_mode::includes_p(machine_mode mode)
{
...
inline opt_scalar_int_mode int_mode_for_size(poly_uint64 size, int limit)
{
...
inline opt_scalar_float_mode float_mode_for_size(poly_uint64 size)
{
...
inline opt_scalar_float_mode decimal_float_mode_for_size(unsigned int size)
{
        return dyn_cast<scalar_float_mode>(mode_for_size(size, MODE_DECIMAL_FLOAT, 0));
}
...
inline scalar_int_mode smallest_int_mode_for_size(poly_uint64 size)
{
...
inline opt_machine_mode mode_for_int_vector(machine_mode mode)
{
        return mode_for_int_vector(GET_MODE_UNIT_BITSIZE(mode), GET_MODE_NUNITS(mode));
}
...
{
      public:
        bit_field_mode_iterator(HOST_WIDE_INT, HOST_WIDE_INT, poly_int64, poly_int64, unsigned int,
                                bool);
...
extern bool get_best_mode(int, int, poly_uint64, poly_uint64, unsigned int, unsigned HOST_WIDE_INT,
...
#define NARROWEST_INT_MODE                                                                         \
        (scalar_int_mode(scalar_int_mode::from_int(class_narrowest_mode[MODE_INT])))

/* Return the narrowest mode in T's class.  */

template <typename T> inline T get_narrowest_mode(T mode)
{
        return typename mode_traits<T>::from_int(class_narrowest_mode[GET_MODE_CLASS(mode)]);
}
...
#define TRULY_NOOP_TRUNCATION_MODES_P(MODE1, MODE2)                                                \
        (targetm.truly_noop_truncation(GET_MODE_PRECISION(MODE1), GET_MODE_PRECISION(MODE2)))

/* Return true if MODE is a scalar integer mode that fits in a
   HOST_WIDE_INT.  */

inline bool HWI_COMPUTABLE_MODE_P(machine_mode mode)
{
...
inline bool HWI_COMPUTABLE_MODE_P(scalar_int_mode mode)
{
...
template <typename T> inline bool is_int_mode(machine_mode mode, T *int_mode)
{
...
template <typename T> inline bool is_float_mode(machine_mode mode, T *float_mode)
{
...
template <typename T> inline bool is_complex_int_mode(machine_mode mode, T *cmode)
{
...
template <typename T> inline bool is_complex_float_mode(machine_mode mode, T *cmode)
{
...
inline bool is_narrower_int_mode(machine_mode mode, scalar_int_mode limit)
{
...
template <typename T> inline void start(opt_mode<T> *iter, enum mode_class mclass)
{
...
inline void start(machine_mode *iter, enum mode_class mclass)
{
...
template <typename T> inline bool iterate_p(opt_mode<T> *iter)
{
        return iter->exists();
}

inline bool iterate_p(machine_mode *iter)
{
...
template <typename T> inline void get_wider(opt_mode<T> *iter)
{
        *iter = GET_MODE_WIDER_MODE(iter->require());
}

inline void get_wider(machine_mode *iter)
{
...
template <typename T> inline void get_known_wider(T *iter)
{
        *iter = GET_MODE_WIDER_MODE(*iter).require();
}
...
template <typename T> inline void get_2xwider(opt_mode<T> *iter)
{
        *iter = GET_MODE_2XWIDER_MODE(iter->require());
}

inline void get_2xwider(machine_mode *iter)
{
...
#define FOR_EACH_MODE_IN_CLASS(ITERATOR, CLASS)                                                    \
        for (mode_iterator::start(&(ITERATOR), CLASS); mode_iterator::iterate_p(&(ITERATOR));      \
...
#define FOR_EACH_MODE(ITERATOR, START, END)                                                        \
        for ((ITERATOR) = (START); (ITERATOR) != (END); mode_iterator::get_known_wider(&(ITERATOR)))
...
#define FOR_EACH_MODE_FROM(ITERATOR, START)                                                        \
        for ((ITERATOR) = (START); mode_iterator::iterate_p(&(ITERATOR));                          \
             mode_iterator::get_wider(&(ITERATOR)))
...
#define FOR_EACH_WIDER_MODE(ITERATOR, START)                                                       \
        for ((ITERATOR) = (START), mode_iterator::get_wider(&(ITERATOR));                          \
             mode_iterator::iterate_p(&(ITERATOR)); mode_iterator::get_wider(&(ITERATOR)))
...
#define FOR_EACH_2XWIDER_MODE(ITERATOR, START)                                                     \
        for ((ITERATOR) = (START), mode_iterator::get_2xwider(&(ITERATOR));                        \
             mode_iterator::iterate_p(&(ITERATOR)); mode_iterator::get_2xwider(&(ITERATOR)))

template <typename T> void gt_ggc_mx(pod_mode<T> *)
{
}

template <typename T> void gt_pch_nx(pod_mode<T> *)
{
}

template <typename T> void gt_pch_nx(pod_mode<T> *, void (*)(void *, void *), void *)
{
}
