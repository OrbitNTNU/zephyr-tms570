...
typedef int __assert_gimple_bb_smaller_rtl_bb[(int)sizeof(struct rtl_bb_info) -
...
#define BB_FLAGS_TO_PRESERVE                                                                       \
        (BB_DISABLE_SCHEDULE | BB_RTL | BB_NON_LOCAL_GOTO_TARGET | BB_HOT_PARTITION |              \
         BB_COLD_PARTITION)
...
#define BB_SET_PARTITION(bb, part)                                                                 \
        do {                                                                                       \
                basic_block bb_ = (bb);                                                            \
                bb_->flags = ((bb_->flags & ~(BB_HOT_PARTITION | BB_COLD_PARTITION)) | (part));    \
        } while (0)
...
#define FOR_EACH_BB_FN(BB, FN)                                                                     \
        FOR_BB_BETWEEN(BB, (FN)->cfg->x_entry_block_ptr->next_bb, (FN)->cfg->x_exit_block_ptr,     \
                       next_bb)

#define FOR_EACH_BB_REVERSE_FN(BB, FN)                                                             \
        FOR_BB_BETWEEN(BB, (FN)->cfg->x_exit_block_ptr->prev_bb, (FN)->cfg->x_entry_block_ptr,     \
                       prev_bb)
...
#define FALLTHRU_EDGE(bb)                                                                          \
        (EDGE_SUCC((bb), 0)->flags & EDGE_FALLTHRU ? EDGE_SUCC((bb), 0) : EDGE_SUCC((bb), 1))

/* BB is assumed to contain conditional jump.  Return the branch edge.  */
#define BRANCH_EDGE(bb)                                                                            \
        (EDGE_SUCC((bb), 0)->flags & EDGE_FALLTHRU ? EDGE_SUCC((bb), 1) : EDGE_SUCC((bb), 0))
...
#define EDGE_CRITICAL_P(e) (EDGE_COUNT((e)->src->succs) >= 2 && EDGE_COUNT((e)->dest->preds) >= 2)
...
static inline bool single_succ_p(const_basic_block bb)
{
...
static inline bool single_pred_p(const_basic_block bb)
{
...
static inline edge single_succ_edge(const_basic_block bb)
{
...
static inline edge single_pred_edge(const_basic_block bb)
{
...
static inline basic_block single_succ(const_basic_block bb)
{
...
static inline basic_block single_pred(const_basic_block bb)
{
...
static inline vec<edge, va_gc> *ei_container(edge_iterator i)
{
...
static inline edge_iterator ei_start_1(vec<edge, va_gc> **ev)
{
        edge_iterator i;
...
/* Return an iterator pointing to the last element of an edge
   vector.  */
static inline edge_iterator ei_last_1(vec<edge, va_gc> **ev)
{
        edge_iterator i;
...
static inline bool ei_end_p(edge_iterator i)
{
...
/* Is the iterator `i' at one position before the end of the
   sequence?  */
static inline bool ei_one_before_end_p(edge_iterator i)
{
        return (i.index + 1 == EDGE_COUNT(ei_container(i)));
}

/* Advance the iterator to the next element.  */
static inline void ei_next(edge_iterator *i)
{
...
static inline void ei_prev(edge_iterator *i)
{
...
static inline edge ei_edge(edge_iterator i)
{
...
/* Return an edge pointed to by the iterator.  Do it safely so that
   ...
   sequence.  */
static inline edge ei_safe_edge(edge_iterator i)
{
...
static inline bool ei_cond(edge_iterator ei, edge *p)
{
        if (!ei_end_p(ei)) {
                *p = ei_edge(ei);
                return 1;
...
#define FOR_EACH_EDGE(EDGE, ITER, EDGE_VEC)                                                        \
        for ((ITER) = ei_start((EDGE_VEC)); ei_cond((ITER), &(EDGE)); ei_next(&(ITER)))
...
static inline bool bb_in_transaction(basic_block bb)
{
...
static inline bool bb_has_abnormal_pred(basic_block bb)
{
        edge e;
        edge_iterator ei;
...
static inline edge find_fallthru_edge(vec<edge, va_gc> *edges)
{
        edge e;
        edge_iterator ei;
...
static inline void check_probability(int prob)
{
...
static inline int combine_probabilities(int prob1, int prob2)
{
...
static inline gcov_type apply_scale(gcov_type freq, gcov_type scale)
{
...
static inline gcov_type apply_probability(gcov_type freq, int prob)
{
...
static inline int inverse_probability(int prob1)
{
        check_probability(prob1);
...
static inline bool has_abnormal_or_eh_outgoing_edge_p(basic_block bb)
{
        edge e;
        edge_iterator ei;
...
static inline bool has_abnormal_call_or_eh_pred_edge_p(basic_block bb)
{
        edge e;
        edge_iterator ei;
